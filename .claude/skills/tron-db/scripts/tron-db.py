...
Environment:
    TRON_DB - Path to database (default: ~/.tron/internal/database/log.db)
...
def get_db_path():
    ...
    db_path = Path.home() / ".tron" / "internal" / "database" / "log.db"
    if db_path.exists():
        return str(db_path)

    print("Error: No Tron database found at ~/.tron/internal/database/log.db", file=sys.stderr)
