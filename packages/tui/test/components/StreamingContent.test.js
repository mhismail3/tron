...
describe('StreamingContent Component', () => {
    ...
    it('should render content text', () => {
        const { lastFrame } = render(<StreamingContent content="Hello world" isStreaming={false}/>);
        expect(lastFrame()).toContain('Hello world');
    });
    it('should show cursor when streaming', () => {
        const { lastFrame } = render(<StreamingContent content="Hello" isStreaming={true}/>);
    ...
    it('should not show cursor when not streaming', () => {
        const { lastFrame } = render(<StreamingContent content="Complete" isStreaming={false}/>);
        const frame = lastFrame() ?? '';
        expect(frame).not.toMatch(/[█▌]/);
    });
    it('should handle multiline content', () => {
        const { lastFrame } = render(<StreamingContent content="Line 1\nLine 2\nLine 3" isStreaming={false}/>);
        const frame = lastFrame() ?? '';
...
//# sourceMappingURL=StreamingContent.test.js.map