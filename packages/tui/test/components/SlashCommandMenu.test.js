...
describe('SlashCommandMenu Component', () => {
    ...
    it('should handle custom command list', () => {
        ...
        const { lastFrame } = render(<SlashCommandMenu {...defaultProps} commands={customCommands}/>);
        const frame = lastFrame() ?? '';
        expect(frame).toContain('custom');
        expect(frame).toContain('A custom command');
    });
    it('should limit visible items', () => {
        const { lastFrame } = render(<SlashCommandMenu {...defaultProps} maxVisible={3}/>);
        const frame = lastFrame() ?? '';
...
//# sourceMappingURL=SlashCommandMenu.test.js.map