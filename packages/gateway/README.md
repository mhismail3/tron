...
is bound to the connection, scope, materialization, offset, and revision. RuntimeRegistry supplies a disposable generation identity for indexed catalog cuts; the pagination store validates and freezes one row vector per generation and shares it across clients instead of cloning or JSON-sizing the complete catalog per lease. Single-page results are not retained as generations. A Gateway-owned `gateway/catalog-metadata-v1.json` acceleration file may persist only bounded identity/summary metadata, canonical file identity, size/mtime/EOF, and a fixed tail-boundary hash. It is versioned and root-bound, written through a 0600 temp-file/fsync/rename/directory-fsync transaction, and is discarded on any schema/root/inode/size/mtime/boundary mismatch; JSONL remains authoritative and persistence failure degrades to the canonical in-memory projection. The current query remains a bounded shared materialization rather than offset-seekable catalog pagination; true keyset hydration is a later phase. Traversal
