...
`src/extensions/compatibility-manifest.ts`. Pi 0.87 `context_edit` records remain
canonical JSONL entries but are not chat rows; branch projection, restart/resume,
compaction and export/import must preserve the edited model context. Gateway
session import creates a new canonical identity with `SessionManager.forkFrom`
while retaining imported entry IDs and branch references, rather than replacing
a source session. The SDK-written `parentSession` header is provenance to the import
input path, which can be removed with the upload; it is not a guaranteed
...
- Hashed mobile devices: `gateway/devices.json` (bounded owner-UID-only regular non-symlink file;
  auth contexts expose only `kind` and `deviceId`)
...
- Delegated provider artifacts: the installed `pi-subagents` owner receives the
  ...
  provider-owned. Discovery reads the bounded first-property lifecycle projection
  even when the full report is larger than the legacy document cap; headerless
  artifacts remain subject to that cap, and an oversized legacy read is diagnosed
  separately from an atomic-replacement retry. Restart discovery can therefore
  recover active runs without parsing report bodies or relaxing the artifact I/O
  bound.
...
The Gateway also owns a bounded 512-entry user-facing notification inbox in the same owner-only state transaction as delivery admission. A visible `queued` inbox row proves durable admission, not relay or APNs acceptance. New notification content, exact session route, per-target APNs request identities, delivery outcome, and global read state are canonical there; the iOS cache is only a bounded disposable projection. The inbox is part of the version-1 document shape; a document without it is rejected as malformed. `notification.inbox.list` pages newest-first under an exact branch-independent revision/cursor, while `notification.inbox.read` accepts exactly one public inbox ID or APNs request ID and `notification.inbox.readAll` clears all unread entries through ordinary command receipts. Every admission, terminal delivery transition, and read mutation broadcasts `notification.inbox.changed`. Unknown, unavailable, suppressed, and rate-limited attempts create no inbox row. Preview-disabled explicit model text remains generic in both APNs and inbox storage.
...
ownership, and public `upload:<uuid>` identity remain logical metadata. The folder's version-2 metadata is
the only accepted shape; a superseded shape is discarded as corrupt during inventory. Ordinary inventory
checks inode/size ownership without rehashing every retained object, and repairs a logical path that has
lost its object by stream-hashing it once, re-adopting the object, and fsyncing the object, its shard
directories, and the folder. The first read verifies a digest once per
...
and its retention scan. Full payload copies use `/bin/cp -c -R` to request APFS
...
strict. When a promoted candidate fails after its restart request, the helper reads the
newest 256 KB of `logs/gateway.jsonl` and `logs/deploy.jsonl` for a `gateway.fatal-startup` or
