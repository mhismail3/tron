...
dashboard immediately without broadcasting full transcripts or changing the structural list revision. A transient `busy` discovery cut caused by a concurrent child-session write may serve one previously complete user-scoped catalog cut while the next authoritative read converges; `session.list` all-sessions/admin reads still fail closed. Clients subscribe
