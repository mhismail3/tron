...
| External owner | `pi-agent-browser-native` retains `~/.pi/config/pi-agent-browser-native/config.json`; browser profiles, cookies, Keychain data, and provider credentials remain external capability stores. The migration does not copy or rewrite them. |
