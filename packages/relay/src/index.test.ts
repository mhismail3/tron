 ...
 *
 * Key contract under test: the request body must include `bundle_id`, and the
...
describe("relay worker — bundle_id routing", () => {
  ...
  test("request without bundle_id is rejected", async () => {
    const { captured } = stubApnsFetch();
    const env = makeEnv();
    const worker = await freshWorker();
    ...
    expect(await resp.json()).toEqual({ error: "bundle_id must be a non-empty string" });
    expect(captured).toHaveLength(0);
  });

  test("empty string bundle_id is rejected", async () => {
    const { captured } = stubApnsFetch();
    const env = makeEnv();
    const worker = await freshWorker();
