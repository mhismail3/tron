...
indirect enum ToolSchemaFieldKind: Equatable, Sendable {
    case object
    case array
    case string
    case number
    ...
    case nullable(ToolSchemaFieldKind)
    case unsupported(String)
}

struct ToolSchemaField: Identifiable, Equatable, Sendable {
    var id: String { path.joined(separator: ".") }
    var path: [String]
    var title: String
    var kind: ToolSchemaFieldKind
    var required: Bool
    var description: String?
    var defaultValue: AnyCodable?
    var examples: [AnyCodable]
    var uiHint: String?
    var children: [ToolSchemaField]
}

struct ToolSchemaFormModel: Equatable, Sendable {
    var fields: [ToolSchemaField]
    var unsupportedPaths: [String]

    static func build(from schema: AnyCodable?) -> ToolSchemaFormModel {
        guard let object = schema?.dictionaryValue else {
            return ToolSchemaFormModel(fields: [], unsupportedPaths: [])
        }
    ...
    private static func field(
        ...
        let children = properties
            .compactMap { key, value -> ToolSchemaField? in
    ...
    private static func kindFor(typeName: String, enumValues: [String]?) -> ToolSchemaFieldKind {
