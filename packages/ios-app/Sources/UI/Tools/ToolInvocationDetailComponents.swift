    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack(alignment: .top, spacing: 12) {
                ...
                HStack(spacing: 8) {
                    ...
                    if let duration = data.formattedDuration {
                        ToolHeaderDurationBadge(duration: duration, color: accent)
                    }
                }
            ...
            if let traceId = data.identity.traceId?.nilIfEmpty {
                ToolHeaderMetric(label: "Trace", value: String(traceId.prefix(12)), tint: tint)
            }
        }
        .padding(16)
        .background {
...
struct ToolExecutionGroupView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if !detailRows.isEmpty {
                ...
                ToolReadableRows(rows: detailRows, tint: tint)
            }
        }
        .frame(maxWidth: .infinity, alignment: .leading)
        .padding(12)
    ...
    private var summary: String? {
        switch group.title {
        case "Resolution":
            return [value("Mode"), value("Target").map(humanizeTool)].compactMap { $0 }.joined(separator: " · ").nilIfEmpty
        case "Preparation":
    ...
    private var detailRows: [ToolDisplayRow] {
    ...
    private func humanizeTool(_ id: String) -> String {
...
struct ToolHeaderDurationBadge: View {
    let duration: String
    let color: Color
...
struct ToolArtifactRow: View {
    let artifact: ToolArtifactData
...
struct ToolReadableRows: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ForEach(rows) { row in
                ToolReadableRow(row: row, tint: tint)
            }
        }
    }
}

struct ToolReadableRow: View {
