...
struct ProjectResourcePromptContent: View {
    ...
    var body: some View {
        ...
        .tronSettingsCaption(detail.contentTruncated == true
            ? "This prompt exceeds the content limit. Showing the first \(CommandResourceDetailPolicy.maximumContentBytes.formatted()) bytes; the complete template remains in its source file on the Mac."
            : nil)
    }
}
