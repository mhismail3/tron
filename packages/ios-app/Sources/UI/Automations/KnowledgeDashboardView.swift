...
struct KnowledgeDetailView: View {
    ...
    private var evidence: some View { VStack(alignment: .leading, spacing: 8) { Text("Evidence").font(.headline); ForEach(Array(record.provenance.evidence.enumerated()), id: \.offset) { _, ref in if let citation = ref.sessionEntry { Button("Open session \(citation.sessionId) · entry \(citation.entryId)") { openSession(citation) }.font(.footnote) } else { Text("Record \(ref.recordId ?? "object")").font(.footnote).foregroundStyle(Color.tronTextSecondary) } }; if let evidenceMessage { Text(evidenceMessage).font(.footnote).foregroundStyle(Color.tronTextSecondary) }; if let objectPreview { Text(objectPreview).font(.footnote.monospaced()).textSelection(.enabled).lineLimit(24) } } }
    ...
    private func openSession(_ citation: KnowledgeSessionEntryCitation) { guard admitsOrigin else { evidenceMessage = "Gateway changed; reopen this entry."; return }; Task { @MainActor in do { _ = try await model.knowledge.readSessionEntry(sessionID: citation.sessionId, entryID: citation.entryId); guard admitsOrigin else { return }; onOpenSession(citation.sessionId, citation.entryId); evidenceMessage = "Loaded exact entry \(citation.entryId); opening its originating session." } catch { guard admitsOrigin else { return }; evidenceMessage = error.localizedDescription } } }
