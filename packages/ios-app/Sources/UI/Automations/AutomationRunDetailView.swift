...
struct AutomationRunDetailView: View {
    ...
    var body: some View {
        NavigationStack {
            ScrollView { VStack(alignment: .leading, spacing: TronSpacing.lg) {
                ...
                section("Action snapshot", icon: run.actionSnapshot.typedKind?.icon ?? "bolt") { Text(run.actionSnapshot.content).font(TronTypography.body).foregroundStyle(Color.tronTextPrimary).textSelection(.enabled).fixedSize(horizontal: false, vertical: true) }
    ...
    private func resolve(_ outcome: String) async { guard let client else { return }; do { _ = try await client.resolve(id: automation.summary.id, runId: run.runId, revision: automation.summary.revision, outcome: outcome); dismiss() } catch { resolution = (error as? GatewayFailure)?.message ?? "Unable to resolve run." } }
}
