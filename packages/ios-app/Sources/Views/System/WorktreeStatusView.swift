                        ...
                        if let onCommit {
                            Button(action: onCommit) {
                                Label("Commit", systemImage: "checkmark.circle")
                                    .font(TronTypography.caption)
                                    .lineLimit(1)
                                    .foregroundStyle(canCommit ? .white : .tronEmerald)
                                    .padding(.horizontal, 8)
                                    .padding(.vertical, 3)
                                    .background(canCommit ? Color.accentColor : Color.tronEmerald.opacity(0.15))
                                    .clipShape(Capsule())
                            }
                            .buttonStyle(.plain)
                            .disabled(!canCommit)
                        ...
                        if let onMerge {
                            Button(action: onMerge) {
                                Label("Merge", systemImage: "arrow.triangle.merge")
                                    .font(TronTypography.caption)
                                    .lineLimit(1)
                                    .foregroundStyle(canMerge ? .white : .tronEmerald)
                                    .padding(.horizontal, 8)
                                    .padding(.vertical, 3)
                                    .background(canMerge ? Color.tronEmerald : Color.tronEmerald.opacity(0.15))
                                    .clipShape(Capsule())
                            }
                            .buttonStyle(.plain)
                            .disabled(!canMerge)
