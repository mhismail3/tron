...
/// Status of a tool activity line.
...
// MARK: - Tool Risk Color

/// Type-safe tool color that maps tool presentation hint names to SwiftUI colors.
/// Replaces the stringly-typed `presentationColorName` → `String.resolvedToolColor` pattern.
enum ToolColor: String, Codable, Equatable, CaseIterable, Sendable {
    ...
    /// Parse from a tool presentation hint color string. Falls back to `.tronTextMuted`.
    init(fromDescriptorName name: String) {
        self = ToolColor(rawValue: name) ?? .tronTextMuted
    }

    static func fromTool(_: ToolIdentity) -> ToolColor {
        .tronInfo
    }
}
...
struct ActivityLine: Identifiable, Codable, Sendable {
    ...
    enum CodingKeys: String, CodingKey {
        case kind, text, icon, iconColor, toolName, displayName, summary, duration, status, toolIdentity
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        self.toolIdentity = try c.decodeIfPresent(ToolIdentity.self, forKey: .toolIdentity)
...
extension ActivityLine: Equatable {
    static func == (lhs: ActivityLine, rhs: ActivityLine) -> Bool {
        ...
        lhs.toolIdentity == rhs.toolIdentity
    }
}
