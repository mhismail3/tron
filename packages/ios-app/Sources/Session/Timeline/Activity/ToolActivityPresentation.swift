...
enum ToolActivityPresentation {
    ...
    static func summary(
        ...
        if let hinted = ToolPresentation.presentationString("summary", for: identity)
            ?? ToolPresentation.presentationString("subtitle", for: identity)
            ?? ToolPresentation.presentationString("commandText", for: identity) {
    ...
    private static func simpleSummary(from object: [String: Any]) -> String? {
        if let command = ToolInvocationDisplayModel.commandString(
