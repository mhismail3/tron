...
extension ToolInvocationDisplayModel {
    ...
    static func requestRows(
        ...
        if rows.isEmpty, let payloadSummary {
            rows.append(ToolDisplayRow(label: "Request", value: payloadSummary))
        }
        return rows
    }

    static func appendPayloadRows(_ payload: [String: Any], into rows: inout [ToolDisplayRow]) {
        func append(_ label: String, _ value: String?) {
            guard let value = value?.nilIfEmpty else { return }
            rows.append(ToolDisplayRow(label: label, value: value))
        }
        ...
        let extraRows = payload
            ...
            .sorted { $0.key < $1.key }
            .prefix(3)
            .compactMap { key, value -> ToolDisplayRow? in
                simplePayloadExtraValue(value).map { ToolDisplayRow(label: humanizeKey(key), value: $0) }
            }
    ...
    static func technicalRows(data: ToolInvocationData) -> [ToolDisplayRow] {
        ...
        func append(_ label: String, _ value: String?) {
            guard let value = value?.nilIfEmpty else { return }
            rows.append(ToolDisplayRow(label: label, value: value, isTechnical: true))
        }
        append("Invocation", data.id)
        append("Tool", identity.toolName)
        ...
        append("Observed duration", data.observedDurationMs.map(ToolInvocationData.formatDuration))
    ...
    static func appendRow(
        _ label: String,
        _ value: String?,
        to rows: inout [ToolDisplayRow],
        technical: Bool = false
    ) {
        guard let value = value?.nilIfEmpty else { return }
        rows.append(ToolDisplayRow(label: label, value: value, isTechnical: technical))
    }

    static func argumentObject(from data: ToolInvocationData) -> [String: Any] {
    ...
    static func outputObject(from data: ToolInvocationData) -> [String: Any] {
