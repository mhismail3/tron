...
// =============================================================================
// MARK: - Streaming RPC Event Types (from server core/src/rpc/types.ts)
// =============================================================================

/// All streaming RPC event types sent over WebSocket.
/// This EXACTLY mirrors the server's RpcEventType union in rpc/types.ts.
///
/// These events are received in real-time during agent execution and
/// are used for live UI updates (streaming text, tool progress, etc.)
enum StreamingEventType: String, CaseIterable {
    // Agent events (real-time during execution)
    case agentTurnStart = "agent.turn_start"
    case agentTurnEnd = "agent.turn_end"
    case agentTextDelta = "agent.text_delta"
    case agentThinkingDelta = "agent.thinking_delta"
    case agentToolStart = "agent.tool_start"
    case agentToolEnd = "agent.tool_end"
    case agentError = "agent.error"
    case agentComplete = "agent.complete"

    // Session events (lifecycle notifications)
    case sessionCreated = "session.created"
    case sessionEnded = "session.ended"
    case sessionUpdated = "session.updated"
    case sessionForked = "session.forked"
    case sessionRewound = "session.rewound"

    // Event sync events (for real-time event broadcasting)
    case eventsNew = "events.new"
    case eventsBatch = "events.batch"

    // Tree events (branch/fork notifications)
    case treeUpdated = "tree.updated"
    case treeBranchCreated = "tree.branch_created"

    // System events (connection management)
    case systemConnected = "system.connected"
    case systemDisconnected = "system.disconnected"
    case systemError = "system.error"

    // MARK: - Classification

    /// Whether this is an agent execution event (affects chat display)
    var isAgentEvent: Bool {
        switch self {
        case .agentTurnStart, .agentTurnEnd, .agentTextDelta, .agentThinkingDelta,
             .agentToolStart, .agentToolEnd, .agentError, .agentComplete:
            return true
        default:
            return false
        }
    ...
    /// Whether this is a session lifecycle event
    var isSessionEvent: Bool {
        switch self {
    ...
    /// Whether this is a system/connection event
