...
final class RepoClient: RPCDomainClient {
    ...
    /// flows; this RPC does not mutate other sessions.
    func listSessions(sessionId: String) async throws -> [RepoSessionSummary] {
        ...
        let result: RepoListSessionsResult = try await ws.send(
            method: "repo.listSessions",
            params: params
        )
    ...
    func getDivergence(sessionId: String) async throws -> RepoDivergence {
        ...
        return try await ws.send(method: "repo.getDivergence", params: params)
    }
}
