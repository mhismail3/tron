...
- **Native checkpoint gap:** `BrowserLiveMountedViewingTests.floatingFrameGeometryAdaptsWithoutRemounting` intermittently times out waiting for two matching requests in grouped runs, including with `BrowserLiveViewingTests`, while some narrower runs pass. The same two-suite failure reproduces on clean source baseline `218510f154127591c802d0c86c75409ddf4be93a`; the reconnect/composer changes are not required to reproduce it. Its browser preparation/activity/handoff cause remains unresolved, so the full native gate is not certified clean.
