...
**Status: source-hardening checkpoints are implemented, with follow-up reconnect and composer visibility fixes on `agent/reconnect-progressive-loading`; deployment and physical qualification are not complete.** Catalog producer invalidation/cost, interrupted pre-hello recovery accounting, and the device-native opening-settlement trigger remain open. The phases below remain the acceptance contract; automated evidence is not a blanket reliability guarantee.
...
- **Native checkpoint gap:** `BrowserLiveMountedViewingTests.floatingFrameGeometryAdaptsWithoutRemounting` has failed on rapid activity restoration after DELETE with no replacement POST, for both live producers; narrow passes do not refute the grouped failure. Source inspection identifies redundant SwiftUI phase-to-native-generation feedback into the same task ID, rather than evidence of a decode queue stall. Native notifications now exclusively own the generation fence; phase directly gates admission/rendering without another callback. The focused resume/coalesced-activity regressions and grouped native checkpoint still require validation before this gate is certified clean; see [display lifecycle](display-artifacts.md#live-browser-and-native-window-views).
