...
The Tron logo is a native `UIButton`/`UIMenu` selector: a 56-point bottom-right
glass circle with a 34-point logo on Sessions (replacing the + button), and leading in the toolbar on Automations and Knowledge. On Sessions its inline sections are
Sessions/Automations/Knowledge, Search/Filter, Settings, and New Session. Filter is a single-line entry without subtext; each action
retains its existing search or managed-sheet owner. One generic selection callback feeds an exhaustive root-mode switch between the
...
are not persisted. All-mode search retains its bottom-left reveal/close interaction;
Sessions opens its existing search overlay from the logo menu instead. Neither search field permanently
occupies dashboard space. Upcoming retains its current agenda or neutral empty state while
