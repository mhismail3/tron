...
tool grouping retain their visual row identities. Run at least fifteen sequential tool-only assistant messages while prior runtime states remain retained: each exact finalized producer group must keep its own chip, the newest single call must never become **Using 15 tools**, and pressing Stop must not be required to restore grouping. Agent text and thinking traces reveal only newly admitted words while
...
On a physical device verify solving-to-thinking-to-hidden expiry, simultaneous synchronous and asynchronous rows, and live-to-terminal updates. The composer subagent orb must enter and leave with the same scoped spring as the catch-up arrow, and its Subagents sheet opens at medium; Subagent History and a tapped child transcript retain their larger initial presentation. Row taps present a bottom sheet instead of a rightward push; active/completed rows keep compact lifecycle/mode pills beneath the title, plain right-aligned durations, Liquid Glass containers only in the bounded active sheet, scroll-optimized history containers, and duration-scaled solving/thinking orbs; short/empty child transcripts stay top-aligned while long newest pages open at the tail; and canonical history paging, transcript refresh, VoiceOver, large Dynamic Type, and Reduce Motion remain correct. Assistant bash—including `nohup x &`—remains ordinary transcript/tool activity and never appears in Subagents.
