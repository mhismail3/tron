...
detail titles, Done actions, icons, and cards explicitly own their category theme rather than inheriting the overview tint; Technical JSON stays neutral gray. Skill chips use the same cyan as their picker, not the general information-blue palette. Subagent pills use their card accent for icons and text, with compact vertical padding. Started timestamps and terminal timestamps (history, immediately before elapsed duration) share monospace styling; missing terminal times stay absent. Session History entry details have no end-of-content or metadata footer; navigation controls appear only for multipart content. Verify package/inline extension names
