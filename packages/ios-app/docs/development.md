...
Runtime Behavior uses the standard Liquid Glass group surface; Model Defaults order is Model, Thinking, Context Window, and the shared slider host and row identity fences remain unchanged. Custom Models uses the standard blue field theme for its Models editor.
