...
Sessions, Automations, and Knowledge compose `DashboardChrome`: a fixed, leading
...
dashboard's accent; long headings fit narrow/accessibility layouts without shrinking
on scroll. Only the backdrop observes `DashboardHeaderState`; each concrete list or
scroll view feeds inset-normalized geometry through `tronDashboardScroll`. Content,
...
stability, geometry, accents, accessibility sizing, and search routing.
