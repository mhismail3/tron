...
post-reveal marker/geometry evidence settles the opening target and the exact release callback is consumed. `ChatTranscriptPresentationStoreTests` use a
...
evidence. The 256-record process-local ring evicts informational records before warning/error evidence;
when every retained record has diagnostic priority it falls back to FIFO. It it intentionally records no prompt,
...
The native orb is a minimal emerald adaptation of the upstream thinking-orbs motion language rather than a geometry port. Active work uses sixteen larger depth-aware points in a slowly rotating, slice-shifting sphere; recent resting work preserves the original spherical ribbon projection and two traveling waves while joining reduced mirrored samples into eleven continuous cubic strands with rounded caps instead of 208 separate dots. A stable animatable Canvas owner crossfades the two live geometries over 340 milliseconds when lifecycle changes mode, computes both only during that transition, and otherwise renders one mode. Keep `Sources/Resources/ThirdPartyNotices/thinking-orbs-LICENSE.txt` in the application resources and preserve deterministic coverage for primitive counts, bounds, motion, and painter order. The composer's matched-geometry glass transition owns the button's morph-away instead of inventing cross-mode geometry. Reduce Motion renders a deterministic frame, and explicit visibility plus scene inactivity pause the `TimelineView`.
