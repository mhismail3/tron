...
evidence. The 256-record process-local ring discards an unchanged composer-availability repeat outright and reclaims the oldest composer-availability samples before the geometry, viewport-transition, and submission-checkpoint samples, which go before any other info record; `chat.context.begin` outlives them and warnings and errors are evicted only as a last resort, falling back to FIFO when every retained record has diagnostic priority. Command publication, applied native-target ownership, and pending release are separate diagnostic bits. It intentionally records no prompt,
...
Every Project asks the Gateway for the global scope and Current Project for the project path; an untrusted project states that its hooks are not loaded and links to Project Trust. The Hooks row uses the Agent group's purple like its siblings, while the sheet keeps the hook inventory's teal content, as Extensions keeps its resource colors; its By Event view is the default, while By Extension remains available for ownership-oriented inspection.
