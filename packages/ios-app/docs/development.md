...
enable Send; attachment-only submissions show their chips without an empty user bubble. Prompt-template submissions show the original typed text as entered, never a placeholder or the expanded instructions; with no input text, only the prompt chip and any attachments remain. Verify this remains stable across optimistic → pending/queued → canonical handoff and history reload, with files/photos retained; the chip still opens the template contents. Long-press a user text bubble to open an anchored **Copy** popover. Its contents are ordinary app-authored buttons, not a system context menu, so iOS cannot inject Ask Siri into this surface. Compact-width devices retain a popover rather than expanding it into a sheet. Copy captures the complete displayed text at opening, including whitespace and Unicode; it never includes expanded prompt instructions, resource metadata, or attachment bytes. Sending/pending cards and Automation prompts share this surface; queues retain authorized reorder/clear actions alongside Copy, while read-only/busy queues retain Copy alone. Empty text offers no Copy, and empty read-only cards do not open an empty popover. The bounded bubble owns the hit region; separate resource/attachment actions remain unchanged. VoiceOver exposes Message actions, choosing an action dismisses the popover, and removing the source clears its ephemeral selection. No context-menu lifecycle scan, Siri-specific hook, polling, or copied-state timer is used. `SessionSheetPresentationTests` guards the absence of system context-menu interactions across user-message states and the prompt-provenance boundary. Template substitution determines model-input ordering, not when the chip was added. Queue editing and sending retain raw arguments; the display-only suppression never changes model input. Skill-only submissions retain their existing chip-only presentation. Ordinary
