...
- **8E blobs/exports in progress:** all export blob responses now use the ephemeral bounded transport and reject declared/streamed bodies above 25 MiB before publication. Gateway transient blobs also reject one value above 25 MiB and enforce exact-content-deduplicated ceilings of 128 items/200 MiB plus 30-minute idle expiry without evicting an ID already published to a client. Excess projected images degrade to bounded omission text, and generated exports are size-checked before their first full-file read. Gateway filenames reduce to a bounded final component, unique protected/backup-excluded temporary directories prevent overwrite/traversal, route retirement removes late artifacts, and each export prunes malformed or older-than-24-hour entries. Single-flight presentation ownership prevents concurrent result races, cancellation removes late artifacts, dismissal discards the current artifact, and app-model construction performs best-effort age cleanup. File-streamed downloads remain.
