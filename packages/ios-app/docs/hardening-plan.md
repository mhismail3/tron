...
- **3C scoped configuration/drafts — in progress:** 3C.1 moved security-scoped file access, read/upload sequencing, and exact lifecycle/profile admission into `SessionImportCoordinator`; the existing `session.import` wire mutation remains in `SessionMutationService`. Remaining work moves typed configuration and composer attachment/draft owners without duplicating Gateway state.
