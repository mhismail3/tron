...
Status: implementation in progress — Phases 0–2, Phase 3A–3C, and the separate provisional UI removal milestone are complete. Phase 6.0 source characterization/provisional budget decisions and the Phase 6.1 source implementation are complete, but their physical pixel, accessibility, frame, and memory acceptance remains pending and the Phase 6 exit gate is not met. The common Phase 4/5 chat path now has deterministic scroll ownership, exact-tagged off-main atomic projection, one raw-atom/global assembler with exact ordinal fragment reuse, flat sparse runtime-tool overrides, isolated streaming suffix sharing, bounded latest-tail/page contracts, exact post-layout opening-tail settlement, the completed tool-detail redesign, and role-aligned animated chat activity presentation. Phase 6.2 bounded Markdown/thinking caching, Phase 6.3 bounded media loading, final physical-device chat acceptance, and the remaining Phase 3D/3E cleanup are still pending.
...
- Direct Request/Result JSON formatting is correctly detail-only, but very large technical payloads still need the remaining Phase 6 rendering budget and laziness review.
...
2. Implement the approved Phase 6.2 Markdown/thinking cache ratchets, then Phase 6.3 bounded deduplicated media/image loading; physically calibrate both before tightening provisional limits.
...
- **6.2 pending:** cache Markdown revisions and attributed thinking segments under a shared provisional 4 MiB accounted source/presentation-byte budget, at most 512 Markdown revisions, at most 4,096 thinking segments within the same budget, and at most 320,000 bytes for any individual cached source (the exact Gateway projected-content ceiling). Preparation concurrency is two and admission is newest-only per identity. These are ratchets pending physical memory/frame calibration, not permission to retain canonical mirrors.
...
- **6.3 pending:** add a byte-bounded attachment thumbnail loader keyed by profile/auth epoch plus blob ID. Its provisional ratchets are 4 MiB decoded, 64 items, 192-pixel maximum thumbnails, one fetch/decode per identity, 25 MiB encoded admission, and one uncached full preview. It must deduplicate requests, downsample off-main, respect cancellation/memory pressure, and preserve characterized retry/loading/preview behavior while limiting full-resolution lifetime to the preview flow. Physical calibration is still required.
