...
**Milestone 1 — Identity and invalidation defects: in progress.** Settings, provider/model catalog, package, and custom-model views now key reloads to explicitly named event-only invalidation generations. Successful publications no longer advance those generations, and duplicate initial settings tasks were merged into one owner. Settings surfaces pass their selected scope/CWD explicitly, and global updates no longer inherit a selected project's path. Watchdog-bounded scripted Gateway tests prove successful reads cannot trigger self-sustaining reload loops. Remaining typed target identity, route, attachment/editor ownership, dashboard-state, and bounded-notice work remains pending in this milestone.
