...
- **7F file/UI ownership — terminal split complete; remaining UI splits pending:** terminal sheet composition, presentation lifecycle/error state, and the native SwiftTerm/keyboard renderer now have separate source owners. Continue splitting oversized onboarding/settings/chat files and move large resolved resources/packages behind existing on-demand detail sheets while preserving the same UX.
