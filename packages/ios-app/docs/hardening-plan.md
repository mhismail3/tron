...
Status: repository implementation and Phase 9 cleanup are complete for Phases 0–7 and Phase 8A, 8B, 8D–8G. The common chat path has deterministic scroll ownership, exact-tagged off-main atomic projection, installation-owned tool payloads, bounded latest-tail/page contracts, exact post-layout opening-tail settlement, bounded Markdown/thinking preparation and media loading, and the completed tool-detail presentation. Remaining work is external or approval-gated: Phase 8C Share Inbox/retention policy, physical pixel/accessibility/frame/peak-memory acceptance, and safe forward paging for detached-history eviction. Incremental-prefix Markdown parsing remains a non-gating optimization deferred behind exact cold differential proof.
...
**Provisional UI removal — complete as a separate user-directed serial milestone.** The composer waveform is kept as a subtle nonstructural bottom activity effect, while the custom subagent session-management sheet remains removed from chat presentation. Bounded extension widget/status state remains canonical, but all native extension widgets and extension status pills are temporarily presentation-gated while chat layout acceptance proceeds. Interaction sheets, working/retry state, title/editor behavior, notices, and durable extension transcript entries remain active. Session kind/classification and dashboard filtering remain canonical; the dashboard retains user sessions and ordinary forks while hiding subagent backing sessions. This milestone does not advance or implement any pending numbered hardening milestone.
