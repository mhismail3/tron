...
- **8B profiles/enrollment in progress:** transactional metadata/Keychain replacement and attempt-owned pairing commit are complete. Profile metadata now commits as one document after atomic Keychain upsert; deterministic failures prove exact credential rollback, new-token cleanup, removal rollback, corrupt-document cleanup, and legacy migration. Validated persisted endpoints and removal of force-unwrapped URL construction remain.
