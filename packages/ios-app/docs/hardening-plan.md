...
- **3C scoped configuration/drafts — in progress:** 3C.1 moved security-scoped file access, read/upload sequencing, and exact lifecycle/profile admission into `SessionImportCoordinator`; 3C.2 extracted settings/trust projection and request ownership; 3C.3 extracted provider/model catalog and operation-keyed authentication ownership. Existing session wire mutations remain in `SessionMutationService`. Remaining work moves package/custom-model configuration and composer attachment/draft owners without duplicating Gateway state.
