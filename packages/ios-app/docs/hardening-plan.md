...
- **7F file/UI ownership — terminal and settings-shell splits complete; remaining UI splits pending:** terminal sheet composition, presentation lifecycle/error state, and the native SwiftTerm/keyboard renderer have separate source owners. The settings shell now delegates unchanged appearance, connection/import, provider, and agent-default presentations to cohesive files. Continue splitting oversized onboarding/chat files and move large resolved resources/packages behind existing on-demand detail sheets while preserving the same UX.
