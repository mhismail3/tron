...
**Session subagent activity — complete as a separate user-directed serial milestone.** The composer owns one session-level subagent control driven by a shallow Gateway-authored overview: a sparse sphere of larger emerald points represents active synchronous/asynchronous subagents, and rounded continuous curved strands represent only terminal subagents still inside the exact five-minute recency window. Assistant bash remains ordinary transcript/tool activity. Per-extension pills and ambient extension status/widget/service sections are retired; Manage Session owns canonical paginated Subagent History. Structured async lifecycle remains active independently of its settled launcher tool. Validated subagent rows may open a connection-owned, parent-authorized read-only canonical transcript lease without acquiring another runtime; interactive extension sheets, working/retry state, title/editor behavior, notices, and durable extension transcript entries remain active. Session kind/classification and dashboard filtering remain canonical; the dashboard retains user sessions and ordinary forks while hiding subagent backing sessions.
...
- **Complete:** eligible text/thinking/image streaming shares the immutable canonical row/identity prefix and computes only the isolated live suffix. Structural changes, canonical-result joins, ambiguous tool placement, and cold rebuilds still use fragment reuse plus the sole linear global assembler; cold/incremental parity is exact across rows, order, tools, and semantic maps.
...
- **Complete:** exact page install advances a layout epoch carried by the row geometry transform, and prior marker frames cannot be re-admitted as current evidence when their numeric frame is unchanged. Prepend passively waits for an exact current-epoch sample, pairs a strictly newer exact sample with a newer geometry revision after each correction, permits at most one late correction, completes the corrected programmatic point correction after semantic settlement, and succeeds only within one point. Catch-up, opening-tail ownership, and outstanding non-prepend commands reject prepend admission. No next-frame assumption or total tail/content height drives correction.
