...
- **8D uploads in progress:** the Gateway serializes staging admission against 25 MiB per upload/prompt, 128-entry, and 200 MiB default aggregate ceilings; duplicate prompt IDs fail closed. HTTP body accumulation separately admits at most four default maximum-sized concurrent bodies, accounting for chunk plus contiguous-buffer ownership, and releases admission on every terminal path. Unclaimed uploads expire after 24 hours, malformed/partial folders self-clean, successful import staging is removed, and canonical session deletion removes its claimed attachment folders without turning cleanup failure into command ambiguity. iOS now rejects non-regular/empty/oversized import and file-picker values before reading, verifies exact size after mapped reads, releases security-scoped access before network suspension, and admits at most 10 staged attachments totaling 25 MiB across completed and concurrent uploads. Upload response accumulation uses the shared ephemeral transport with a 64 KiB ceiling and revalidates the exact connection epoch. Draft image chips now render only orientation-correct 192-pixel PNG previews after upload, with 1 MiB decoded and encoded ceilings; invalid images retain the established generic photo fallback. The original payload remains within the existing presentation-wide 25 MiB budget solely to preserve the explicit preview, whose full decode is now off-main and bounded to 4,096 pixels/64 MiB. Streamed file-backed request transport remains; presentation-retention changes remain approval-gated.
- **8E blobs/exports in progress:** export blob responses use a file-backed URLSession download and reject declared/streamed bodies above 25 MiB without response `Data` accumulation. Completed files move into protected staging capped at 32 files/24 hours before atomic artifact adoption. Gateway transient blobs also reject one value above 25 MiB and enforce exact-content-deduplicated ceilings of 128 items/200 MiB plus 30-minute idle expiry without evicting an ID already published to a client. Excess projected images degrade to bounded omission text, and generated exports are size-checked before their first full-file read. Gateway filenames reduce to a bounded final component, unique protected/backup-excluded temporary directories prevent overwrite/traversal, route retirement removes late staging and artifacts, and each export prunes malformed or older-than-24-hour entries. Single-flight presentation ownership prevents concurrent result races, cancellation removes late artifacts, dismissal discards the current artifact, and app-model construction performs best-effort age cleanup. Gateway export blobs remain memory-backed pending active-reader file leases; upload streaming remains separate.
- **8F dynamic data/pagination in progress:** fresh per-operation Gateway coders now enforce dynamic JSON depth, total-node, per-collection member, per-string UTF-8 byte, and aggregate-string ceilings before observable publication. Non-finite numbers fail coding and integer projection is exact/range-safe; focused boundary and concurrent-decoder tests ratchet the policy. Canonical session discovery now omits every file sharing a duplicate embedded session ID and rejects open/delete deterministically until repair, without choosing by traversal order. Provider projection is atomically bounded to 1,000 unique IDs and 4 MiB of strings. Model projection is bounded to 50 pages, 25,000 compound-identity-unique rows, and 16 MiB of retained strings, and rejects pages above the requested 500 items. Gateway model cursors bind offsets to a whole-catalog fingerprint so a mutation fails closed instead of mixing generations. Audit remaining page/item ceilings and duplicate-ID admission across other non-session catalogs.
