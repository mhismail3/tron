...
**Milestone 3 — Domain-owner extraction: in progress.** Phase 3A is complete. `SessionCatalogCoordinator` owns the disposable dashboard summary rows, retained monotonic live-summary overlays, typed latest-load admission, safe cache/disconnect installation, and removal. `GatewayLifecycleCoordinator` now solely owns enrollment, selected-profile lifecycle, observable connection projection, generation/connection admissions, serial retire/close transitions, reconnect, foreground reconciliation, and final teardown. `AppModel` composes both through observable compatibility façades and narrow projection hooks; it no longer stores a second lifecycle phase, pairing/reconnect/foreground task, connection identity, or transition waiter collection. Same-profile receipt resolution retains generation admission while connection-owned terminal results must reattach after replacement. Transport pagination, hidden/local selection policy, cache I/O, and navigation remain explicitly outside these owners. Phase 3B is next.
