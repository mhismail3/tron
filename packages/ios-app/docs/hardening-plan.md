...
- **8E blobs/exports in progress:** all export blob responses now use the ephemeral bounded transport and reject declared/streamed bodies above 25 MiB before publication. Gateway filenames reduce to a bounded final component, unique protected/backup-excluded temporary directories prevent overwrite/traversal, route retirement removes late artifacts, and each export prunes malformed or older-than-24-hour entries. Single-flight presentation ownership prevents concurrent result races, cancellation removes late artifacts, dismissal discards the current artifact, and app-model construction performs best-effort age cleanup. File-streamed downloads remain.
- **8F dynamic data/pagination in progress:** fresh per-operation Gateway coders now enforce dynamic JSON depth, total-node, per-collection member, per-string UTF-8 byte, and aggregate-string ceilings before observable publication. Non-finite numbers fail coding and integer projection is exact/range-safe; focused boundary and concurrent-decoder tests ratchet the policy. Canonical session discovery now omits every file sharing a duplicate embedded session ID and rejects open/delete deterministically until repair, without choosing by traversal order. Audit remaining page/item ceilings and duplicate-ID admission across non-session catalogs.
