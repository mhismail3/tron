...
- **8F dynamic data/pagination in progress:** fresh per-operation Gateway coders now enforce dynamic JSON depth, total-node, per-collection member, per-string UTF-8 byte, and aggregate-string ceilings before observable publication. Non-finite numbers fail coding and integer projection is exact/range-safe; focused boundary and concurrent-decoder tests ratchet the policy. Audit remaining page/item ceilings and duplicate-ID admission across non-session catalogs.
