...
Status: implementation in progress — Phases 0–2, Phase 3A–3C, and the separate provisional UI removal milestone are complete. The common Phase 4/5 chat path now has deterministic scroll ownership, exact-tagged off-main atomic projection, canonical-prefix reuse for ordinary streaming, a single raw-atom/global-assembler cold kernel, bounded latest-tail/page contracts, exact post-layout opening-tail settlement, the completed tool-detail redesign, and role-aligned animated chat activity presentation. Exact fragment reuse, sparse affected-tool patching, Markdown/thinking and media budgets, final physical-device chat acceptance, and the remaining Phase 3D/3E cleanup are still pending.

Source and owning documentation in the current worktree are authoritative. The justified-user-text and animated notification/tool-pill checkpoint is committed at `d893205b4`; the subsequent common projection-kernel extraction is the current uncommitted checkpoint. Untracked `.pi` runtime artifacts and historical audit line numbers are not.
...
2. Add sparse affected-tool and changed-canonical-entry projection with exact cold/incremental parity.
3. Budget/cache Markdown and thinking parsing, then add bounded deduplicated media/image loading.
4. Resume Phase 3D/3E terminal/event reducer and direct-boundary cleanup.
5. Do not implement bidirectional detached-history eviction until Gateway provides a safe forward-paging/reload contract.
...
- **Kernel checkpoint complete:** one Sendable cold kernel builds exact per-entry raw atoms and is the sole global assembler for bootstrap filtering, call/result joining, orphan suppression, exact ordinals, barriers, grouping, runtime-tool placement, compatibility normalization, and complete semantic maps. The public cold oracle and detached worker both use it; the output-producing test builder was replaced by a delay-only `HOSTED_TEST`-only work gate immediately before the production kernel. Aggregate work reports contain only mode and numeric entry/fragment/tool/atom/rendered counts. Raw fragments retain all explicitly loaded visible history rather than imposing the 512-item page cap. Fragment reuse and tool-row patching remain pending.
