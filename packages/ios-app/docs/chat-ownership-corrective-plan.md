...
Status: active SwiftUI-preserving hardening. Milestone 3 transcript ownership remains implemented: the session snapshot is the sole whole-session authority and the mounted transcript window retains only an exact prefix before the Gateway tail. The completed first slices give anchored and anchorless paging one coordinator-owned operation, cancel it on background, propagate layout settlement/abandonment/overflow to the sole viewport command owner, revision-scope repeating participants, close the pre-admission entrance race, promote pending insertions after abandonment, freeze transcript-coupled composer chrome with the installed commit, and prevent opening starvation by revealing one complete same-runtime commit before coalescing to newest. A sparse positive-start open now resolves one optional exact page inside the opaque opening transaction, bounded to a combined 512-row recent window and a one-second request deadline, so hydration cannot install a second physical spine immediately after readiness. SwiftUI remains the renderer; composer ownership, animation constants, and visual styling are unchanged. Physical rows retain one lazy collection parent through send, canonical acknowledgement, and successor insertion. One surrounding transcript target layout contains the exact row IDs and eager marker. Native underflow alignment replaces the forced minimum-height machinery; short and long sends use the same materialization path. Canonical lease transfer consumes the original entrance entitlement, transient entrance geometry cannot retarget an unsettled lease, and marker jitter cannot renew a repair budget. Hosted regressions measure mounted native frames, composer clearance, and persistent SwiftUI host identity across short sends, oversized sends entering overflow, long mixed-height history, streaming/appends through the inset boundary, and viewport contraction. Bounded content-free trace records distinguish evidence freshness and retired contexts. Further coordinator reduction, integrated reconnect coverage, and physical flow acceptance remain open; this status makes no physical acceptance claim.
...
- Implemented without extending the sync quarantine: before the first render commit mounts, a positive-start window below 512 rows resolves one optional exact backward page through the mounted subscription owner. Exact runtime/leaf/total/range/next-ID/target admission is reused, the combined recent window is trimmed to 512, and a failure or one-second deadline silently retains the usable tail.
...
- Complete for opening churn: automatic intake pauses behind the opaque surface, optional recent-tail hydration resolves before the first render commit, and the external presentation remains noninteractive through current non-lifted post-reveal physical evidence and exact target release. Missing proof fails behind the cover instead of becoming visible and repairing later.
