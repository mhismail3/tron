...
- During open/sync, if a fitted legacy/current baseline has canonical history but fewer than the bounded recent-window target, fetch one exact backward page under the provisional subscription before publishing the presentation. Validate runtime, total, range, next ID, branch, and token after suspension.
