...
semantic-restore, or prepend settlement evidence arrives, then release only that token on the next presented frame. Submission binds its exact stable physical row target to the `ChatLayoutTransaction` shared by outgoing-row entrance, morph, keyboard, and submission. Materialization is necessary but not sufficient: after every participant settles, two unchanged display boundaries and a final evidence check must cross before the binding is cleared. One mode-qualified native size-change anchor then owns continuous streaming and payload growth. A genuinely new lazy physical row
