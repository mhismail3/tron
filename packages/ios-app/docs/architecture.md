...
records stay visible. Observation coverage is an informational overview card (settled breakdown plus one
button naming the cuts that need attention) that opens `KnowledgeCoverageDetailSheet`, a standard
medium/large managed sheet listing those cuts; the Gateway page is filtered to
