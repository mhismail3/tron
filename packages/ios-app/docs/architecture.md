...
surface, and removal is an explicit ID list. Ambient extension statuses, widgets, and service activity are not composer content. The semantic extension host remains the transport owner for interactive prompts, editor leases, and form v1 interactions. A form is one authoritative, epoch/revision-scoped batch of one to four choice questions. The app keeps only an ephemeral ID-keyed draft, submits every answer atomically, and never mirrors or replays the form; read-only extension presentation does not create a composer affordance or Manage Session destination.
