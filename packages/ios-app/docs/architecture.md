...
queued, edited-queue, and canonical text identical. The inline glass picker remains inside the sole composer safe-area owner, below attachments and the skill chip but immediately above the input row. One permanently mounted, bottom-aligned host receives the finite keyboard-reduced chat height, reserves the input and accessory surfaces, and gives only the picker panel the remaining flexible height; the picker's list is the only internally scrolling region. Picker opening and closing use one bounded aggregate-height and fade/slide transaction, so the native safe-area inset pushes the transcript in step with the panel instead of installing a final height before its first visible frame. Extension and subagent pills are not clipped by a structural host, multiline growth shortens the picker before moving the input, and Reduce Motion retains only the nonspatial fade. No composer geometry path issues a scroll-coordinator command. Multiline measurement remains direct, UIKit owns keyboard motion plus the one responder, and UTF-16 selection and caret geometry stay native. The active lease is the immutable session/presentation generation plus lifecycle generation.
