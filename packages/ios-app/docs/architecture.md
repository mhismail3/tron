...
entrance only when the outgoing, pending, or queued representation first appears. That receipt is owned outside lazy row-local state and retained only while the lifecycle identity remains installed, so eviction/remount and opening-to-ready changes cannot replay it. Structural transcript transactions are disabled at the stack boundary; only explicitly tagged entrance, shallow tool-chip, and continuous native-control transactions cross it. A lifecycle-only graft requests its own pinned tail follow without consuming the semantic-anchor mutation boundary reserved for a newer authoritative payload install. Canonical settlement consumes the exact
