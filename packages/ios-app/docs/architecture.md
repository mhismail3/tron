...
while a possibly-sent transport outcome retains the row and captured IDs without replay. Once transport returns an operation ID, canonical reconciliation uses only that exact identity; text and attachment matching remain a bounded pre-ID compatibility path. A later sequenced operation failure restores the exact accepted submission once, retires its lifecycle row, and releases the composer rather than leaving a permanent Sending owner. Retired completions publish neither restoration
