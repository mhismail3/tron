...
Command/prompt display excerpts retain at most 480 characters and 10 source lines after Markdown preparation;
skill bodies retain the complete admitted projection. A muted-gray footer reports either local or Gateway truncation.
These are display bounds only: the 96-KiB transport admission, invocation identity, canonical resource, and loaded
byte-count facts are unchanged.
...
constructed by the mounted history sheet. Live extension activity remains a
separate bounded snapshot projection; it has no independent mobile history cache.
...
is unchanged. Project Resources, Session History, and Subagent History use blue titles and
