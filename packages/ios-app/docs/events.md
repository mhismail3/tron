...
- custom messages carrying Gateway-authored `sessionInput` evidence enter the ordered timeline as right-aligned conversation input, including hidden extension wake messages that triggered or continued an agent turn. The receipt and target are canonical, while producer attribution is optional and never guessed. Non-triggering custom messages remain ordinary extension/tool rows;
