...
The event consumer retains bounded category/phase aggregates rather than per-event traces; session event intake places suffixes in the synchronization quarantine synchronously and hands open/sync work to one owned task, so a slow mounted read cannot block unrelated lifecycle, catalog, auth, or terminal delivery. Optional notification inbox
...
ordinary relaunch only; test clients remain memory-only, and persistence failures cannot affect transport or presentation. The production app composes this sink explicitly; it retains only structurally admitted iOS incident records under a fixed count, byte and age cap, while test-created `AppModel` instances have no sink by default. Offline Logs publish local rows before any remote profile fetch and preserve failed-source status; the Logs surface also fences publication and cleanup to its captured presentation activity and request generation. Export Diagnostics combines the bounded iOS AppLog records, the last retained Gateway log tail, app build identity, and OS version as JSONL provenance data; these are bounded projections rather than canonical state. What each record stream contains, at which level, and where it lives is owned by [observability](../../gateway/docs/observability.md). When connected to a Gateway advertising `diagnostic-export.v1`, the existing `system.logs.export` RPC saves the bundle in the Gateway's private device-export store and returns its path; the app copies the path in the same action. Offline or failed uploads open the native activity sheet immediately with a bounded device-local artifact, without a second preparation/share tap. The export mutation continues if its presenting Logs surface retires, while stale presentation work cannot show the sheet or success notice. Persisted invalid-response incidents retain only the typed event code at the storage boundary. Gateway heartbeat/close diagnostics include bounded inbound/write-progress ages and outbound queue counters without payloads. Reconnect scheduling/attempt/failure, foreground/background transitions, and coarse Network.framework path changes are recorded as bounded lifecycle incidents independently of event-consumer delivery; successful ping/pong and socket writes update progress evidence, while unknown-before-success remains distinct.
...
- `notification.inbox.changed` is a global invalidation only. On connect or invalidation, each selected or admitted background profile requests one newest page plus the Gateway-authoritative unread count; older pages are requested only when the mounted history scroll reaches its paging sentinel. Every page stays on its captured connection and exact revision, and the coordinator bounds retained rows and rejects duplicate identities. Profile buckets aggregate newest-first, while Gateway command receipts own mark-one/mark-all read settlement. A synchronized chat's fresh visible-presentation transition separately acknowledges that session's existing inbox rows; background/technical opens and ordinary visibility renewals do not. The Gateway persists and retries the exact captured notification IDs independently of navigation, then broadcasts the same invalidation. APNs taps may use their bounded request ID to mark the exact canonical row read, but that best-effort mutation never delays exact machine/session navigation;
...
- `session.extensionPresentation` v3 remains the leased transport for semantic updates and native select/confirm/input/editor sheets plus one bounded atomic form sheet. Closing an interaction sheet is local presentation intent, never a cancellation response: the pending Gateway interaction continues waiting, and a bounded device-local draft keyed by session/interaction/epoch/revision restores selections, text, Other activation, and form page across navigation or app restart. Successful response or authoritative retirement removes that draft. The mounted chat route waits for its first ready transcript frame before presenting a pending interaction and remains the sole sheet owner, preventing sheet coverage from cancelling session opening or creating presentation loops. The managed item-sheet identity carries the exact session, interaction ID, host epoch, and presentation revision, so a same-ID successor retires the old presentation token before replacement; accepted responses continue under the mutation owner while stale presentation callbacks are suppressed. Post-await effects use the shared token authority and fail closed when a coordinator is present without a token; only standalone sheets fall back to ambient activity. A dismissed pending Tron-owned or explicitly admitted compatibility `ask_user` form reopens through that route from its operation/owner-matched tool chip, while canonical completed tool-result details reconstruct the same questions and submitted answers read-only without retaining a second interaction mirror. Tron-owned `ask_user` uses the release source `tron:ask-user.v1`; the audited foreign source remains historical compatibility rendering only. Read-only statuses, widgets, and service activity create no ambient transcript or composer content and no Manage Session surface; retained string widgets, bounded read-only widget frames, and owner-attributed keyed statuses are reachable only through the single user-opened **Extension content** sheet, whose compact composer entry point appears only while presentable retained content exists, while discrete extension events remain notification pills. A retained status is presentable on its own, so state that outlives its widget (a paused goal clears its widget and keeps only its status) stays inspectable instead of disappearing. Interactive prompts and editor ownership are unchanged. In particular, there is no Pi Subagents composer pill or generic extension summary route. Normal canonical `subagent` calls remain ordinary transcript tool chips, while the process orb/native **Subagents** sheet is the sole above-composer progress surface;
...
A temporary catch-up notice is deduplicated, removed on successful synchronization and presentation navigation/close, and automatically expires after a bounded interval if a terminal retry path cannot converge. It never outlives its presentation owner. Transport recovery has a separate episode-scoped two-second presentation grace driven by the owning injected clock: it preserves the mounted Chat projection and never grants command authority or Send admission. The timer is fenced and clears its stored task on cancellation or stale completion. Only successful mounted reconciliation clears its Retry/Logs warning; a provisional hello or path hint cannot clear a warning while synchronization remains unusable. Session event reduction is synchronous at intake; a bounded coordinator quarantine is claimed before any owned synchronization task starts, and no extra admitted-event queue may bypass event-hub pressure. Satisfied path hints are fanned to admitted secondary profiles, where each entry/generation fence decides whether a parked recovery can accelerate; planned secondary stopping uses its own 90-second maintenance intent without debiting ordinary recovery. Optional provider/settings/device/catalog reads are disposable owner work and cannot delay the mounted baseline or accepted-command receipt owner. Auth completion commits its exact operation/presentation outcome synchronously; any resulting provider catalog convergence runs in the provider owner task and cannot block global control-event intake. Catalog failure admission rejects repeated invalidations/manual refresh after the bounded application-failure allowance until the scoped owner is rearmed.
