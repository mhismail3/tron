...
final class ToolArgumentExtractorTests: XCTestCase {
    ...
    private func makeToolCall(arguments: String) -> ToolCallPayload? {
    ...
    func testExtractsFromToolCallArguments() {
        let toolCall = makeToolCall(arguments: "{\"path\":\"/test.txt\"}")
    ...
    func testToolCallArgumentsTakePriorityOverContentBlock() {
        let toolCall = makeToolCall(arguments: "{\"from_tool_call\":true}")
        let block: [String: Any] = [
