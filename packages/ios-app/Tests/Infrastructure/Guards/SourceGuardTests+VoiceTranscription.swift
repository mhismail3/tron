...
extension SourceGuardTests {
    ...
    func testVoiceWaveformUsesRecorderMetering() throws {
        ...
        #expect(recorderSource.contains("private static let maxRecordingDuration: TimeInterval = 300"))
        #expect(recorderSource.contains("func startRecording() async throws"))
        #expect(recorderSource.contains("Task.sleep(for: .seconds(Self.maxRecordingDuration))"))
        #expect(!recorderSource.contains("func startRecording(maxDuration:"))
