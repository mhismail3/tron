...
extension SourceGuardTests {
    ...
    func testSessionListRowsUseInsetLiquidGlassContainers() throws {
        ...
        #expect(sessionList.contains("static let rowContainerHorizontalInset: CGFloat = 16"))
        #expect(sessionList.contains("static let rowContentHorizontalPadding: CGFloat = 12"))
        #expect(sessionList.contains("static var headerLeadingPadding: CGFloat"))
        #expect(sessionList.contains("rowContainerHorizontalInset + rowContentHorizontalPadding"))
        #expect(sessionList.contains("static var headerTrailingPadding: CGFloat"))
        #expect(sessionList.contains("static let rowContainerCornerRadius: CGFloat = 12"))
        #expect(sessionList.contains("leading: rowContainerHorizontalInset"))
        #expect(sessionList.contains("trailing: rowContainerHorizontalInset"))
        #expect(sessionList.contains(".padding(.leading, SessionListLayout.headerLeadingPadding)"))
        #expect(sessionList.contains(".padding(.trailing, SessionListLayout.headerTrailingPadding)"))
        #expect(sessionList.contains("HStack(alignment: .center, spacing: SessionListLayout.iconTextSpacing)"))
        #expect(sidebar.contains("Button {"))
        #expect(sidebar.contains("selectedSessionId = session.id"))
        #expect(sidebar.contains(".glassEffect("))
        #expect(sidebar.contains(".regular.tint(Color.tronEmerald.opacity(isSelected ? 0.22 : 0.14)).interactive()"))
        #expect(sidebar.contains(".buttonStyle(.plain)"))
        #expect(sidebar.contains(".listRowInsets(SessionListLayout.rowInsets)"))
        #expect(!sessionList.contains("DragGesture(minimumDistance: 0)"))
        #expect(!sessionList.contains("@GestureState"))
        #expect(!sessionList.contains(".offset(boundedDragOffset)"))
        #expect(!sessionList.contains("rowPressedScale"))
        #expect(!sessionList.contains("rowPressedBrightness"))
        #expect(!sessionList.contains("SessionListRowButtonStyle"))
        #expect(!sessionList.contains("outerHorizontalPadding"))
        #expect(!sessionList.contains(".sectionFill("))
    }
