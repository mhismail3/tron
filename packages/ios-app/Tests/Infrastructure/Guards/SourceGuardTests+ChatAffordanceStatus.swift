...
extension SourceGuardTests {
    ...
    func testChatConversationDoesNotMountPassiveEngineCockpit() throws {
        ...
        let groupStart = try #require(cockpitDiscoveryViews.range(of: "struct CapabilityGroupCard"))
        let groupEnd = try #require(cockpitDiscoveryViews.range(of: "struct CatalogVerificationRow"))
        let groupSource = cockpitDiscoveryViews[groupStart.lowerBound..<groupEnd.lowerBound]
        #expect(groupSource.contains("Text(group.narrative)"))
        #expect(groupSource.contains("compactSummaryMetric(group.ownerSummary)"))
        #expect(!groupSource.contains("group.question"))
        #expect(!groupSource.contains(#""OK""#))
        #expect(!groupSource.contains("countBadge"))
        #expect(!groupSource.contains("workerTriggerExplanation"))
        ...
        #expect(cockpitDiscoveryViews.contains("Text(operation.displayName)"))
        #expect(cockpitDiscoveryViews.contains("functionDisplayName(function.id)"))
        ...
        #expect(cockpitOperationDetailViews.contains("Text(operation.displayName)"))
        #expect(cockpitOperationDetailViews.contains("Text(operation.description)"))
        #expect(!cockpitOperationDetailViews.contains("capabilityEvolutionPath"))
        #expect(cockpitOperationDetailViews.contains("provenanceLabel(operation.projectionSourceLabel)"))
        #expect(!cockpitViews.contains("struct AgentStatusCapsuleView"))
        #expect(cockpitViews.contains(#"SheetTitle(title: "Dashboard", color: .tronEmerald)"#))
        ...
        #expect(cockpitTabViews.contains("[.capabilities, .engine, .activity]"))
        ...
        #expect(activitySource.contains("CapabilityIssuesCard(overview: viewModel.overview)"))
