...
struct AppRuntimeModeTests {
    ...
    private let explicitValues: [String?] = [
        nil,
        "",
        "malformed",
        "application",
    ...
    @Test("each Apple hosted marker is authoritative for every audit value")
    func markerCrossProduct() {
        for marker in markers {
            for value in explicitValues {
                var environment = [marker: ""]
                environment["TRON_APP_RUNTIME_MODE"] = value
                #expect(
                    AppRuntimeMode.resolve(environment: environment, arguments: []) == .hostedUnitTests,
                    "marker \(marker), audit value \(value ?? "absent")"
                )
            }
        }
    }

    @Test("ambient audit values never disable an application process")
    func noMarkerCrossProduct() {
        for value in explicitValues {
            var environment: [String: String] = [:]
            environment["TRON_APP_RUNTIME_MODE"] = value
            #expect(
                AppRuntimeMode.resolve(environment: environment, arguments: []) == .application,
                "audit value \(value ?? "absent")"
            )
        }
    }

    @Test("onboarding UI-test arguments remain application launches")
    func onboardingArgumentsRemainApplication() {
        for argument in [
            "--tron-ui-test-onboarding-complete",
            "--tron-ui-test-onboarding-incomplete",
        ] {
            #expect(
                AppRuntimeMode.resolve(environment: [:], arguments: [argument]) == .application
            )
        }
    }

    @Test("hosted bootstrap never evaluates production factory")
    func hostedBootstrapIsLazy() {
        var factoryCount = 0
        let bootstrap = AppBootstrap(mode: .hostedUnitTests) {
            factoryCount += 1
            return "production"
        }

        #expect(factoryCount == 0)
        #expect(bootstrap.productionRoot == nil)
    }

    @Test("application bootstrap evaluates and stores production root once")
    func applicationBootstrapConstructsOnce() {
        var factoryCount = 0
        let bootstrap = AppBootstrap(mode: .application) {
            factoryCount += 1
            return "production"
        }

        #expect(factoryCount == 1)
        #expect(bootstrap.productionRoot == "production")
    }

    @Test("application runtime storage uses supplied defaults and no suite")
    func applicationStorageUsesInjectedDefaults() {
        IsolatedTestState.withDefaults(label: "runtime-application") { defaults in
            let capture = LifecycleCapture(forwardToStandard: true)
            var namedFactoryCalls = 0
            let storage = AppRuntimeStorage.make(
                mode: .application,
                processIdentity: "app",
                applicationDefaults: defaults,
                namedSuiteFactory: { _ in
                    namedFactoryCalls += 1
                    return nil
                },
                lifecycleEmitter: capture.emitter,
                cleanupRegistry: AppRuntimeCleanupRegistry()
            )

            storage.defaults.set("value", forKey: "sentinel")
            #expect(defaults.string(forKey: "sentinel") == "value")
            #expect(storage.suiteName == nil)
            #expect(namedFactoryCalls == 0)
            storage.cleanup()
            #expect(defaults.string(forKey: "sentinel") == "value")
    ...
    @Test("hosted runtime storage owns an observable collision-proof suite")
    func hostedStorageIsDistinctAndCleanable() throws {
        try IsolatedTestState.withDefaults(label: "runtime-sentinel") { sentinelDefaults in
            let capture = LifecycleCapture(forwardToStandard: true)
            let cleanupRegistry = AppRuntimeCleanupRegistry()
            sentinelDefaults.set("protected", forKey: "sentinel")
            var createdNames: [String] = []
            let storage = AppRuntimeStorage.make(
                mode: .hostedUnitTests,
                processIdentity: "process identity",
                applicationDefaults: sentinelDefaults,
                namedSuiteFactory: { name in
                    createdNames.append(name)
                    return IsolatedTestState.namedDefaults(for: name)
                },
                lifecycleEmitter: capture.emitter,
                cleanupRegistry: cleanupRegistry
            )
            let suiteName = try #require(storage.suiteName)

            #expect(createdNames == [suiteName])
            #expect(suiteName.contains("process-identity"))
            storage.defaults.set("hosted", forKey: "sentinel")
            #expect(sentinelDefaults.string(forKey: "sentinel") == "protected")
            #expect(storage.defaults.string(forKey: "sentinel") == "hosted")
            #expect(capture.records == [
                HostedTestSuiteLifecycleRecord(event: .registered, suite: suiteName),
            ])

            storage.cleanup()
            storage.cleanup()
            #expect(
                IsolatedTestState.persistentDomain(named: suiteName)?.isEmpty != false
            )
