...
final class MentionDetectorTests: XCTestCase {
    ...
    func testDetectMention_atTrigger_returnsQuery() {
        XCTAssertEqual(SkillMentions.detectMention(in: "hello @foo"), "foo")
    }

    func testDetectMention_atStartOfString() {
        XCTAssertEqual(SkillMentions.detectMention(in: "@test"), "test")
    }

    func testDetectMention_emptyQuery() {
        XCTAssertEqual(SkillMentions.detectMention(in: "hello @"), "")
    }

    func testDetectMention_noTrigger_returnsNil() {
        XCTAssertNil(SkillMentions.detectMention(in: "hello"))
    }

    func testDetectMention_triggerInsideWord_returnsNil() {
        XCTAssertNil(SkillMentions.detectMention(in: "email@test"))
    }

    // MARK: - Whitespace / Boundary Rules

    func testDetectMention_afterNewline() {
        XCTAssertEqual(SkillMentions.detectMention(in: "line1\n@foo"), "foo")
    }

    func testDetectMention_afterTab() {
        XCTAssertEqual(SkillMentions.detectMention(in: "text\t@foo"), "foo")
    }
    ...
    func testDetectMention_triggerFollowedBySpace_returnsNil() {
        XCTAssertNil(SkillMentions.detectMention(in: "@foo bar"))
    }

    func testDetectMention_triggerFollowedByNewline_returnsNil() {
        XCTAssertNil(SkillMentions.detectMention(in: "@foo\nbar"))
    }

    // MARK: - Backtick Escaping

    func testDetectMention_insideSingleBackticks_returnsNil() {
        XCTAssertNil(SkillMentions.detectMention(in: "`code @foo`"))
    }
    ...
    func testDetectMention_afterClosedBackticks_returnsMention() {
        XCTAssertEqual(SkillMentions.detectMention(in: "`code` @foo"), "foo")
    }

    func testDetectMention_emptyBacktickPair_thenMention() {
        XCTAssertEqual(SkillMentions.detectMention(in: "`` @foo"), "foo")
    }
    ...
    func testDetectCompletedMention_matchesExactSkillName() {
        ...
        let result = SkillMentions.detectCompletedMention(in: "@typescript-rules ", skills: skills, alreadySelected: [])
    ...
    func testDetectCompletedMention_alreadySelected_returnsNil() {
        ...
        let result = SkillMentions.detectCompletedMention(in: "@typescript-rules ", skills: [skill], alreadySelected: [skill])
        XCTAssertNil(result)
    }

    func testDetectCompletedMention_noMatch_returnsNil() {
        let skills = [makeSkill("typescript-rules")]
        let result = SkillMentions.detectCompletedMention(in: "@nonexistent ", skills: skills, alreadySelected: [])
        XCTAssertNil(result)
    }
    ...
    func testDetectCompletedMention_insideBackticks_returnsNil() {
        ...
        let result = SkillMentions.detectCompletedMention(in: "`@foo `", skills: skills, alreadySelected: [])
        XCTAssertNil(result)
    }

    func testDetectCompletedMention_midWord_returnsNil() {
        let skills = [makeSkill("bar")]
        let result = SkillMentions.detectCompletedMention(in: "foo@bar ", skills: skills, alreadySelected: [])
        XCTAssertNil(result)
    }
    ...
    func testFilterSkills_emptyQuery_returnsAll() {
        ...
        let result = SkillMentions.filterSkills(skills, query: "")
        XCTAssertEqual(result.count, 3)
    }

    func testFilterSkills_matchesName() {
        let skills = [makeSkill("typescript-rules"), makeSkill("api-design")]
        let result = SkillMentions.filterSkills(skills, query: "type")
    ...
    func testFilterSkills_matchesDescription() {
        ...
        let result = SkillMentions.filterSkills(skills, query: "typescript")
        XCTAssertEqual(result.count, 1)
    }
    ...
    func testFilterSkills_noMatch_returnsEmpty() {
        let skills = [makeSkill("typescript-rules"), makeSkill("api-design")]
        let result = SkillMentions.filterSkills(skills, query: "zzz")
        XCTAssertTrue(result.isEmpty)
    }

    func testFilterSkills_prefixMatchesSortedFirst() {
        let skills = [makeSkill("api-design"), makeSkill("typescript-api")]
        let result = SkillMentions.filterSkills(skills, query: "api")
