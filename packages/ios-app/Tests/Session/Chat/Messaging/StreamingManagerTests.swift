...
final class StreamingManagerTests: XCTestCase {
    ...
    func testFlushPendingTextIdempotent() {
        ...
        // Second call should not send another update
        manager.flushPendingText()
        XCTAssertEqual(callbackCount, 1, "Second flushPendingText should not send another update")
    }
    ...
    func testResetDuringAnimation() {
        ...
        // Next flush should not send another callback
        manager.flushPendingTextIfNeeded()
