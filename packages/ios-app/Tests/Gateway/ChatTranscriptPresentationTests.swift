...
struct ChatTranscriptPresentationTests {
    ...
    func chatOpenPresentationState() {
        ...
        let wrongVisibleSession = state.beginVisibleReveal(sessionID: "session-b", epoch: epoch)
        let staleVisibleEpoch = state.beginVisibleReveal(sessionID: "session-a", epoch: epoch - 1)
        let visible = state.beginVisibleReveal(sessionID: "session-a", epoch: epoch)
        #expect(!wrongVisibleSession)
        #expect(!staleVisibleEpoch)
        #expect(visible)
        #expect(state.phase == .presented)
        let wrongReadySession = state.installReadyViewport(sessionID: "session-b", epoch: epoch)
        let staleReadyEpoch = state.installReadyViewport(sessionID: "session-a", epoch: epoch - 1)
        let ready = state.installReadyViewport(sessionID: "session-a", epoch: epoch)
        #expect(!wrongReadySession)
