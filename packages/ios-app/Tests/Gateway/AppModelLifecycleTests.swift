...
@MainActor
@Suite("Composer draft app lifecycle", .serialized)
struct ComposerDraftAppLifecycleTests {
    @Test("teardown joins the latest composer checkpoint")
    func teardownComposerCheckpoint() async throws {
        let root = FileManager.default.temporaryDirectory.appending(
            path: "teardown-composer-\(UUID().uuidString)",
            directoryHint: .isDirectory
        )
        defer { try? FileManager.default.removeItem(at: root) }
        let store = ComposerDraftStore(root: root)
        let defaultsName = "TeardownComposerCheckpoint.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: defaultsName)!
        defer { defaults.removePersistentDomain(forName: defaultsName) }
        let model = AppModel(
            client: GatewayClient(),
            profiles: GatewayProfileStore(defaults: defaults),
            cache: SnapshotCache(root: root.appending(path: "snapshots")),
            composerDraftStore: store
        )
        let scope = model.composerDrafts.prepareDraft(
            profileID: "profile", sessionID: "session", initialText: nil
        )
        model.composerDrafts.setText("latest teardown edit", for: scope)

        await model.teardown()

        #expect(await store.load(scope)?.text == "latest teardown edit")
    }

    @Test("background lifecycle checkpoints the latest composer edit")
    func backgroundComposerCheckpoint() async throws {
        let root = FileManager.default.temporaryDirectory.appending(
            path: "background-composer-\(UUID().uuidString)",
            directoryHint: .isDirectory
        )
        defer { try? FileManager.default.removeItem(at: root) }
        let store = ComposerDraftStore(root: root)
        let defaultsName = "BackgroundComposerCheckpoint.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: defaultsName)!
        defer { defaults.removePersistentDomain(forName: defaultsName) }
        let model = AppModel(
            client: GatewayClient(),
        ...
        model.composerDrafts.setText("latest background edit", for: scope)

        await model.enteredBackground().value

        #expect(await store.load(scope)?.text == "latest background edit")
        await model.teardown()
    }
}

@MainActor
@Suite("App background checkpoint ownership")
struct AppBackgroundCheckpointCoordinatorTests {
    @Test("completion ends the assertion and expiration cancels bounded work")
    func completionAndExpiration() async {
        let recorder = BackgroundTaskAccessRecorder()
        let completed = AppBackgroundCheckpointCoordinator(access: recorder.access)
        completed.retain(Task {})
        for _ in 0..<20 where completed.hostedHasActiveCheckpoint { await Task.yield() }
        #expect(!completed.hostedHasActiveCheckpoint)
        #expect(recorder.ended == [1])

        let (cancellations, cancellationContinuation) = AsyncStream<Void>.makeStream(
            bufferingPolicy: .bufferingNewest(1)
        )
        let expiring = AppBackgroundCheckpointCoordinator(access: recorder.access)
        let checkpoint = Task {
        ...
        expiring.retain(checkpoint)
        #expect(expiring.hostedHasActiveCheckpoint)
        recorder.expireCurrent()
        var iterator = cancellations.makeAsyncIterator()
        #expect(await iterator.next() != nil)
        #expect(!expiring.hostedHasActiveCheckpoint)
        #expect(recorder.ended == [1, 2])
    }
}

@MainActor
private final class BackgroundTaskAccessRecorder {
    private(set) var ended: [Int] = []
    private var nextID = 0
    private var expiration: (@MainActor () -> Void)?

    var access: AppBackgroundTaskAccess {
        AppBackgroundTaskAccess(
            begin: { [weak self] expiration in
                guard let self else { return nil }
                nextID += 1
                self.expiration = expiration
                return .init(rawValue: nextID)
            },
            end: { [weak self] identifier in
                self?.ended.append(identifier.rawValue)
            }
        )
    }
