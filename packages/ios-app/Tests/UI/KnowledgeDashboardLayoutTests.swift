...
final class KnowledgeDashboardLayoutTests: XCTestCase {
    ...
    fileprivate static func sourceRecord(title: String = "Fixture retained article", disposition: KnowledgeCaptureDisposition = .complete, admission: KnowledgeSourceAdmissionState? = nil) -> KnowledgeRecord {
        ...
        let savedText = disposition == .referenceOnly || disposition == .metadataOnly ? nil : "Captured fixture text only; this is not a live source."
        let sourceSummary = savedText.map { KnowledgeSourceSummary(text: "A synthetic source fixture saved to verify the library summary layout.", tags: [KnowledgeSourceSummaryTag(label: "Library design", kind: "semantic")], generatedAt: "2026-01-01T00:00:00Z", sourceRevisionId: "fixture-source-revision", evidenceDigest: KnowledgeSourcePresentationPolicy.evidenceDigest(title: title, text: $0), coverage: "full") }
        return KnowledgeRecord(schemaVersion: 1, id: "fixture-source-\(fixtureID)", revisionId: "fixture-source-revision", kind: .source, scope: .research,
                               ...
                               content: .source(KnowledgeSourceContent(title: title, uri: "https://example.test/article", text: savedText, object: KnowledgeObjectRef(hash: hash, mediaType: "text/plain", bytes: 50), mediaType: "text/plain", captureDisposition: disposition, captureReason: disposition == .partial ? "Synthetic partial capture" : nil, annotations: nil, sourcePublishedAt: nil, sourceSavedAt: "2025-12-31T00:00:00Z", capturedAt: "2026-01-01T00:00:00Z", origin: "connector", origins: [KnowledgeSourceOrigin(kind: .connector, capturedAt: "2026-01-01T00:00:00Z", annotation: "Synthetic hosted UI fixture", uri: "https://example.test/article", identity: nil)], identity: nil, assessment: KnowledgeSourceAssessment(summary: "Useful or uncertain intake classification; retained by policy.", contribution: nil, whyItMatters: nil, evidenceQuality: .unknown, freshness: .unknown, possibleUse: nil, generatedAt: "2026-01-01T00:00:00Z", model: "fixture/model", coverage: "full", classification: "technical"), contentSummary: sourceSummary, admission: admission)))
    }
