...
final class WAVFileWritingTests: XCTestCase {
    ...
    func test_headerIs44Bytes() {
        ...
        let url = AudioCaptureEngine.writeWAVFile(pcmData: pcm, sampleRate: 44100)
        XCTAssertNotNil(url)
    ...
    func test_sampleRateInHeader() {
        ...
        let url = AudioCaptureEngine.writeWAVFile(pcmData: pcm, sampleRate: 48000)!
        let data = try! Data(contentsOf: url)
    ...
    func test_emptyPCM_returnsNil() {
        let url = AudioCaptureEngine.writeWAVFile(pcmData: Data(), sampleRate: 44100)
        XCTAssertNil(url)
    }
}
...
final class AudioRecorderTests: XCTestCase {
    ...
    func test_sessionOptions_containsDefaultToSpeaker() {
        XCTAssertTrue(AudioCaptureEngine.sessionOptions.contains(.defaultToSpeaker))
    }
}
