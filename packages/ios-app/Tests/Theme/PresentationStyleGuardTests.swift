...
struct PresentationStyleGuardTests {
    ...
    func topBlurCoverage() throws {
        ...
        #expect(blur.contains("static let height: CGFloat = 56"))
        #expect(blur.contains("static let safeAreaTranslation: CGFloat = 44"))
        ...
        #expect(blur.contains(".repeatForever(autoreverses: true)"))
        ...
        #expect(chat.contains(".offset(y: ChatBottomActivityBlurLayout.safeAreaTranslation)"))
    ...
    func toolDetailPresentation() throws {
        ...
        #expect(runOwner.contains("resolveDetails(run.tools.map(\\.id), installationTag)"))
        ...
        #expect(technicalDetail.contains("accessibilityLabel(\"Inspect \\(title) JSON, \\(summary)\")"))
