...
struct PresentationStyleGuardTests {
    ...
    func boundedTextPreparationOwnership() throws {
        ...
        #expect(store.contains("preparedTextByRenderedID: slices"))
    ...
    func chatStructuralAnimationBoundary() throws {
        ...
        #expect(chat.contains("transcriptRevealAnimation,"))
        #expect(chat.contains("completionCriteria: .logicallyComplete"))
        #expect(chat.contains("beginVisibleReveal("))
        #expect(chat.contains("completeVisibleOpeningReveal()"))
    ...
    func chatScopedMotionOwnership() throws {
        ...
        #expect(morph.contains("freeze the first"))
        #expect(morph.contains("guard flight.phase == .waitingForDestination else { return }"))
        #expect(morph.contains("func failOpen(lifecycleID:"))
        #expect(morph.contains("onFlightEnded"))
        ...
        #expect(entrances.contains("ownership == .flight || ownership == .completed"))
        ...
        #expect(chatView.contains("holdsSubmissionHeight: morphRegistry.flight != nil"))
    ...
    func topBlurCoverage() throws {
        ...
        #expect(blur.contains("case .toolDetail: 108"))
    ...
    func progressiveSettingsSheets() throws {
        ...
        #expect(providerAuth.contains("systemImage: prompt.kind == .manualCode ? \"checkmark.shield\" : TronSaveActionPresentation.systemImage"))
        ...
        #expect(presentation.contains("static let systemImage = \"externaldrive\""))
        #expect(saveToolbarButton.contains("systemImage: TronSaveActionPresentation.systemImage"))
    ...
    func gatewayDetailPresentations() throws {
        ...
        #expect(context.contains("let workingTreeStatus = dirty"))
        #expect(context.contains("subtitle: branch,"))
        #expect(context.contains("Text(workingTreeStatus)"))
        #expect(!context.contains("subtitle: dirty ?"))
    ...
    func removedProvisionalChatSurfaces() throws {
        ...
        #expect(processSheets.contains("SessionProcessRow(process: process, surfaceStyle: .glass)"))
        #expect(processSheets.contains("SessionProcessRow(process: process, surfaceStyle: .scrollOptimized)"))
        ...
        #expect(processSheets.contains("ChatCompactPillSurface(tone: .neutral, material: .flat)"))
        #expect(!processSheets.contains("SessionProcessPill(icon: \"circle.fill\""))
        #expect(processSheets.contains("case .inProgress: .tronAmber"))
        #expect(processSheets.contains("case .succeeded: .tronSuccess"))
        #expect(processSheets.contains("case .unsuccessful: .tronError"))
    ...
    func inlineDisplayContainer() throws {
        ...
        #expect(source.contains("withAnimation(disclosureFadeAnimation, completionCriteria: .logicallyComplete)"))
        #expect(source.contains("withAnimation(disclosureLayoutAnimation)"))
        #expect(!source.contains("DisplayDisclosureClippingModifier"))
        #expect(!source.contains("disclosureClipActive"))
        #expect(source.contains("@Environment(\\.colorScheme) private var colorScheme"))
        #expect(source.contains("Color(uiColor: .systemBackground)"))
        #expect(source.contains("Color(uiColor: .black)"))
    ...
    func floatingDisplayChrome() throws {
        ...
        #expect(chat.contains("bottomExclusion: composerHeightLedger.current"))
    }
    ...
    func thinkingTraceAccessibility() throws {
        ...
        let compactViewport = (block.components(separatedBy: "private func traceViewport").dropFirst().first ?? "")
            .components(separatedBy: "private func paragraph").first ?? ""
    ...
    func sendToChatMotion() throws {
        ...
        #expect(motion.contains("transaction.admitsChatIncrementalGrowthAnimation = true"))
    ...
    func queuedMessagePresentation() throws {
        ...
        #expect(queue.contains("Image(systemName: TronSaveActionPresentation.systemImage)"))
    ...
    func compactToolAndNoticePresentation() throws {
        ...
        let toolPreviewViewport = (toolRuns.components(separatedBy: "private struct ToolRowPreviewViewport").dropFirst().first ?? "")
            .components(separatedBy: "private struct ToolRunSummaryRow").first ?? ""
        let toolSummaryRow = (toolRuns.components(separatedBy: "private struct ToolRunSummaryRow").dropFirst().first ?? "")
            .components(separatedBy: "private struct ToolElapsedClock").first ?? ""
        ...
        #expect(!toolRunLabel.contains("surfaceToneOverride"))
        #expect(compactPill.contains("case .warning: .tronAmber"))
        #expect(compactPill.contains("case .error: .tronError"))
        ...
        #expect(toolRuns.contains("private struct ToolRunSummaryRow"))
        #expect(toolRuns.contains("ToolRunRowPresentation"))
        #expect(toolSummaryRow.contains("HStack(alignment: .center, spacing: 8)"))
        #expect(toolSummaryRow.contains("edge: .bottom"))
        #expect(toolSummaryRow.contains("edge: .top"))
        #expect(!toolSummaryRow.contains("frame(width: 48"))
        #expect(!toolSummaryRow.contains("Output preview bounded; open for full details"))
        #expect(toolPreviewViewport.contains("ToolRowPreviewFadePolicy.showsFade"))
        #expect(toolPreviewViewport.occurrences(of: "LinearGradient(") == 2)
        #expect(toolRuns.contains("@State private var selectedToolPresentation: ChatToolPresentation?"))
        ...
        #expect(chat.contains("SessionContextSheet(sessionID: sessionID) { route in"))
        #expect(chat.contains("forkNavigation.stage(route)"))
        #expect(chat.contains("onDismiss: completeForkNavigationAfterContextDismissal"))
        ...
        #expect(transcriptView.contains("current.hasIndependentViewportMovement(from: prior)"))
        ...
        #expect(chat.contains("TronPulseLoadingIndicator(accent: .tronEmerald, size: 44)"))
        #expect(!chat.contains("Text(\"Opening conversation…\")"))
        #expect(chat.contains(".accessibilityLabel(\"Opening conversation\")"))
    ...
    func composerResourcePickerPresentation() throws {
        ...
        let contentEnd = try #require(
            picker.range(of: "private func loadDetail", range: contentStart.upperBound..<picker.endIndex)
        )
        let contentSection = String(picker[contentStart.lowerBound..<contentEnd.lowerBound])
        ...
        #expect(picker.contains("Circle().fill(accent.opacity(0.15)).frame(width: 28, height: 28)"))
        #expect(picker.contains("Text(entry.friendlyName)"))
        #expect(picker.contains("ChatCompactPillSurface("))
        #expect(picker.contains("ChatCompactPillLabel("))
        #expect(picker.contains("model.commandDetail(sessionID: sessionID"))
        #expect(picker.contains("TronTechnicalMetadataSection("))
        #expect(picker.contains("TronMarkdownView(text: content, streaming: false)"))
        #expect(contentSection.contains(".tronScrollSurface("))
        #expect(!contentSection.contains(".tronGlassSurface("))
        #expect(picker.contains("ComposerResourceContentPresentation.body($0, source: entry.source)"))
