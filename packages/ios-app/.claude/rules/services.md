...
- `DiagnosticsRedactor` - PII scrubber for local diagnostic bundles. Strips bearer tokens (length-only), home-directory paths, and chat message fields before data is attached to feedback. Shared implementation between iOS and Mac (`packages/mac-app/Sources/Services/Observability/DiagnosticsRedactor.swift` is a near-mirror).
- `MetricKitDiagnosticsStore` - Local-only MetricKit subscriber. Stores metric and diagnostic payload JSON under Application Support with retention caps; it never transmits payloads by itself.
- `DiagnosticsBundleBuilder` - Creates the explicit user-generated diagnostics JSON attachment. Includes bounded local/server logs, session summaries, allowlisted event summaries, MetricKit payloads, and a manifest with truncation flags.
- `FeedbackComposer` - Pure value type that builds the subject + body for "Send Feedback". `FeedbackMailView` wraps `MFMailComposeViewController` and attaches the diagnostics JSON when Mail is configured; otherwise Settings presents a share sheet so the user can save or attach the file manually.
