...
9. `scripts/tron-release-notes` generates a bounded draft changelog from first-parent git history since the previous release tag, with DMG, SHA256, and full compare-link details included. The beta2 release intentionally compares against the historical Mac-scoped beta1 tag so the tag-prefix rename does not turn beta2 into an all-history changelog.
