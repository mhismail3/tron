...
`Tron.app` embeds the Rust agent and program worker inside signed helper apps at `Contents/Library/LoginItems/Tron Server.app/Contents/MacOS/` and `Contents/Library/LoginItems/Tron Server Dev.app/Contents/MacOS/`. `Tron Server.app` has bundle id `com.tron.server` for production/local Release; `Tron Server Dev.app` has bundle id `com.tron.server.dev` for isolated Debug install testing. `tron` is the LaunchAgent entrypoint, and `tron-program-worker` must sit beside it for `execute(mode: "program")`. Both helper binaries are gitignored under each helper's `Contents/MacOS/` and produced by:
