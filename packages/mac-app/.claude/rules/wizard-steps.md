...
paths:
  ...
  - "packages/mac-app/**/MacPermissionProbe*"
...
WizardView
  └─ switch state.step in:
       welcome → tailscale → install → permissions → iosBeta → pairingInfo → done
...
| `install` | `InstallStep` | YES — first click starts install; second click continues after `.success` / `.registered` | owns existing-install detection, validates the bundled helper/plist/signature, registers or refreshes the LaunchAgent through `SMAppService`, waits for `system::ping`, and renders registered-service/install-complete states from explicit user action rather than background mutation |
| `permissions` | `PermissionsStep` | Full Disk Access REQUIRED | runs native wrapper probes via `MacPermissionProbe`, opens the Full Disk Access Settings pane, starts a short-lived grant watcher after the gear click, and leaves helper restarts to explicit menu/server paths |
| `iosBeta` | `IOSBetaStep` | NO | shows the public TestFlight invite QR plus copy/open fallbacks |
| `pairingInfo` | `PairingInfoStep` | NO (display-only); "I'm paired" disabled until bearer token, server heartbeat, and a real Tailscale IP are resolved | reads `auth.json`, confirms `system::ping`, probes Tailscale live, resolves the user-facing macOS Computer Name as the iOS server name, then caches `server.tailscaleIp` into `profiles/user/profile.toml`; profile TOML is a fallback/cache, not a fresh-install prerequisite; generates QR via `QRCodeGenerator`; copy actions |
...
Permissions has one row: Full Disk Access. The page intro is the short user-facing sentence "Tron needs Full Disk Access to work with your files." The row subtext says "Lets Tron Server read and edit files.", and the instruction line names the wrapper app the user must enable in Full Disk Access. Permission cards use moderate row geometry (`PermissionsStepLayout.cardHorizontalPadding`, `statusIconColumnWidth`, and `iconTextSpacing`) so the page has breathing room without losing a single-line helper. The inline Re-check action is left-padded to align its icon with the permission status column and says "Checking permissions..." while it runs the native wrapper probe. Do not restore Screen Recording, Accessibility, draggable app shortcuts, or agent-side permission prompt calls in the Mac wizard.

Pairing uses the same glass card language as the other wizard pages: the QR lives in a glass container, and the four manual values (host, port, token, server name) are separate copyable `WizardInfoCard` rows. The QR and manual rows form one centered `pairingCluster`; the value rows have a stable column width and are vertically centered against the QR, and the whole cluster is centered in the remaining body space between intro copy and the pinned buttons. Do not show a raw `tron://pair?...` caption under the QR; it is too long to be useful in the fixed wizard viewport, and the QR/manual rows are the intended user affordances. The initial pairing refresh is delayed briefly when no payload is already cached so a fast QR generation cannot pop in at the final coordinate while the page body is still sliding.
...
1. **`InstallPlanner.plan(paths:existingInstall:) -> Result<InstallPlan, Failure>`**
   Pure-value planner. Takes bundled helper/plist paths plus an existing-install snapshot, returns either a plan or a typed failure (`applicationNotInApplications`, `helperBundleMissing`, `helperBinaryMissing`, `plistMissing`, `helperSignatureInvalid`, `missingTeamIdentifier`, `requiresUserApproval`).
   Also renders LaunchAgent metadata through the bundled plist and expected helper path.
   Tests in `Tests/Services/InstallPlannerTests.swift`.

2. **`LaunchAgentManaging.registerOrRefresh(label:) -> LaunchAgentOutcome` + `InstallLaunchAgentRunner.ensureLoaded(...)`**
   Protocol surface for SMAppService registration/refresh plus bounded launchd restart. The live implementation uses `SMAppService` for registration and falls back to launchd diagnostics/restart only where the current primitive server lifecycle requires it. Tests use `MockLaunchAgentManager`.

3. **`InstallStep` view** — uses short user-facing copy and does not mutate launchd until `WizardState.requestInstall()` increments `installRequestID`. It is the single install gate: `.none` and `.partial` detection results wait for the Install CTA, while `.registered` renders as a completed service state. Before the first click, the stage area shows only "Installation not started"; it must not list all pending work. `WizardState.handledInstallRequestID` records the latest consumed request so remounting the page after navigating back from Permissions cannot replay the pipeline; only a new Install/Retry click creates new work. During the active run, completed rows are derived synchronously from terminal `installOutcome` so check icons slide with the rest of the page instead of popping in from a later `.task`. After explicit user action, it progressively reveals only stages that have started (`validateApplication` → `validateHelper` → `registerAgent` → `awaitPing`). Fast stages intentionally hold the running state briefly so users can perceive the sequence instead of watching three checks appear at once. The ping client uses a string request ID (matching the Rust engine wire type), ignores broadcast frames, and waits for the matching response. Each stage has a pending/running/succeeded/failed(String) state and a retry path. Failure surfaces an `InstallOutcome` that the Pairing step uses for gating.
...
- **Bundled helper artifacts are app resources.** The wizard registers the helper apps and LaunchAgent plists already sealed into `Tron.app/Contents/Library`; it does not copy a new helper into `~/.tron/system/`.
- **Uninstall preserves user data.** `TronUninstaller` removes wrapper-owned registration/runtime artifacts only; auth tokens, provider auth, settings, databases, sessions, workspace files, and non-empty dev/deploy/update artifacts are out of scope.
...
- **Full Disk Access names the wrapper app.** Permissions UI must tell the user to enable the current wrapper app (`Tron.app` in Release, `TronMac.app` in Debug companion), because macOS attributes the LaunchAgent helper through the wrapper bundle identifiers.
...
- **No wizard step writes provider auth or bearer tokens.** `auth.json` is owned by the agent; the wizard only reads the bearer token. If the file is missing on the Pairing step, the user sees "(not generated)" and the pipeline has failed earlier.
...
- `Sources/App/TronMacApp.swift` — `RootView` owns mode + `wizardEntryStep`; `MenuBarHostView` observes `.tronWizardShowPairingInfo`
- `Sources/MenuBar/MenuBarItemBuilder.swift` — emits the `.tronWizardShowPairingInfo` notification
- `Sources/Support/Onboarding/InstallPlanner.swift` — pure planner + plist renderer
- `Sources/Support/Onboarding/{ExistingInstallDetector,MacPermissionProbe,TailscaleProbe}.swift`
- `Sources/Server/TronPaths.swift` — single source of truth for wrapper paths and bundled helper locations
- `Tests/Wizard/{WizardState,WizardStep,MockLaunchAgentManager}Tests.swift`
- `Tests/Services/{InstallPlanner,BearerTokenReader,TronPaths,…}Tests.swift`
