...
| `pairingInfo` | `PairingInfoStep` | NO (display-only); "I'm paired" disabled until bearer token, server heartbeat, and a real Tailscale IP are resolved | reads `auth-token.json`, confirms `system.ping`, probes Tailscale live, then caches `server.tailscaleIp` into `settings.json`; `settings.json` is a fallback/cache, not a fresh-install prerequisite; generates QR via `QRCodeGenerator`; copy actions |
