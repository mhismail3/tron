...
export class EventStoreOrchestrator extends EventEmitter {
  ...
  constructor(config: EventStoreOrchestratorConfig) {
    ...
    this.sessionManager = createSessionManager({
      ...
      createAgentForSession: (sessionId, workingDirectory, model, systemPrompt) =>
        this.createAgentForSession(sessionId, workingDirectory, model, systemPrompt),
      emit: (event, data) => this.emit(event, data),
  ...
  private async createAgentForSession(
    ...
    return this.agentFactory.createAgentForSession(sessionId, workingDirectory, model, systemPrompt);
  }
  ...
  async querySubagent(
    ...
    success: boolean;
    status?: import('@tron/core').SubagentStatusInfo;
    events?: import('@tron/core').SubagentEventInfo[];
    logs?: import('@tron/core').SubagentLogInfo[];
    output?: string;
    error?: string;
  }> {
  ...
  async waitForSubagents(
    ...
    success: boolean;
    results?: import('@tron/core').SubagentResult[];
    error?: string;
    timedOut?: boolean;
  }> {
    ...
    try {
      let results: import('@tron/core').SubagentResult[];
