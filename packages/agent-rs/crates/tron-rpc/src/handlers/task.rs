...
fn task_error_to_rpc(e: &tron_tasks::TaskError, entity: &str, id: &str) -> RpcError {
    match e {
        tron_tasks::TaskError::NotFound { .. } => RpcError::NotFound {
...
impl MethodHandler for CreateTaskHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let task = tron_tasks::TaskService::create_task(
            &conn,
            &tron_tasks::TaskCreateParams {
                title,
                description,
                project_id,
                ..Default::default()
...
impl MethodHandler for GetTaskHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let details = tron_tasks::TaskService::get_task(&conn, &task_id)
...
impl MethodHandler for UpdateTaskHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let mut updates = tron_tasks::TaskUpdateParams::default();
        ...
        let task = tron_tasks::TaskService::update_task(&conn, &task_id, &updates, None)
            .map_err(|e| task_error_to_rpc(&e, "Task", &task_id))?;
...
impl MethodHandler for ListTasksHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let status_filter = params
            ...
            .and_then(Value::as_str)
            .and_then(|s| {
                serde_json::from_value::<tron_tasks::TaskStatus>(Value::String(s.into())).ok()
            });
        ...
        let filter = tron_tasks::TaskFilter {
        ...
        let result = tron_tasks::TaskRepository::list_tasks(&conn, &filter, limit, offset)
            .map_err(|e| RpcError::Internal {
                message: e.to_string(),
            })?;
...
impl MethodHandler for DeleteTaskHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let deleted = tron_tasks::TaskService::delete_task(&conn, &task_id, None)
            .map_err(|e| task_error_to_rpc(&e, "Task", &task_id))?;
...
impl MethodHandler for SearchTasksHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let results = tron_tasks::TaskRepository::search_tasks(&conn, &query, limit)
            .map_err(|e| RpcError::Internal {
                message: e.to_string(),
            })?;
...
impl MethodHandler for GetTaskActivityHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let activity = tron_tasks::TaskRepository::get_activity(&conn, &task_id, limit)
            .map_err(|e| RpcError::Internal {
                message: e.to_string(),
            })?;
...
impl MethodHandler for CreateProjectHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let project = tron_tasks::TaskService::create_project(
            &conn,
            &tron_tasks::ProjectCreateParams {
                title,
                description,
                area_id,
                ..Default::default()
...
impl MethodHandler for ListProjectsHandler {
    async fn handle(&self, _params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let result = tron_tasks::TaskRepository::list_projects(
            &conn,
            &tron_tasks::ProjectFilter::default(),
            100,
            0,
        )
        .map_err(|e| RpcError::Internal {
...
impl MethodHandler for GetProjectHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let project =
            tron_tasks::TaskRepository::get_project(&conn, &project_id).map_err(|e| {
                RpcError::Internal {
                    message: e.to_string(),
                }
            })?;
...
impl MethodHandler for UpdateProjectHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let mut updates = tron_tasks::ProjectUpdateParams::default();
        ...
        let project = tron_tasks::TaskService::update_project(&conn, &project_id, &updates)
...
impl MethodHandler for DeleteProjectHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let deleted = tron_tasks::TaskRepository::delete_project(&conn, &project_id)
            .map_err(|e| RpcError::Internal {
                message: e.to_string(),
            })?;
...
impl MethodHandler for GetProjectDetailsHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let task_result = tron_tasks::TaskRepository::list_tasks(&conn, &filter, 1000, 0)
            .map_err(|e| RpcError::Internal {
                message: e.to_string(),
            })?;
...
impl MethodHandler for CreateAreaHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let area = tron_tasks::TaskService::create_area(
            &conn,
            &tron_tasks::AreaCreateParams {
                title,
                description,
                ..Default::default()
            },
...
impl MethodHandler for ListAreasHandler {
    async fn handle(&self, _params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let result = tron_tasks::TaskRepository::list_areas(
            &conn,
            &tron_tasks::AreaFilter::default(),
            100,
            0,
        )
        .map_err(|e| RpcError::Internal {
...
impl MethodHandler for GetAreaHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let area = tron_tasks::TaskRepository::get_area(&conn, &area_id).map_err(|e| {
            RpcError::Internal {
                message: e.to_string(),
            }
        })?;
...
impl MethodHandler for UpdateAreaHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let mut updates = tron_tasks::AreaUpdateParams::default();
        if let Some(title) = params
            .as_ref()
            .and_then(|p| p.get("title"))
            .and_then(Value::as_str)
        ...
        let area = tron_tasks::TaskRepository::update_area(&conn, &area_id, &updates)
...
impl MethodHandler for DeleteAreaHandler {
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let deleted = tron_tasks::TaskRepository::delete_area(&conn, &area_id)
            .map_err(|e| RpcError::Internal {
                message: e.to_string(),
            })?;
