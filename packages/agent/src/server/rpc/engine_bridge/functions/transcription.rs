...
async fn transcribe_audio_value(payload: &Value, deps: &RpcEngineDeps) -> Result<Value, RpcError> {
    let audio_base64 =
...
fn download_model_value(deps: &RpcEngineDeps) -> Result<Value, RpcError> {
