...
async fn register_token(payload: &Value, deps: &RpcEngineDeps) -> Result<Value, RpcError> {
...
async fn unregister_token(payload: &Value, deps: &RpcEngineDeps) -> Result<Value, RpcError> {
    let device_token = require_string_param(Some(payload), "deviceToken")?;
    let event_store = Arc::clone(&deps.event_store);
    deps.rpc_context
...
async fn respond(payload: &Value, deps: &RpcEngineDeps) -> Result<Value, RpcError> {
