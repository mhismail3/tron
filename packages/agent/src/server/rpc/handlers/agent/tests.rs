...
#[tokio::test]
async fn prompt_rejects_oversized_prompt() {
    let ctx = make_test_context();
    let sid = ctx
        .session_manager
        .create_session("m", "/tmp", None, None)
...
async fn prompt_accepts_attachments() {
    ...
    let result = PromptHandler
        .handle(
            Some(
                json!({"sessionId": sid, "prompt": "hi", "attachments": [{"path": "/tmp/f.txt"}]}),
            ),
            &ctx,
        )
        .await
...
async fn prompt_error_emits_agent_error_event() {
    ...
    while let Ok(event) = rx.try_recv() {
        if let crate::core::events::TronEvent::Error {
            error,
            provider,
            category,
            retryable,
...
async fn prompt_success_no_agent_error() {
    ...
    while let Ok(event) = rx.try_recv() {
        assert!(
            !matches!(&event, crate::core::events::TronEvent::Error { .. }),
...
fn register_test_skill(ctx: &RpcContext, name: &str, content: &str) {
    ...
    registry.insert(crate::skills::types::SkillMetadata {
        name: name.to_string(),
...
fn make_event_store() -> Arc<crate::events::EventStore> {
...
#[test]
fn payload_skills_combine_with_images_and_metadata() {
    let images = vec![json!({"data": "i", "mediaType": "image/png"})];
...
fn user_content_override_none_without_multimodal_input() {
    let override_content = prompt_runtime::build_user_content_override("hello", "mock", None, None);
...
fn user_content_override_strips_images_for_non_image_models() {
    ...
    match override_content {
        crate::core::messages::UserMessageContent::Blocks(blocks) => {
            assert_eq!(blocks.len(), 2, "text + document");
            assert!(matches!(
                blocks[0],
                crate::core::content::UserContent::Text { .. }
            ));
            assert!(matches!(
                blocks[1],
                crate::core::content::UserContent::Document { .. }
            ));
        }
        crate::core::messages::UserMessageContent::Text(text) => {
...
fn user_content_override_keeps_images_for_image_models() {
    ...
    match override_content {
        crate::core::messages::UserMessageContent::Blocks(blocks) => {
            ...
            assert!(matches!(
                blocks[1],
                crate::core::content::UserContent::Image { .. }
            ));
        }
        crate::core::messages::UserMessageContent::Text(text) => {
            panic!("unexpected text override content: {text:?}")
