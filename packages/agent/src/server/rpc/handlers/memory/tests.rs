...
use crate::server::rpc::memory_ledger::user_message_len;
use serde_json::json;
use std::sync::Arc;
...
#[tokio::test]
async fn get_ledger_empty_working_dir_returns_all() {
    let ctx = make_test_context();
    let _ = seed_ledger_event(&ctx, "/tmp/a", json!({"title": "Entry 1"}));
    let _ = seed_ledger_event(&ctx, "/tmp/b", json!({"title": "Entry 2"}));

    let result = GetLedgerHandler
        .handle(Some(json!({"workingDirectory": ""})), &ctx)
        .await
        .unwrap();
...
async fn update_ledger_with_working_directory_targets_most_recent_session() {
    ...
    match rx.recv().await.unwrap() {
        crate::core::events::TronEvent::MemoryUpdating { base } => {
    ...
    match rx.recv().await.unwrap() {
        crate::core::events::TronEvent::MemoryUpdated { base, .. } => {
            assert_eq!(base.session_id, newer);
        }
...
use crate::llm::models::types::Provider as LlmProvider;
...
async fn execute_ledger_write_cron_source() {
    ...
    let subagent = Arc::new(
        crate::runtime::orchestrator::subagent_manager::SubagentManager::new(
    ...
    subagent.set_tool_factory(Arc::new(crate::tools::registry::ToolRegistry::new));
...
fn prepare_cron_transcript_strips_long_user_message() {
    ...
    let messages = vec![
        ...
        Message::Assistant {
            content: vec![crate::core::content::AssistantContent::Text {
...
fn prepare_cron_transcript_preserves_assistant_messages() {
    let messages = vec![
        ...
        Message::Assistant {
            content: vec![
                crate::core::content::AssistantContent::Text {
                    text: "response".into(),
                },
                crate::core::content::AssistantContent::ToolUse {
                    id: "t1".into(),
                    name: "search".into(),
...
fn cron_assistant_text_len_ignores_user_and_tool_result_messages() {
    ...
    let messages = vec![
        ...
        Message::ToolResult {
            ...
            content: crate::core::messages::ToolResultMessageContent::Text(
