...
impl MethodHandler for CreateHandler {
    ...
    async fn handle(&self, params: Option<Value>, ctx: &RpcContext) -> Result<Value, RpcError> {
        ...
        let schedule: crate::cron::types::Schedule =
        ...
        let payload: crate::cron::types::Payload =
        ...
        let delivery: Vec<crate::cron::types::Delivery> = job_param
            .get("delivery")
        ...
        // Validate
        crate::cron::config::validate_job(&job).map_err(|e| RpcError::InvalidParams {
            message: e.to_string(),
        })?;
...
mod tests {
    ...
    fn make_cron_context() -> (RpcContext, tempfile::TempDir) {
        ...
        let scheduler = std::sync::Arc::new(crate::cron::CronScheduler::new(
            pool.clone(),
            std::sync::Arc::new(crate::cron::SystemClock),
            deps,
            config_path,
            backup_path,
            cancel,
    ...
    async fn create_job_with_tool_restrictions() {
        ...
        let result = CreateHandler.handle(Some(params), &ctx).await.unwrap();
        let job_id = result["job"]["id"].as_str().unwrap().to_string();
        assert!(result["job"]["toolRestrictions"]["allowedTools"].is_array());
    ...
    #[tokio::test]
    async fn create_job_with_denied_tools_rejected() {
        let (ctx, _dir) = make_cron_context();
        let params = serde_json::json!({
            "job": {
                "name": "Bad",
        ...
        let err = CreateHandler.handle(Some(params), &ctx).await.unwrap_err();
        assert!(format!("{err:?}").contains("deniedTools"));
    }
    ...
    async fn update_job_change_tool_restrictions() {
        ...
        let params = serde_json::json!({
            "job": {
                ...
                "schedule": {"type": "every", "intervalSecs": 60},
                "payload": {"type": "shellCommand", "command": "echo hi"},
                "toolRestrictions": {"allowedTools": ["Bash"]}
            }
        });
        let result = CreateHandler.handle(Some(params), &ctx).await.unwrap();
        let job_id = result["job"]["id"].as_str().unwrap().to_string();

        // Change to different allowed tools
        let update_params = serde_json::json!({
            "jobId": job_id,
    ...
    #[tokio::test]
    async fn update_job_with_denied_tools_rejected() {
        let (ctx, _dir) = make_cron_context();
        let params = serde_json::json!({
            "job": {
                "name": "Update Bad",
