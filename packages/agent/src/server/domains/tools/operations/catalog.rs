...
use crate::tools::backends::{RealFileSystem, ReqwestHttpClient, TokioProcessRunner};
...
pub(crate) fn builtin_tool_specs(
    ...
    let mut tools: Vec<Arc<dyn TronTool>> = vec![
        Arc::new(crate::tools::fs::read::ReadTool::new(fs.clone())),
        Arc::new(crate::tools::fs::write::WriteTool::new(fs.clone())),
        Arc::new(crate::tools::fs::edit::EditTool::new(fs.clone())),
        Arc::new(
            crate::tools::system::bash::BashTool::new(runner.clone(), Some(blob_store.clone()))
        ...
        Arc::new(crate::tools::search::search_tool::SearchTool::new(
            runner.clone(),
        )),
        Arc::new(crate::tools::fs::find::FindTool::new()),
        Arc::new(crate::tools::ui::ask_user::AskUserQuestionTool::new()),
        Arc::new(crate::tools::ui::get_confirmation::GetConfirmationTool::new()),
        Arc::new(crate::tools::ui::notify::NotifyAppTool::new(
    ...
    let summarizer = deps.subagent_manager.as_ref().map(|manager| {
        Arc::new(
            crate::runtime::agent::compaction_handler::SubagentContentSummarizer {
    ...
    if let Some(summarizer) = summarizer {
        tools.push(Arc::new(
            crate::tools::web::web_fetch::WebFetchTool::new_with_summarizer(
                http.clone(),
                summarizer,
            ),
        ));
    } else {
        tools.push(Arc::new(crate::tools::web::web_fetch::WebFetchTool::new(
            http.clone(),
        )));
    }
    tools.push(Arc::new(
        crate::tools::web::web_search::WebSearchTool::new_with_auth_path(
            http.clone(),
            deps.auth_path.clone(),
        ),
    ));

    let mut display_tool = crate::tools::ui::display::DisplayTool::new(Some(blob_store));
    display_tool = display_tool.with_event_tx(deps.orchestrator.broadcast().sender());
    tools.push(Arc::new(display_tool));

    tools.push(Arc::new(
        crate::tools::ui::computer_use::ComputerUseTool::new(
            runner,
            deps.tool_runtime
    ...
    tools.push(Arc::new(crate::tools::engine::EngineDiscoverTool::new(
        deps.engine_host.clone(),
    )));
    tools.push(Arc::new(crate::tools::engine::EngineInspectTool::new(
        deps.engine_host.clone(),
    )));
    tools.push(Arc::new(crate::tools::engine::EngineWatchTool::new(
        deps.engine_host.clone(),
    )));
    tools.push(Arc::new(crate::tools::engine::EngineInvokeTool::new(
        deps.engine_host.clone(),
    )));

    if let Some(router) = deps.mcp_router.as_ref() {
        tools.push(Arc::new(crate::mcp::search_tool::McpSearchTool::new(
            router.clone(),
        )));
        tools.push(Arc::new(crate::mcp::call_tool::McpCallTool::new(
            router.clone(),
        )));
    }

    if let Some(subagent_manager) = deps.subagent_manager.as_ref() {
        let spawner: Arc<dyn crate::tools::traits::SubagentSpawner> = subagent_manager.clone();
        tools.push(Arc::new(
            crate::tools::subagent::spawn::SpawnSubagentTool::with_profile_runtime(
                spawner,
    ...
    if let Some(job_manager) = deps.job_manager.as_ref() {
        tools.push(Arc::new(
            crate::tools::system::manage_process::ManageJobTool::new(job_manager.clone()),
        ));
        tools.push(Arc::new(crate::tools::system::wait::WaitTool::new(
            job_manager.clone(),
        )));
    }
...
fn execution_mode_metadata(mode: crate::tools::traits::ExecutionMode) -> Value {
    match mode {
        crate::tools::traits::ExecutionMode::Parallel => json!({"kind": "parallel"}),
        crate::tools::traits::ExecutionMode::Serialized(group) => {
