...
use crate::engine::{
    ...
    Provenance, PublishStreamEvent, RiskLevel, VisibilityScope, WorkerId,
};
...
pub(super) async fn mcp_status_value(deps: &Deps) -> Result<Value, CapabilityError> {
...
pub(super) async fn mcp_add_server_value(
    params: Option<&Value>,
...
pub(super) async fn mcp_remove_server_value(
    params: Option<&Value>,
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
...
pub(super) async fn mcp_enable_server_value(
    params: Option<&Value>,
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
...
pub(super) async fn mcp_disable_server_value(
    params: Option<&Value>,
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
...
pub(super) async fn mcp_restart_server_value(
    params: Option<&Value>,
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
...
pub(super) async fn mcp_reload_value(
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let router = require_router(deps)?.clone();
...
pub(super) async fn mcp_list_tools_value(
    params: Option<&Value>,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let router = require_router(deps)?;
...
async fn publish_mcp_status_changed(invocation: &Invocation, deps: &Deps) {
    ...
    if let Err(error) = deps
        ...
        .await
    {
        tracing::warn!(topic, error = %error, "mcp stream publication failed");
    }
}
...
pub(super) fn mcp_tool_function_id(server: &str, tool: &str) -> String {
...
pub(super) fn mcp_model_tool_name(server: &str, tool: &str) -> String {
    format!(
        "mcp_{}_{}",
        sanitize_id_part(server),
        sanitize_id_part(tool)
