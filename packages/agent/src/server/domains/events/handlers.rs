...
pub(crate) async fn handle(
    ...
    match operation_key {
        "get_history" => events_get_history_value(Some(payload), deps).await,
        "get_since" => events_get_since_value(Some(payload), deps).await,
        "append" => events_append_value(Some(payload), invocation, deps).await,
        "subscribe" => events_subscribe_value(Some(payload), invocation, deps).await,
        "unsubscribe" => events_unsubscribe_value(Some(payload), deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("events method {operation_key} is not engine-owned"),
        }),
    }
}
