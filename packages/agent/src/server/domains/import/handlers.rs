...
pub(crate) async fn handle(
    ...
    match operation_key {
        "list_sources" => list_sources(deps).await,
        "list_sessions" => list_sessions(&invocation.payload, deps).await,
        "preview_session" => preview_session(&invocation.payload, deps).await,
        "execute" => execute_import(&invocation.payload, deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("import method {operation_key} is not engine-owned"),
        }),
    }
}
