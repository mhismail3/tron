...
pub(crate) async fn handle(
    ...
    match operation_key {
        "enter" => plan_set_value(Some(payload), deps, true),
        "exit" => plan_set_value(Some(payload), deps, false),
        "get_state" => plan_get_state_value(Some(payload), deps),
        _ => Err(CapabilityError::Internal {
            message: format!("plan method {operation_key} is not engine-owned"),
        }),
    }
}
