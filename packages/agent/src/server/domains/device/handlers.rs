...
pub(crate) async fn handle(
    ...
    match operation_key {
        "register" => register_token(&invocation.payload, deps).await,
        "unregister" => unregister_token(&invocation.payload, deps).await,
        "respond" => respond(&invocation.payload, deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("device method {operation_key} is not engine-owned"),
        }),
    }
}
