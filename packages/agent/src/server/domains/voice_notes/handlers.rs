...
pub(crate) async fn handle(
    ...
    match operation_key {
        "list" => list(&invocation.payload, deps).await,
        "save" => save(&invocation.payload, deps).await,
        "delete" => delete(&invocation.payload, deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("voice notes method {operation_key} is not engine-owned"),
        }),
    }
}
