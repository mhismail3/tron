//! Settings-owned engine stream publication.
...
/// Typed publisher for settings-triggered status broadcasts.
pub(crate) struct SettingsStreamPublisher<'a> {
    engine_host: &'a EngineHostHandle,
}

impl<'a> SettingsStreamPublisher<'a> {
    pub(crate) fn new(engine_host: &'a EngineHostHandle) -> Self {
        Self { engine_host }
    }

    pub(crate) async fn mcp_status_changed(&self, invocation: &Invocation, status: Value) {
        ...
        if let Err(error) = self
            ...
            .await
        {
            tracing::warn!(error = %error, "failed to publish settings stream event");
        }
    }
}
