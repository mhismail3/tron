...
pub(crate) async fn handle(
    ...
    match operation_key {
        "get" => serde_json::to_value(&deps.profile_runtime.current().settings).map_err(|error| {
            CapabilityError::Internal {
                message: error.to_string(),
            }
        }),
        "update" => settings_update_value(Some(payload), invocation, deps).await,
        "reset_to_defaults" => settings_reset_to_defaults_value(deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("settings method {operation_key} is not engine-owned"),
        }),
    }
}
