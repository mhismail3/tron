...
pub(crate) async fn handle(
    ...
    match operation_key {
        "audio" => transcribe_audio_value(&invocation.payload, deps).await,
        "list_models" => list_models_value(deps),
        "download_model" => download_model_value(deps),
        _ => Err(CapabilityError::Internal {
            message: format!("transcription method {operation_key} is not engine-owned"),
        }),
    }
}
