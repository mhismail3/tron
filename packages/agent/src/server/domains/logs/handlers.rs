...
pub(crate) async fn handle(
    ...
    match operation_key {
        "ingest" => ingest_logs_value(Some(payload), deps).await,
        "recent" => recent_logs_value(Some(payload.clone()), deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("logs method {operation_key} is not engine-owned"),
        }),
    }
}
