...
pub(crate) async fn handle(
    ...
    match operation_key {
        "list_containers" => list_containers(deps).await,
        "start_container" => run_container_command("start", &invocation.payload).await,
        "stop_container" => run_container_command("stop", &invocation.payload).await,
        "kill_container" => run_container_command("kill", &invocation.payload).await,
        "remove_container" => remove_container(&invocation.payload, deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("sandbox method {operation_key} is not engine-owned"),
        }),
    }
}
