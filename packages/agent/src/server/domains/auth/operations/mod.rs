...
use super::*;
use crate::engine::{Invocation, PublishStreamEvent, VisibilityScope};
...
pub(super) async fn auth_get(deps: &Deps) -> Result<Value, CapabilityError> {
...
pub(super) async fn auth_update(
...
pub(super) async fn auth_oauth_begin(
    payload: &Value,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
...
pub(super) async fn auth_oauth_complete(
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let payload = &invocation.payload;
...
pub(super) async fn auth_rename_account(
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let payload = &invocation.payload;
...
pub(super) async fn auth_set_active(
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let payload = &invocation.payload;
...
pub(super) async fn auth_remove_account(
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let payload = &invocation.payload;
...
pub(super) async fn auth_remove_api_key(
    invocation: &Invocation,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let payload = &invocation.payload;
...
async fn broadcast_auth_updated(deps: &Deps, invocation: &Invocation, masked_state: &Value) {
    ...
    if let Err(error) = deps
        ...
        .await
    {
        tracing::warn!(topic, error = %error, "auth stream publication failed");
    }
}
