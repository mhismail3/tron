...
pub(crate) async fn handle(
    ...
    match operation_key {
        "get" => auth_get(deps).await,
        "update" => auth_update(invocation, deps).await,
        "clear" => auth_clear(invocation, deps).await,
        "oauth_begin" => auth_oauth_begin(&invocation.payload, deps).await,
        "oauth_complete" => auth_oauth_complete(invocation, deps).await,
        "rename_account" => auth_rename_account(invocation, deps).await,
        "set_active" => auth_set_active(invocation, deps).await,
        "remove_account" => auth_remove_account(invocation, deps).await,
        "remove_api_key" => auth_remove_api_key(invocation, deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("auth method {operation_key} is not engine-owned"),
        }),
    }
}
