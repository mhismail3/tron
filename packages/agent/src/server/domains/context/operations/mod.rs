...
pub(super) async fn get_snapshot(payload: &Value, deps: &Deps) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
    ContextQueryService::get_snapshot(deps, session_id).await
}

pub(super) async fn get_detailed_snapshot(
    payload: &Value,
    deps: &Deps,
...
pub(super) async fn get_audit_trace(
    payload: &Value,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
...
pub(super) async fn should_compact(payload: &Value, deps: &Deps) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
    ContextQueryService::should_compact(deps, session_id).await
}

pub(super) async fn preview_compaction(
    payload: &Value,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
...
pub(super) async fn can_accept_turn(
    payload: &Value,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
...
pub(super) async fn confirm_compaction(
    payload: &Value,
    deps: &Deps,
) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
...
pub(super) async fn clear(payload: &Value, deps: &Deps) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
    ContextCommandService::clear(deps, session_id).await
}

pub(super) async fn compact(payload: &Value, deps: &Deps) -> Result<Value, CapabilityError> {
    let session_id = require_string_param(Some(payload), "sessionId")?;
