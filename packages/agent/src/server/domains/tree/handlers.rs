...
pub(crate) async fn handle(
    ...
    match operation_key {
        "get_visualization" => get_visualization(&invocation.payload, deps).await,
        "get_branches" => get_branches(&invocation.payload, deps).await,
        "get_subtree" => get_subtree(&invocation.payload, deps).await,
        "get_ancestors" => get_ancestors(&invocation.payload, deps).await,
        "compare_branches" => compare_branches(&invocation.payload).await,
        _ => Err(CapabilityError::Internal {
            message: format!("tree method {operation_key} is not engine-owned"),
        }),
    }
}
