...
pub(crate) async fn handle(
    ...
    match operation_key {
        "list" => notifications_list_value(Some(payload), deps).await,
        "mark_read" => notifications_mark_read_value(Some(payload), deps).await,
        "mark_all_read" => notifications_mark_all_read_value(Some(payload), deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("notifications method {operation_key} is not engine-owned"),
        }),
    }
}
