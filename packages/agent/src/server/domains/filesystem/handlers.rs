...
pub(crate) async fn handle(
    ...
    match operation_key {
        "list_dir" => filesystem_list_dir_value(Some(payload), deps).await,
        "get_home" => filesystem_get_home_value(deps).await,
        "read_file" => file_read_value(Some(payload), deps).await,
        "create_dir" => filesystem_create_dir_value(Some(payload), deps).await,
        _ => Err(CapabilityError::Internal {
            message: format!("filesystem method {operation_key} is not engine-owned"),
        }),
    }
}
