...
struct PromptRunPlan {
    ...
    context_artifacts: Arc<crate::server::services::session_context::ContextArtifactsService>,
...
async fn execute_prompt_run(plan: PromptRunPlan) {
    ...
    if result.error.is_none() && !result.interrupted && retain_eligible(&result.stop_reason) {
        let deps = crate::server::services::memory_retain::RetainDeps {
        ...
        drop(tokio::spawn(async move {
            crate::server::services::memory_retain::auto_retain::maybe_fire(
                &deps,
