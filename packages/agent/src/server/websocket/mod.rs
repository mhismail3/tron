...
//! | `event_bridge` | Orchestrator events → engine `events.session` stream → WebSocket broadcast for migrated event classes, with direct fallback for deferred classes |
...
//! publish `RpcEvent` payloads into engine streams first, then the stream pump
//! broadcasts compatible frames to clients.
