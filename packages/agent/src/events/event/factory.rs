...
//! IDs and timestamps.
//!
//! [`EventChainBuilder`] wraps an [`EventStore`](crate::events::store::EventStore) and
...
///
/// Uses the [`EventStore`](crate::events::store::EventStore) for persistence.
pub struct EventChainBuilder {
    store: crate::events::store::EventStore,
    session_id: String,
    head: String,
}
...
impl EventChainBuilder {
    ...
    pub fn append(
        ...
        event_type: EventType,
        payload: Value,
    ) -> crate::events::errors::Result<crate::events::sqlite::row_types::EventRow> {
        let event = self.store.append(&crate::events::store::AppendOptions {
...
mod tests {
    ...
    fn chain_builder_auto_threads() {
        ...
        let store = crate::events::store::EventStore::new(pool.clone());

        let cr = store
            .create_session("claude-opus-4-6", "/tmp/project", None, None, None)
            .unwrap();

        let store2 = crate::events::store::EventStore::new(pool);
