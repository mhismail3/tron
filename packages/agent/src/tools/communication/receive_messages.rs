...
use crate::tools::traits::{MessageBus, MessageFilter, ToolContext, TronTool};
use crate::tools::utils::schema::ToolSchemaBuilder;
use crate::tools::utils::validation::{get_optional_bool, get_optional_string, get_optional_u64};
...
impl TronTool for ReceiveMessagesTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<TronToolResult, ToolError> {
        ...
        match self.bus.receive_messages(&ctx.session_id, &filter).await {
            Ok(messages) => {
                if messages.is_empty() {
                    return Ok(TronToolResult {
                        content: ToolResultBody::Blocks(vec![
                            crate::core::content::ToolResultContent::text("No messages found."),
                        ]),
                        details: Some(json!({
                            "messages": [],
                            "count": 0,
                ...
                let summary = messages
                    ...
                    .map(|m| {
                        ...
                        let truncated =
                            crate::core::text::truncate_with_suffix(&payload_str, 103, "...");
                        format!(
                            "- [{}] from {}: {}",
            ...
            Err(e) => Ok(crate::core::tools::error_result(format!(
...
mod tests {
    ...
    use crate::tools::traits::{MessageSendResult, OutgoingMessage, ReceivedMessage};
