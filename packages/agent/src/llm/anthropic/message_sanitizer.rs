...
fn is_valid_user_content(content: &crate::core::messages::UserMessageContent) -> bool {
    match content {
        crate::core::messages::UserMessageContent::Text(text) => !text.trim().is_empty(),
        crate::core::messages::UserMessageContent::Blocks(blocks) => !blocks.is_empty(),
    }
}
...
mod tests {
    ...
    fn empty_messages_filtered() {
        let messages = vec![
            Message::user("hello"),
            Message::User {
                content: crate::core::messages::UserMessageContent::Text("  ".into()),
                timestamp: None,
            },
    ...
    fn first_message_assistant_gets_user_prepended() {
        ...
        assert!(result[0].is_user());
        if let Message::User {
            content: crate::core::messages::UserMessageContent::Text(text),
            ..
        } = &result[0]
        {
