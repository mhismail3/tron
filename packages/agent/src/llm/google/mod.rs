...
//! | Module                | Content |
//! |-----------------------|---------|
//! | [`provider`]          | [`GoogleProvider`] — implements the shared `Provider` trait ([`crate::llm::provider`]); stream orchestration, retry, and tool translation |
//! | [`message_converter`] | `Vec<Message>` → Gemini `contents` array; handles tool calls, tool results, and multimodal parts |
//! | [`stream_handler`]    | Gemini SSE `v1beta/{model}:streamGenerateContent` → `StreamEvent` sequence ([`crate::core::events`]) |
...
//!   provider enum
//! - [`GoogleAuth`], [`GoogleConfig`] — consumed by [`crate::llm::auth`]
//!
//! ## Invariants
//!
...
//!   ([`crate::llm::provider`]) before reaching the orchestrator;
