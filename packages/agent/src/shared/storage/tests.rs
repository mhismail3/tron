...
#[test]
fn nullable_observation_migration_preserves_rows_and_is_idempotent() {
    ...
    conn.execute_batch(
        "CREATE TABLE evidence (
         ...
         CREATE INDEX evidence_payload ON evidence(payload);
         INSERT INTO evidence (id, authority_id, payload)
         VALUES ('one', 'legacy-grant', 'kept');",
    )
    .unwrap();
    let table_sql = "CREATE TABLE IF NOT EXISTS evidence (
        id TEXT PRIMARY KEY,
        authority_id TEXT,
        payload TEXT NOT NULL
    );";
    let index_sql = "CREATE INDEX IF NOT EXISTS evidence_payload ON evidence(payload);";

    ensure_nullable_text_observation(
        &conn,
        "evidence",
        "authority_id",
        table_sql,
        index_sql,
        &["id", "authority_id", "payload"],
    )
    .unwrap();
    ensure_nullable_text_observation(
        &conn,
    ...
    let legacy: (String, String) = conn
        .query_row(
            "SELECT authority_id, payload FROM evidence WHERE id = 'one'",
            [],
            |row| Ok((row.get(0)?, row.get(1)?)),
        )
        .unwrap();
    assert_eq!(legacy, ("legacy-grant".to_owned(), "kept".to_owned()));
    conn.execute(
        "INSERT INTO evidence (id, authority_id, payload) VALUES ('two', NULL, 'local')",
        [],
    )
    .unwrap();
    let null_rows: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM evidence WHERE authority_id IS NULL",
            [],
            |row| row.get(0),
        )
        .unwrap();
    ...
    let not_null: i64 = conn
        .query_row(
            "SELECT \"notnull\" FROM pragma_table_info('evidence') WHERE name = 'authority_id'",
            [],
            |row| row.get(0),
        )
        .unwrap();
