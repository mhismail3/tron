...
pub async fn execute_with_retries(
    ...
    loop {
        ...
        if attempt > 0
            && let Ok(false) = crate::cron::store::is_job_enabled(&deps.pool, &job.id)
        {
            return make_run(
                run_id,
                &job.id,
...
mod tests {
    ...
    #[async_trait::async_trait]
    impl crate::cron::executor::SystemEventInjector for MockInjector {
    ...
    async fn auto_disable_after_failures() {
        ...
        for _ in 0..3 {
            let _ = crate::cron::store::increment_consecutive_failures(&deps.pool, "cron_auto");
        }
        let failures = crate::cron::store::get_runtime_state(&deps.pool, "cron_auto")
            .unwrap()
            .unwrap()
