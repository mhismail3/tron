...
use crate::engine::invocation::host::{
    CatalogWatchRequest, CatalogWatchResponse, EngineHostHandle,
};
use crate::engine::invocation::model::{CausalContext, Invocation, InvocationResult};
...
use crate::engine::runtime::triggers::{EngineTriggerRuntime, TriggerDispatchRequest};
    ...
    pub async fn invoke(
        ...
        if let Err(error) = reject_noncanonical_namespace(&function_id) {
            return InvocationResult::error(
                ...
                WorkerId::new("agent").expect("valid static worker id"),
                FunctionRevision(0),
                CatalogRevision(0),
                error,
            );
        ...
        if let Err(error) = reject_agent_blocked_function(&function_id) {
            let (worker_id, revision) = self
                ...
                .unwrap_or_else(|_| {
                    (
                        WorkerId::new(function_id.namespace()).unwrap_or_else(|_| {
                            WorkerId::new("agent").expect("valid static worker id")
                        }),
                        FunctionRevision(0),
                    )
