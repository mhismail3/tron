...
//!
//! `catalog` owns revision counters, visibility, provenance, and health;
//! `function` owns the executable catalog definition. Persistent
//! worker bundles and lifecycle state belong to `domains::worker_kernel`.

mod catalog;
mod function;

pub use catalog::{CatalogRevision, FunctionHealth, FunctionRevision, Provenance, VisibilityScope};
