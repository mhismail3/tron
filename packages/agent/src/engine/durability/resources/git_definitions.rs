...
pub(super) fn git_resource_type_definitions() -> Vec<RegisterResourceType> {
    vec![
        git_index_change_definition(),
        git_commit_definition(),
        git_branch_start_definition(),
    ]
}
...
fn git_branch_start_definition() -> RegisterResourceType {
    RegisterResourceType {
        kind: GIT_BRANCH_START_KIND.to_owned(),
        schema_id: GIT_BRANCH_START_SCHEMA_ID.to_owned(),
        schema: json!({
            "type": "object",
            "required": [
                "schemaVersion",
            ...
            "properties": {
                "schemaVersion": {"type": "string"},
                "operation": {"type": "string", "enum": ["branch_start"]},
                "state": {"type": "string", "enum": ["started", "archived"]},
                "repository": {"type": "object"},
        ...
        lifecycle_states: ["started", "archived"]
            .into_iter()
            .map(str::to_owned)
            .collect(),
        versioning_mode: EngineResourceVersioningMode::AppendOnly,
