//! External worker restart soak tests.

use super::*;

struct ExternalWorkerSoakInvoker;

#[async_trait]
impl external::ExternalWorkerInvoker for ExternalWorkerSoakInvoker {
...
#[tokio::test]
async fn external_worker_soak_registers_invokes_disconnects_and_reopens() {
    ...
    for cycle in 0..6 {
        let namespace = format!("external_soak_{cycle}");
        let worker_id = wid(&format!("external-soak-worker-{cycle}"));
        let function_id = format!("{namespace}::echo");
        let session_id = format!("external-soak-session-{cycle}");
            ...
            .register_function(RegisterFunction {
                definition: external_visible_function(
                    FunctionDefinition::new(
                        fid(&function_id),
                        worker_id.clone(),
                        format!("external worker soak echo cycle {cycle}"),
        ...
        let result = handle
            .invoke(host_invocation(
                &function_id,
                json!({"cycle": cycle, "proof": "external-worker-soak"}),
                causal()
                    .with_scope(format!("{namespace}.read"))
                    .with_session_id(&session_id)
                    .with_workspace_id("external-soak-workspace"),
            ))
            .await;
        ...
        assert_eq!(
            ...
            "external-worker-soak"
        );

        runtime
            .disconnect(WorkerDisconnect {
                worker_id: worker_id.clone(),
                reason: format!("external worker soak cycle {cycle} complete"),
            })
            .await
            .unwrap();
        assert!(!runtime.connections().contains(&worker_id));
