...
pub async fn observe_local_model_availability() -> LocalModelAvailability {
    ...
    let preferred = [
        GEMMA4_26B,
        crate::domains::model::routing::models::model_ids::GEMMA4_E4B,
    ];
    for id in preferred {
...
fn preferred_deep_model(default_model: &str) -> &str {
    if crate::domains::model::routing::models::registry::is_model_supported(CLAUDE_OPUS_4_7) {
        CLAUDE_OPUS_4_7
    } else if crate::domains::model::routing::models::registry::is_model_supported(
        CLAUDE_SONNET_4_6,
    ) {
        CLAUDE_SONNET_4_6
    } else {
