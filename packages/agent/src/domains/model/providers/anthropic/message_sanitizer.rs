...
//!
//! Invariants enforced:
//! 1. Every `tool_invocation` block has a corresponding `ToolResult` message
//! 2. No empty messages
...
/// - Duplicate `tool_invocation` IDs deduplicated
/// - Synthetic tool results injected for unmatched `tool_invocation` blocks
/// - Placeholder user message prepended if first message isn't user
pub fn sanitize_messages(messages: Vec<Message>) -> Vec<Message> {
    // PHASE 1: Filter invalid messages, deduplicate tool_invocation IDs
    let mut valid: Vec<Message> = Vec::with_capacity(messages.len());
    let mut seen_tool_invocation_ids: HashSet<String> = HashSet::new();
    // tool_invocation_id → index in `valid` where the assistant message lives
    let mut tool_invocation_locations: HashMap<String, usize> = HashMap::new();
        ...
        match &msg {
            ...
            Message::Assistant { content, .. } => {
                ...
                for block in content {
                    match block {
                        AssistantContent::ToolInvocation { id, .. } => {
                            if seen_tool_invocation_ids.contains(id) {
                                debug!(tool_invocation_id = %id, "Removed duplicate tool_invocation block");
                                continue;
                            }
                            let _ = seen_tool_invocation_ids.insert(id.clone());
                ...
                // Track tool_invocation locations for synthetic result injection
                for block in &filtered {
                    if let AssistantContent::ToolInvocation { id, .. } = block {
                        let _ = tool_invocation_locations.insert(id.clone(), idx);
                    }
                }
            ...
            Message::ToolResult {
                ...
                if invocation_id.is_empty() {
                    debug!("Removed tool result with empty invocation_id");
                    continue;
                }
                if !is_valid_tool_result_content(content) {
                    debug!(invocation_id = %invocation_id, "Removed empty tool result");
                    continue;
                }
                valid.push(msg);
            }
    ...
    // PHASE 3: Inject synthetic tool results for unmatched tool_invocation blocks
    // Group missing IDs by assistant message index
    let mut missing_by_index: HashMap<usize, Vec<String>> = HashMap::new();
    for (tool_invocation_id, assistant_idx) in &tool_invocation_locations {
        if !existing_result_ids.contains(tool_invocation_id.as_str()) {
            missing_by_index
                .entry(*assistant_idx)
                .or_default()
                .push(tool_invocation_id.clone());
        }
    }
    ...
    for assistant_idx in sorted_indices {
        if let Some(missing_ids) = missing_by_index.get(&assistant_idx) {
            // Insert in reverse to maintain original tool_invocation order
            for invocation_id in missing_ids.iter().rev() {
                debug!(invocation_id = %invocation_id, "Injected synthetic tool result for interrupted execution");
                valid.insert(
                    assistant_idx + 1,
                    Message::ToolResult {
...
mod tests {
    ...
    fn tool_invocation(id: &str, name: &str) -> AssistantContent {
    ...
    fn tool_result(invocation_id: &str, text: &str) -> Message {
        Message::ToolResult {
    ...
    #[test]
    fn duplicate_tool_invocation_ids_removed() {
        let messages = vec![
            Message::user("hello"),
            assistant_with_content(vec![
                tool_invocation("tc-1", "test_tool"),
                AssistantContent::text("some text"),
            ]),
            tool_result("tc-1", "output"),
            // Second assistant reuses the same tool_invocation ID
            assistant_with_content(vec![
                tool_invocation("tc-1", "test_tool"),
        ...
        let result = sanitize_messages(messages);
        // The second assistant message should have the duplicate tool_invocation removed
    ...
    // ── Insert synthetic tool results ─────────────────────────────

    #[test]
    fn unmatched_tool_invocation_gets_synthetic_result() {
        let messages = vec![
            Message::user("hello"),
            assistant_with_content(vec![tool_invocation("tc-1", "test_tool")]),
    ...
    #[test]
    fn matched_tool_invocation_no_synthetic_result() {
        let messages = vec![
            Message::user("hello"),
            assistant_with_content(vec![tool_invocation("tc-1", "test_tool")]),
            tool_result("tc-1", "output"),
    ...
    #[test]
    fn multiple_unmatched_tool_invocations_all_get_synthetic_results() {
        let messages = vec![
            Message::user("hello"),
            assistant_with_content(vec![
                tool_invocation("tc-1", "test_tool"),
                tool_invocation("tc-2", "inspect"),
                tool_invocation("tc-3", "search"),
            ]),
        ];
        let result = sanitize_messages(messages);
    ...
    fn cross_provider_signed_thinking_converted_to_text() {
        let messages = vec![
            ...
            assistant_with_content(vec![
                ...
                tool_invocation("call_abc123", "test_tool"),
            ]),
