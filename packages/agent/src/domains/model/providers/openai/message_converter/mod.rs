...
pub fn generate_capability_instruction_text(capabilities: &[ModelCapability]) -> String {
    ...
    format!(
        ...
        Context-control operations use `context_control_status`, `context_control_snapshot`, `context_control_compact`, `context_control_clear`, `context_control_action_list`, `context_control_action_inspect`, `context_survivor_record`, `context_survivor_list`, `context_survivor_disable`, `context_exclusion_record`, `context_exclusion_list`, `context_exclusion_disable`, and `context_policy_snapshot` for read-only current-session context composition, snapshots, compact/clear action records, epoch audit details, server-owned survivor/exclusion policy refs, and policy snapshots that replacement summarizers must preserve; use `context_control_status` for read-only context state before recording a snapshot; compact/clear and policy writes require `reason`, `idempotencyKey`, exact session-scoped authority, and `networkPolicy: none`, disables require exact policy resource selectors, and projections expose token estimates, prompt block labels, redacted refs, timeline/audit refs, policy refs, and redaction/truncation proof only, never raw prompt bodies, hidden system/soul prompt text, secrets, local paths, commands, logs, grant ids, authority ids, raw file contents, or hidden chain-of-thought. \
