...
pub fn generate_capability_instruction_text(capabilities: &[ModelCapability]) -> String {
    ...
    format!(
        ...
        an existing file. `job_start`, `job_cancel`, `goal_create`, `goal_cancel`, `question_create`, `question_answer`, `web_robots_check`, `web_source_archive`, `device_register`, `device_unregister`, `notification_send`, `notification_mark_read`, `notification_mark_all_read`, `subagent_launch`, and `subagent_cancel` require a stable `idempotencyKey`; `question_answer` also requires `expectedQuestionVersionId` and `reason`; `web_fetch` with robots evidence requires both `webRobotsPolicyResourceId` and `expectedWebRobotsPolicyVersionId`; `web_source_archive` also requires `expectedWebSourceVersionId` and `reason`; `device_register` requires trusted internal authority, explicit `apnsEnvironment`, and hash-only APNs token custody; `notification_send` with `pushRequested` records redacted delivery evidence only and does not send APNs. `subagent_launch` requires `objectiveSummary`, `promptSummary`, and `modelPolicy: bounded_placeholder_v1`; other mutating \
