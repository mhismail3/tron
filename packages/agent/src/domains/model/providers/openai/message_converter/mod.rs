...
pub fn generate_capability_instruction_text(capabilities: &[ModelCapability]) -> String {
    ...
    format!(
        ...
        an existing file. `job_start`, `job_cancel`, `goal_create`, `goal_cancel`, `question_create`, `question_answer`, `web_robots_check`, `web_source_archive`, `media_create`, `media_archive`, `import_history_record`, `repository_tree_snapshot`, `import_preview_record`, `program_execution_record`, `prompt_artifact_record`, `device_register`, `device_unregister`, `notification_send`, `notification_mark_read`, `notification_mark_all_read`, `subagent_launch`, and `subagent_cancel` require a stable `idempotencyKey`; `question_answer` also requires `expectedQuestionVersionId` and `reason`; `web_fetch` with robots evidence requires both `webRobotsPolicyResourceId` and `expectedWebRobotsPolicyVersionId`; `web_source_archive` also requires `expectedWebSourceVersionId` and `reason`; `media_archive` also requires `expectedMediaVersionId` and `reason`; `import_history_record` also requires `subjectKind`, `subjectId`, and bounded lineage refs only; `repository_tree_snapshot` also requires `repositoryRef`, `rootRef`, `treeObjectRef`, and content-free path metadata only; `import_preview_record` also requires `importHistoryRef`, `repositoryTreeRef`, `previewFingerprint`, and content-free path metadata only; `program_execution_record` also requires `runtimeId`, `languageId`, `programFingerprint`, and metadata-only I/O envelope fields; `prompt_artifact_record` also requires `artifactKind`, `title`, and `contentFingerprint`, and accepts content refs/fingerprints only instead of raw prompt bodies; `device_register` requires trusted internal authority, explicit `apnsEnvironment`, and hash-only APNs token custody; `notification_send` with `pushRequested` records redacted delivery evidence only and does not send APNs. `subagent_launch` requires `objectiveSummary`, `promptSummary`, and `modelPolicy: bounded_placeholder_v1`; other mutating \
