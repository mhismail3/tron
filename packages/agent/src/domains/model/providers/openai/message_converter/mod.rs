...
pub fn generate_capability_instruction_text(capabilities: &[ModelCapability]) -> String {
    ...
    format!(
        ...
        `web_fetch`, `web_robots_check`, `web_source_list`, `web_source_inspect`, `web_source_archive`, `tool_source_list`, `tool_source_inspect`, `subagent_launch`, `subagent_status`, `subagent_result`, `subagent_cancel`, `subagent_task_list`, `subagent_task_inspect`, `worker_package_list`, `worker_package_inspect`, `trace_list`, `trace_get`, `log_recent`, `replay_manifest`, \
        ...
        an existing file. `job_start`, `job_cancel`, `goal_create`, `goal_cancel`, `question_create`, `question_answer`, `web_robots_check`, `web_source_archive`, `subagent_launch`, and `subagent_cancel` require a stable `idempotencyKey`; `question_answer` also requires `expectedQuestionVersionId` and `reason`; `web_fetch` with robots evidence requires both `webRobotsPolicyResourceId` and `expectedWebRobotsPolicyVersionId`; `web_source_archive` also requires `expectedWebSourceVersionId` and `reason`; `subagent_launch` requires `objectiveSummary`, `promptSummary`, and `modelPolicy: bounded_placeholder_v1`; other mutating \
