...
pub(super) const SNAPSHOT_ID_MAX_BYTES: usize = 160;
...
pub(super) fn reject_raw_repository_tree_fields(payload: &Value) -> Result<(), CapabilityError> {
    for field in [
        "rawImportPayload",
        "importPayload",
        "repositoryTree",
        ...
        if payload.get(field).is_some() {
            return Err(invalid(format!(
                "{field} is not accepted; repository tree snapshots store bounded metadata only"
            )));
        }
    }
    Ok(())
...
pub(super) fn idempotency_key(
    ...
    optional_string(payload, "idempotencyKey")?
        ...
        .transpose()?
        .ok_or_else(|| invalid("repository_tree_snapshot requires an idempotencyKey"))
}
...
pub(super) fn resource_scope(
    ...
    invocation
        ...
        .ok_or_else(|| {
            invalid("repository tree operations require trusted session or workspace scope")
        })
}
...
pub(super) fn retention_policy(payload: &Value) -> Result<Value, CapabilityError> {
    ...
    Ok(json!({
        "privacyClass": "repository_tree_metadata",
        "policy": "content_free_tree_snapshot_metadata_only",
...
pub(super) fn tree_counts(
    ...
    let file_count = bounded_count(payload, "fileCount", total_entries)?;
    let directory_count = bounded_count(payload, "directoryCount", total_entries)?;
    let symlink_count = bounded_count(payload, "symlinkCount", total_entries)?;
    let submodule_count = bounded_count(payload, "submoduleCount", total_entries)?;
...
fn sanitize_path_entry(index: usize, value: &Value) -> Result<Value, CapabilityError> {
    ...
    for key in ["mode", "objectRef", "contentHash"] {
    ...
    if item.keys().any(|key| {
        !matches!(
            key.as_str(),
            "path" | "kind" | "mode" | "objectRef" | "contentHash" | "sizeBytes"
        )
    }) {
        return Err(invalid(format!(
            "{label} may contain only path, kind, mode, objectRef, contentHash, and sizeBytes"
        )));
    }
    Ok(Value::Object(sanitized))
}
