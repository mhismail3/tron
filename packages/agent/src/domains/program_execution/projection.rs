...
pub(super) fn program_execution_summary(
    ...
    json!({
        ...
        "programId": projected_string(payload, "programId", PROJECTION_ID_BYTES),
        "runtimeId": projected_string(payload, "runtimeId", PROJECTION_ID_BYTES),
        "languageId": projected_string(payload, "languageId", PROJECTION_ID_BYTES),
        "programFingerprint": projected_string(payload, "programFingerprint", PROJECTION_ID_BYTES),
        "sourceRef": projected_ref_item(payload.get("sourceRef")),
        "inputRef": projected_ref_item(payload.get("inputRef")),
        "outputRef": projected_ref_item(payload.get("outputRef")),
        "resourceLimits": projected_resource_limits(payload.get("resourceLimits")),
        "ioEnvelope": projected_io_envelope(payload.get("ioEnvelope")),
        ...
        "resourceRefs": [version_ref(resource, version, "program_execution")]
    })
}

pub(super) fn inspected_program_execution(
    resource: &EngineResource,
    version: &EngineResourceVersion,
    payload: &Value,
) -> Value {
    json!({
        ...
        "projection": {
            "allowlist": "program_execution_record_redacted_v1",
...
fn projected_resource_limits(value: Option<&Value>) -> Value {
    let Some(Value::Object(limits)) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in ["maxWallClockMs", "maxMemoryMb", "maxOutputBytes"] {
        if let Some(value) = limits.get(key).and_then(Value::as_u64) {
            projected.insert(key.to_owned(), json!(value));
        }
    }
    for key in ["declaredOnly", "enforcedByRuntime"] {
        if let Some(value) = limits.get(key).and_then(Value::as_bool) {
            projected.insert(key.to_owned(), json!(value));
        }
    }
    Value::Object(projected)
...
fn projected_io_envelope(value: Option<&Value>) -> Value {
    let Some(Value::Object(envelope)) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in [
        ...
        if let Some(value) = envelope.get(key).and_then(Value::as_bool) {
            projected.insert(key.to_owned(), json!(value));
        }
    }
    for key in ["inputFingerprint", "outputFingerprint"] {
        insert_projected_string(envelope, &mut projected, key, PROJECTION_ID_BYTES);
    }
    for key in ["inputRef", "outputRef"] {
        if let Some(value) = envelope.get(key) {
            projected.insert(key.to_owned(), projected_ref_item(Some(value)));
        }
    }
    Value::Object(projected)
}
...
fn projected_authority(value: Option<&Value>) -> Value {
    ...
    json!({
        ...
        "contentFreeProgramExecution": authority.get("contentFreeProgramExecution").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value)),
        "runtimeExecutionPerformed": authority.get("runtimeExecutionPerformed").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value)),
        "processLaunched": authority.get("processLaunched").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value))
    })
}
