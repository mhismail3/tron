...
use crate::shared::server::error_mapping::map_import_error;

async fn list_sources(_deps: &Deps) -> Result<Value, CapabilityError> {
    run_blocking_task("import::list_sources", move || {
            let claude_projects =
                PathBuf::from(crate::shared::paths::home_dir()).join(".claude").join("projects");
            let Ok(projects) = crate::domains::import::discover_projects(&claude_projects) else {
...
async fn list_sessions(payload: &Value, deps: &Deps) -> Result<Value, CapabilityError> {
    ...
    run_blocking_task("import::list_sessions", move || {
        let claude_projects = PathBuf::from(crate::shared::paths::home_dir())
            .join(".claude")
            .join("projects");
        let project_dir = claude_projects.join(&encoded_dir);
        let sessions =
            crate::domains::import::discover_sessions(&project_dir).map_err(map_import_error)?;
    ...
    run_blocking_task("import::preview_session", move || {
            ...
            for spec in &result.events {
                match spec.event_type {
                    crate::domains::session::event_store::EventType::MessageUser => {
    ...
    run_blocking_task("import::execute", move || {
            ...
            let wd = if working_directory.is_empty() {
                path.parent()
                    ...
                    .and_then(|n| n.to_str())
                    .map(crate::domains::import::parser::decode_project_dir)
            ...
            match crate::domains::import::import_session(&event_store, &path, &wd, &tags, Some(&origin)) {
                Ok(result) => {
                ...
                Err(crate::domains::import::ImportError::AlreadyImported { tron_session_id }) => {
                    Ok(json!({
...
fn import_warning_to_json(warning: &crate::domains::import::ImportWarning) -> Value {
