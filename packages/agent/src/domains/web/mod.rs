...
//!
//! Slice 8A restores only explicit direct URL fetch as source evidence. The
...
//! side channels, or public `/engine` web functions. Network access must fail
//! closed unless the trusted runtime context carries a derived grant whose
//! network policy explicitly permits direct declared fetch.
