...
pub(super) fn module_lifecycle_summary(
    ...
    json!({
        ...
        "transitionId": projected_string(payload, "transitionId", PROJECTION_ID_BYTES),
        "installDecision": projected_ref(payload.get("installDecision")),
        "transition": projected_transition(payload.get("transition")),
        "runtimeAuthorization": projected_runtime_authorization(payload.get("runtimeAuthorization")),
        ...
        "resourceRefs": [version_ref(resource, version, "module_lifecycle_state")]
    })
}

pub(super) fn inspected_module_lifecycle(
    resource: &EngineResource,
    version: &EngineResourceVersion,
    payload: &Value,
) -> Value {
    json!({
        ...
        "moduleLifecycle": {
            ...
            "installDecision": projected_ref(payload.get("installDecision")),
            "transition": projected_transition(payload.get("transition")),
            "previous": projected_previous(payload.get("previous")),
...
fn projected_transition(value: Option<&Value>) -> Value {
    let Some(Value::Object(transition)) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in ["action", "from", "to", "reason"] {
        insert_projected_string(transition, &mut projected, key, PROJECTION_STRING_BYTES);
    }
    for key in [
        "metadataOnly",
        "stateMutationOnly",
        ...
        if let Some(value) = transition.get(key).and_then(Value::as_bool) {
...
fn projected_previous(value: Option<&Value>) -> Value {
    let Some(Value::Object(previous)) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in ["state", "versionId"] {
        insert_projected_string(previous, &mut projected, key, PROJECTION_ID_BYTES);
    }
    if let Some(value) = previous
        .get("currentVersionRevalidated")
        .and_then(Value::as_bool)
    {
        projected.insert("currentVersionRevalidated".to_owned(), json!(value));
    }
    Value::Object(projected)
}
...
fn projected_policy(value: Option<&Value>) -> Value {
    ...
    let mut projected = Map::new();
    for key in ["status", "metadataOnly", "rollbackExecuted"] {
    ...
    if let Some(child) = policy.get("proofRefs") {
        projected.insert("proofRefs".to_owned(), projected_refs(Some(child)));
    }
    Value::Object(projected)
}

fn projected_runtime_authorization(value: Option<&Value>) -> Value {
    let Some(Value::Object(authz)) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in [
        "failClosed",
        "enabledAllowsRuntime",
        "disabledDenied",
        "quarantinedDenied",
        "rolledBackDenied",
    ] {
        if let Some(value) = authz.get(key).and_then(Value::as_bool) {
            projected.insert(key.to_owned(), json!(value));
        }
    }
    Value::Object(projected)
...
fn projected_scope(resource: &EngineResource, value: Option<&Value>) -> Value {
    ...
    let mut projected = Map::new();
    insert_projected_string(scope, &mut projected, "kind", PROJECTION_ID_BYTES);
    insert_projected_string(scope, &mut projected, "value", PROJECTION_ID_BYTES);
    Value::Object(projected)
}
...
fn projection_policy() -> Value {
    json!({
        "allowlist": "module_lifecycle_metadata_redacted_v1",
        "metadataOnly": true,
