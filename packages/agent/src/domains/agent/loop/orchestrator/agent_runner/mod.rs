...
use crate::domains::agent::r#loop::types::{AgentRunTrigger, RunContext, RunResult};
...
/// Run an agent from a closed durable trigger.
#[instrument(skip_all, fields(session_id = agent.session_id()))]
pub(crate) async fn run_agent_trigger(
    agent: &mut TronAgent,
    ...
    // Run the agent.
    let result = agent.run_trigger(trigger, ctx).await;
