...
fn make_agent_with_responder(responder: Arc<dyn ModelResponder>) -> (TronAgent, JournalCleanup) {
...
async fn cancelled_turn_uses_session_ordinal_and_row_backed_sequence() {
    ...
    let failure_payload: serde_json::Value = serde_json::from_str(&failures[0].payload).unwrap();
...
async fn partial_cancellation_atomically_persists_message_and_failure() {
    ...
    let assistant_payload: serde_json::Value = serde_json::from_str(&assistant.payload).unwrap();
...
async fn run_agent_error_still_defers_terminal_lifecycle() {
    let (mut agent, _journal) = make_agent_with_responder(Arc::new(StreamBackedResponder::new(
        vec![Err(ModelResponseError::other("expired"))],
    )));
