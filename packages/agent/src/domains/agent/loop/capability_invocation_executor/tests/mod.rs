...
use crate::engine::{
    ActorId, AuthorityGrantId, AuthorityRequirement, EffectClass, FunctionDefinition, FunctionId,
...
#[tokio::test]
async fn direct_tool_uses_typed_payload_and_creates_no_authority_grant() {
    let host = EngineHostHandle::new_in_memory().unwrap();
    ...
    .with_required_authority(AuthorityRequirement::scope("legacy.scope.must.be.ignored"))
