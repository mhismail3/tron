...
use super::rules_discovery::DiscoveredRulesFile;
use super::rules_index::RulesIndex;
use super::types::RulesLevel;
...
mod tests {
    ...
    fn touch_path_unrelated_causes_no_activation() {
        ...
        let activated = tracker.touch_path("src/domains/agent/runner/agent.ts");
        assert!(!activated);
        assert_eq!(tracker.activated_scoped_rules_count(), 0);
    }
    ...
    fn get_touched_paths() {
        ...
        let _ = tracker.touch_path("src/context/loader.ts");
        let _ = tracker.touch_path("src/domains/agent/runner/agent.ts");

        let touched = tracker.touched_paths();
        assert_eq!(touched.len(), 2);
        assert!(touched.contains("src/context/loader.ts"));
        assert!(touched.contains("src/domains/agent/runner/agent.ts"));
    }
