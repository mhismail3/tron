...
impl CompactionTrigger {
    ...
    pub fn should_compact(&self, current_token_ratio: f64) -> CompactionTriggerResult {
        // 1. Token threshold — primary trigger
        if current_token_ratio >= self.config.trigger_token_threshold {
            #[allow(clippy::cast_possible_truncation, clippy::cast_sign_loss)]
            let pct = (current_token_ratio * 100.0) as u32;
...
mod tests {
    ...
    fn test_token_threshold_triggers() {
        ...
        let result = trigger.should_compact(0.75);
    ...
    fn test_token_threshold_exact_boundary() {
        let trigger = CompactionTrigger::new(CompactionTriggerConfig::default());
        let result = trigger.should_compact(0.70);
        assert!(result.compact);
    }
    ...
    fn test_below_token_threshold_no_trigger() {
        let trigger = CompactionTrigger::new(CompactionTriggerConfig::default());
        let result = trigger.should_compact(0.69);
        assert!(!result.compact);
    }
    ...
    fn test_no_turn_count_recovery_even_after_many_turns() {
        ...
        for _ in 0..100 {
            let result = trigger.should_compact(0.3);
