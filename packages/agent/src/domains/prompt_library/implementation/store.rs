...
//! contexts (inside the supervised blocking-task boundary for async callers).

use base64::Engine;
use chrono::Utc;
use rusqlite::{OptionalExtension, params};

use crate::domains::prompt_library::normalize::{hash_hex, is_blank, normalize_for_hash};
use crate::domains::prompt_library::types::{HistoryItem, HistoryPage, RecordOutcome, Snippet};
use crate::domains::session::event_store::sqlite::contention::{
    BusyRetryPolicy, RetryError, is_rusqlite_busy, retry_on_busy,
};
use crate::domains::session::event_store::{ConnectionPool, EventStoreError, Result};
...
pub fn record_prompt(pool: &ConnectionPool, text: &str) -> Result<RecordOutcome> {
    ...
    // from a supervised blocking task and must tolerate brief contention.
