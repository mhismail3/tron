...
pub(super) fn module_validation_report_summary(
    ...
    json!({
        ...
        "reportId": projected_string(payload, "reportId", PROJECTION_ID_BYTES),
        ...
        "commandRefCount": ref_count(payload.pointer("/evidence/commands")),
        "resultRefCount": ref_count(payload.pointer("/evidence/results")),
        "failureEvidenceCount": ref_count(payload.pointer("/evidence/failures")),
        "projectionParity": projected_projection_parity(payload.get("projectionParity")),
        "validation": projected_validation(payload.get("validation")),
        "lifecycle": projected_lifecycle(payload.get("lifecycle")),
        "noInstallNoExecutionProof": projected_safety_proof(payload.get("noInstallNoExecutionProof")),
        ...
        "resourceRefs": [version_ref(resource, version, "module_validation_report")]
    })
}

pub(super) fn inspected_module_validation_report(
    resource: &EngineResource,
    version: &EngineResourceVersion,
    payload: &Value,
) -> Value {
    json!({
        ...
        "validationReport": {
            ...
            "identity": projected_identity(payload.get("identity")),
            "subjectRefs": projected_subject_refs(payload.get("subjectRefs")),
            "projectionParity": projected_projection_parity(payload.get("projectionParity")),
            "evidence": projected_evidence(payload.get("evidence")),
            "validation": projected_validation(payload.get("validation")),
            "lifecycle": projected_lifecycle(payload.get("lifecycle")),
        ...
        "projection": {
            "allowlist": "module_validation_report_metadata_redacted_v1",
            "metadataOnly": true,
            "rawValidationReportBodyReturned": false,
...
fn projected_validation(value: Option<&Value>) -> Value {
    ...
    json!({
        "status": validation
            ...
            .map(|text| projected_text(text, PROJECTION_ID_BYTES))
            .unwrap_or(Value::Null),
        "checks": projected_checks(validation.get("checks"))
    })
}
...
fn projected_subject_refs(value: Option<&Value>) -> Value {
    match value {
        Some(Value::Object(map)) => {
            let mut projected = Map::new();
            for key in ["modules", "proposals"] {
        ...
        _ => json!({
            "modules": {"items": [], "total": 0, "truncated": false},
            "proposals": {"items": [], "total": 0, "truncated": false}
        }),
    }
}

fn projected_projection_parity(value: Option<&Value>) -> Value {
    match value {
        Some(Value::Object(map)) => {
            let mut projected = Map::new();
            for key in ["manifest", "resource", "provider"] {
                if let Some(child) = map.get(key) {
                    projected.insert(key.to_owned(), projected_checks(Some(child)));
                }
            }
            Value::Object(projected)
        }
        _ => json!({
            "manifest": {"items": [], "total": 0, "truncated": false},
            "resource": {"items": [], "total": 0, "truncated": false},
            "provider": {"items": [], "total": 0, "truncated": false}
        }),
    }
}

fn projected_evidence(value: Option<&Value>) -> Value {
    match value {
        Some(Value::Object(map)) => {
            let mut projected = Map::new();
            for key in [
        ...
        _ => json!({
            ...
            "commands": {"items": [], "total": 0, "truncated": false},
            "results": {"items": [], "total": 0, "truncated": false},
            "failures": {"items": [], "total": 0, "truncated": false},
...
fn projected_checks(value: Option<&Value>) -> Value {
    match value {
        Some(Value::Array(items)) => json!({
            "items": items.iter().take(MAX_PROJECTED_REFS).map(projected_check_item).collect::<Vec<_>>(),
            "total": items.len(),
...
fn projected_check_item(value: &Value) -> Value {
    let Value::Object(item) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in ["name", "status", "summary", "fingerprint"] {
