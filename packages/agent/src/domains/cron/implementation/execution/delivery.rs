...
use crate::domains::cron::executor::ExecutorDeps;
use crate::domains::cron::types::{CronJob, CronRun, Delivery, DeliveryOutcome};
...
///
/// A naive single-shot UPDATE silently swallows contention — a brief
/// lock collision on a busy DB leaves `cron_runs.delivery_status` in
/// whatever state the previous run left it (often NULL), which makes
/// the distinction between "not yet delivered" and "delivery status
/// lost" un-observable. Three attempts with a short linear backoff
/// cover the common case; after that we log the attempted status at
/// warn so the operator can reconstruct the intent from the log even
/// without the DB row.
async fn persist_delivery_status_with_retry(
    ...
    for attempt in 1..=ATTEMPTS {
        match crate::domains::cron::store::update_delivery_status(&deps.pool, run_id, status) {
            Ok(()) => return,
            Err(e) => {
                if attempt == ATTEMPTS {
                    tracing::warn!(
...
async fn deliver_engine_stream(
    ...
    let event_publisher = deps.event_publisher.get().ok_or_else(|| {
        crate::domains::cron::errors::CronError::Execution("no event publisher".into())
    })?;
...
async fn deliver_apns(
    ...
    let notifier = deps.push_notifier.as_ref().ok_or_else(|| {
        crate::domains::cron::errors::CronError::Execution("no push notifier".into())
    })?;

    let title = title.unwrap_or(&job.name);
    let body = match &run.status {
        crate::domains::cron::types::RunStatus::Completed => run
            .output
            .as_deref()
...
async fn deliver_webhook(
    ...
    let payload = serde_json::to_value(run)
        .map_err(|e| crate::domains::cron::errors::CronError::Execution(e.to_string()))?;
    req = req.json(&payload);

    let resp = req.send().await.map_err(|e| {
        crate::domains::cron::errors::CronError::Execution(format!("delivery webhook: {e}"))
    })?;
    ...
    if !status.is_success() {
        ...
        return Err(crate::domains::cron::errors::CronError::Execution(format!(
...
mod tests {
    ...
    impl crate::domains::cron::executor::PushNotifier for MockPushNotifier {
        async fn notify(
            &self,
            _title: &str,
            _body: &str,
