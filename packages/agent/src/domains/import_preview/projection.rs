...
pub(super) fn import_preview_summary(
    ...
    json!({
        ...
        "previewId": projected_string(payload, "previewId", PROJECTION_ID_BYTES),
        "importHistoryRef": projected_ref_item(payload.get("importHistoryRef")),
        "repositoryTreeRef": projected_ref_item(payload.get("repositoryTreeRef")),
        "repositoryRef": projected_ref_item(payload.get("repositoryRef")),
        "rootRef": projected_ref_item(payload.get("rootRef")),
        "previewFingerprint": projected_string(payload, "previewFingerprint", PROJECTION_ID_BYTES),
        ...
        "resourceRefs": [version_ref(resource, version, "import_preview")]
    })
}

pub(super) fn inspected_import_preview(
    resource: &EngineResource,
    version: &EngineResourceVersion,
    payload: &Value,
) -> Value {
    json!({
        ...
        "projection": {
            "allowlist": "import_preview_record_redacted_v1",
            "rawPayloadReturned": false,
            "rawPreviewPayloadReturned": false,
            "rawPreviewPayloadReturned": false,
            "rawImportPayloadReturned": false,
...
fn projected_metadata(value: Option<&Value>) -> Value {
    ...
    for key in ["previewLabel", "previewSummary", "changeSummary"] {
    ...
    for key in [
        ...
        "rawPreviewPayloadStored",
        "rawRepositoryContentsStored",
        "rawBlobBytesStored",
        "absolutePathsStored",
        "importExecutionPerformed",
...
fn projected_authority(value: Option<&Value>) -> Value {
    ...
    json!({
        ...
        "contentFreePreview": authority.get("contentFreePreview").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value)),
        "importExecutionPerformed": authority.get("importExecutionPerformed").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value)),
        "gitMutationPerformed": authority.get("gitMutationPerformed").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value))
    })
}
