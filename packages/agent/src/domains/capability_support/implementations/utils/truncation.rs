...
//!
//! Provides character-based head+tail truncation with blob reference markers
//! for process and job output.
...
/// Maximum output shown per job in wait-capability results.
...
pub fn truncate_output(output: &str, max_tokens: usize, options: &TruncateOptions) -> String {
    ...
    if available > 0 && available < output.len() {
        let safe_slice = crate::shared::text::truncate_str(output, available);
