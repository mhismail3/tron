...
pub(super) fn prompt_artifact_summary(
    ...
    json!({
        ...
        "artifactId": projected_string(payload, "artifactId", PROJECTION_ID_BYTES),
        "artifactKind": projected_string(payload, "artifactKind", PROJECTION_ID_BYTES),
        ...
        "preview": projected_string(payload, "preview", PROJECTION_STRING_BYTES),
        "content": projected_content(payload.get("content")),
        ...
        "resourceRefs": [version_ref(resource, version, "prompt_artifact")]
    })
}

pub(super) fn inspected_prompt_artifact(
    resource: &EngineResource,
    version: &EngineResourceVersion,
    payload: &Value,
) -> Value {
    json!({
        ...
        "payload": {
            ...
            "createdAt": projected_string(payload, "createdAt", PROJECTION_TIMESTAMP_BYTES),
            "updatedAt": projected_string(payload, "updatedAt", PROJECTION_TIMESTAMP_BYTES),
            "retention": projected_object(payload.get("retention"), &["privacyClass", "policy", "state"], &["maxAgeDays"]),
            "metadata": projected_metadata(payload.get("metadata")),
        ...
        "projection": {
            "allowlist": "prompt_artifact_redacted_v1",
            "rawPayloadReturned": false,
            "rawPromptReturned": false,
            "providerVisibleRawPayloadReturned": false,
...
fn projected_metadata(value: Option<&Value>) -> Value {
    ...
    for key in [
        "promptArtifactMetadataOnly",
        "explicitOptIn",
        "automaticCapturePerformed",
        "promptInjectionPerformed",
        "promptContextIncluded",
        "learnedBehaviorUpdated",
        "fileWritesPerformed",
        "networkAccessPerformed",
        "rawPromptStored",
        "providerVisibleRawPayloadStored",
...
fn projected_content(value: Option<&Value>) -> Value {
    let Some(Value::Object(content)) = value else {
        return Value::Null;
    };
    let mut projected = Map::new();
    for key in ["contentFingerprint"] {
        insert_projected_string(content, &mut projected, key, PROJECTION_ID_BYTES);
    }
    for key in [
        "metadataOnly",
        "rawPromptStored",
    ...
    if let Some(value) = content.get("contentRef") {
        projected.insert("contentRef".to_owned(), projected_ref_item(Some(value)));
    }
    Value::Object(projected)
}
...
fn projected_authority(value: Option<&Value>) -> Value {
    ...
    json!({
        ...
        "promptArtifactMetadataOnly": authority.get("promptArtifactMetadataOnly").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value)),
        "rawPromptStored": authority.get("rawPromptStored").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value)),
        "promptInjectionPerformed": authority.get("promptInjectionPerformed").and_then(Value::as_bool).map_or(Value::Null, |value| json!(value))
    })
}
