...
async fn rebase_on_main_session_not_found() {
    ...
    assert!(
        matches!(
            err,
            crate::domains::worktree::errors::WorktreeError::NotFound { .. }
        ),
...
async fn rebase_on_main_pending_merge_exists() {
    ...
    // Inject a pending merge directly into coordinator state.
    {
        ...
        state.pending_merges.insert(
            session_id.clone(),
            crate::domains::worktree::types::PendingMergeState {
    ...
    assert!(
        matches!(
            err,
            crate::domains::worktree::errors::WorktreeError::PendingMergeExists
        ),
...
async fn rebase_on_main_missing_base_branch_and_no_override() {
    ...
    assert!(
        matches!(
            err,
            crate::domains::worktree::errors::WorktreeError::MissingBaseBranch
        ),
...
async fn rebase_on_main_conflict_resolve_continue_pops_stash_and_fires_rebased_on_main() {
    ...
    coord
        .resolve_conflict(
            &session_id,
            "f.txt",
            crate::domains::worktree::types::ConflictResolution::Ours,
        )
        .await
        .unwrap();
...
async fn stash_pop_continue_rejects_unresolved_paths() {
    ...
    assert!(matches!(
        err,
        crate::domains::worktree::errors::WorktreeError::Git(_)
    ));
...
async fn stash_pop_abort_is_idempotent_under_repeat_call() {
    ...
    assert!(matches!(
        err,
        crate::domains::worktree::errors::WorktreeError::NoPendingMerge
    ));
}
