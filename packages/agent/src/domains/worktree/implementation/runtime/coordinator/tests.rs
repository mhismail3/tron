use super::*;
use crate::domains::session::event_store::{ConnectionConfig, new_in_memory, run_migrations};
...
async fn acquire_mode_never() {
    ...
    let config = WorktreeConfig {
        mode: crate::domains::settings::types::IsolationMode::Never,
...
async fn rebuild_from_events_restores_repo_tracking_for_lazy_mode() {
    ...
    let lazy_coord = WorktreeCoordinator::new(
        WorktreeConfig {
            mode: crate::domains::settings::types::IsolationMode::Lazy,
            ..WorktreeConfig::default()
        },
        store,
    );
...
async fn delete_branch_rejects_active_branch() {
    ...
    assert!(
        matches!(
            err,
            crate::domains::worktree::WorktreeError::BranchActive(_)
        ),
...
async fn delete_branch_rejects_wrong_prefix() {
    ...
    let err = coord
        ...
        .unwrap_err();
    assert!(
        matches!(err, crate::domains::worktree::WorktreeError::Git(_)),
...
async fn list_local_branches_errors_without_fallback_for_untracked_session() {
    ...
    assert!(
        matches!(
            err,
            crate::domains::worktree::WorktreeError::NotFound { .. }
        ),
...
async fn coordinator_commit_amend_on_empty_repo_returns_err() {
    ...
    let err = coord
        ...
        .unwrap_err();
    assert!(
        matches!(err, crate::domains::worktree::WorktreeError::Git(ref m) if m.contains("amend")),
...
async fn concurrent_acquire_same_session_creates_exactly_one_worktree() {
    ...
    let acquired_count = events
        ...
        .filter(|e| {
            e.event_type
                == crate::domains::session::event_store::EventType::WorktreeAcquired.as_str()
        })
        .count();
    assert_eq!(
        acquired_count, 1,
