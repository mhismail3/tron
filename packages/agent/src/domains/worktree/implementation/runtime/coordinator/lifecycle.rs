...
use crate::domains::worktree::isolation;
use crate::domains::worktree::types::{AcquireResult, DeferralReason};
...
impl WorktreeCoordinator {
    ...
    pub async fn maybe_acquire_with_override(
        ...
        // Serialize concurrent acquire attempts for the same session.
        // Without this, two in-flight prompts for the same session_id
        // (fast double-tap, reconnection mid-send) would both pass the
        // cache check, both call `lifecycle::create`, and both emit
        // `worktree.acquired` — leaking one of the resulting worktrees.
        // Holding the lock across the entire cache-check → create →
        // track window makes the second caller see the first's tracked
        // state on its cache check and return the already-acquired
        // worktree.
        ...
        let info = crate::domains::worktree::lifecycle::create(
            session_id,
            working_dir,
            &self.config,
            &self.git,
    ...
    pub async fn release(&self, session_id: &str) -> Result<()> {
        ...
        let release_info =
            match crate::domains::worktree::lifecycle::remove(&info, &self.config, &self.git).await
            {
