...
pub(super) const SUMMARY_MAX_BYTES: usize = 1_000;
pub(super) const IDEMPOTENCY_KEY_MAX_BYTES: usize = 256;
pub(super) const MAX_REFS: usize = 25;
pub(super) const TIMEOUT_MS_DEFAULT: u64 = 30_000;
pub(super) const TIMEOUT_MS_MAX: u64 = 120_000;
...
pub(super) fn idempotency_key(
    ...
    optional_string(payload, "idempotencyKey")?
        ...
        .transpose()?
        .ok_or_else(|| invalid("module_runtime write operations require an idempotencyKey"))
}
...
pub(super) fn resource_scope(
    ...
    invocation
        ...
        .ok_or_else(|| invalid("module runtime requires trusted session or workspace scope"))
}

pub(super) fn runtime_state(payload: &Value) -> Result<String, CapabilityError> {
    let state = optional_string(payload, "runtimeState")?.unwrap_or_else(|| "running".to_owned());
    if matches!(
        state.as_str(),
        "requested" | "running" | "completed" | "failed"
    ) {
        bounded_provider_visible_token("runtimeState", &state, TOKEN_MAX_BYTES)
    } else {
        Err(invalid(format!("unsupported module runtime state {state}")))
    }
}
...
pub(super) fn validate_module_lifecycle_resource_id(value: &str) -> Result<(), CapabilityError> {
...
pub(super) fn validate_module_runtime_resource_id(value: &str) -> Result<(), CapabilityError> {
    if !value.starts_with("module_runtime_state:") {
        return Err(invalid(
            "moduleRuntimeResourceId has unsupported resource kind",
        ));
    }
    bounded_provider_visible_token("moduleRuntimeResourceId", value, TOKEN_MAX_BYTES).map(|_| ())
}
...
fn sanitize_ref_item(label: &str, value: &Value) -> Result<Value, CapabilityError> {
    ...
    for key in ["status", "fingerprint", "summary"] {
        if let Some(value) = item.get(key).and_then(Value::as_str) {
            let sanitized_value = if key == "summary" {
                bounded_text("ref.summary", value, SUMMARY_MAX_BYTES)?
            } else {
    ...
    if item.keys().any(|key| {
        ...
        return Err(invalid(format!(
            "{label} may contain only kind, id/resourceId, role, versionId, status, fingerprint, and summary"
        )));
    }
    ...
    match value {
        Value::Object(object) => {
            for (key, child) in object {
                ...
                {
                    return Err(invalid(format!(
                        "{key} is not accepted; module runtime records store bounded metadata and refs only"
                    )));
                }
...
fn reject_shell_command_like(field: &str, value: &str) -> Result<(), CapabilityError> {
    ...
    if tokens.iter().any(|token| {
        ...
        return Err(invalid(format!(
            "{field} must not contain command-like material"
        )));
    }
    Ok(())
}
