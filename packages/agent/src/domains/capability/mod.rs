...
//! | `registry` | Durable catalog projection, plugin manifests, binding decisions, inspection handles, pause/run records, and program runs; search ranking lives in `registry/index.rs`, primer rendering in `registry/primer.rs`, recipes in `registry/recipes.rs`, and persistence in `registry/store.rs` |
