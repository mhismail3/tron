    ...
    match function_id {
        EXECUTE_FUNCTION_ID => json!({
            ...
            "capabilitySchema": {
                "name": "execute",
                "description": concat!(
                    "Intent-first Work router for Tron worker abilities: resolve, prepare, approve when needed, run, and observe one worker ability per call. ",
                    "Start with natural-language intent alone when the target is not already known; provide target only when the user supplied an exact capability id, a prior execute result selected it, or a worker-guide recipe makes it unambiguous. ",
                    "For non-trivial work, act as the orchestrator: delegate focused investigation, implementation, or verification slices to workers with agent::spawn_subagent or spawned helper abilities, fan out before collecting results, then gather outputs through status/result calls or returned artifacts. ",
                    "Put target arguments inside arguments when possible, keep wrapper fields top-level, and never invent targets to satisfy a discovery or shape test. Use operation=discover, or a clear discovery-only intent, when you need ability ids, required fields, schemas, examples, or a safe sequence without executing the target. ",
                    "For filesystem repo discovery, list only known directories such as `.` or paths returned by a prior call; do not call filesystem::list_dir on guessed module, file, or extensionless paths. Locate uncertain names with filesystem::find, filesystem::glob, or filesystem::search_text before listing or reading the exact returned path. ",
                    "For approval-gated high-risk write commands or shell redirection, target process::run with executionMode=sandbox_materialized and expectedOutputs; do not target filesystem::write_file or approval::request. ",
                    "For resource lifecycle targets such as artifact::promote, artifact::discard, materialized_file::promote, materialized_file::discard, and patch::merge, pass prior result version ids as expectedCurrentVersionId; use expectedCurrentVersionId, not versionId, as the target CAS guard. Worktree discard uses worktree::discard_files with repository-relative paths only and pauses for user approval. ",
                    "If you accidentally place target argument fields at the execute root, execute may move them into arguments and select the target by schema fit, but arguments is the canonical shape. If you accidentally set target to capability::execute itself, execute removes that self-target and resolves the real target from intent; do not intentionally wrap execute inside execute. ",
                    "Do not call separate search, inspect, or approval::request tools; this execute primitive owns discovery, freshness, approval, correction, and child execution. A needs_input result means retry the same selected target with the missing arguments. ",
                    "A needs_decomposition result means the request spans multiple target invocations; call execute once per suggested call only when the user still wants the underlying work performed, and report the decomposition result without running suggestions when the user only asked to test or inspect decomposition. Harmless shape mistakes may be corrected, but mutating or elevated-risk work still pauses for freshness and approval before child execution. ",
                    "For autonomous Worker extension, first target self_extension::grant_workspace_autonomy with absolute workspacePath, optional sessionId/reason, and omit workspaceId for the current workspace so execute binds the invocation context; only pass workspaceId when a prior result gave the exact context id. ",
                    "After user approval the result says Safe in this workspace, and workspace-visible helper spawns must pass grantId as workspaceAutonomyGrantId plus the returned workspaceId as workspaceId; if resourceSelectors is omitted, worker::spawn derives the child selector as workspace:<workspaceId>. Use that same returned workspaceId as execute's top-level workspaceId when watching, inspecting, or invoking the workspace-visible helper. ",
                    "Then target worker::protocol_guide, author the worker, target worker::spawn, watch catalog::watch_snapshot or capability::inspect, run conformance/test evidence, and invoke the new function through execute. For Worker Packs, use execute with module::register_package, module::inspect_package, module::configure, module::activate, module::disable, module::rollback, module::revoke_source_approval, and module::remove_package; remove only after live activations are disabled or quarantined and never use remote package discovery. ",
                    "Use engine::promote for governed workspace/system promotion, stop sandbox helpers with sandbox::stop_spawned_worker, discard temporary repo helper files with approval-gated worktree::discard_files using relative paths, and use worker::disconnect only for raw volatile worker cleanup. ",
                    "Report Work status, outcomes, blockers, and cleanup state in chat. Keep grant ids, trace ids, resource refs, catalog revision, child invocation ids, function ids, and raw schemas in Audit unless the user explicitly asks for audit/debug details. ",
                    "Do not invent constraints such as riskMax for ordinary work; use constraints only when the user explicitly gives a hard bound. Network reads such as web::search and web::fetch are medium-risk pure reads, so riskMax=low intentionally rejects them."
                ),
...
mod tests {
    ...
    fn execute_description_teaches_self_modifying_worker_lifecycle() {
        ...
        assert!(
            !description.contains("cite trace ids"),
            "execute model description must not instruct normal chat to expose raw evidence ids"
        );
    }
