...
fn execute_model_request_schema() -> serde_json::Value {
    ...
    properties.insert(
        "sourceRefs".to_owned(),
        json!({"type": "array", "description": "Bounded non-secret source refs for notification_send, media_create, import_history_record, repository_tree_snapshot, import_preview_record, program_execution_record, or update_diagnostic_record replay evidence."}),
    );
