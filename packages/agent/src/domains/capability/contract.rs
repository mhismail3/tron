...
use super::{import_history_contract, media_contract, scheduler_contract};
...
fn execute_model_request_schema() -> serde_json::Value {
    ...
    properties.insert(
        "sourceRefs".to_owned(),
        json!({"type": "array", "description": "Bounded non-secret source refs for notification_send, media_create, or import_history_record replay evidence."}),
    );
