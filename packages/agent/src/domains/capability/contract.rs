...
use super::{
    capability_binding_contract, context_control_contract, import_history_contract,
    import_preview_contract, media_contract, module_dependencies_contract, module_install_contract,
    module_lifecycle_contract, module_runtime_contract, module_validation_contract,
    operation_list_text, program_execution_contract, prompt_artifacts_contract,
...
fn execute_model_request_schema() -> serde_json::Value {
    ...
    insert_string(
        &mut properties,
        "command",
        "Shell command for process_run, job_start, or module_program_execution_start; module execution traces and results redact this raw value.",
    );
    insert_string(
        &mut properties,
        "jobResourceId",
        "Durable job_process resource id for job_status, job_log, job_cancel, or module_program_execution status/cancel/cleanup.",
    );
    insert_string(
        &mut properties,
        "expectedJobVersionId",
        "Expected current job_process version id for exact module_program_execution_cleanup freshness.",
    );
    insert_string(
        &mut properties,
        "goalResourceId",
    ...
    insert_string(
        &mut properties,
        "queryResourceId",
        "Memory query evidence resource id for memory_query_inspect.",
    );
    insert_string(
        &mut properties,
        "decisionResourceId",
        "Memory decision evidence resource id for memory_decision_inspect.",
    );
    insert_string(
        &mut properties,
        "webSourceResourceId",
    ...
    insert_string(
        &mut properties,
        "modelPolicy",
        "Required explicit subagent_launch policy; currently accepted_jobs_program_execution_v1 activates the reviewed jobs/program-execution module pack.",
    );
    insert_string(
        &mut properties,
        "workerKind",
        "Required explicit subagent_launch worker selection; currently module_program_execution.",
    );
    insert_string(
        &mut properties,
        "modulePackId",
        "Required explicit subagent_launch module pack selection; currently jobs_program_execution.",
    );
    properties.insert(
        "handoffRefs".to_owned(),
        json!({"type": "array", "description": "Summary-only subagent_launch handoff refs and fingerprints; raw prompts, results, commands, logs, paths, and output payloads are rejected."}),
    );
    insert_string(
        &mut properties,
    ...
    insert_string(
        ...
        "Durable module_manifest resource id for module_inspect.",
    );
    insert_string(
        &mut properties,
        "moduleProposalResourceId",
        "Durable module_proposal resource id for module_proposal_inspect.",
    );
    ...
    module_dependencies_contract::append_schema_properties(&mut properties);
    capability_binding_contract::append_schema_properties(&mut properties);
    web_research_contract::append_schema_properties(&mut properties);
    module_lifecycle_contract::insert_module_lifecycle_request_fields(&mut properties);
    module_runtime_contract::insert_module_runtime_request_fields(&mut properties);
    context_control_contract::insert_context_control_fields(&mut properties);
    insert_string(
        &mut properties,
        "proposalId",
        "Optional caller-visible module proposal id for module_proposal_record.",
    );
    properties.insert(
        "intendedModuleRefs".to_owned(),
        json!({"type": "array", "description": "Bounded resource-backed refs to intended module identities for module_proposal_record."}),
    );
    properties.insert(
        "docRefs".to_owned(),
        json!({"type": "array", "description": "Bounded resource-backed documentation refs for module_proposal_record."}),
    );
    properties.insert(
        "testRefs".to_owned(),
        json!({"type": "array", "description": "Bounded resource-backed test refs for module_proposal_record."}),
    );
    insert_string(
        &mut properties,
        "proceduralKind",
        "Procedural kind for procedural definition, state, activation request, or activation decision operations: skill, rule, hook, or procedure.",
    );
    insert_string(
        &mut properties,
        "proceduralRecordResourceId",
        "Durable procedural_record resource id for procedural_state_inspect or activation request/decision prerequisites.",
    );
    insert_string(
        &mut properties,
        "definitionId",
        "Optional bounded procedural definition id for procedural_definition_record.",
    );
    insert_string(
        &mut properties,
        "activationRequestId",
        "Optional bounded request id for procedural_activation_request_record.",
    );
    insert_string(
        &mut properties,
        "proceduralActivationRequestResourceId",
        "Durable procedural_activation_request resource id for activation decision/inspect.",
    );
    insert_string(
        &mut properties,
        "activationDecisionId",
        "Optional bounded decision id for procedural_activation_decision_record.",
    );
    insert_string(
        &mut properties,
        "proceduralActivationDecisionResourceId",
        "Durable procedural_activation_decision resource id for activation decision inspect.",
    );
    insert_string(
        &mut properties,
        "requestedAction",
        "Procedural activation request action: activate, deactivate, or rollback.",
    );
    insert_string(
        &mut properties,
        "decision",
        "Procedural activation decision: approve_activation, deny_activation, approve_deactivation, or approve_rollback.",
    );
    properties.insert(
        "triggerDeclarations".to_owned(),
        json!({"type": "array", "description": "Bounded metadata-only trigger declarations for procedural definition/request records; declarations do not register triggers."}),
    );
    properties.insert(
        "validationEvidenceRefs".to_owned(),
        json!({"type": "array", "description": "Bounded validation evidence refs for procedural definition/request records."}),
    );
    properties.insert(
        "rollbackProofRefs".to_owned(),
        json!({"type": "array", "description": "Bounded rollback proof refs for procedural activation request/decision records."}),
    );
    properties.insert(
        "deactivationProofRefs".to_owned(),
        json!({"type": "array", "description": "Bounded deactivation proof refs for procedural activation decision records."}),
    );
    insert_integer(
        &mut properties,
        "maxEvidenceItems",
    ...
    insert_string(
        &mut properties,
        "networkPolicy",
        "Optional module_program_execution_start network policy; only none is accepted in this slice.",
    );
    insert_integer(
        &mut properties,
        "maxResponseBytes",
...
mod tests {
    ...
    fn execute_schema_exposes_primitive_operations_not_catalog_targets() {
        ...
        for field in capability_binding_contract::CAPABILITY_BINDING_SCHEMA_FIELDS {
            assert!(
        ...
        assert!(description.contains("no runtime routing"));
        ...
        assert!(schema["properties"].get("workerKind").is_some());
        assert!(schema["properties"].get("modulePackId").is_some());
        ...
        assert!(schema["properties"].get("expectedJobVersionId").is_some());
        assert!(schema["properties"].get("networkPolicy").is_some());
        assert!(
            schema["properties"]
                .get("moduleProposalResourceId")
                .is_some()
        );
        assert!(schema["properties"].get("proposalId").is_some());
        assert!(schema["properties"].get("intendedModuleRefs").is_some());
        assert!(schema["properties"].get("docRefs").is_some());
        assert!(schema["properties"].get("testRefs").is_some());
        for property in module_validation_contract::MODULE_VALIDATION_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
        for property in module_install_contract::MODULE_INSTALL_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
        for property in module_dependencies_contract::MODULE_DEPENDENCIES_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
        for property in web_research_contract::WEB_RESEARCH_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
        for property in module_lifecycle_contract::MODULE_LIFECYCLE_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
        for property in module_runtime_contract::MODULE_RUNTIME_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
        for property in context_control_contract::CONTEXT_CONTROL_SCHEMA_FIELDS {
            assert!(schema["properties"].get(property).is_some());
        }
