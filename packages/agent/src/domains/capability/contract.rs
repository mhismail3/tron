...
fn execute_model_request_schema() -> serde_json::Value {
    ...
    insert_string(
        &mut properties,
        "lifecycleState",
        "Module proposal lifecycle for module_proposal_record: draft, submitted, superseded, or archived.",
    );
    insert_string(
        &mut properties,
        "validationStatus",
        "Optional bounded placeholder validation status for module_proposal_record.",
    );
    properties.insert(
...
mod tests {
    ...
    fn execute_schema_exposes_primitive_operations_not_catalog_targets() {
        ...
        assert!(schema["properties"].get("lifecycleState").is_some());
        assert!(schema["properties"].get("validationStatus").is_some());
