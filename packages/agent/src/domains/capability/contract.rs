...
use super::{
    import_history_contract, import_preview_contract, media_contract, repository_tree_contract,
...
pub(crate) fn model_metadata(function_id: &str) -> serde_json::Value {
    match function_id {
        EXECUTE_FUNCTION_ID => json!({
                    ...
                    "Choose one operation per call. Catalog discovery operations inspect metadata and conformance only; they do not execute discovered capabilities. Import-preview operations store refs, path metadata, counts, summaries, and fingerprints only; they never execute/apply imports, mutate Git, visualize repositories, or store raw import payloads, preview payloads, file contents, or blob bytes. Update diagnostic operations store signed-release/provenance metadata only; they never perform live network checks, install, restart, deploy, register packages, or store production endpoint details/package bytes. Keep mutation reasons and idempotency keys in this payload when they matter for evidence."
                ),
...
fn execute_model_request_schema() -> serde_json::Value {
    ...
    properties.insert(
        "sourceRefs".to_owned(),
        json!({"type": "array", "description": "Bounded non-secret source refs for notification_send, media_create, import_history_record, repository_tree_snapshot, import_preview_record, or update_diagnostic_record replay evidence."}),
    );
