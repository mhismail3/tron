...
fn execute_model_request_schema() -> serde_json::Value {
    json!({
        ...
        "properties": {
            ...
            "arguments": {"type": "object", "description": "Arguments for the resolved target capability only. Example for process::run: {\"command\":\"date\",\"executionMode\":\"read_only\"}. Omit arguments for pure discovery if required fields are not known yet. If execute returns needs_input, retry the same selected target with the missing fields. If execute returns needs_decomposition, make one execute call per suggested call instead of packing multiple target invocations into one arguments object. Do not include wrapper fields such as target, contractId, capabilityId, functionId, implementationId, payload, mode, inspectionHandle, reason, or expectedRevision here. Filesystem paths inside arguments must be exact known paths for list_dir/read_file; find or glob uncertain module names before passing a path. Keep idempotencyKey top-level; when the selected target schema itself requires idempotencyKey, execute copies the top-level key into the target arguments safely. Harmless target argument key casing/separator mistakes are corrected only when they uniquely match the selected target schema; conflicting aliases still fail validation. Unknown top-level fields are accepted only so execute can correct flattened target arguments into arguments and audit that correction."},
            "constraints": {
                "type": "object",
                "additionalProperties": true,
                "properties": {
