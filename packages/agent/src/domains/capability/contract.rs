...
//!
//! This worker is the model-facing harness collapse point: it owns the fully
//! composed `capability::execute` definition, and providers see that one
//! `execute` primitive. The primitive can observe, touch agent-owned state, use hardened
...
use crate::domains::registration::contract::{
    CapabilityContract, function_definition_for_capability,
};
use crate::engine::{
...
/// The single fully composed function definition exposed by this domain worker.
pub(crate) fn execute_function_definition() -> EngineResult<FunctionDefinition> {
    ...
    .build()?;
    let mut definition = function_definition_for_capability(&contract);
    definition
        .metadata
        .as_object_mut()
        .expect("capability contract metadata must be an object")
        .extend(
            execute_model_metadata()
                .as_object()
                .expect("execute model metadata must be an object")
                .clone(),
        );
    Ok(definition)
}

fn execute_model_metadata() -> serde_json::Value {
    json!({
...
mod tests {
    ...
    #[test]
    fn execute_definition_is_model_facing() {
        let definition = execute_function_definition().expect("execute definition");
        assert_eq!(definition.id.as_str(), EXECUTE_FUNCTION_ID);
        assert_eq!(definition.metadata["capabilityPrimitive"], json!(true));
        assert_eq!(definition.metadata["modelPrimitiveName"], json!("execute"));
    }

    #[test]
    fn execute_schema_exposes_primitive_operations_not_catalog_targets() {
        let metadata = execute_function_definition()
