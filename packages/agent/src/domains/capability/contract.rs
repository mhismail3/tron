...
fn execute_model_request_schema() -> serde_json::Value {
    json!({
        ...
        "properties": {
            ...
            "target": {"type": "string", "description": "Optional target hint such as process::run or filesystem::read_file. Omit when discovering or comparing capabilities; use only when the user supplied an exact id, a prior execute result selected it, or a primed recipe makes it unambiguous. For filesystem::list_dir, the path must already be a known directory; use filesystem::find, filesystem::glob, or filesystem::search_text before targeting guessed module names, file names, or extensionless source paths."},
            ...
            "arguments": {"type": "object", "description": "Arguments for the resolved target capability only. Example for process::run: {\"command\":\"date\",\"executionMode\":\"read_only\"}. Omit arguments for pure discovery if required fields are not known yet. If execute returns needs_input, retry the same selected target with the missing fields. If execute returns needs_decomposition, make one execute call per suggested call instead of packing multiple target invocations into one arguments object. Do not include execute wrapper fields such as target, contractId, capabilityId, functionId, implementationId, payload, inspectionHandle, reason, or expectedRevision here unless the selected target schema itself requires a field with that name; for example module::check_health requires arguments.mode. Filesystem paths inside arguments must be exact known paths for list_dir/read_file; find or glob uncertain module names before passing a path. Keep idempotencyKey top-level; when the selected target schema itself requires idempotencyKey, execute copies the top-level key into the target arguments safely. Harmless target argument key casing/separator mistakes are corrected only when they uniquely match the selected target schema; conflicting aliases still fail validation. Unknown top-level fields are accepted only so execute can correct flattened target arguments into arguments and audit that correction."},
            "constraints": {
                "type": "object",
                "additionalProperties": true,
                "properties": {
