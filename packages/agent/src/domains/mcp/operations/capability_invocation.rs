...
use super::{InProcessFunctionHandler, async_trait, mcp_result_to_capability_result};
use crate::domains::mcp::Deps;
use crate::engine::Invocation;
use serde_json::Value;
use serde_json::json;

pub(crate) struct McpCapabilityFunctionHandler {
...
#[async_trait]
impl InProcessFunctionHandler for McpCapabilityFunctionHandler {
    async fn invoke(&self, invocation: Invocation) -> Result<Value, crate::engine::EngineError> {
        ...
        let result = guard
            .call(&self.server, &self.mcp_name, invocation.payload)
            .await
            .map_err(|error| crate::engine::EngineError::DomainFailure {
                domain: "mcp".to_owned(),
                code: "MCP_CAPABILITY_ERROR".to_owned(),
                message: error.to_string(),
                details: Some(json!({
                    "server": self.server,
                    "mcpCapabilityName": self.mcp_name,
                })),
            })?;
        let tron_result = mcp_result_to_capability_result(&result, &self.server, &self.mcp_name);
...
#[cfg(test)]
mod tests {
    use crate::domains::mcp::operations::classify_mcp_capability;
    use crate::engine::{EffectClass, RiskLevel};

    #[test]
    fn classifier_marks_obvious_reads_as_low_risk_pure_reads() {
        let classification = classify_mcp_capability("list_projects", "List project metadata");
    ...
    fn classifier_marks_mutation_words_as_approval_required_side_effects() {
        let classification = classify_mcp_capability("send_email", "Send a message to a recipient");
    ...
    #[test]
    fn classifier_defaults_unknown_capabilities_to_conservative_side_effects() {
        let classification = classify_mcp_capability("frobnicate", "Perform the server operation");
        assert_eq!(classification.effect_class, EffectClass::ExternalSideEffect);
        assert_eq!(classification.risk_level, RiskLevel::Medium);
        assert_eq!(classification.authority_scope, "mcp.write");
        assert!(classification.approval_required);
        assert_eq!(
            classification.reason,
            "unknown_mcp_capability_defaults_to_safe_external_side_effect"
        );
    }
}
