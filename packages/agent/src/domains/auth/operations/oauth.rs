...
pub(crate) async fn auth_oauth_begin(
    ...
    let (auth_url, verifier_or_state) = match provider.as_str() {
        "anthropic" => {
            let pair = crate::domains::auth::provider_credentials::pkce::generate_pkce();
            let config = crate::domains::auth::provider_credentials::anthropic::default_config();
            let url = crate::domains::auth::provider_credentials::anthropic::get_authorization_url_with_state(
                &config,
                &pair.challenge,
                Some(&pair.verifier),
            );
        ...
        "openai-codex" => {
            let pair = crate::domains::auth::provider_credentials::pkce::generate_pkce();
            let config = crate::domains::auth::provider_credentials::openai::default_config();
            let url = crate::domains::auth::provider_credentials::openai::get_authorization_url_with_state(
                &config,
                &pair.challenge,
                Some(&pair.verifier),
            );
        ...
        "google" => {
            ...
            let base_cfg =
                crate::domains::auth::provider_credentials::google::cloud_code_assist_config();
            let config = crate::domains::auth::provider_credentials::google::GoogleOAuthConfig {
                oauth: crate::domains::auth::provider_credentials::types::OAuthConfig {
                    client_id,
                    client_secret,
                    ..base_cfg.oauth
                },
            ...
            let pair = crate::domains::auth::provider_credentials::pkce::generate_pkce();
            let url = crate::domains::auth::provider_credentials::google::get_authorization_url(
                &config,
                &pair.challenge,
            );
            (url, pair.verifier)
    ...
    let _ = flows.insert(
        flow_id.clone(),
        crate::domains::auth::flows::PendingOAuthFlow {
...
pub(crate) async fn auth_oauth_complete(
    ...
    let tokens = match flow.provider.as_str() {
        "anthropic" => {
            let config = crate::domains::auth::provider_credentials::anthropic::default_config();
            crate::domains::auth::provider_credentials::anthropic::exchange_code_for_tokens(
                &config,
                &code,
                &flow.verifier,
                Some(&flow.verifier),
        ...
        "openai-codex" => {
            let config = crate::domains::auth::provider_credentials::openai::default_config();
            crate::domains::auth::provider_credentials::openai::exchange_code_for_tokens(
                &config,
                &code,
                &flow.verifier,
            )
        ...
        "google" => {
            ...
            crate::domains::auth::provider_credentials::google::exchange_code_for_tokens(
                &config,
                &code,
                &flow.verifier,
            )
...
pub(crate) async fn auth_rename_account(
    ...
    write_auth_and_broadcast(deps, invocation, "auth::rename_account", move |auth_path| {
        crate::domains::auth::provider_credentials::storage::rename_account(
...
pub(crate) async fn auth_remove_account(
    ...
    write_auth_and_broadcast(deps, invocation, "auth::remove_account", move |auth_path| {
        crate::domains::auth::provider_credentials::storage::remove_account(
...
pub(crate) async fn auth_remove_api_key(
    ...
    write_auth_and_broadcast(deps, invocation, "auth::remove_api_key", move |auth_path| {
        crate::domains::auth::provider_credentials::storage::remove_named_api_key(
            auth_path, &provider, &label,
        )
        .map_err(map_auth_error)
    })
