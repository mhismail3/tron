...
pub(crate) fn clear_service_auth(
    auth_path: &Path,
    service: &str,
) -> Result<(), crate::domains::auth::provider_credentials::errors::AuthError> {
...
pub(crate) fn build_masked_state(
    auth_path: &Path,
) -> Result<Value, crate::domains::auth::provider_credentials::errors::AuthError> {
    let storage = load_auth_storage(auth_path)?;
    let mut providers = serde_json::Map::new();

    for &provider in KNOWN_PROVIDERS {
        if provider == "google" {
            let google = storage.as_ref().and_then(
                crate::domains::auth::provider_credentials::types::AuthStorage::get_google_auth,
            );
            ...
            let info = if let Some(ref pa) = pa {
                ...
                if let Some(first_acct) = pa.accounts.as_ref().and_then(|accounts| accounts.first())
                {
                    ...
                    let is_expired = crate::domains::auth::provider_credentials::types::now_ms()
                        >= first_acct.oauth.expires_at;
...
pub(crate) fn build_provider_info(pa: &ProviderAuth) -> serde_json::Map<String, Value> {
    ...
    let effective_active = crate::domains::auth::provider_credentials::resolve_credential(pa, None)
        .map(|resolved| match resolved {
            crate::domains::auth::provider_credentials::ResolvedCredential::OAuthAccount(
                account,
            ) => crate::domains::auth::provider_credentials::ActiveCredential::OAuth {
                label: account.label.clone(),
            },
            crate::domains::auth::provider_credentials::ResolvedCredential::ApiKey(key) => {
                crate::domains::auth::provider_credentials::ActiveCredential::ApiKey {
...
pub(crate) async fn publish_auth_updated(
    ...
    crate::domains::auth::stream::AuthStreamPublisher::new(&deps.engine_host)
