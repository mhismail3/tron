...
pub(crate) fn build_masked_state(
    auth_path: &Path,
) -> Result<Value, crate::domains::auth::credentials::errors::AuthError> {
    ...
    for &provider in KNOWN_PROVIDERS {
        if provider == "google" {
            let google = storage
                .as_ref()
                .and_then(crate::domains::auth::credentials::types::AuthStorage::get_google_auth);
            ...
            let info = if let Some(ref pa) = pa {
                ...
                if let Some(first_acct) = pa.accounts.as_ref().and_then(|accounts| accounts.first())
                {
                    ...
                    let is_expired = crate::domains::auth::credentials::types::now_ms()
...
pub(crate) fn build_provider_info(pa: &ProviderAuth) -> serde_json::Map<String, Value> {
    ...
    let effective_active =
        crate::domains::auth::credentials::resolve_credential(pa, None).map(|resolved| {
            match resolved {
                crate::domains::auth::credentials::ResolvedCredential::OAuthAccount(account) => {
                    crate::domains::auth::credentials::ActiveCredential::OAuth {
                        label: account.label.clone(),
                    }
                }
                crate::domains::auth::credentials::ResolvedCredential::ApiKey(key) => {
                    crate::domains::auth::credentials::ActiveCredential::ApiKey {
