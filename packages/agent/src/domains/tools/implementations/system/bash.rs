...
use crate::domains::tools::implementations::traits::{
    BlobStore, ProcessOptions, ProcessRunner, ToolContext, TronTool,
};
use crate::domains::tools::implementations::utils::schema::ToolSchemaBuilder;
use crate::domains::tools::implementations::utils::truncation::estimate_tokens;
...
impl BashTool {
    ...
    async fn execute_managed(
        ...
        let config = crate::domains::tools::implementations::traits::ManagedProcessConfig {
            label: command.to_owned(),
            kind: crate::domains::tools::implementations::traits::ProcessKind::Shell,
        ...
        let output_buffer = std::sync::Arc::new(
            crate::domains::agent::runner::orchestrator::output_buffer::SharedOutputBuffer::new(),
        );
            ...
            while let Some(chunk) = output_rx.recv().await {
                ...
                if let Some(ref emitter) = forwarder_emitter {
                    let _ = emitter.emit(crate::shared::events::TronEvent::ToolExecutionUpdate {
                        base: crate::shared::events::BaseEvent::now(&forwarder_session_id),
                    ...
                    && throttle.ready(std::time::Instant::now())
                {
                    ...
                    let _ = persister
                        .append_background(
                            ...
                            crate::domains::session::event_store::EventType::ToolProgress,
                            Value::Object(obj),
                        )
                        .await;
                }
        ...
        let task: std::pin::Pin<
            Box<
                dyn std::future::Future<Output = crate::domains::tools::implementations::traits::ManagedProcessResult> + Send,
            >,
        > = Box::pin(async move {
            ...
            match runner.run_command(&cmd, &opts).await {
                Ok(output) => {
                    ...
                    crate::domains::tools::implementations::traits::ManagedProcessResult {
                ...
                Err(e) => crate::domains::tools::implementations::traits::ManagedProcessResult {
                    process_id: String::new(),
                    output: e.to_string(),
                    exit_code: None,
                    duration_ms: start.elapsed().as_millis() as u64,
        ...
        match handle.result {
            ...
            None => {
                ...
                let user_initiated =
                    handle.backgrounded == Some(crate::domains::tools::implementations::traits::BackgroundReason::UserAction);
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<TronToolResult, ToolError> {
        ...
        let sandbox_workspace = if let Some(sandbox_val) = sandbox_mode {
            ...
            if is_lightweight {
                ...
                let config =
                    crate::domains::tools::implementations::system::sandbox::SandboxConfig {
                        copy_paths: Vec::new(),
                        readonly_mounts: sandbox_mounts,
                    };
                match crate::domains::tools::implementations::system::sandbox::SandboxWorkspace::create(&config).await {
                    Ok(ws) => Some(ws),
                ...
                let docker_config =
                    crate::domains::tools::implementations::system::sandbox::DockerSandboxConfig {
                ...
                if let Err(e) =
                    crate::domains::tools::implementations::system::sandbox::check_docker_available(
                    )
                    .await
                {
                ...
                let docker_cmd =
                    crate::domains::tools::implementations::system::sandbox::build_docker_command(
                        &command,
                        &docker_config,
                    );
                ...
                return Ok(TronToolResult {
                    content: ToolResultBody::Blocks(vec![
                        crate::shared::content::ToolResultContent::text(&combined),
                    ]),
