...
use crate::shared::tools::{Tool, ToolCategory, TronToolResult};
...
use crate::domains::tools::implementations::utils::validation::{
    validate_path_not_root, validate_required_string,
};
...
impl TronTool for WriteTool {
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<TronToolResult, ToolError> {
        ...
        Ok(TronToolResult {
            content: crate::shared::tools::ToolResultBody::Blocks(vec![
                crate::shared::content::ToolResultContent::text(message),
            ]),
