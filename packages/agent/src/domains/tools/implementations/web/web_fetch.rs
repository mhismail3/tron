...
use crate::domains::tools::implementations::traits::{
    ContentSummarizer, HttpClient, HttpRequest, ToolContext, TronTool,
};
...
use crate::domains::tools::implementations::web::cache::{CachedResult, WebCache, WebCacheConfig};
use crate::domains::tools::implementations::web::html_parser::parse_html;
use crate::domains::tools::implementations::web::url_validator::{
    UrlValidatorConfig, validate_url,
};
...
impl WebFetchTool {
    ...
    async fn execute_summarization(
            ...
            if let Some(cached) = cache.get(url, prompt) {
                ...
                return Ok(TronToolResult {
                    content: ToolResultBody::Blocks(vec![
                        crate::shared::content::ToolResultContent::text(&cached.answer),
                    ]),
                    details: Some(json!({
                        "mode": "summarization",
                        "url": cached.url,
        ...
        let truncated_content = if parsed.markdown.len() > max_bytes {
            let prefix = crate::shared::text::truncate_str(&parsed.markdown, max_bytes);
            format!("{prefix}...\n[Content truncated]")
        } else {
    ...
    async fn execute_raw(
        ...
        let display_body = if body_text.len() > max_bytes {
            let prefix = crate::shared::text::truncate_str(&body_text, max_bytes);
            format!(
...
mod tests {
    use super::*;
    use crate::domains::tools::implementations::traits::{HttpResponse, SummarizerResult};
    ...
    async fn summarization_emits_fetch_start_and_completed_progress() {
        ...
        let (ctx, store, session_id) =
            crate::domains::tools::implementations::testutil::make_ctx_with_persister().await;
        ...
        let events = crate::domains::tools::implementations::testutil::drain_progress_events(
            &store,
            &session_id,
        )
        .await;
