...
pub fn text_search(
    ...
    let token_count =
        crate::domains::tools::implementations::utils::truncation::estimate_tokens(output.len());
    let truncated = token_count > MAX_OUTPUT_TOKENS;
    if truncated {
        let max_chars = crate::domains::tools::implementations::utils::truncation::tokens_to_chars(
            MAX_OUTPUT_TOKENS,
        );
