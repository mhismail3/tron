...
pub struct ToolContext {
    ...
    pub event_emitter:
        Option<Arc<crate::domains::agent::runner::agent::event_emitter::EventEmitter>>,
...
impl ToolContext {
    ...
    pub async fn emit_progress(&self, message: Option<String>, percent: Option<f64>) {
        if let Some(emitter) = self.event_emitter.as_ref() {
            let _ = emitter.emit(crate::shared::events::TronEvent::ToolExecutionProgress {
                base: crate::shared::events::BaseEvent::now(self.session_id.clone()),
...
pub trait BlobStore: Send + Sync {
    ...
    async fn store(
        ...
        content: &[u8],
        mime_type: &str,
    ) -> Result<String, crate::domains::tools::implementations::errors::ToolError>;
}
...
mod tests {
    ...
    async fn emit_progress_broadcasts_tron_event_when_emitter_present() {
        let emitter =
            Arc::new(crate::domains::agent::runner::agent::event_emitter::EventEmitter::new());
        ...
        match event {
            crate::shared::events::TronEvent::ToolExecutionProgress {
                tool_call_id,
                message,
                percent,
                base,
