...
use crate::domains::tools::implementations::traits::{
    SubagentConfig, SubagentMode, SubagentSpawner, ToolContext, TronTool,
};
...
impl SpawnSubagentTool {
    ...
    pub fn new(spawner: Arc<dyn SubagentSpawner>) -> Self {
        ...
        crate::shared::constitution::ensure_tron_home_at(&home)
            .expect("test profile runtime seeded home");
        let profile_runtime = Arc::new(
            crate::domains::agent::runner::ProfileRuntime::load(&home)
                .expect("test profile runtime"),
        );
    ...
    fn process_for_mode(&self, mode: &SubagentMode) -> crate::shared::profile::ProcessSpec {
...
- Full output, token usage, duration statistics, status",
    ...
    async fn execute(&self, params: Value, ctx: &ToolContext) -> Result<TronToolResult, ToolError> {
        ...
        let task_preview = crate::shared::text::truncate_str(&task, 120);
                ...
                if let Some(output) = handle.output {
                    ...
                    Ok(TronToolResult {
                        content: ToolResultBody::Blocks(vec![
                            crate::shared::content::ToolResultContent::text(&output),
                        ]),
                        details: Some(json!({
                            "sessionId": handle.session_id,
                            "success": handle.success.unwrap_or(true),
                            "totalTurns": turns,
                            "resultSummary": crate::shared::text::truncate_str(&output, 200),
...
mod tests {
    ...
    use crate::domains::tools::implementations::traits::{
        SubagentHandle, SubagentResult, WaitMode,
    };
