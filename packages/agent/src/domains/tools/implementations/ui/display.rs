...
use crate::domains::tools::implementations::utils::validation::{
    get_optional_string, validate_required_string,
};
...
pub struct DisplayTool {
    blob_store: Option<Arc<dyn crate::domains::tools::implementations::traits::BlobStore>>,
    /// Broadcast sender for emitting DisplayFrame events during streaming.
    event_tx: Option<tokio::sync::broadcast::Sender<crate::shared::events::TronEvent>>,
}
...
impl DisplayTool {
    ...
    pub fn with_event_tx(
        mut self,
        tx: tokio::sync::broadcast::Sender<crate::shared::events::TronEvent>,
    ) -> Self {
...
impl DisplayTool {
    ...
    async fn handle_stream(
        ...
        let config = crate::domains::tools::implementations::ui::display_stream::StreamConfig {
            interval: std::time::Duration::from_millis(
                crate::domains::tools::implementations::ui::display_stream::DEFAULT_INTERVAL_MS,
            ),
        ...
        > = Box::pin(async move {
            ...
            let last_frame_data = crate::domains::tools::implementations::ui::display_stream::screen_capture_loop(
                event_tx,
                config,
                stream_cancel,
            )
        ...
        let pm_config = crate::domains::tools::implementations::traits::ManagedProcessConfig {
            label: format!("display_stream:{stream_id}"),
            kind: crate::domains::tools::implementations::traits::ProcessKind::DisplayStream,
            timeout_ms: None,
        ...
        if let Some(process_id) = pm.find_by_label(&ctx.session_id, &label) {
            ...
            Ok(TronToolResult {
                content: ToolResultBody::Blocks(vec![
                    crate::shared::content::ToolResultContent::text(format!(
...
mod tests {
    use super::*;
    use crate::domains::tools::implementations::testutil::make_ctx;
    use std::io::Write;
    ...
    fn make_stream_ctx() -> ToolContext {
        ...
        let pm = std::sync::Arc::new(
            crate::domains::agent::runner::orchestrator::process_manager::ProcessManager::new(),
        );
    ...
    async fn stream_stop_cancels_active_stream() {
        let (tx, _rx) = tokio::sync::broadcast::channel::<crate::shared::events::TronEvent>(64);
