...
pub(crate) fn capabilities() -> EngineResult<Vec<CapabilitySpec>> {
    Ok(vec![
        ...
        CapabilityContract::new("filesystem::write_file", "filesystem", EffectClass::IdempotentWrite, RiskLevel::Medium, Some("filesystem.write"))
            .description("Create a new file or overwrite an existing file with exact content. Use this for new scratch files, docs-sandbox notes, and other file creation; read or diff first before overwriting important existing files.")
            .tags(vec!["write", "file", "save", "create file", "overwrite", "filesystem"])
            .request_schema(json!({"additionalProperties":false,"properties":{"content":{"type":"string","description":"Exact file contents to write."},"path":{"type":"string","description":"Path for the new or existing file. Use write_file, not apply_patch, when the file may not exist yet."},"sessionId":{"type":"string"},"workspaceId":{"type":"string"}},"required":["path","content"],"type":"object"}))
        ...
        CapabilityContract::new("filesystem::apply_patch", "filesystem", EffectClass::ReversibleSideEffect, RiskLevel::Medium, Some("filesystem.write"))
            .description("Use filesystem::write_file for new files or scratch notes. Apply exact-string patches only to an existing known file and return the resulting diff. To append bytes to an existing file, pass oldString as an empty string and newString as the exact bytes to append.")
            .tags(vec!["patch", "apply patch", "edit", "replace", "append", "file", "filesystem"])
            .request_schema(json!({"additionalProperties":false,"properties":{"newString":{"type":"string","description":"Exact replacement or appended bytes."},"oldString":{"type":"string","description":"Exact existing text to replace; use an empty string only to append to an existing file."},"path":{"type":"string","description":"Existing file path. Do not use apply_patch for new file creation; use filesystem::write_file instead."},"replaceAll":{"type":"boolean"},"sessionId":{"type":"string"},"workspaceId":{"type":"string"}},"required":["path","oldString","newString"],"type":"object"}))
            .response_schema(filesystem_resource_backed_response(json!({"diff":{"type":"string"},"path":{"type":"string"},"replacements":{"type":"integer"}}), vec!["path", "replacements", "diff"]))
            .idempotency(IdempotencyContract::caller_session_engine_ledger())
            .output_contract(DurableOutputContract::resource_backed(["materialized_file", "patch_proposal"]))
            .resource_lease(filesystem_write_lease())
