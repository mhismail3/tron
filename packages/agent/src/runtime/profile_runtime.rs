...
impl SessionExecutionPlan {
    ...
    #[must_use]
    pub fn runtime_context_policy(&self) -> crate::runtime::context::local_policy::ContextPolicy {
        let provider_is_local = self.provider.is_some_and(|provider| {
            crate::runtime::context::local_policy::provider_is_local_for_spec(
                provider,
                &self.resolved_profile.spec,
            )
        });
        crate::runtime::context::local_policy::ContextPolicy::from_resolved_parts(
...
impl ProfileRuntime {
    ...
    pub fn resolve_profile(&self, name: &str) -> std::io::Result<Arc<ResolvedProfile>> {
        crate::core::profile::resolve_profile_at(&self.home, name).map(Arc::new)
    }
    ...
    pub fn reload_now(&self, reason: &str) -> std::io::Result<Arc<ResolvedHarnessSpec>> {
        match load_harness_spec(&self.home) {
            Ok(next) => {
                ...
                crate::settings::init_settings(next.settings.clone());
                tracing::info!(
                    reason,
    ...
    pub fn plan_session(
        ...
        let provider = crate::llm::models::registry::detect_provider_from_model(&request.model);
...
fn profile_tree_hash(home: &Path) -> std::io::Result<String> {
    let profiles_dir = home.join(crate::core::paths::dirs::PROFILES);
    ...
    for entry in WalkDir::new(&profiles_dir).follow_links(false) {
        ...
        if path.file_name().and_then(|name| name.to_str())
            == Some(crate::core::paths::files::AUTH_JSON)
        {
            continue;
        }
...
fn load_harness_spec(home: &Path) -> std::io::Result<ResolvedHarnessSpec> {
    let profile = Arc::new(crate::core::profile::resolve_active_profile_at(home)?);
...
fn build_session_plan(
    ...
    let context_policy_id = if provider.is_some_and(|provider| {
        crate::runtime::context::local_policy::provider_is_local_for_spec(provider, spec)
    }) {
        entrypoint
...
fn effective_settings(settings: &TronSettings) -> std::io::Result<TronSettings> {
    ...
    crate::settings::loader::apply_env_overrides(&mut settings);
    settings.validate();
...
mod tests {
    ...
    fn reload_updates_global_settings_snapshot() {
        let _settings_guard = crate::settings::test_settings_lock()
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        crate::settings::reset_settings();
        let (_dir, runtime) = seeded_runtime();
        crate::settings::init_settings(runtime.current().settings.clone());
        let profile_path = runtime
            .home()
            .join(crate::core::paths::dirs::PROFILES)
            .join(NORMAL_PROFILE)
        ...
        assert_eq!(
            crate::settings::get_settings().server.default_model,
            "reload-test-model"
        );
        crate::settings::reset_settings();
    }
        ...
        tokio::time::timeout(Duration::from_secs(2), async {
            loop {
                if runtime.current().settings.server.default_model == "watcher-test-model"
                    && crate::settings::get_settings().server.default_model == "watcher-test-model"
                {
                    break;
                }
