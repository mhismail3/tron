...
const AGENT_DIRS: &[&str] = &[".claude", ".tron/memory/rules", ".agent"];
...
mod tests {
    ...
    fn case_insensitive_filenames() {
        ...
        write_file(tmp.path(), ".tron/memory/rules/agents.md", "# lowercase agents");

        let results = discover_rules_files(&make_config(tmp.path()));
        assert_eq!(results.len(), 2);
        let mut paths: Vec<_> = results.iter().map(|r| r.relative_path.as_str()).collect();
        paths.sort_unstable();
        assert_eq!(paths, vec![".claude/claude.md", ".tron/memory/rules/agents.md"]);
    }
    ...
    fn discovers_deeply_nested_agent_dir_rules() {
        ...
        write_file(tmp.path(), "src/lib/.tron/memory/rules/AGENTS.md", "# Nested rule");
    ...
    fn discovers_files_across_multiple_agent_dirs() {
        let tmp = setup();
        write_file(tmp.path(), "packages/foo/.claude/CLAUDE.md", "# Claude");
        write_file(tmp.path(), "packages/foo/.tron/memory/rules/AGENTS.md", "# Tron Agents");
    ...
    fn scope_dir_deeply_nested_tron_rules() {
        assert_eq!(
            compute_scope_dir("a/b/c/.tron/memory/rules/AGENTS.md", false, Some(".tron/memory/rules")),
            "a/b/c"
        );
    }
    ...
    fn scope_dir_root_tron_rules() {
        assert_eq!(
            compute_scope_dir(".tron/memory/rules/CLAUDE.md", false, Some(".tron/memory/rules")),
            ""
        );
    }
