...
- `~/.tron/memory/sessions/` — Session summaries (auto-generated by the retain system). Do not write here manually.
- `~/.tron/memory/reports/` — Analysis, research, and investigation results.
- `~/.tron/memory/knowledge/` — Living documents: patterns, preferences, technical notes, accumulated context that spans sessions.
- `~/.tron/memory/scratch/` — Throwaway output, intermediate results. Subdirectories OK for multi-file experiments.

At session start: read recent session files in `~/.tron/memory/sessions/` to recover context.
...
| Reports, analysis, research results | `~/.tron/memory/reports/` |
| Long-term knowledge, preferences, technical notes | `~/.tron/memory/knowledge/` |
