...
- SESSIONS — Session summaries (auto-generated by the retain system). Do not write here manually.
- REPORTS — Analysis, research, and investigation results.
- KNOWLEDGE — Living documents: patterns, preferences, technical notes, accumulated context that spans sessions.
- SCRATCH — Throwaway output, intermediate results. Subdirectories OK for multi-file experiments.

See PATH REFERENCE below for full paths. At session start: read recent session files in SESSIONS to recover context.
...
| MODS | `~/.tron/system/mods/` | Optional modules: `apns/`, `google/`, `twitter/`, `transcribe/` |
...
| Name | Path | Contents |
|------|------|----------|
| KNOWLEDGE | `~/.tron/workspace/knowledge/` | Wiki: `SCHEMA.md`, `index.md`, `log.md`, `sources/`, `wiki/` |
...
| SESSIONS | `~/.tron/workspace/sessions/` | Auto-generated session summaries (retain system) |
| SCRATCH | `~/.tron/workspace/scratch/` | Throwaway output, experiments |
| RULES | `~/.tron/workspace/rules/` | `SYSTEM.md` and other global rules |
