...
export class EventStoreOrchestrator extends EventEmitter {
  ...
  readonly embeddings: EmbeddingController;

  /** Skill loading (exposed for integration testing — prefer using via agentRunner) */
  ...
  constructor(config: EventStoreOrchestratorConfig) {
    ...
    // Initialize EmbeddingController (delegated module)
    this.embeddings = createEmbeddingController({ eventStore: this.eventStore });
    ...
    this.agentFactory = createAgentFactory({
      ...
      memoryConfig: {
        ...
        embedMemory: (eventId, workspaceId, payload) =>
          this.embeddings.embedMemory(eventId, workspaceId, payload),
  ...
  async initialize(): Promise<void> {
    ...
    await this.embeddings.initialize({
      enabled: embeddingConfig?.enabled ?? true,
