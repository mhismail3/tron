...
//!   `ArcSwap`-backed `crate::domains::settings::get_settings()` is already
...
pub fn spawn(
    ...
    tokio::spawn(async move {
        use crate::domains::settings::types::UpdateFrequency;
        ...
        let initial_frequency = crate::domains::settings::get_settings()
            .server
            .update
            .frequency;
        let initial_enabled = crate::domains::settings::get_settings()
            .server
            .update
            .enabled;
        debug!(
        ...
        loop {
            tokio::select! {
                _ = tokio::time::sleep(next_sleep) => {
                    let cfg = crate::domains::settings::get_settings().server.update.clone();
...
mod tests {
    use super::*;
    use crate::domains::settings::types::{UpdateChannel, UpdateFrequency};
    use crate::engine::EngineHostHandle;
    use crate::platform::updater::{MockReleaseFetcher, ReleaseInfo};
    ...
    fn install_update_settings(
        update: crate::domains::settings::types::UpdateSettings,
    ) -> std::sync::MutexGuard<'static, ()> {
        let guard = crate::domains::settings::test_settings_lock()
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner());
        let mut settings = crate::domains::settings::types::TronSettings::default();
        settings.server.update = update;
        crate::domains::settings::init_settings(settings);
        guard
    }

    fn default_enabled(channel: UpdateChannel) -> crate::domains::settings::types::UpdateSettings {
        let mut s = crate::domains::settings::types::UpdateSettings::default();
        s.enabled = true;
        s.channel = channel;
    ...
    async fn disabled_flag_short_circuits_before_fetcher() {
        ...
        let mut settings = crate::domains::settings::types::UpdateSettings::default();
