...
use tron_core::messages::{Provider, TokenUsage};
...
pub(super) fn build_token_record_json(
    ...
    let cost = token_usage
        .and_then(|usage| tron_llm::tokens::calculate_cost(model, usage).map(|c| c.total));
...
pub(super) fn add_assistant_message_to_context(
    ...
    let has_thinking = stream_result
        ...
        .content
        .iter()
        .any(|c| matches!(c, tron_core::content::AssistantContent::Thinking { .. }));
    let thinking_text = stream_result.message.content.iter().find_map(|c| {
        if let tron_core::content::AssistantContent::Thinking { thinking, .. } = c {
            Some(thinking.clone())
        } else {
            None
        }
    ...
    context_manager.add_message(tron_core::messages::Message::Assistant {
