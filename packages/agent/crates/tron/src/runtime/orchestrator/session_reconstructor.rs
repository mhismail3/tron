...
use crate::core::messages::{Message, TokenUsage};
use crate::events::{EventStore, SessionState};
...
mod tests {
    ...
    fn reconstruct_tool_use_survives_serde_roundtrip() {
        ...
        if let Message::Assistant { content, .. } = &state.messages[1] {
            ...
            if let crate::core::content::AssistantContent::ToolUse {
                id,
                name,
                arguments,
                ..
    ...
    fn reconstruct_multimodal_user_message() {
        ...
        if let Message::User { content, .. } = &state.messages[0] {
            match content {
                ...
                crate::core::messages::UserMessageContent::Text(_) => {
