...
impl Provider for GoogleProvider {
    fn provider_type(&self) -> crate::core::messages::Provider {
        crate::core::messages::Provider::Google
    }
...
mod tests {
    ...
    fn oauth_tokens() -> OAuthTokens {
        OAuthTokens {
            ...
            expires_at: crate::llm::auth::now_ms() + 3_600_000, // 1 hour
        }
    }
    ...
    fn thinking_config_gemini3_custom_level() {
        let mut config = oauth_config();
        config.thinking_level = Some(crate::llm::google::types::GeminiThinkingLevel::Low);
        let provider = GoogleProvider::new(config);
        let opts = ProviderStreamOptions::default();
        let tc = provider.build_thinking_config(true, &opts).unwrap();
    ...
    async fn refresh_tokens_success() {
        ...
        assert!(new_tokens.expires_at > crate::llm::auth::now_ms());
    }
