...
use crate::events::memory::types::LedgerWriteResult;
...
use crate::runtime::agent::compaction_handler::SubagentManagerSpawner;
use crate::runtime::context::ledger_writer::{LedgerEntry, LedgerParseResult};
use crate::runtime::context::llm_summarizer::SubsessionSpawner;
use crate::runtime::context::summarizer::serialize_messages;
...
async fn summarize_cycle(
    ...
    let spawner = SubagentManagerSpawner {
        ...
        system_prompt: crate::runtime::context::system_prompts::MEMORY_LEDGER_PROMPT.to_string(),
        model: Some("claude-haiku-4-5-20251001".to_string()),
    };
    let result = spawner.spawn_summarizer(&transcript).await;
    if result.success {
        result.output.as_deref().and_then(|output| {
            crate::runtime::context::ledger_writer::parse_ledger_response(output).ok()
        })
    } else {
...
async fn persist_and_embed(
    ...
    let event_id = match run_store_task("memory.persist_ledger", move || {
        event_store.append(&crate::events::AppendOptions {
    ...
    .await
    {
        ...
        Ok(Err(error)) => {
            ...
            return match error {
                crate::events::EventStoreError::Busy { .. } => {
...
fn ledger_read_failure(
    session_id: &str,
    error: &crate::events::EventStoreError,
) -> LedgerWriteResult {
...
async fn run_store_task<T, F>(
    ...
    f: F,
) -> Result<Result<T, crate::events::EventStoreError>, crate::server::rpc::errors::RpcError>
where
    T: Send + 'static,
    F: FnOnce() -> Result<T, crate::events::EventStoreError> + Send + 'static,
{
    crate::server::rpc::context::run_blocking_task(task_name, move || {
        Ok::<_, crate::server::rpc::errors::RpcError>(f())
    })
    .await
}
