...
use crate::server::rpc::memory_ledger::{LedgerWriteDeps, execute_ledger_write};
...
impl MemoryCommandService {
    ...
    fn emit_memory_updated(
        ctx: &RpcContext,
        session_id: &str,
        result: &crate::events::memory::types::LedgerWriteResult,
    ) {
    ...
    fn rpc_response(result: &crate::events::memory::types::LedgerWriteResult) -> Value {
        json!({
