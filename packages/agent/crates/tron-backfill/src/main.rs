...
mod tests {
    ...
    #[test]
    fn cli_import_subcommand_parses() {
    ...
    #[test]
    fn cli_embed_subcommand_parses() {
        let cli = Cli::parse_from(["tron-backfill", "embed"]);
        assert!(matches!(cli.command, Command::Embed { force: false }));
    }

    #[test]
    fn cli_embed_force() {
        let cli = Cli::parse_from(["tron-backfill", "embed", "--force"]);
        assert!(matches!(cli.command, Command::Embed { force: true }));
    }

    #[test]
    fn cli_all_subcommand_parses() {
        let cli = Cli::parse_from([
            "tron-backfill",
            "all",
            "--ledger-path",
