...
async fn deliver_ws(
    ...
    let broadcaster = deps
        .broadcaster
        .get()
        .ok_or_else(|| crate::errors::CronError::Execution("no broadcaster".into()))?;
...
async fn deliver_apns(
    ...
    let notifier = deps
        .push_notifier
        .as_ref()
        .ok_or_else(|| crate::errors::CronError::Execution("no push notifier".into()))?;
...
async fn deliver_webhook(
    ...
    let payload = serde_json::to_value(run)
        .map_err(|e| crate::errors::CronError::Execution(e.to_string()))?;
    req = req.json(&payload);

    let resp = req
        .send()
        .await
        .map_err(|e| crate::errors::CronError::Execution(format!("delivery webhook: {e}")))?;

    if !resp.status().is_success() {
        return Err(crate::errors::CronError::Execution(format!(
