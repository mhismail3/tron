...
use crate::rpc::session_context::{RuleFileLevel, collect_dynamic_rule_paths};
...
fn snapshot_response(snapshot: &tron_runtime::context::types::ContextSnapshot) -> Value {
    json!({
...
fn build_detailed_messages(
    messages: &[tron_runtime::context::types::DetailedMessageInfo],
) -> Vec<Value> {
    messages
        .iter()
        .map(|message| {
...
fn build_composed_system_prompt(
    context_manager: &tron_runtime::context::context_manager::ContextManager,
    ...
    tron_llm::compose_context_parts(&composed_context).join("\n\n")
}
...
fn build_rules_info(
    ...
    session_id: &str,
    session: &tron_events::sqlite::row_types::SessionRow,
    artifacts: &crate::rpc::session_context::SessionContextArtifacts,
    rules_tokens: u64,
) -> Option<Value> {
...
fn build_memory_info(memory: Option<&crate::rpc::session_context::LoadedMemory>) -> Option<Value> {
    let memory = memory?;
...
fn build_session_memory_info(
    memories: &[tron_runtime::context::types::SessionMemoryEntry],
) -> Option<Value> {
