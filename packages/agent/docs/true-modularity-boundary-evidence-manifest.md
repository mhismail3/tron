...
| TMB-8 | passed_after_fix | Defined boundary-local error contracts by deleting the unused shared `ProviderError`/`RateLimitInfo`, removing `TronError::Provider`, moving `OpenAIAuthPath` into auth credentials, mapping auth/settings JSON failures to operation-named domain error variants, mapping strict Google provider decode failures inside auth storage, and narrowing the implementation-error guard to owner-local model, event-store, engine SQLite, observability SQLite, transport, and server error-mapping boundaries. | `cargo test --manifest-path packages/agent/Cargo.toml --test true_modularity_boundary_invariants provider_internals_do_not_escape_model_domain -- --nocapture`; `cargo test --manifest-path packages/agent/Cargo.toml --test true_modularity_boundary_invariants boundary_errors_do_not_leak_impl_errors -- --nocapture`; `cargo test --manifest-path packages/agent/Cargo.toml domains::auth::credentials::errors -- --nocapture`; `cargo test --manifest-path packages/agent/Cargo.toml domains::settings::profile::errors -- --nocapture`; `cargo test --manifest-path packages/agent/Cargo.toml infer_auth_path -- --nocapture` all pass. Full TMB target now runs 11 tests with 10 passed and 1 failed (`final_modularity_closeout_is_complete`). | TMB-9 and TMB-10 remain open. Provider and engine dead-code warnings remain visible and are carried into later cleanup; no `allow` attributes were added. | checkpoint commit |
| TMB-9 | passed_after_fix | Updated final architecture docs after the boundary implementation: README no longer lists crate-private domain registration as a caller surface and now documents OpenAI auth-path ownership; settings module docs record boundary-local JSON error mapping; iOS architecture docs and the TMB inventory summary carry TMB-9 verification markers; the TSV inventory already covered the current tracked source set and required no row changes. | Audit commands: `rg "true-modularity\|True Modularity\|TMB\|boundary\|ModelResponder\|EngineClient\|ChatSessionServices\|ProviderError\|OpenAIAuthPath\|settings/auth wire\|boundary-error\|open work\|open loop" README.md packages/agent/docs packages/agent/src/domains/auth/credentials/mod.rs packages/agent/src/domains/model/mod.rs packages/agent/src/domains/model/responder/mod.rs packages/agent/src/domains/settings/profile/mod.rs packages/agent/src/shared/foundation/errors/mod.rs packages/ios-app/docs/architecture.md -n`; `wc -l packages/agent/docs/true-modularity-boundary-inventory.tsv`. Passing checks: `cargo test --manifest-path packages/agent/Cargo.toml --test true_modularity_boundary_invariants true_modularity_scorecard_stays_formalized -- --nocapture`; `cargo test --manifest-path packages/agent/Cargo.toml --test true_modularity_boundary_invariants boundary_inventory_covers_tracked_sources -- --nocapture`; `cargo fmt --manifest-path packages/agent/Cargo.toml --all -- --check`; `git diff --check`. Full TMB target still runs 11 tests with 10 passed and 1 failed (`final_modularity_closeout_is_complete`). | TMB-10 final closeout remains open. Provider and engine dead-code warnings remain visible and are carried into final closeout; no `allow` attributes were added. | checkpoint commit |
| TMB-10 | passed_after_fix | Completed the final adversarial closeout. Fixed the first full-CI blocker by adding TMB-created files to the PCC, HRA, HRA iOS, and TPC inventories; removed the final iOS concrete-client leak by routing diagnostics through `DiagnosticsEngineEndpoint` and `ClientLogIngestionEndpoint` adapters owned by `Support/Composition`; strengthened `ios_ui_uses_repositories_not_engine_transport` to reject concrete `EngineClient` outside Engine-owned code and composition; updated iOS architecture docs; and corrected the `packages/agent/src/main.rs` composition-root owner label. | Full CI closeout: `scripts/tron ci fmt check clippy test`. Final static scans: agent-domain provider internals scan; provider re-export scan; production storage import/pool escape scan plus `cargo test --manifest-path packages/agent/Cargo.toml --test true_modularity_boundary_invariants state_stores_are_owner_private -- --nocapture`; iOS Session/UI raw Engine DTO scan; concrete `EngineClient` confinement scan; composition-root owner scan. Focused verification: `xcodegen generate`; `xcodebuild test -scheme Tron -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:TronMobileTests/ClientLogIngestionServiceTests -only-testing:TronMobileTests/DiagnosticsBundleBuilderTests -only-testing:TronMobileTests/SourceGuardTests`; `cargo test --manifest-path packages/agent/Cargo.toml --test true_modularity_boundary_invariants ios_ui_uses_repositories_not_engine_transport -- --nocapture`; PCC/HRA/HRA iOS/TPC inventory gates. Final hygiene: `scripts/personal-info-guard.sh`; `cd packages/ios-app && xcodegen generate && cd ../.. && git diff --exit-code -- packages/ios-app/TronMobile.xcodeproj`; `git ls-files -ci --exclude-standard`; `git diff --check`; `git status --short`. | None. | checkpoint commit |
...
- After TMB-8, `provider_internals_do_not_escape_model_domain` and
  `boundary_errors_do_not_leak_impl_errors` pass. Provider errors remain owned
  by the model responder/provider boundary, auth owns OpenAI credential-path
  inference, auth/settings JSON failures surface as boundary-local error
  variants, and raw SQL/transport implementation errors are allowed only in
  their owner-local adapters.
- After TMB-9, README, affected Rust module docs, iOS architecture docs, and
  the TMB inventory summary reflect the enforced boundary architecture. The
  machine-readable inventory still covers every tracked Rust/Swift production
  source and did not require row changes.
- After TMB-10, `final_modularity_closeout_is_complete` passes. Final scans
  found and closed the remaining concrete `EngineClient` diagnostics support
  leak, corrected composition-root inventory ownership, refreshed cross-campaign
  inventories, and the full TMB target is green.
