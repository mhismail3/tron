...
| 41 | Phase 2 Slice 14B: Import And Session/Resource Graph Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@495881c1e4f30604640cf91761515ac9f3a97279`; implementation is in progress on the focused branch `codex/phase-2-slice-14b-import-session-resource-graph-foundation` and remains pending independent review. | Discovery thread `019effd6-4548-7991-b9ce-cefee80394be`; implementation thread `019effd9-089b-7f41-a035-3ec11620f1ae`; implementation worktree `/Users/<USER>/Workspace/tron`. | `packages/agent/src/domains/import_history/`; `packages/agent/src/domains/capability/import_history_contract.rs`; `packages/agent/src/domains/capability/operations/import_history.rs`; `packages/agent/src/domains/capability/contract.rs`; `packages/agent/src/engine/durability/resources/import_history_definitions.rs`; runtime grant/provider guidance; Phase 2 scorecard/evidence/inventory; README capability and import-history boundary docs; HRA/TMB/TPC/PCC/SACB/CSD/BPRC/DESI/IARM/DRC/public-protocol/performance-resource guards. | Implementation candidate adds durable `import_history_record` resources plus execute-only `import_history_record`, `import_history_list`, and `import_history_inspect` operation values behind `capability::execute`. The candidate requires trusted current-session/workspace context, explicit non-wildcard `import_history_record` authority/resource-kind grants and bounded selectors, bounded generic lineage refs only, retention fields, source/evidence refs, trace/replay refs, deterministic injected timestamps, lifecycle evidence, redacted/bounded projections, and fingerprinted idempotency evidence instead of raw caller keys. Raw import payloads, repository trees, unsafe paths, import preview/execute behavior, repository visualization, and native iOS session/import/tree UI remain out of scope. Focused `cargo check` and import-history/grant tests are passing while static inventory and documentation closeout is still being completed before adversarial review. | Native import/session tree UI, repository browsing/divergence visualization, import preview/execute behavior, update diagnostics, public `/engine` expansion, trust/MCP/package/catalog/browser/result-merge/autonomous-planning changes, repo-managed first-party skills, and production deploy remain deferred. | Open. Pending independent review after implementation closeout validation and candidate commit. |
