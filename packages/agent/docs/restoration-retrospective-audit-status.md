...
| 32 | Phase 2 Slice 8F: Web Fetch Robots Evidence Linkage Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@419433985790f35f5ef514e9f508b4f8906d37a1`; implementation branch `codex/phase-2-slice-8f-web-fetch-robots-evidence-linkage`; candidate implementation commit pending independent review. | Source handoff thread `019ef914-ed80-78f2-b253-229240d49444`; discovery thread `019efc32-30b1-7811-9959-7e539ba8062f`; implementation worktree `/Users/<USER>/.codex/worktrees/07b9/tron`. | `packages/agent/src/domains/web/robots_link.rs`; `packages/agent/src/domains/web/fetch.rs`; `packages/agent/src/domains/web/source.rs`; `packages/agent/src/domains/web/web_fetch_robots_link_tests.rs`; `packages/agent/src/domains/capability/contract.rs`; `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs`; `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant_tests.rs`; `packages/agent/src/domains/model/providers/openai/message_converter/`; `packages/agent/src/engine/authority/grants/authorization.rs`; `packages/agent/src/engine/durability/resources/web_definitions.rs`; Phase 2 scorecard/evidence/inventory; README capability docs; HRA/TMB/TPC/PCC/SACB/BPRC/IARM/DESI/public-protocol guards. | Implementation candidate adds optional `web_fetch` inputs `webRobotsPolicyResourceId` and `expectedWebRobotsPolicyVersionId`, validates current-session allow `web_robots_policy` evidence for exact origin/target compatibility before target HTTP client construction, fails closed for stale/malformed/wrong-kind/wrong-session/wrong-version/non-current/non-allow evidence, persists bounded `robotsPolicyRefs` into `web_source`, exposes bounded refs through source list/inspect, and conditionally derives/authorizes robots-policy read grants only for robots-linked fetches. | Search providers, browser automation, crawling, sitemap traversal/fetching, login/cookies/session reuse, credential reuse, shell/process network side channels, native iOS web/source UI, public `/engine` web APIs, deletion/pruning/automatic TTL cleanup, settings/profile fields, database migrations, network-enabled jobs, and global robots requirement remain deferred. | Pending independent review; do not mark accepted/current baseline from this implementation branch. |
