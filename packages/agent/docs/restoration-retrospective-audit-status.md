...
- Consolidation update: `codex/restoration-mainline-consolidation` starts from
  ...
  commits are now present on this consolidated line. Order 21 remains blocked
  because no completed Phase 2 Slice 6 Git/Worktrees/Source Control source
  branch, range, canonical artifacts, or accepted fix commits were found.
...
| 20 | Phase 2 Slice 5A: Durable Jobs and Process Lifecycle + Race Hardening | Phase 2 agent-execution restoration | Baseline `1d35ac848` on `2650dd0b9`; original commits `404290d06`, `4fb134c64`, `0d76be40d`; accepted fix commits `8702a7e8cb6346d29ac5247ee87344f34b766143`, `d57aaf83e94693e2eaefde16108202ea4e254e5e` | Integrated branch evidence at `codex/phase-2-jobs-process-lifecycle-current@0d76be40d` plus accepted fix refs `codex/order-20-jobs-process-lifecycle-fix@8702a7e8cb6346d29ac5247ee87344f34b766143` and `codex/order-20-jobs-process-lifecycle-reconcile-targeted@d57aaf83e94693e2eaefde16108202ea4e254e5e`. | Integrated files: Phase 2 scorecard/evidence/inventory; jobs domain docs/tests; SACB/TMB/CSD/HRA/PCC/TPC/BPRC/IARM/OPSAA/PRG/DRC/DESI invariants on the source/fix branches. | Accepted by independent re-audit thread `019ef83d-dc76-7031-ae09-8774db2443f4` after focused fix commits `8702a7e8cb6346d29ac5247ee87344f34b766143` and `d57aaf83e94693e2eaefde16108202ea4e254e5e`; no findings remain. Integrated evidence records durable `jobs` domain, `job_process` resources, bounded `execution_output`, lifecycle streams, start/status/list/log/cancel `execute` ops, process-group cleanup, cancellation/finalization race fixes, stale running-state reconciliation after restart/crash, targeted status/log/cancel reconciliation after scope validation, and SACB/CSD coverage for `packages/agent/src/domains/jobs/race_tests.rs`. Consolidated-line acceptance requires validation on this integrated branch before promotion. | PTY/interactive terminals, interpreter runtimes, git/source control, web/network research, subagents, scheduling, notifications, native iOS process panels, public DTO expansion, production deploy behavior, unrelated settings/auth/provider changes, and queue-backed internal dispatch remain deferred. | Closed. Order 21 is blocked pending a resolvable Phase 2 Slice 6 source branch/range. |
| 21 | Phase 2 Slice 6: Git, Worktrees, And Source Control | Phase 2 agent-execution restoration | Not started / not auditable on this tracker branch: no baseline, implementation commit range, final ref, canonical artifacts, or accepted fix commits were found for a completed Slice 6. | Awaiting explicit source branch/ref from orchestration or the user. Source-resolution branch `codex/order-21-source-resolution` started from `codex/order-20-jobs-closeout@58edb6f41d474f1ee61cb58cf2aead3bfe7d4eab` and records only this tracker correction. | None yet. Roadmap-only references exist in historical Phase 2 planning artifacts; they are not implementation evidence. | Source-resolution found no matching local or remote `codex/phase-2-*git*`, `*worktree*`, `*source-control*`, or Order 21 implementation branch after `git fetch --all --prune`. `git grep` across refs found Slice 6 only as roadmap/planning text on Phase 2 artifacts and as older pre-restoration/productization worktree implementation references under `origin/next/modular-capability-engine` and session snapshots. Local database searches for the audit IDs, Order 21, Phase 2 Slice 6, and source-resolution terms found no hidden source ref. Current-line truth: Slice 6 remains deferred/not started, and no integrated or branch-scoped acceptance is claimed. | Git/source-control/worktree restoration remains deferred exactly as described by the Phase 2 roadmap: future module-owned worktree graph, branch status, diffs, staging, commits, conflicts, merges, rebases, pushes, PR handoff, resources, streams, authority, replay, trace, approval decisions, and iOS SourceChanges only after stable server contracts. | Blocked. Pause orchestration for an explicit source ref or a new implementation order; do not dispatch an independent audit until a resolvable branch/range and canonical evidence exist. No later completed Phase 2 slice is documented in current local/remote refs to skip to. |
...
- Searched tracker/docs/history/all refs for Order 21, Phase 2 Slice 6,
  Git/Worktrees/Source Control, `domains/git`, and `domains/worktree`. The
