...
| 36 | Phase 2 Slice 10B: Subagent Worker Launch Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@414cb54119453afbdaf496c7063fcea7dd8e694f`; implementation branch `codex/phase-2-slice-10b-subagent-worker-launch-foundation`; pending independent review. | Source handoff thread `019ef914-ed80-78f2-b253-229240d49444`; discovery thread `019efddb-9995-71f3-8e44-175d81b87adc`; implementation worktree `/Users/<USER>/.codex/worktrees/03e5/tron`. | `packages/agent/src/domains/subagents/`; `packages/agent/src/domains/capability/operations/subagents.rs`; `packages/agent/src/domains/capability/contract.rs`; `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs`; `packages/agent/src/engine/authority/grants/authorization.rs`; `packages/agent/src/engine/durability/resources/subagent_definitions.rs`; provider message converter guidance; Phase 2 scorecard/evidence/inventory; README capability docs; HRA/TMB/TPC/PCC/SACB/CSD/BPRC/DESI/DRC/PMBD/public-protocol/performance-resource guards. | Implementation candidate keeps `subagent_task` as the durable parent causality anchor and adds controlled `subagent_launch`, `subagent_status`, `subagent_result`, and `subagent_cancel` operation values behind `capability::execute`. Launch requires trusted current-session/workspace context, exact `kind:subagent_task` selectors, explicit read/write subagent and resource authority, idempotency, `modelPolicy: bounded_placeholder_v1`, bounded parent refs/summaries, one-running-task-per-scope concurrency, no-network policy, and stored side-effect proof. Status/result/cancel are bounded resource-state projections or transitions only; no worker, job, process, package, tool, network, browser, search, login, public `/engine`, settings, migration, result merge, catalog registration, trust promotion, or native fixed UI scope is added. Validation passed focused subagent/capability/provider/grant tests, fmt, check, personal-info/diff/ignored-file guards, and relevant static gates; DRC failed only the known non-subagents UTC allow-list gap in `goals`, `web`, and `tool_sources`. | Real child-agent execution, actual worker/package/job/process start, tool execution, scheduler/autonomy, cancellation signalling to workers, result merge into conversation state, model-profile policy, approval triggers, public `/engine`, settings/profile/migrations, network/browser/search/login, catalog registration, trust promotion, and native fixed subagent UI remain deferred. | Pending independent review. Stop for review before integration. |
