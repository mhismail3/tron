...
- Consolidation update: `codex/restoration-mainline-consolidation` starts from
  ...
  `019efa7c-7bb9-7803-bf80-224f8b799c1b`.
...
| 26 | Phase 2 Slice 7A: Goal And Question Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@9950ea484299901e09af9077f33466021118ca33`; implementation candidate branch `codex/phase-2-slice-7a-goal-question-foundation-v2`. | Source handoff thread `019ef914-ed80-78f2-b253-229240d49444`; implementation candidate in assigned worktree `/Users/<USER>/.codex/worktrees/a8a7/tron`. | `packages/agent/src/domains/goals/`; `packages/agent/src/domains/capability/operations/goals.rs`; `packages/agent/src/domains/capability/contract.rs`; provider message converter guidance/tests; `packages/agent/src/engine/durability/resources/goal_definitions.rs`; Phase 2 scorecard/evidence/inventory; README capability docs; HRA/TMB/TPC/PCC inventories. | Implementation candidate adds execute-only goal/question lifecycle operations, durable `goal`, `user_question`, and `goal_answer` resource evidence, lifecycle stream evidence on `goals.lifecycle`, bounded summaries/truncation flags, expected-version answer guarding, answer authority/freshness/idempotency provenance, fail-closed scope/expiry/malformed-input behavior, and execute idempotency replay coverage without double-answering. | Autonomous goal runner, planner/task decomposition, hidden prompt queue, scheduler/reminder, notification/APNs, subagents, native Work dashboard, native question sheet, public `/engine` goal API expansion, settings/profile fields, unrelated DB tables, production deploy behavior, and copied historical DTOs remain deferred. | Pending review. Do not treat as accepted/current baseline until independent review and mainline integration complete. |
...
- Phase 2 Slice 6 remains deferred/not started until orchestration or the user
  provides a resolvable source branch/range or starts an explicit implementation
  order.
- Do not skip to an independent Order 21 audit without a source ref and
  canonical evidence.
- No later completed Phase 2 slice is documented in current local/remote refs to
  skip to. Orchestration should pause for user input or an explicit new
  implementation order.

This consolidation branch now provides the requested single-line integration of
the accepted retrospective source/fix/closeout commits through Order 21. Before
any remote promotion or future all-slices acceptance claim, run validation on
this consolidated line and keep Order 21 blocked until a resolvable Slice 6
source branch/range exists.
