...
- Consolidation update: `codex/restoration-mainline-consolidation` starts from
  ...
  `019efa4a-ffdd-7e32-b08e-f175e2a44f1a`.
...
| 25 | Phase 2 Slice 6E: Git Branch Inventory Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@2241def83033d3bb49836b0d6b1ecf3c36fc8c39`; candidate branch `codex/phase-2-slice-6e-branch-inventory-v2`. | Discovery thread `019efa54-f63f-7211-ade4-16912ddee971`; stalled first implementation thread `019efa5c-411c-7e30-9acd-3bcd3e60d7bf`; candidate implementation thread/takeover `019efa5f-d74a-7432-8ee5-3682c0457eab`. | `packages/agent/src/domains/git/`; `packages/agent/src/domains/capability/operations/git.rs`; `packages/agent/src/domains/capability/contract.rs`; provider message converter guidance/tests; Phase 2 scorecard/evidence/inventory; README capability docs; BPRC guard. | Candidate implementation adds execute-only `git_branch_inventory`, backend read-only branch inventory service code, trusted-root and nested-repo rejection, deterministic sorted local branch rows, current/detached HEAD evidence, local upstream/ahead-behind evidence, bounded last-commit metadata, explicit count/byte truncation metadata, and provider/schema guard updates. Focused `git_branch_inventory` tests passed before independent review. | Branch checkout/switch, branch deletion/rename, upstream setup, merge/rebase/reset/revert/cherry-pick, stash/clean, fetch/pull/push, PR handoff, conflict workflows, worktree graph resources, public API expansion, production deploy behavior, native SourceChanges UI, and durable branch-inventory resources remain deferred. | Pending independent review and mainline integration. |

Current source-control candidate after accepted Slice 6D and Slice 6E
discovery: review Slice 6E, a read-only Git branch inventory foundation from
fresh `origin/main`. Arbitrary checkout, branch deletion/rename, remotes,
conflict workflows, worktree graph resources, PR handoff, production deploy
behavior, and native SourceChanges remain deferred.
