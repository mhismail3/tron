...
| 37 | Phase 2 Slice 11A: Procedural State Provenance And Inspection Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@85579a78da54113321cb32de9eb90dd8cd330aef`; implementation candidate on branch `codex/phase-2-slice-11a-procedural-state-provenance-foundation`; pending independent review. | Source handoff thread `019efe16-09d7-73d3-9708-6c6ba5bc6493`; discovery thread `019efe17-adc8-7e83-9974-4bfb9b7256ed`; implementation worktree `/Users/<USER>/.codex/worktrees/e852/tron`. | `packages/agent/src/domains/procedural/`; `packages/agent/src/domains/capability/operations/procedural.rs`; `packages/agent/src/domains/capability/contract.rs`; `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs`; `packages/agent/src/engine/authority/grants/authorization.rs`; `packages/agent/src/engine/durability/resources/procedural_definitions.rs`; provider message converter guidance; Phase 2 scorecard/evidence/inventory; README capability docs; HRA/TMB/TPC/PCC/SACB/BPRC/SSARR/DESI/public-protocol/performance-resource guards. | Implementation candidate adds inert `procedural_record` skill/rule/hook/procedure resources plus read-only `procedural_state_list` and `procedural_state_inspect` operation values behind `capability::execute`. Operations require trusted current-session/workspace context, `procedural.read`, `resource.read`, explicit non-wildcard `procedural_record` authority, exact `kind:procedural_record` and `proceduralKind:*` selectors, stored kind/schema/version/status revalidation, bounded/redacted projections, and `networkPolicy: none`. | Actual activation, trigger firing, prompt injection, learned behavior, autonomous execution, scheduler work, tool execution, worker/package/job/process/network launch, MCP lifecycle, package install/catalog registration, trust promotion, public `/engine`, settings/profile migrations, browser/search/crawl/login scope, native fixed procedural UI, result merge, repo-managed skills, and skill-copy/bootstrap prompt wiring remain absent. | Implementation under review. Do not treat this row as acceptance or current baseline until an independent review/fix loop closes it. |
