...
- Consolidation update: `codex/restoration-mainline-consolidation` starts from
  ...
  `019ef942-3a15-74c0-8230-7065c4e0000d`. Slice 6B implementation work is
  tracked on `codex/phase-2-slice-6b-git-index-mutation` as the narrow
  index-only mutation foundation.
...
| 22 | Phase 2 Slice 6B: Git Index Mutation Foundation | Phase 2 agent-execution restoration | Baseline `origin/main@743b783976b0515372e5525c0c1671f5dbec37bd`; implementation branch `codex/phase-2-slice-6b-git-index-mutation`. | Implementation thread `019ef914-ed80-78f2-b253-229240d49444`; Slice 6B discovery/handoff is integrated on `origin/main@743b783976b0515372e5525c0c1671f5dbec37bd`. | `packages/agent/src/domains/git/`; `packages/agent/src/domains/capability/operations/git.rs`; `packages/agent/src/engine/durability/resources/git_definitions.rs`; Phase 2 scorecard/evidence/inventory; README capability docs; BPRC invariant guard; provider message converter tests. | Implementation candidate adds only `git_stage`/`git_unstage` execute operations, backend `git::stage`/`git::unstage` contracts, trusted-root relative path checks, expected HEAD freshness, idempotency/replay, reason capture, conflict and nested-repository rejection, bounded before/after evidence, `git_index_change` resources, and `git.lifecycle` stream events. | Commits, branch create/checkout/delete, merge, rebase, reset, stash, clean, fetch/pull/push, PR handoff, conflict resolution workflows, worktree graph resources, public API expansion, production deploy behavior, and native iOS SourceChanges remain deferred. | Candidate ready for independent review after implementation validation. |
