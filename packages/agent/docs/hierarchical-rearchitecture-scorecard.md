...
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/tests/grant_tests.rs` | capability runtime grant test owner | Rust hard limit 900 LOC | 1136 LOC | Accepted Slice 24D keeps memory query/decision runtime-grant regressions with existing execute-resource and delegated subagent grant tests; split resource-family, memory, and delegated subagent fixtures before adding more execute-resource families. | accepted_budget |
