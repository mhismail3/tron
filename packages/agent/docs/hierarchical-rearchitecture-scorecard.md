...
| `packages/agent/src/domains/git/service.rs` | git domain owner | Rust hard limit 900 LOC | 1184 LOC | Split read-only status/diff helpers, staged-index tree evidence, and mutation command helpers into owner submodules before adding more source-control operations. | accepted_budget |
| `packages/agent/src/domains/git/tests.rs` | git test owner | Rust hard limit 900 LOC | 1660 LOC | Split read-only Git status/diff tests from index-mutation, commit evidence, resource, and replay tests before adding more source-control coverage. | accepted_budget |
...
| `packages/agent/tests/baseline_pre_restoration_closure_invariants.rs` | BPRC invariant owner | Rust hard limit 900 LOC | 906 LOC | Split scorecard/inventory parsing helpers and Phase 2 lineage assertions into folder-backed modules before extending BPRC closure guards. | accepted_budget |
