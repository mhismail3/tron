...
| `packages/agent/src/shared/protocol/model_audit.rs` | provider audit protocol owner | Rust warning band 850 LOC | 855 LOC | Keep bounded request projection and reasoning-status evidence together for replay contract parity; split payload projection helpers from DTO definitions before adding another provider audit evidence family. | accepted_budget |
...
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitStateTests.swift` | iOS cockpit projection test owner | Swift hard limit 700 LOC | 710 LOC | Existing projection, degradation, presentation, and action tests share large server-fact fixtures; split interaction/action cases into a sibling file before adding another cockpit projection family. | accepted_budget |
