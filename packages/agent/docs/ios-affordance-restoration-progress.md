...
Phase 1 local-native/user-facing affordance restoration is closed after the
session-list/cockpit placement cleanup, settings/onboarding polish, chat visual
cue fixes, recent-input persistence fixes, voice transcription cancellation
fixes, camera/native attachment fixes, and the notification/inbox defer
decision.
