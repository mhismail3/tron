...
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 893 | worker lifecycle projection owner | Split capability-pool role projection, operation-row mapping, and summary derivation into focused cockpit projection extensions before adding more presentation fields; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
| `packages/ios-app/Sources/UI/AgentCockpit/AgentCockpitViews.swift` | 600 | cockpit shell UI owner | Keep sheet orchestration and top-level cockpit chrome here only; move future tab/card composition into focused AgentCockpit subviews before adding more cockpit surface logic. | watch |
...
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitStateTests.swift` | 695 | worker lifecycle projection test owner | Keep capability-pool role and summary projection assertions here only; split future operation-detail or activity-state cases into focused cockpit projection test files. | watch |
