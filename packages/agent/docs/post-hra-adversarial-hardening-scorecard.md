...
| Path | Current LOC | Owner | Warning-band action | Status |
|------|-------------|-------|---------------------|--------|
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 736 | worker lifecycle projection owner | Split capability-cockpit DTO mapping and display-row formatting into focused cockpit projection extensions before adding more presentation fields; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
...
| `packages/ios-app/Tests/Engine/Protocol/WorkerLifecycleDTOTests.swift` | 634 | worker lifecycle protocol test owner | Split new worker lifecycle DTO cases into owner-family suites before broadening this transport/projection test file. | watch |
...
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitViewModelTestFixtures.swift` | 649 | worker lifecycle test fixture owner | Keep cockpit DTO builders and repository doubles here only; split protocol-family fixtures before adding more fixture state. | watch |
