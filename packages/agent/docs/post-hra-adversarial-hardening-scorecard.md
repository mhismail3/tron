...
| Path | Current LOC | Owner | Warning-band action | Status |
|------|-------------|-------|---------------------|--------|
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 843 | worker lifecycle projection owner | Split capability-pool role projection, operation-row mapping, and summary derivation into focused cockpit projection extensions before adding more presentation fields; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
...
| `packages/ios-app/Tests/Engine/Protocol/WorkerLifecycleDTOTests.swift` | 780 | worker lifecycle protocol test owner | Split worker lifecycle DTO cases into capability-cockpit, package, and surface-family suites before broadening this transport/projection test file; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
...
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitStateTests.swift` | 625 | worker lifecycle projection test owner | Keep capability-pool role and summary projection assertions here only; split future operation-detail or activity-state cases into focused cockpit projection test files. | watch |
