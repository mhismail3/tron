...
| AHA-4 | Xcode project hygiene and Mac test execution | 8 | passed_after_fix | Apple CI owner | `tracked_ignored_files_stay_absent` owns ignored and untracked XcodeGen output for both clients. CI and release workflows generate and consume those projects, while Mac CI keeps `build-for-testing` plus focused `TronPathsTests`, `ServerStatusPollerTests`, and `TailscaleProbeTests` execution. | Closed; final closeout still reruns project generation/build checks and focused Mac tests. |
| AHA-5 | Rust module ownership cleanup | 10 | passed_after_fix | Rust architecture owner | Production `#[path]` aliases and module-inception allowances are removed. Provider shared helpers live under `providers::shared`, settings loader paths use `profile::storage::loader`, OpenAI provider tests use a normal folder module, and the orchestrator coordinator lives under `orchestrator::core`. | Closed; AHA-6 owns source-local documentation for the new ownership roots. |
| AHA-6 | Rust progressive docs | 6 | passed_after_fix | Rust docs/tests owner | Ownership-critical Rust roots touched by AHA-5 carry source-local documentation for their submodules, entry points, dependency direction, invariants, and tests. | Closed; final closeout still reruns the full Rust static targets. |
...
| AHA-8 | iOS hierarchy, budgets, and docs | 9 | passed_after_fix | iOS architecture owner | SourceGuard enforces deep hierarchy/count/budget gates for Engine clients, shared capability UI, settings shell, shared components, and Session/Chat tests; iOS resource docs are current, and redundant deployment-target annotations are removed. | Closed; final closeout reruns XcodeGen drift and focused iOS tests. |
| AHA-9 | Inventory and provenance integrity | 8 | passed_after_fix | inventory/provenance owner | The HRA campaign maps and plan digest served closeout, then were retired after durable source-owner documentation and guards became authoritative. | Closed; no point-in-time HRA inventory remains active. |
