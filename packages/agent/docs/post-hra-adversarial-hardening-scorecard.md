...
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 908 | worker lifecycle projection owner | Issue aggregation now lives in `AgentCockpitIssueState.swift`; split capability-pool role projection, operation-row mapping, and summary derivation into further focused cockpit projection extensions before adding more presentation fields. | over_limit_followup |
...
| `packages/ios-app/Tests/Engine/Protocol/WorkerLifecycleDTOTests.swift` | 801 | worker lifecycle protocol test owner | Split worker lifecycle DTO cases into capability-cockpit, package, and surface-family suites before broadening this transport/projection test file; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
...
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitStateTests.swift` | 720 | worker lifecycle projection test owner | Keep capability-pool role and summary projection assertions here only; split future operation-detail or activity-state cases into focused cockpit projection test files. | over_limit_followup |
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitViewModelTestFixtures.swift` | 705 | worker lifecycle test fixture owner | Keep cockpit DTO builders and repository doubles here only; split protocol-family fixtures before adding more fixture state; this file is at the hard guard edge and needs a focused fixture split before growth. | over_limit_followup |
