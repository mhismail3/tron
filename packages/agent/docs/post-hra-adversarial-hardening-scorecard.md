...
| `packages/ios-app/Tests/Session/Chat/ViewModel/ChatViewModelEventRoutingTests.swift` | 707 | chat event-routing test owner | Split event-routing coverage into coordinator- or event-family tests before adding more cases; this file is over the hard guard and needs a focused follow-up split. | over_limit_followup |
...
| `packages/ios-app/Tests/Session/Chat/TurnLifecycleCoordinatorTests.swift` | 617 | turn lifecycle test owner | Add new lifecycle cases under focused coordinator tests before increasing this broad suite. | watch |
...
| `packages/ios-app/Sources/Session/Chat/ViewModel/ChatViewModel.swift` | 654 | chat view-model owner | Move new observation, reconstruction, or lifecycle behavior into focused view-model extensions before expanding the root coordinator further. | watch |
| `packages/ios-app/Sources/UI/Chat/Sheets/ContextControlSheet.swift` | 609 | session briefing owner | Move additional context/model/audit presentation into focused Session Briefing subviews before expanding this sheet further. | watch |
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 729 | worker lifecycle projection owner | Split capability-cockpit DTO mapping and display-row formatting into focused cockpit projection extensions before adding more presentation fields; this file is over the hard guard after cockpit visibility and needs a focused follow-up split. | over_limit_followup |
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitViewModelTests.swift` | 817 | worker lifecycle test owner | Put new cockpit protocol and briefing-specific coverage in focused client or briefing test files before growing this broad cockpit suite; this file is over the hard guard after cockpit visibility and needs a focused follow-up split. | over_limit_followup |
| `packages/ios-app/Sources/UI/AgentCockpit/AgentCockpitDiscoveryViews.swift` | 846 | cockpit capabilities UI owner | Move operation detail and attempt/rollback blocks into focused capability-detail view files before adding more progressive disclosure layers; this file is over the hard guard after cockpit visibility and needs a focused follow-up split. | over_limit_followup |
