...
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 902 | worker lifecycle projection owner | Split capability-pool role projection, operation-row mapping, and summary derivation into focused cockpit projection extensions before adding more presentation fields; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
| `packages/ios-app/Sources/UI/AgentCockpit/AgentCockpitViews.swift` | 618 | cockpit shell UI owner | Keep sheet orchestration and top-level cockpit chrome here only; move future tab/card composition into focused AgentCockpit subviews before adding more cockpit surface logic. | watch |
...
| `packages/ios-app/Tests/Engine/Persistence/SessionRepositoryTests.swift` | 598 | session persistence test owner | Split pagination/listing cases from event reconstruction and projection persistence cases before adding another repository behavior. | watch |
...
| `packages/ios-app/Tests/Engine/Protocol/WorkerLifecycleDTOTests.swift` | 795 | worker lifecycle protocol test owner | Split worker lifecycle DTO cases into capability-cockpit, package, and surface-family suites before broadening this transport/projection test file; this file remains over the hard guard and needs a focused follow-up split. | over_limit_followup |
...
| `packages/ios-app/Tests/Session/Chat/TurnLifecycleCoordinatorTests.swift` | 616 | turn lifecycle test owner | Add new lifecycle cases under focused coordinator tests before increasing this broad suite. | watch |
| `packages/ios-app/Tests/Session/Chat/ViewModel/ChatViewModelEventRoutingTests.swift` | 602 | chat event-routing test owner | Terminal/error event coverage now lives in a focused sibling suite; split future event-family cases before this broad routing suite grows again. | watch |
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitStateTests.swift` | 708 | worker lifecycle projection test owner | Keep capability-pool role and summary projection assertions here only; split future operation-detail or activity-state cases into focused cockpit projection test files. | over_limit_followup |
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitViewModelTestFixtures.swift` | 701 | worker lifecycle test fixture owner | Keep cockpit DTO builders and repository doubles here only; split protocol-family fixtures before adding more fixture state; this file is at the hard guard edge and needs a focused fixture split before growth. | over_limit_followup |
