...
| SOL-6 | Session/event-store lifecycle | 10 | passed_after_fix | session/event store/orchestrator projections | Deleted dead single-event physical delete state; added `sol_session_event_store_lifecycle_is_source_backed`; proved create/resume/fork/archive/unarchive/delete/end, append ordering, projection-cache eviction and reconstruction, projection cleanup, payload refs, export, and session-scoped cleanup. | Closed. | SOL-6 session event-store lifecycle checkpoint |
