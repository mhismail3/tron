...
| SOL-1 | Whole-repo state inventory for Rust server, iOS app, scripts/CI state, docs-owned state claims | 10 | pending | docs/static gates | Not started. | Inventory must cover every tracked production Rust/Swift file containing lifecycle markers plus scripts/CI state surfaces. | pending |
