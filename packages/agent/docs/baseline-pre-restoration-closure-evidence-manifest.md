...
| BPRC-2 | passed | Added 24 restoration backlog rows to the BPRC TSV, preserving the frozen comparison's complete bucket lineage. | BPRC invariant requires `BPRC-FEATURE-01` through `BPRC-FEATURE-24`, `future_restoration`, and `not_in_baseline`. | Future slices consume one row at a time. |
...
- Initial scope review found that the existing baseline had a frozen comparison but
...
- `self_sufficient_agent_runtime_readiness_invariants` initially rejected the
  frozen comparison as successor vocabulary without SSARR classification.
  Fixed by classifying the BPRC artifact family as future-restoration
...
- Full CI initially failed in `primitive_code_cleanup_invariants` because the
  PCC file inventory did not yet classify the BPRC artifacts and the frozen
  comparison repeated retired iOS product-panel names. Fixed by adding PCC
  inventory rows and replacing the literal panel names with a generic
  retired-panel category.
- Full CI then failed in `hierarchical_rearchitecture_invariants` because the
  ...
  Fixed by adding the missing HRA rows.
- Full CI then failed in `security_authority_capability_boundaries_invariants`
  ...
  artifacts. Fixed by adding static/backlog authority-guard rows with no secret
  custody.
- Full CI then failed in the off-plan-authorship cleanup invariant because BPRC
  comparison wording reused stale successor terms. Fixed by rewording
