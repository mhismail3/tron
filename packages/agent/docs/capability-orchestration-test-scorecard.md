...
Current manual-test substrate confidence: **95/100**.
...
| Provider parity | 6 | 3 | Anthropic Opus 4.7 now passes intent-only read-file, safe read-only process, and missing-input guidance paths with one `execute` call each. Provider boundary tests also cover schema conversion. | OpenAI, Anthropic, Gemini, and configured local providers must expose identical `execute` behavior across read, process, and `needs_input` prompts. Kimi is skipped until credentials/subscription is available. |
...
| RWO-012 provider parity | partial | `sess_019e70fb-62da-7113-be4e-0af3b6ff9dc2` | Anthropic Opus 4.7 A/B/C: (A) one `execute` call with intent only, `Read only the first line of README.md`, no target or arguments; (B) one `execute` call with no target and target arguments for a safe read-only process command; (C) one `execute` call with no target and no arguments for `read a file`. | A resolves to `filesystem::read_file`; B resolves to `process::run`; C resolves to `filesystem::read_file` as `needs_input`; A/B require no approval and create one child invocation each; C creates no child invocation. | A: execute `019e70fc-0af7-7dc3-9f58-8b8bd79b4331`, child `filesystem::read_file` `019e70fc-0dcb-7712-9615-123c74624601`, corrections `intent_file_path_to_target_argument` and `intent_first_lines_to_target_arguments`, first line `# Tron`. B: execute `019e7100-12d8-7472-b0ab-451e699fc492`, child `process::run` `019e7100-1447-7453-a958-c0542880c8a5`, stdout contained the isolated session worktree and `# Tron`, stderr empty, exit code `0`. C: execute `019e7105-6532-7773-a00f-705e5d36980b`, status `needs_input`, selected `filesystem::read_file`, `missingArgumentPaths=["arguments.path"]`, `childInvocationIds=[]`, no approval, and no failed invocation. | none | none | Earlier Anthropic run in `sess_019e70ae-1630-7592-b7cb-b750ded1022b` needed multiple tool calls because intent-only `README.md` did not deterministically route or bind `arguments.path`. Fixed in the execute orchestrator by extracting only safe relative file paths and first-line ranges from intent for pure-read `filesystem::read_file`; unsafe absolute, parent, URL, and shell-like paths remain unbound. B/C passed without code changes, proving argument-schema-fit resolution and structured missing-input guidance are provider-neutral for safe process execution and incomplete filesystem reads. | current checkpoint | +3 |
