...
Current manual-test substrate confidence: **99/100**.
...
|---|---:|---:|---|---|
| Execute portal usability | 12 | 12 | One `execute` tool can resolve, prepare, correct, and run without fragile wrapper guessing. RWO-004 proved web search/fetch now run through clean `execute` calls after guidance and alias hardening; RWO-014 proved self-target correction, multi-file decomposition, and model-visible suggested calls. | Stale-plan recovery and vector-warmup degradation remain tracked in their dedicated scenarios. |
| Core first-party capability usefulness | 12 | 12 | Filesystem, process, web, browser/display discovery, git/worktree inspection, model listing, settings reads, and observability trace listing paths are useful in real tasks. | Continue sampling less-used first-party domains under real prompts; do not raise confidence on untested domains without DB-backed evidence. |
...
| Resource and durable output truth | 10 | 9 | Prompt library, voice notes, process materialization, filesystem mutations, and resource inspection/not-found semantics are resource-backed, inspectable, and idempotent in covered paths. | Damage/CAS/hash/discard failure paths and resource-backed package evidence under live scenarios. |
...
| Runtime resilience | 10 | 10 | Reconnect tests, queue/idempotency unit coverage, manual process retries, truthful background auto-retain skip/failure reporting, mounted-chat reconnect after `tron dev -bdt`, and in-flight interruption/reconnect after a fast `tron dev -d` restart are covered. | Keep this full only if queue retry under live load, crash/partial-failure recovery, and activation cleanup continue to stay inspectable in later tests. |
...
| Provider parity | 6 | 6 | Anthropic Opus 4.7, Gemini 3.1 Pro, OpenAI GPT-5.5, and Ollama Gemma 4 E4B pass intent-only read-file, safe read-only process, and missing-input guidance paths with one `execute` call each. Provider boundary tests also cover schema conversion. | Kimi is skipped until credentials/subscription is available. Keep full credit only if future provider adapters preserve tool-call ownership inside provider modules. |
...
| RWO-005 browser/display probe | passed | `sess_019e6f69-300b-7c62-9d67-a929f161d7a9` | Discover browser/display capabilities and inspect safe status without arbitrary browsing. | `browser::*` and/or `display::*` status capabilities, or `needs_capability` if unavailable | Three `capability::execute` calls; one clean `browser::get_status` child `019e6f6a-1083-7d42-8e47-3c1e5f36e2f6` returned `hasBrowser=false`, `isStreaming=false`; no approvals; no `process::run`; no target failures; display discovery found only side-effecting `display::stop_stream` and did not invoke it. | none | none | Browser/display discovery is safe and auditable, but display absence currently surfaces as a structured `needs_selection` execute result with `isError=true`, so the UI can color a non-mutating discovery outcome red. Track as UX/ergonomics cleanup, not substrate failure. | n/a | +1 |
| RWO-006 state queue stream substrate | passed | `sess_019e6f69-300b-7c62-9d67-a929f161d7a9` | Create/read state, enqueue or inspect queue if available, inspect stream/trigger status, report invocation IDs. | `state::*`, `queue::*`, `stream::*` through `execute` | Initial run set and read `state::set`/`state::get`, discovered queue/stream candidates, enqueued one `state::get` queue item, and listed queue `rwo-006`. It exposed a harmless shape miss where `functionid` did not satisfy required `functionId`. The retest after schema-key normalization used one `state::set` child `019e6f7c-82a8-7c82-8ea1-95191b716033`, one `state::get` child `019e6f7c-8bab-7963-87ef-c1aa29b3c55f`, one `queue::enqueue` child `019e6f7c-97a0-7350-a19b-e53cf2795de1`, and one `queue::list` child `019e6f7c-a0cc-7972-8fec-28bb6694d446`; zero failed invocations and zero approvals. Stream availability was safely discovered but not invoked without a subscription id. | none | none | Generic execute normalization now canonicalizes target argument property names against the selected schema when the match is unique, recorded as `schema_property_name_alias`; conflicting aliases still fail closed. | current checkpoint | +2 |
| RWO-007 subagent real use case | passed | `sess_019e6fe0-683e-7711-92a6-f24deafbcec6` | Delegate exactly one subagent to read `packages/agent/src/domains/capability/mod.rs` through `execute`, summarize ownership, and report parent/child lineage ids. | Parent `capability::execute` -> `agent::spawn_subagent`; child `capability::execute` -> `filesystem::read_file` | Parent execute `019e6fe1-4490-7a61-ad34-205fc834d30c`; spawn child `019e6fe1-4576-7203-a13b-73a5868e8c96`; child session `sess_019e6fe1-4579-71e0-81f9-d0f7c1da7d02`; child execute `019e6fe1-55c4-7ca0-b995-bc5b2f1801fb`; child read `019e6fe1-56bb-7641-b1a9-fc383e4a2b66`; no parent `filesystem::read_file` or `process::run`; no approvals. | none | none | Earlier RWO-007 attempts exposed three foundational gaps: Anthropic provider code expected internal `capability_invocation` terms instead of provider-wire `tool_use`; `current-session` subagents could persist literal sentinel ids and empty working directories; `execute` observations omitted the execute invocation id. Fixed by isolating Anthropic wire terms in the provider, resolving subagent session/worktree from trusted invocation context, and projecting execute lineage into result/audit details. | current checkpoint | +3 |
| RWO-008 memory auto-retain context | passed | `sess_019e6ffc-391d-76e0-a909-7f80157e35ea` | Summarize a durable project lesson by reading the scorecard and `memory/mod.rs`; do not run shell commands, write files, or edit memory. | `capability::execute` -> `filesystem::read_file`; background `memory::auto_retain_fire` lineage | Two visible `capability::execute` calls, two `filesystem::read_file` children (`019e6fff-1896-7103-bfb8-b73b1fe8a83e`, `019e6fff-2197-72d1-851d-df19b3c6fcbe`), no process/write/edit/memory-retain target calls, no approvals. Background `memory::auto_retain_fire` returned `{"fired":false,"status":"skipped","reason":"below_threshold","interval":10,"userMessagesSinceRetain":1}`. | none beyond normal agent-result and prompt-history session plumbing | none | First run exposed misleading hidden capability output: `memory::auto_retain_fire` returned `fired:true` even when the policy skipped and no memory was retained. Fixed in the memory domain by returning structured `fired/status/reason/interval/userMessagesSinceRetain` outcomes from the auto-retain service. | current checkpoint | +2 |
| RWO-009 high-risk approval boundary | passed | `sess_019e703d-b774-7630-a442-05fab52c7154` | Attempt one sandbox-materialized `process::run`, deny it, then repeat with approval and deliberate double tap. | Approval records plus target child invocation only after approval | Denial approval `019e703e-b75c-7eb1-8d3d-42aa36e65113` ended `denied`; corresponding `capability::execute` had `childInvocationIds=[]`, `resourceRefs=[]`, and no `process::run` child. Approval `019e7042-f177-7312-9fc6-51ad78bfdb0b` ended `executed`; deliberate double tap produced one `approval::resolve`, one `process::run` child `019e7043-8401-7233-bb19-0a42de5526ec`, and exactly one materialized output file with content `rwo-009-approved`. | `materialized_file:ace58a8bae52bd88b1163f9b93405c0062a12e0eb9e18c25e2d5e35e1fd94ab1`; execution output `res_019e7043-8415-7972-ad63-708e939613ee` | Denied `019e703e-b75c-7eb1-8d3d-42aa36e65113`; executed `019e7042-f177-7312-9fc6-51ad78bfdb0b` | none | n/a | +1 |
| RWO-010 server restart reconnect | passed | `sess_019e7067-eed7-7c20-b373-432ba954e05a`; `sess_019e71a7-826c-7442-befb-c94c9f57fa44` | Restarted the dev server while chat was mounted, then exercised a fast in-flight restart during a long read-only `process::run` and validated a new post-reconnect `execute` turn. | connection events, preserved invocation state, no duplicate turn | Mounted reconnect after `./scripts/tron dev -bdt` left chat input enabled and completed two read-file children (`019e706d-66b7-78f2-8351-4e52a6ebf3be`, `019e706d-6f66-7e62-bc01-17ff05a511f2`). The fast in-flight restart used `./scripts/tron dev -d` while `process::run` child `019e71c4-ac83-7772-96e8-7f4e0ccd6bca` was running; events recorded `notification.interrupted`, no approval, no resource refs, and no duplicate `process::run`. After reconnect, one intent-only `execute` resolved `filesystem::read_file` and created child `019e71c6-9377-7612-8db6-25ac09c589d4`, no approval, no resource refs, and final assistant output reported the first line. | none | none | `dev -bdt`/`dev -bd` build before takeover, so they validate mounted reconnect but can miss in-flight interruption. Use no-build `dev -d` for deterministic interruption tests. The interrupted process is inspectable as `exitCode=-1` plus `notification.interrupted`; a future UX polish may label this as interrupted instead of ordinary process failure, but no duplicate or hidden side effect occurred. | n/a | +1 |
| RWO-011 module worker package activation | partial | `sess_019e7099-56ad-7743-9c83-d9bff97f16b9` plus automated fixture | Discovery retest only through simulator: use `execute` with `operation=discover` to find the canonical package lifecycle capabilities and safe sequence without creating resources, spawning workers, running shell commands, or mutating package state. Automated substrate fixture separately exercises the full lifecycle. | Discovery-only `capability::execute` for module package lifecycle capabilities; automated fixture exercises `module::*`, `worker::spawn`, health, disable, recovery, grants/resources. Remaining manual gap: model-orchestrated app activation fixture. | Simulator discovery: 18 `capability::execute` calls; 0 `module::*` child invocations; 0 approvals; 0 worker spawns; no failed engine invocations. Targeted lifecycle calls returned `capability_discovery` for `module::register_package`, `module::verify_source`, `module::inspect_package`, `module::verify_integrity`, `module::disable`, `module::recover_activation`, `module::approve_source`, `module::configure`, `module::activate`, `module::check_health`, and `module::audit_policy`, each with `childInvocationIds=[]` and `resourceRefs=[]`. The only resource writes were normal prompt-history and final agent-result session plumbing. Automated fixture `e2e_local_process_module_activation_health_and_disable_use_real_worker_spawn` passed on 2026-05-28 and was refreshed on 2026-05-29; it covered register, verify source, approve source, configure, two activate/health/disable cycles, worker cleanup, grant revocation, and disabled diagnostics. | none from simulator module/package targets; automated fixture asserts activation/resource refs internally | none in simulator discovery | Initial package discovery exposed that discovery-only intents could be routed into target preparation and return noisy `needs_input` guidance for `module::*` capabilities. Fixed by making `operation=discover` and clear no-mutation discovery intent a first-class execute path that returns recipe/schema guidance without child invocation. Also changed `needs_selection`, `needs_input`, and `needs_capability` orchestration outcomes from runtime errors to guidance states, while policy/invalid-payload denials remain errors. Provider schema portability and retired-term gates caught follow-up issues before server restart. Full app-driven activation remains intentionally blocked until a server-owned fixture/operator path exists, because app inventory found zero current `worker_package` and `activation_record` resources. | current checkpoint | 0 |
| RWO-011A package resource inventory prerequisite | passed | `sess_019e71a7-826c-7442-befb-c94c9f57fa44` | Use `execute` to determine whether existing package/activation resources make a safe app-driven module lifecycle test possible without hand-authoring a manifest. | `resource::list` for canonical `worker_package` and `activation_record` kinds, no mutation | First retest exposed that module-resource wording resolved to `capability::*`; fixed resource-kind intent binding. Second retest exposed file-read precedence from the word `manifest`; fixed route precedence. Final retest returned `needs_decomposition` with suggested calls for `resource::list` kinds `worker_package` and `activation_record`, then follow-up RWO-011B executed two pure-read list calls. Children were `resource::list` `019e71e8-f505-7e22-9daa-085dc676c2a1` and `019e71e8-f5e0-7463-99ea-9751e7303301`; both returned `resources=[]`, no approval, no durable resource refs, and no failed invocations. | none | none | Live app testing now proves resource inventory is routed through canonical resource truth, but it also proves there are no existing package/activation resources to drive full app activation. Full RWO-011 still needs a server-owned fixture/operator path or remains covered only by automated lifecycle tests. | current checkpoint | 0 |
| RWO-012 provider parity | passed | `sess_019e70fb-62da-7113-be4e-0af3b6ff9dc2` | Anthropic Opus 4.7 A/B/C, Gemini 3.1 Pro D/E/F, OpenAI GPT-5.5 G/H/I, and Ollama Gemma 4 E4B J/K/L: intent-only read-file, safe read-only process, and missing-input guidance. Kimi skipped because subscription/credentials may be unavailable. | Read-file resolves to `filesystem::read_file`; process resolves to `process::run`; missing input resolves to `filesystem::read_file` as `needs_input`; successful read/process calls require no approval and create one child invocation each; missing-input calls create no child invocation. | Anthropic A/B/C: read execute `019e70fc-0af7-7dc3-9f58-8b8bd79b4331` -> child `019e70fc-0dcb-7712-9615-123c74624601`; process execute `019e7100-12d8-7472-b0ab-451e699fc492` -> child `019e7100-1447-7453-a958-c0542880c8a5`; needs-input execute `019e7105-6532-7773-a00f-705e5d36980b`, `missingArgumentPaths=["arguments.path"]`, `childInvocationIds=[]`. Gemini D/E/F: read execute `019e710a-3864-72d3-b1dd-ce810a8465ac` -> child `019e710a-39d8-7820-b6d2-eaed2ac722c8`; process execute `019e710b-5339-7233-bbcd-579543bb4365` -> child `019e710b-54a6-7d60-a2d1-fda611830113`; needs-input execute `019e710c-bfac-73e2-b2dc-43fbdadfdada`, `missingArgumentPaths=["arguments.path"]`, `childInvocationIds=[]`. OpenAI G/H/I: read execute `019e7114-1064-7790-9b7d-1ef5eb18055a` -> child `019e7114-11bb-7c63-ad9b-6b03115ae6a3`; process execute `019e711b-6bca-7601-8477-d709f88c0c5b` -> child `019e711b-6dbb-74b0-9d8f-e8cd87142dd1`; needs-input execute `019e711d-2f75-78b3-bd19-53c39b242351`, `missingArgumentPaths=["arguments.path"]`, `childInvocationIds=[]`. Ollama J/K/L: read execute `019e712f-cca3-7613-9155-6e57e5bb5fab` -> child `019e712f-cfd0-7c43-b76e-3b2ee3d8f7a4`; process execute `019e7130-e516-7293-a32f-e1ee2ef1a9d1` -> child `019e7130-e688-78b0-9adc-86b9d43734e7`; needs-input execute `019e7131-ed2a-7d80-98e7-22f298bba7cf`, `missingArgumentPaths=["arguments.path"]`, `childInvocationIds=[]`. Across all configured provider subcases there were zero approvals and zero target failures. | none | none | Earlier Anthropic run in `sess_019e70ae-1630-7592-b7cb-b750ded1022b` needed multiple tool calls because intent-only `README.md` did not deterministically route or bind `arguments.path`. Fixed in the execute orchestrator by extracting only safe relative file paths and first-line ranges from intent for pure-read `filesystem::read_file`; unsafe absolute, parent, URL, and shell-like paths remain unbound. The first Ollama J run exposed a provider-boundary defect: Ollama native chat returns tool calls under `message.tool_calls`, but the stream handler only read the internal `capability_invocations` spelling. Fixed in the Ollama provider module with a schema alias and regression test, keeping provider-specific wire details out of the engine. | current checkpoint | +6 |
...
| RWO-014 decomposition and provider-shape hardening | passed | `sess_019e7194-bae5-7ac0-a436-60ce4f5be034`; relaunch sanity `sess_019e71a7-826c-7442-befb-c94c9f57fa44`; focused provider tests | Use only `execute`; make exactly one intent-only execute call to read two files with distinct line bounds, stop on `needs_decomposition`, and report suggested calls without executing them. Earlier attempts also exercised Ollama-native chat shape and accidental execute self-targeting. | `capability::execute` returns `needs_decomposition`; no target child is created; suggested calls include correct per-file line bounds; provider adapters keep wire-schema details inside provider modules; accidental self-targeting is corrected before real target resolution. | Final RWO-010C5 retest used one OpenAI tool call `call_DDTA5ydjHbaiEYZbeH3fLT7L`, completed `needs_decomposition`, created `childInvocationIds=[]`, and projected suggested calls in both the tool observation and final assistant answer: scorecard `startLine=1,endLine=20`; `README.md` `startLine=1,endLine=5`. RWO-014C6 after simulator relaunch used one OpenAI execute call with `target=capability::execute`, applied `execute_self_target_removed` and `intent_file_path_to_target_argument`, selected `filesystem::read_file`, created one child `019e71a8-876b-7933-8bc2-16ec59bf666b`, required no approval, and produced no durable target refs. Focused tests also passed for Ollama native string/image message conversion, execute self-target correction, multi-file conjunction splitting, and decomposition content projection. | none from target calls | none | Earlier C2/C3/C4 attempts exposed four root causes: Ollama sent OpenAI-style content arrays to a native string field, the model could self-target `capability::execute`, line bounds leaked from the first file clause to the second, and `needs_decomposition` suggested calls were stored in details but not visible enough in primary tool content. Fixed in the provider boundary and execute orchestrator without adding fallback routes or compatibility state. | current checkpoint | 0 |
| RWO-015 operator substrate status smoke | passed | `sess_019e71a7-826c-7442-befb-c94c9f57fa44` | Use only `execute`; do not run shell/process and do not mutate settings, state, resources, files, or prompts; report current model/provider, settings summary, and recent server/event/log count if available. | Pure-read `model::list`, `settings::get`, and observability/log status capability calls; no process, approval, or target resource mutation. | Final retest used three successful `capability::execute` calls and three pure-read children: `019e720f-e286-7131-ae8a-a05172829963` -> `model::list` child `019e720f-e395-78e1-9cfc-e5aecac19782`; `019e720f-ee02-7792-bac1-a955672d2f78` -> `settings::get` child `019e720f-eef6-7fc3-966c-9d403a252733`; `019e720f-fd5a-7bb1-853d-bfdf598f02e3` -> `observability::trace_list` child `019e720f-fe49-70f0-bfbd-a046f71c718a`. DB rows after `2026-05-29T04:47:00Z` showed 0 failed invocations, 0 approvals, 0 `process::run`, and no filesystem/state/queue mutations. Normal prompt-history and final agent-result resources were expected session plumbing; background auto-retain skipped below threshold. | none from target calls | none | First run over-read safety wording and status-discovery wording: `do not mutate` made pure-read calls discovery-only, and operator status intent could route to `filesystem::read_file`. Fixed by treating only explicit schema/dry-run/no-child wording as discovery-only, adding deterministic operator-status routes and namespace aliases for model/settings/logs/observability, and adding focused execute tests. | current checkpoint | 0 |
| RWO-016 logs-domain usefulness | passed | `sess_019e71a7-826c-7442-befb-c94c9f57fa44` | Use only `execute`; do not run shell/process and do not mutate settings, state, resources, files, prompts, or memory; fetch recent server/app logs for the current session if available. | `logs::recent` through `execute`, no process, approval, or durable target resource mutation. | Initial RWO-016 selected `settings::get` because the negative guard clause contained `settings`. After the resolver fix and server rebuild, RWO-016B used exactly one `capability::execute` invocation `019e721f-72fa-70b2-8195-a0a35c372512` and one `logs::recent` child `019e721f-7607-7fb2-8cf0-65cdf29991e6`, returned 200 rows, highest severity `warn`, and reported no current-session error rows. DB rows after `2026-05-29T05:00:00Z` showed 0 failed invocations, 0 approvals, 0 `process::run`, and no filesystem/state/queue mutations. | none from target calls | none | Negative safety clauses were still part of positive target-routing words, so `do not mutate settings` could outrank a positive logs request. Fixed by deriving operator-status route words from positive intent clauses only, while still preserving safety constraints for preparation. | current checkpoint | 0 |
| RWO-017 resource truth inspect smoke | passed | `sess_019e71a7-826c-7442-befb-c94c9f57fa44` | Use only `execute`; make exactly two pure-read `resource::inspect` calls for one existing agent-result resource and one missing resource id; do not run shell/process or mutate target resources. | `resource::inspect` through `execute`, no process, approval, or target resource refs. | Two target `capability::execute` calls ran between `2026-05-29T05:18:18Z` and `2026-05-29T05:18:23Z`. Existing-resource execute `019e722b-4a47-71d3-a4e4-f426c637e375` created child `resource::inspect` `019e722b-4b77-7023-b680-8070d478d7d2`, which inspected `res_019e721f-8e7d-72c2-b564-0b4444d8caef` as kind `agent_result`, lifecycle `final`, current version `ver_019e721f-8e7e-7930-9133-0c882dd808ab`, with `produced_resource_refs_json=[]`. Missing-resource execute `019e722b-4b7a-77e1-a8b8-df16b5227b9f` created child `resource::inspect` `019e722b-4c81-70b1-aa2d-fb8743bce724`, returned `inspection:null`, required no approval, and produced `resourceRefs=[]`. The narrow time-window DB query showed only those two target children; normal prompt-history and final agent-result resources remained expected session plumbing. | none from target calls | none | none | n/a | 0 |
| RWO-018 resource mutation guard and prompt-history prune load | passed | `sess_019e71a7-826c-7442-befb-c94c9f57fa44` | Use only `execute`; call `resource::update` with a missing `payload` and no idempotency key to prove fail-closed prepare behavior before any target child execution. Retest after fixing prompt-history prune load. | `resource::update` prepare guidance through `execute`; no child `resource::update`; no approval; no target refs; prompt-history bookkeeping stays bounded. | Initial RWO-018 returned execute `019e7230-97eb-77b1-ad6d-4cca4233cae8`, selected `resource::update`, `status=needs_input`, `missingArgumentPaths=["arguments.payload"]`, `childInvocationIds=[]`, `approvalRequired=false`, and `resourceRefs=[]`. The DB also exposed unrelated hidden load: every prompt-history capture scanned 170+ prompt-history resources during auto-prune even though default limits were `historyMaxEntries=10000` and `historyMaxAgeDays=0`. After fixing prompt-library pruning to skip impossible prune scans, RWO-018B returned execute `019e723a-1534-7190-9769-6d351b04dc3d` with the same fail-closed target outcome, no approvals, and no target refs; the latest `prompt_library::history_record` had exactly one `resource::inspect` child and one `artifact::create` child. | none from target calls | none | Prompt-history auto-prune did unnecessary O(history) resource inspections after every captured prompt even when count/age defaults made pruning impossible. Fixed in the prompt-library owner by listing active history resources once and returning before payload inspection when no prune can occur. | current checkpoint | 0 |

## Already exercised and fixed

| Area | Status | What was proven or fixed | Evidence |
|---|---|---|---|
| Prompt history auto-prune load | Covered by focused test and simulator | Default prompt-history retention now avoids scanning every history artifact on each interactive prompt when pruning is impossible. It still uses resource truth and still prunes when count or age policy can actually remove entries. | RWO-018B DB proof; `prompt_history_record_skips_prune_scan_when_default_limits_cannot_prune`; `cargo test prompt_library --lib -- --nocapture`; `./scripts/tron dev -bdt` |
...
| Subagent delegation | Covered by focused tests and simulator | A parent agent can delegate one narrow task through canonical `agent::spawn_subagent`; the child inherits a real isolated worktree from trusted invocation context, uses `execute` for its own capability call, and returns lineage ids that are reconstructable from DB rows and model-visible observations. Provider wire-format handling remains isolated to each provider. | RWO-007 parent session `sess_019e6fe0-683e-7711-92a6-f24deafbcec6`; child session `sess_019e6fe1-4579-71e0-81f9-d0f7c1da7d02`; parent execute `019e6fe1-4490-7a61-ad34-205fc834d30c`; spawn child `019e6fe1-4576-7203-a13b-73a5868e8c96`; child execute `019e6fe1-55c4-7ca0-b995-bc5b2f1801fb`; child read `019e6fe1-56bb-7641-b1a9-fc383e4a2b66`; `current_session_resolves_to_trusted_invocation_session`; `subagent_working_directory_uses_trusted_runtime_metadata`; Anthropic provider boundary tests |
...
| Memory auto-retain | Covered in simulator | Auto-retain runs as hidden engine capability plumbing after an agent turn and now reports truthful outcome state. A below-threshold fresh session returned `fired=false`, `status=skipped`, `reason=below_threshold`, and created no memory retain target invocation or memory resource events. | RWO-008 session `sess_019e6ffc-391d-76e0-a909-7f80157e35ea`; `memory::auto_retain_fire` invocation `019e6fff-3d61-7301-803e-e6add79c02d0`; `auto_retain_fire_reports_skip_without_memory_side_effects`; auto-retain service tests |
...
```text
Use only execute. Scenario RWO-007 final lineage retest. Delegate to exactly one subagent to inspect one narrow repo topic. The subagent must read packages/agent/src/domains/capability/mod.rs from the current session workspace using execute with filesystem::read_file, summarize what owns capability orchestration in 3 bullets, and include both the executeInvocationId from the execute observation and the filesystem::read_file child invocation id. The parent must not read the file, must not use process::run, and must not retry with an absolute path. Wait for the subagent result, then report the subagent session id, the parent spawn invocation id, the subagent execute invocation id, the subagent filesystem::read_file invocation id, and whether any approval was required.
```
...
2. Restart the server with `./scripts/tron dev -bdt` for mounted reconnect
   after a build/test cycle, then use `./scripts/tron dev -d` during an
   already-running long read-only command to deterministically exercise
...
Current live status: simulator resource inventory for this scenario is now
safe and canonical. `execute` decomposes multi-kind package inventory into
bounded `resource::list` calls, and separate `worker_package` /
`activation_record` lists returned zero current resources in
`sess_019e71a7-826c-7442-befb-c94c9f57fa44`. The full lifecycle remains
covered by the automated local-process fixture until there is a server-owned
app/operator fixture that can create the package resources without hand-written
chat manifests.
