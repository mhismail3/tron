...
Current manual-test substrate confidence: **96/100**.
...
| Provider parity | 6 | 4 | Anthropic Opus 4.7 and Gemini 3.1 Pro both pass intent-only read-file, safe read-only process, and missing-input guidance paths with one `execute` call each. Provider boundary tests also cover schema conversion. | OpenAI and configured local providers still need the same explicit A/B/C parity pass. Kimi is skipped until credentials/subscription is available. |
