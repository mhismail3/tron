...
Current manual-test substrate confidence: **93/100**.
...
| Provider parity | 6 | 1 | Anthropic Opus 4.7 now passes the intent-only read-file path with one `execute` call after engine-owned intent argument binding. Provider boundary tests also cover schema conversion. | OpenAI, Anthropic, Gemini, and configured local providers must expose identical `execute` behavior across read, process, and `needs_input` prompts. Kimi is skipped until credentials/subscription are available. |
...
| RWO-012 provider parity | partial | `sess_019e70fb-62da-7113-be4e-0af3b6ff9dc2` | Anthropic Opus 4.7: use only `execute`; make exactly one execute call with intent only, `Read only the first line of README.md`; no target or arguments. | One `capability::execute` resolving to `filesystem::read_file`, with no approval and one child invocation. | One `capability::execute` parent `019e70fc-0af7-7dc3-9f58-8b8bd79b4331`; one `filesystem::read_file` child `019e70fc-0dcb-7712-9615-123c74624601`; audit corrections `intent_file_path_to_target_argument` and `intent_first_lines_to_target_arguments`; no approvals; no failed invocations; first line `# Tron`. | none | none | Earlier Anthropic run in `sess_019e70ae-1630-7592-b7cb-b750ded1022b` needed multiple tool calls because intent-only `README.md` did not deterministically route or bind `arguments.path`. Fixed in the execute orchestrator by extracting only safe relative file paths and first-line ranges from intent for pure-read `filesystem::read_file`; unsafe absolute, parent, URL, and shell-like paths remain unbound. | current checkpoint | +1 |
