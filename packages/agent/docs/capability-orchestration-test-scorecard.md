...
Current manual-test substrate confidence: **97/100**.
...
| Provider parity | 6 | 5 | Anthropic Opus 4.7, Gemini 3.1 Pro, and OpenAI GPT-5.5 pass intent-only read-file, safe read-only process, and missing-input guidance paths with one `execute` call each. Provider boundary tests also cover schema conversion. | Configured local providers still need the same explicit A/B/C parity pass. Kimi is skipped until credentials/subscription is available. |
