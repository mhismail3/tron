...
Current manual-test substrate confidence: **75/100**.
...
| Generated UI/action gateway | 10 | 7 | Prompt management generated surface, action toasts, live `ui::surface_for_target` resource collection authoring, live `ui::submit_action` through stored action coordinates, stale `ui_surface` version rejection before target child invocation, and renderer stale/offline fail-closed behavior | Malformed input, revoked grant, target revision drift beyond surface version drift, and replay from the fixed iOS management sheet |
...
4. remaining generated-UI work should cover malformed input, revoked/expired
   grants, target revision drift beyond surface version drift, and replay from
   the fixed iOS management sheet itself.
