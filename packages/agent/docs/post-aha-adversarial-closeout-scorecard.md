...
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | 1577 | capability runtime grant owner | concrete split plan: keep cross-domain grant orchestration in `grant.rs`; move the next execute-resource grant family into a focused grant policy helper module before adding behavior here. | watch |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/tests/grant_tests.rs` | 1334 | capability runtime grant test owner | concrete split plan: keep shared grant test fixtures here; move the next resource-family or delegated-module regression batch into focused `tests/grant_*` sibling modules before adding coverage here. | watch |
| `packages/agent/src/domains/capability/contract.rs` | 1107 | capability contract owner | concrete split plan: keep the primitive capability catalog boundary here; move the next execute operation schema guidance block into a focused contract helper module before adding behavior here. | watch |
...
| `packages/agent/src/domains/capability_binding/cockpit_visibility.rs` | 1293 | capability binding cockpit visibility owner | concrete split plan: keep the cockpit overview projection here; move the next operation fact aggregation, action derivation, or DTO row formatting behavior into focused cockpit projection helpers before adding visibility fields here. | watch |
| `packages/agent/src/domains/capability_binding/shadow_trial.rs` | 1654 | capability binding shadow-trial owner | concrete split plan: keep shadow-trial orchestration here; move the next validation, projection, or event-record helper into focused shadow-trial modules before adding replacement-trial semantics here. | watch |
| `packages/agent/src/domains/capability_binding/tests.rs` | 1943 | capability binding test owner | concrete split plan: keep shared binding fixtures here; split the next binding policy, shadow-trial, or cockpit-visibility regression batch into focused sibling test modules before adding coverage here. | watch |
| `packages/agent/src/domains/git/service.rs` | 1517 | git domain owner | concrete split plan: keep git service orchestration here; move the next status/diff, staged-index, command-boundary, or ref-helper behavior into focused git service modules before adding behavior here. | watch |
| `packages/agent/src/domains/git/tests.rs` | 3054 | git test owner | concrete split plan: keep shared git test fixtures here; move the next status/diff, mutation, commit, branch-start, resource/schema, provider-static, or replay batch into focused git test modules before adding coverage here. | watch |
...
| `packages/agent/src/engine/authority/grants/authorization.rs` | 3767 | engine authority owner | concrete split plan: keep shared authorization orchestration here; move the next operation/resource selector extractor or per-domain explicit-grant scanner into focused authority grant modules before adding behavior here. | watch |
| `packages/agent/src/domains/model/providers/openai/stream_handler/tests.rs` | 973 | OpenAI stream handler test owner | concrete split plan: keep shared stream-handler fixtures here; split the next stream delta, tool-call, usage, or error regression batch into focused sibling tests before adding coverage here. | watch |
...
| `packages/agent/tests/capability_modularity_scorecard_invariants.rs` | 1205 | capability modularity invariant owner | concrete split plan: keep shared capability-modularity scorecard parsing here; split the next cockpit, binding, or replacement-phase assertion group into focused invariant helper modules before adding assertions here. | watch |
| `packages/agent/tests/ios_affordance_restoration_map_invariants.rs` | 1111 | IARM invariant owner | concrete split plan: keep shared invariant helpers in the root target; split the next physical-device, queue/phase, APNs defer, or planning-text guard into focused invariant modules before adding assertions here. | watch |
