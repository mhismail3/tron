...
|------|-------------|-------|--------|-------------|
| `packages/agent/src/domains/git/service.rs` | 1397 | Git domain service | Slice 6A/6B/6C/6D source-control service covering trusted repository facts, bounded status/diff command helpers, non-interactive commit execution, pure staged-index tree evidence, and shared Git command status helpers. | PCC-6/P2AER-INV-013 |
| `packages/agent/src/domains/git/tests.rs` | 2612 | Git domain tests | Slice 6A/6B/6C/6D source-control tests covering status/diff evidence, index-only stage/unstage, staged-index commit evidence, branch-start evidence and rollback, path/conflict/idempotency/resource guards, hook/prompt suppression, and provider-visible `capability::execute` routing. | PCC-6/P2AER-INV-013 |
