...
| `packages/agent/src/domains/capability/operations/catalog.rs` | 1938 | Capability discovery adapter owner | Canonical model-facing search, inspect, recovery, and conformance projection for all execute operations remains together so discovery semantics cannot drift; its focused regression module is isolated under `catalog/tests.rs`, and new discovery behavior must first reuse the canonical operation contract. | PCC-10/capability-contract |
