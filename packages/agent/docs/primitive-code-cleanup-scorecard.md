...
|------|-------------|-------|--------|-------------|
| `packages/agent/src/engine/authority/grants/authorization.rs` | 3103 | Engine authority grants owner | Broad capability-invocation authorization scanner retained while accepted Slice 24E adds exact procedural module-pack selectors alongside prior Slice 24D memory query/decision selectors, prior Slice 24C delegated subagent task selectors, and existing module-program-execution exact runtime/job selectors; future authority slices should split per-domain scanner helpers before growing this file further. | PCC-10/P3MSA-S23B/P3MSA-INV-009/P3MSA-INV-011/P3MSA-INV-012/P3MSA-INV-013 |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | 1410 | Capability runtime grant owner | Runtime capability grant derivation retained while accepted Slice 24E derives exact procedural module-pack selectors, prior Slice 24D derives exact memory query/decision selectors, and prior Slice 24C derives delegated subagent task selectors under `capability::execute`; future grant slices should split per-domain grant policies before growing this file further. | PCC-10/SACB-10/P3MSA-INV-009/P3MSA-INV-011/P3MSA-INV-012/P3MSA-INV-013 |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/tests/grant_tests.rs` | 1284 | Capability runtime grant test owner | Accepted Slice 24E keeps procedural module-pack runtime-grant regressions with prior Slice 24D memory query/decision, execute-resource, and subagent delegated-grant tests; split restored resource-family, procedural, memory, and subagent delegated-grant fixtures into focused modules before adding more execute-resource families. | PCC-10/P3MSA-INV-011/P3MSA-INV-012/P3MSA-INV-013 |
| `packages/agent/src/domains/capability/contract.rs` | 1081 | Capability execute contract owner | Provider-visible `capability::execute` schema remains centralized while accepted Slice 24E adds procedural module-pack record/review fields and prior restoration slices preserve the single operation contract surface. | PCC-10/P2AER-INV-013/P3MSA-INV-011/P3MSA-INV-013 |
| `packages/agent/src/domains/capability/operations/catalog/mod.rs` | 1938 | Capability discovery adapter owner | Canonical model-facing search, inspect, recovery, and conformance projection for all execute operations remains together so discovery semantics cannot drift; its focused regression module is isolated under `catalog/tests.rs`, and new discovery behavior must first reuse the canonical operation contract. | PCC-10/capability-contract |
| `packages/agent/src/domains/capability/operations/catalog/tests.rs` | 2253 | Capability discovery regression owner | Exhaustive model-facing discovery, inspection, schema, recovery, effect-filter, and workflow-plan regressions cover the canonical capability catalog bridge without inflating the production adapter file. | PCC-10/capability-contract |
| `packages/agent/src/domains/capability/operations/operation_contract/authority.rs` | 1370 | Canonical operation authority-contract owner | Exact per-operation grants, selectors, network policy, and scope constraints stay in the canonical operation-contract tree so catalog inspection, grant derivation, and runtime validation cannot drift; split deterministic family tables from shared constructors before adding another authority family. | PCC-10/capability-contract |
| `packages/agent/src/domains/capability/operations/operation_contract/governance.rs` | 1549 | Canonical governance input-contract owner | Closed schemas for module, procedural, context, and capability-route governance stay in the canonical operation-contract tree so discovery and runtime validation consume identical request shapes; split deterministic family tables before adding another governance family. | PCC-10/capability-contract |
| `packages/agent/src/domains/capability/operations/operation_contract/output/projection/mod.rs` | 1750 | Canonical provider-evidence projection owner | Provider-safe evidence allowlists and profile dispatch stay behind one output-contract boundary so raw domain details cannot bypass redaction; split profile-specific projectors before adding another output profile. | PCC-10/PMBD |
| `packages/agent/src/domains/capability/operations/operation_contract/output/projection/tests.rs` | 2017 | Canonical provider-evidence projection regression owner | Exhaustive catalog, trace, governance, context, resource, error, and redaction cases validate the real provider envelope; split tests by output profile before adding another profile family. | PCC-10/PMBD |
| `packages/agent/src/domains/capability/operations/operation_contract/records.rs` | 1391 | Canonical record-plane input-contract owner | Closed schemas for durable record operations stay in the canonical operation-contract tree so catalog and runtime request validation cannot drift; split deterministic record-family tables before adding another family. | PCC-10/capability-contract |
| `packages/agent/src/domains/capability/operations/trace.rs` | 1035 | Capability trace operation owner | Provider-safe trace list/get projection, paging, and proof metadata remain together at the capability adapter boundary; split list and inspect projection helpers before expanding trace behavior. | PCC-10/PMBD |
| `packages/agent/src/domains/capability/pool.rs` | 1297 | Capability pool classification owner | The complete operation surface, audience, replacement class, default visibility, minimality decision, and evolution path remain one typed inventory consumed by catalog and cockpit views; split generated rows from validation helpers before adding another classification dimension. | PCC-10/capability-pool |
| `packages/agent/src/domains/capability_binding/route.rs` | 2231 | Governed capability route owner | Exact-scope lookup, activation, disable, rollback, event evidence, stale-version checks, and supervised runtime dispatch remain one fail-closed routing boundary; split read resolution from write lifecycle handlers before adding another routed operation. | PCC-10/capability-dynamic-replacement |
| `packages/agent/src/domains/context_control/service.rs` | 1391 | Context control service owner | Snapshot, compaction, epoch clear, action history, survivor policy, and provider-safe evidence remain under one session-scoped owner; split snapshot projection from action mutation before adding another context action. | PCC-10/context-control |
| `packages/agent/src/domains/context_control/tests.rs` | 1258 | Context control regression owner | Snapshot, compaction, epoch, survivor, replay, selector, and redaction cases remain together while the context contract stabilizes; split read and mutation fixtures before expanding the action set. | PCC-10/context-control |
| `packages/agent/src/domains/jobs/tests.rs` | 1030 | Durable jobs regression owner | Start, status, list, log, cancellation, replay, truncation, and custody tests cover one adapter-replaceable job contract; split lifecycle and output-custody fixtures before adding another runner. | PCC-10/jobs |
| `packages/agent/src/domains/model/providers/openai/stream_handler/tests.rs` | 1139 | OpenAI streaming regression owner | Response event parsing, reasoning summaries, tool calls, token accounting, and malformed-stream recovery remain exhaustive for one provider stream; split event-family fixtures before adding another event class. | PCC-10/PMBD |
| `packages/agent/src/engine/durability/resources/capability_binding_definitions.rs` | 1023 | Capability binding resource-schema owner | Candidate, binding, activation, event, rollback, and policy resource schemas remain one versioned governance definition set; split immutable candidate schemas from route lifecycle schemas before adding another record kind. | PCC-10/capability-dynamic-replacement |
| `packages/ios-app/Sources/Session/WorkerLifecycle/AgentCockpitState.swift` | 892 | Native worker-cockpit state owner | Package catalog, launch, stop, retire, and refresh state stay behind one observable session owner used by the progressive cockpit UI; split command orchestration from view projection before adding another lifecycle action. | PCC-10/worker-cockpit |
| `packages/agent/src/domains/capability_binding/cockpit_visibility.rs` | 1293 | Capability binding cockpit projection owner | Capability modularity cockpit projection keeps operation ownership, binding/shadow activity, truncation, and redaction policy together while replacement visibility stabilizes; split aggregation, operation projection, activity summary, and redaction-policy helpers before adding write controls. | PCC-10/capability-modularity |
| `packages/agent/src/domains/capability_binding/shadow_trial.rs` | 1654 | Capability binding shadow-trial owner | Capability modularity shadow trial keeps request, decision, run, evidence projection, and stale-version guards together for the first metadata-only replacement proof; split request/decision/run/evidence owner modules before adding a second shadow-trial target. | PCC-10/capability-modularity |
| `packages/agent/src/domains/capability_binding/tests.rs` | 1943 | Capability binding test owner | Capability binding regressions keep binding-policy, shadow-trial, cockpit projection, selector, stale-version, redaction, and no-routing coverage together for the first modularity campaign; split binding-policy, shadow-trial, cockpit-visibility, and authorization test modules before expanding replacement routing. | PCC-10/capability-modularity |
| `packages/agent/src/domains/procedural/service.rs` | 1917 | Procedural domain owner | Accepted Slice 24E keeps metadata-only procedural definition, activation request, activation decision, projection-safety, idempotency, and exact-authority checks in the existing procedural owner without restoring repo-managed skills or hook execution. | PCC-10/P3MSA-INV-013 |
| `packages/agent/src/domains/procedural/tests.rs` | 1149 | Procedural domain test owner | Accepted Slice 24E adds definition replay, activation/deactivation/rollback review, exact-selector denial, redaction, and provider-safe projection regressions to the existing inert procedural state tests. | PCC-10/P3MSA-INV-013 |
| `packages/agent/src/domains/module_registry/tests.rs` | 1028 | Module registry test owner | Accepted Slice 24E extends consolidated module-registry manifest tests with the pending-review procedural module pack while existing tests still cover manifest schema, seed redaction, authority scope, lifecycle, and side-effect coverage. | PCC-10/P3MSA-INV-001/P3MSA-INV-013 |
...
| `packages/agent/src/domains/memory/tests.rs` | 1784 | Memory domain tests | Accepted Slice 24D retrieval, prompt-inclusion, retention-policy, and provider-safe projection tests extend the existing memory foundation coverage for disabled mode, record lifecycle, redaction, prompt trace, migration, scope isolation, idempotency, authority denial, and provider-safe inspection. | PCC-10/P2AER-INV-015/P2AER-INV-016/P3MSA-INV-012 |
...
| `packages/agent/tests/capability_modularity_scorecard_invariants.rs` | 1205 | Capability modularity invariant owner | Capability modularity invariant keeps inventory, registry/dispatch parity, kernel/governance locks, adapter seams, cockpit visibility, and docs linkage in one closeout gate; split parser helpers, ownership rules, source-backed seam checks, and artifact-link checks before adding runtime routing invariants. | PCC-10/capability-modularity |
| `packages/agent/tests/ios_affordance_restoration_map_invariants.rs` | 1111 | IARM invariant owner | Large static invariant suite covering historical map closure, stale wording, physical-device redaction, queue/phase anchors, and APNs defer guards. | PCC-6/IARM-9 |
