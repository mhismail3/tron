...
|------|-------------|-------|--------|-------------|
| `packages/agent/src/engine/authority/grants/authorization.rs` | 3103 | Engine authority grants owner | Broad capability-invocation authorization scanner retained while the Slice 24E implementation candidate adds exact procedural module-pack selectors alongside prior Slice 24D memory query/decision selectors, prior Slice 24C delegated subagent task selectors, and existing module-program-execution exact runtime/job selectors; future authority slices should split per-domain scanner helpers before growing this file further. | PCC-10/P3MSA-S23B/P3MSA-INV-009/P3MSA-INV-011/P3MSA-INV-012/P3MSA-INV-013 |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | 1410 | Capability runtime grant owner | Runtime capability grant derivation retained while the Slice 24E implementation candidate derives exact procedural module-pack selectors, prior Slice 24D derives exact memory query/decision selectors, and prior Slice 24C derives delegated subagent task selectors under `capability::execute`; future grant slices should split per-domain grant policies before growing this file further. | PCC-10/SACB-10/P3MSA-INV-009/P3MSA-INV-011/P3MSA-INV-012/P3MSA-INV-013 |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant_tests.rs` | 1284 | Capability runtime grant test owner | The Slice 24E implementation candidate keeps procedural module-pack runtime-grant regressions with prior Slice 24D memory query/decision, execute-resource, and subagent delegated-grant tests; split restored resource-family, procedural, memory, and subagent delegated-grant fixtures into focused modules before adding more execute-resource families. | PCC-10/P3MSA-INV-011/P3MSA-INV-012/P3MSA-INV-013 |
| `packages/agent/src/domains/capability/contract.rs` | 1081 | Capability execute contract owner | Provider-visible `capability::execute` schema remains centralized while the Slice 24E implementation candidate adds procedural module-pack record/review fields and prior restoration slices preserve the single operation contract surface. | PCC-10/P2AER-INV-013/P3MSA-INV-011/P3MSA-INV-013 |
| `packages/agent/src/domains/procedural/service.rs` | 1917 | Procedural domain owner | The Slice 24E implementation candidate keeps metadata-only procedural definition, activation request, activation decision, projection-safety, idempotency, and exact-authority checks in the existing procedural owner without restoring repo-managed skills or hook execution. | PCC-10/P3MSA-INV-013 |
| `packages/agent/src/domains/procedural/tests.rs` | 1149 | Procedural domain test owner | The Slice 24E implementation candidate adds definition replay, activation/deactivation/rollback review, exact-selector denial, redaction, and provider-safe projection regressions to the existing inert procedural state tests. | PCC-10/P3MSA-INV-013 |
| `packages/agent/src/domains/module_registry/tests.rs` | 1028 | Module registry test owner | The Slice 24E implementation candidate extends consolidated module-registry manifest tests with the pending-review procedural module pack while existing tests still cover manifest schema, seed redaction, authority scope, lifecycle, and side-effect coverage. | PCC-10/P3MSA-INV-001/P3MSA-INV-013 |
