...
|------|-------------|-------|--------|-------------|
| `packages/agent/src/engine/authority/grants/authorization.rs` | 2199 | Engine authority grants owner | Broad capability-invocation authorization scanner retained while Slice 24A adds exact file/source-control module-pack scopes, evidence resource kinds/selectors, file-root enforcement, and wildcard denial; future authority slices should split per-domain scanner helpers before growing this file further. | PCC-10/P3MSA-S23B/P3MSA-INV-009 |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | 1026 | Capability runtime grant owner | Runtime capability grant derivation retained while Slice 24A removes broad state fallback for file/source-control operations and adds exact filesystem, Git, resource-kind, and selector grants under `capability::execute`; future grant slices should split per-domain grant policies before growing this file further. | PCC-10/SACB-10/P3MSA-INV-009 |
