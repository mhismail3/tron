...
| `packages/agent/src/domains/git/tests.rs` | 2470 | Git domain tests | Slice 6A/6B/6C/6D source-control tests covering status/diff evidence, index-only stage/unstage, staged-index commit evidence, branch-start evidence, path/conflict/idempotency/resource guards, hook/prompt suppression, and provider-visible `capability::execute` routing. | PCC-6/P2AER-INV-013 |
