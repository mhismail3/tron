...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  action catalogs, stored outcome projection, and grant records/bootstrap/event
  builders stay in focused submodules.
...
- Split the next engine-fabric hotspots by ownership boundary:
  ...
  `registry.rs`, and generated-UI files where the audit finds a direct
  simplification path.
