...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  trust-audit boundaries must stay split: parent roots remain below 1,000 LOC while
  artifact/goal curation, materialized-file and patch mutation, payload
  parsing, shared resource-ref helpers, resource/UI schemas, resource-store
  events/ids, SQLite schemas/codecs, store tests, queue draining, queue
  lifecycle stream projection, approval tests, catalog change DTOs, control
  action catalogs, stored outcome projection, grant records/bootstrap/event
  builders, and worker protocol guide/template projection stay in focused
  submodules; registry invocation lifecycle, output-contract enforcement, and
  catalog-change recording stay out of the live-catalog root; trust-audit
  schedule parsing and due-bucket calculation stay out of the module audit
  action root.
...
- Split the next engine-fabric hotspots by ownership boundary:
  ...
  `primitives/ui/authoring/mod.rs`, and generated-UI files where the audit finds a direct
  simplification path.
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 5948 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates. | 5950 | CLC-9 |
