...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  derivation/narrowing checks, and function registrations stay out of the
  lifecycle root; trust-audit schedule parsing and due-bucket calculation stay
  out of the module audit action root; source-trust registration,
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 6021 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full source-trust, manifest, and grant-boundary subtree checks. | 6050 | CLC-9 |
| `packages/agent/src/engine/primitives/module.rs` | 2239 | CLC-2 engine primitives | Module lifecycle and package/config/activation orchestration still need ownership splits after grant, manifest, registration, trust-audit, and source-trust extraction. | 2250 | CLC-2 |
