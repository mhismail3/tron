...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  catalogs, and function registrations stay out of the lifecycle root; engine
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 6121 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full host-meta, host-runtime-host, source-trust, manifest, grant, resource, schema, payload, and action-catalog subtree checks. | 6150 | CLC-9 |
| `packages/agent/src/engine/primitives/module.rs` | 1346 | CLC-2 engine primitives | Module lifecycle and package/config/activation orchestration still need ownership splits after resource, grant, manifest, registration, schema, payload, action, trust-audit, and source-trust extraction. | 1400 | CLC-2 |
...
| `packages/agent/src/engine/host.rs` | 2446 | CLC-2 engine host | Host orchestration and mutation concerns remain dense after extracting meta-function vocabulary, watch DTOs, schemas, visibility projection, delegated invocation shaping, payload parsers, and the primitive-runtime host implementation. | 2500 | CLC-2 |
