...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  live-catalog root; trust-audit schedule parsing and due-bucket calculation
  stay out of the module audit action root; source-trust registration,
...
- Split the next engine-fabric hotspots by ownership boundary:
  `primitives/module.rs`, `host.rs`, and generated-UI files where the audit
  finds a direct simplification path.
