...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  manifest/grant/registration/trust-audit boundaries must stay split: parent roots remain below 1,000 LOC while
  ...
  derivation/narrowing checks, resource mutation/projection helpers, and
  function registrations stay out of the lifecycle root; trust-audit schedule
...
- Split the next engine-fabric hotspots by ownership boundary:
  `primitives/module.rs` and `host.rs` where the audit finds a direct
  simplification path. Generated-UI parent/action files are currently under
  budget but remain watched for further action/display decomposition if touched.
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 6038 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full source-trust, manifest, grant, and resource-boundary subtree checks. | 6050 | CLC-9 |
| `packages/agent/src/engine/primitives/module.rs` | 1868 | CLC-2 engine primitives | Module lifecycle and package/config/activation orchestration still need ownership splits after resource, grant, manifest, registration, trust-audit, and source-trust extraction. | 1900 | CLC-2 |
