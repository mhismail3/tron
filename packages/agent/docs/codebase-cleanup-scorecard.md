...
- Split the next engine-fabric hotspots by ownership boundary:
  `primitives/module.rs` where the audit finds a direct simplification path.
  `host.rs` is now below 1,000 LOC but remains watched for concern drift.
  Generated-UI parent/action files are currently under budget but remain
  watched for further action/display decomposition if touched.
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 6163 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full host-meta, host-runtime-host, host-handle-surface, source-trust, manifest, grant, resource, schema, payload, and action-catalog subtree checks. | 6200 | CLC-9 |
