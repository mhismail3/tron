...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  invocation shaping, and payload parsers stay in `host/meta.rs`; trust-audit schedule
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 6108 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full host-meta, source-trust, manifest, grant, resource, schema, payload, and action-catalog subtree checks. | 6150 | CLC-9 |
...
| `packages/agent/src/engine/host.rs` | 2730 | CLC-2 engine host | Host orchestration and mutation concerns remain dense after extracting meta-function vocabulary, watch DTOs, schemas, visibility projection, delegated invocation shaping, and payload parsers. | 2800 | CLC-2 |
