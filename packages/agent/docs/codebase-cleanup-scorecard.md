...
- Large Rust, Swift, and shell/script files over 1,000 LOC must have a row in
  the large-file audit table, keep the table's `Current LOC` value exact, and
  stay below that row's budget.
...
| CLC-7 | Complete | +10 | `packages/ios-app/Sources/Views/`, `packages/ios-app/Sources/Services/Network/`, `packages/ios-app/Sources/Models/`, `packages/ios-app/docs/`, `packages/agent/tests/` | Split Audit Details section/component bodies, capability invocation display/presentation models, capability detail/result renderers, new-session flow types/components, and engine connection types/protocol frames out of broad Swift roots; trimmed redundant transport comments instead of widening private state-machine internals. | `xcodegen generate`; targeted `xcodebuild test` for capability invocation display, new-session flow, engine connection reconnect, and Audit Details state; `ios_thin_client_boundaries_stay_split`. | No chat/session navigation behavior changed, so simulator deep-link smoke was not required for this checkpoint. Large iOS test matrices remain audited rows for the test/harness cleanup lane rather than CLC-7 production-source blockers. |
...
- `Views/AuditDetails/AuditDetailsView.swift` is now the Audit Details
  orchestration root below 1,000 LOC. Section identity lives in
  `AuditDetailsSection.swift`; reusable audit chips, cards, rows, and
  inspection sheet components live in `AuditDetailsComponents.swift`.
...
- `xcodebuild test -scheme Tron -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:TronMobileTests/CapabilityInvocationDisplayModelTests -only-testing:TronMobileTests/NewSessionFlowTests -only-testing:TronMobileTests/EngineConnectionReconnectTests -only-testing:TronMobileTests/AuditDetailsStateTests`: passed, 54 selected tests after an initial split-access failure was fixed.
...
| `packages/agent/src/engine/tests/generated_ui.rs` | 2041 | CLC-9 engine tests | Generated UI primitive matrix. | 2100 | CLC-9 |
...
| `packages/agent/src/domains/agent/runner/orchestrator/subagent_manager_tests.rs` | 1550 | CLC-9 runner tests | Subagent manager orchestration matrix. | 1575 | CLC-9 |
| `packages/agent/src/engine/tests/module_activation/source_trust.rs` | 1600 | CLC-9 engine tests | Module source-trust scenario matrix. | 1600 | CLC-9 |
| `packages/agent/src/engine/primitives/ui/authoring/actions.rs` | 1449 | CLC-2 generated UI | Generated action construction and target-specific payload projection stay together until a new renderer/action boundary appears. | 1500 | CLC-2 |
...
| `packages/ios-app/Tests/ViewModels/AuditDetailsStateTests.swift` | 1043 | CLC-9 iOS tests | Audit Details state matrix spans connection, invocation, and generated UI state transitions after the Worker Artifacts projection cases moved into their own focused suite. | 1100 | CLC-9 |
...
| `packages/agent/tests/integration.rs` | 1304 | CLC-9 harnesses | Integration harness module map and shared test setup stay together while the scenario bodies live under `tests/integration/`. | 1325 | CLC-9 |
...
| `packages/agent/src/shared/protocol/events/tron/catalog.rs` | 1176 | CLC-3 protocol | Exhaustive generated `TronEvent` enum catalog and accessors stay together for serde tagging, grep-ability, and match exhaustiveness without introducing a compatibility macro layer. | 1200 | CLC-3 |
...
| `packages/agent/src/domains/capability/operations/mod.rs` | 1105 | CLC-1 capability operations | Capability operation orchestration keeps validation, lifecycle, and projection together until a narrower operation boundary appears. | 1150 | CLC-1 |
| `packages/agent/src/domains/sandbox/mod.rs` | 1093 | TPROD-C sandbox lifecycle | Sandbox worker lifecycle now owns workspace-autonomy selector derivation, cleanup helpers, live-worker evidence, and product-facing helper labels together until a narrower spawn/cleanup boundary appears. | 1150 | TPROD-C |
