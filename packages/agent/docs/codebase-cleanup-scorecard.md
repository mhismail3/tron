...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  manifest/registration/trust-audit boundaries must stay split: parent roots remain below 1,000 LOC while
  ...
  live-catalog root; module manifest validation and runtime parsing stay out
  of the lifecycle root; module function registrations stay out of the
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 6002 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full source-trust and manifest-boundary subtree checks. | 6050 | CLC-9 |
| `packages/agent/src/engine/primitives/module.rs` | 2785 | CLC-2 engine primitives | Module lifecycle and package/config/activation orchestration still need ownership splits after manifest, registration, trust-audit, and source-trust extraction. | 2800 | CLC-2 |
