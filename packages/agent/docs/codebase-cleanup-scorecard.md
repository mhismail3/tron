...
- CLC-2 resource, resource-store, queue, approval, engine-type, generated-UI,
  ...
  registration/trust-audit boundaries must stay split: parent roots remain below 1,000 LOC while
  ...
  live-catalog root; module function registrations stay out of the lifecycle
  root; trust-audit schedule parsing and due-bucket calculation stay out of
  the module audit action root; source-trust registration, verification,
  approval, lifecycle, policy, inspection, support, and schema concerns stay
  in focused source-trust submodules.
...
|------|-------------|-------|--------|--------|--------------------------|
| `packages/agent/tests/threat_model_invariants.rs` | 5971 | CLC-9 static gates | Cross-cutting architecture gates and cleanup scorecard enforcement, including CLC-1 and CLC-2 split-boundary gates plus full source-trust subtree checks. | 6000 | CLC-9 |
| `packages/agent/src/engine/primitives/module.rs` | 3438 | CLC-2 engine primitives | Module lifecycle and package/config/activation orchestration still need ownership splits after registration, trust-audit, and source-trust extraction. | 3450 | CLC-2 |
