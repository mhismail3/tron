...
addenda close the repo/task DTO, `Attachments`, and SessionTree projection
loops; PET-11 still owns retained `capability_support` host indirection/naming,
...
loop only. Later PET-11 addenda close the repo/task DTO, `Attachments`, and
SessionTree projection loops; PET-11 still owns retained `capability_support`
host indirection/naming, update surfaces, diagnostics/logging, dynamic
...
close the `Attachments` and SessionTree projection loops; PET-11 still owns
retained `capability_support` host indirection/naming, update surfaces,
...
checkpoint. Later addenda close the retained/successor `Attachments` and
SessionTree projection loops; PET-11 still owns retained `capability_support`
host indirection/naming, update surfaces, diagnostics/logging, dynamic
rendering, final fresh server/DB/trace proof, iPhone/iPad closeout screenshots,
and any remaining engine-envelope/catalog
...
and removes the legacy prompt image API. The following SessionTree addendum
closes the fixed iOS tree projection loop. PET-11 still owns retained
...
Residual risk: this closes the fixed iOS SessionTree projection loop. PET-11
still owns retained `capability_support` host indirection/naming, update
