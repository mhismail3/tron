...
| Notification bell/list/detail | ios_action_wiring + server_contract | `NotificationClientTests`, `NotificationStoreTests`, `notification_resources`, and `notifications::tests` cover action idempotency/context, server unread-count responses, resource decision truth, offline failure state, and sheet-content shape. | iPhone 17 Pro manual pass covers bell count/refresh, detail mark-read, deep-link session auto-read, global Read All, and offline failure toast. | `notifications::mark_read` / `notifications::mark_all_read` invocation rows plus notification/decision resource links. |
