...
| FSC-1 | Failure inventory | 8 | passed_after_fix | docs/static gates | Inventory and TSV were re-audited after server, durable replay, iOS, and mapping changes; stale gap rows were removed or replaced with current source-backed semantics. | None; final stale-doc enforcement moves to FSC-10. | inventory closeout checkpoint |
