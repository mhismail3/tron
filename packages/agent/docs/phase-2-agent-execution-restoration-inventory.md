...
`P2AER-INV-023` records accepted Slice 22A's historical Dependency Restoration
Review Foundation. Its artifact-backed dependency policy guard retired with
the frozen comparison oracle; current dependency governance belongs to
`packages/agent/src/domains/module_dependencies/mod.rs` and is verified by
`P3MSA-INV-007` request, decision, and policy tests. Dependency restoration
remains deferred: no dependencies are restored, package/runtime behavior is
unchanged, and no fixed iOS panels or Swift UI work are included in this
historical slice.
