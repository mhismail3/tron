...
Slice 14C is an accepted current-baseline foundation for `BPRC-FEATURE-18`. It adds
...
execution remain deferred for later slices.
...
`P2AER-INV-020` is current baseline after accepted Slice 19A: Event Protocol
Catalog Parity Foundation. The accepted slice keeps the persisted session
...
transport-visible session event variants. The accepted slice deliberately does not
...
`P2AER-INV-021` is current baseline after accepted Slice 20A: Database Schema
Catalog Parity Foundation. The accepted slice adds a source-backed static guard
that compares the README Database Schema table catalog against active SQLite
schema sources, records the existing `engine_stream_subscriptions` table in the
README catalog, and keeps the database/storage row focused on evidence parity.
It deliberately does not add a new migration, compatibility reader, product
...
`P2AER-INV-022` is current baseline after accepted Slice 21A: Settings/Profile
Catalog Parity Foundation. The accepted slice strengthens the retained CPE
static gate with README Key Configuration/default parity against `TronSettings`
and source-backed iOS editable-setting catalog checks across Swift decode,
update, state, UI controls, and parity tests. It fixes documented drift for
...
`P2AER-INV-023` is current baseline after accepted Slice 22A: Dependency
Restoration Review Foundation. The accepted slice keeps dependency restoration
deferred and adds a source-backed dependency policy guard that parses feature
index section 24, treats that removed-dependency catalog as policy evidence,
and checks `packages/agent/Cargo.toml` plus `packages/agent/Cargo.lock` so
removed dependencies fail if they reappear without an owning module and Phase 2
rationale. No dependencies are restored, no package/runtime behavior changes,
and no fixed iOS panels or Swift UI work are included.
