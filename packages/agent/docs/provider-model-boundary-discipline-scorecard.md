...
| PMBD-8 | Provider Request Audit and Replay Boundary | 8 | passed | `model.provider_request` audit payloads are canonical, versioned, redacted and bounded, carry request/audit metadata without secrets, and are the durable replay source for provider request structure and digest evidence. Exact provider envelopes, bounded envelope projections, provider-independent snapshots, and whole-envelope digest fallback are explicit; bulk media is not duplicated into audit events. |
