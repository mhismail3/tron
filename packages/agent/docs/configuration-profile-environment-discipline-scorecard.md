...
| CPE-3 | Sparse user overlay and atomic update discipline | 12 | passed | Existing `SettingsStore` tests prove serialized atomic writes, effective reload, malformed-file preservation, partial update merge, and no default-copy reset; `ProfileRuntime` tests own live-snapshot swapping; CPE inventory and static gate anchor the owning code. |
...
| CPE-6 | iOS settings parity | 12 | passed | Swift settings decode/update/state/UI/tests are inventoried; decoder now fails on missing or mistyped server-owned fields instead of masking malformed server state with local defaults, and Slice 21A adds a source-backed editable-setting catalog guard across decode, update, state, UI, and parity tests. |
...
| CPE-9 | Targeted static gates and verification harness | 8 | passed | Added `configuration_profile_environment_discipline_invariants.rs` with scorecard, inventory, default drift, env ownership, iOS parity, sparse overlay, stale branch, README, and predecessor wiring checks; Slice 21A extends it with README Key Configuration/default parity and iOS editable-setting catalog checks. |
...
- Slice 21A fixed root README drift for `retry.maxRetries`, aligning the Key Configuration example with `TronSettings::default()` and the managed default profile value of `3`.
