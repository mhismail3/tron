...
| PET-11 | End-to-end closeout and "cannot remove more" audit | 8 | running | test_harness | Interim adversarial passes deleted retained hosted OpenAI `tool_search`/`computer_use` DTO and stream support, catalog support flags, stale iOS capability catalog DTOs/renderers, Mac Screen Recording/Accessibility onboarding gates, the remaining iOS capability identity/rendering vocabulary for contracts, implementations, functions, plugins, workers, schemas, trust/risk/effect, bindings, search, and inspect, and the server-side event/dashboard identity projection that still emitted those resolved-catalog fields. `CapabilityEventIdentity`, capability invocation persisted payloads, server activity summaries, and runtime stream adapters now carry only primitive name, operation, trace/root ids, theme color, presentation hints, status/result/duration, and runtime details; the dead `capability.resolution` event is deleted. Fresh Rust/OpenAI, iOS SourceGuard, iOS capability/event reconstruction, Mac permission/wizard, and server event/projection tests pass for these slices. Final row still requires fresh server run, provider-loop fixture, DB/event/ledger/trace inspection, iPhone/iPad screenshots, final `git diff --check`, and a full retained-surface audit. | Remaining PET-11 audit targets include retained/successor iOS `Attachments`, `Process`, `SessionTree`, `UserInteraction`, `session_drafts.skills_json`, repo/task DTOs, retained `capability_support` host indirection/naming, update surfaces, diagnostics/logging, dynamic rendering, and any remaining engine-envelope/catalog terms that are not true primitive resource/version metadata. Successor scorecard starts only after this row passes. | PET-11 server primitive identity teardown checkpoint |
