...
| PET-10 | Absence gates, traceability gates, and dead-code cleanup | 6 | running | test_harness | First-class traceability slice is implemented: `execute` now writes durable Agent Trace-style `trace_records`, exposes `trace_list`/`trace_get`, and static/integration gates prove invocation/session/model/authority/file/content-hash linkage. The approval/observability primitive-plane checkpoint physically deletes the engine approval store, approval primitive, observability primitive, runtime trace-projection functions, approval tests, and approval settings; removes grant-level `approval_required`; replaces generated UI `approvalPolicy` with `authorityPolicy`; and strengthens the PET-10 absence gate across retained engine/settings/transport/session/README paths. The client cleanup checkpoint deletes the remaining iOS plugin-source, audio/transcription, memory-retain, and rules planes from settings, DTOs, event plugins, dispatch protocols, chat state, message rendering, sheet state, local event taxonomy, tests, Info.plist, README, and iOS docs. | iOS stale-symbol scans and focused xcodebuild proof are green. `cargo check` still reports the known 293 library warnings plus 1 binary warning from broader dead source. Full `cargo test --lib` remains blocked by stale `#[cfg(test)]` modules that still reference removed session profile/rules/subagent APIs; PET-10 still owns that backend test-source teardown and warning cleanup. | PET-10 client cleanup checkpoint |
