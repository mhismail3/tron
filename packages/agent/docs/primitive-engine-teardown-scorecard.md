...
| PET-10 | Absence gates, traceability gates, and dead-code cleanup | 6 | pending | test_harness | Static gates reject removed namespaces, old UI modes, skills/rules/hooks, capability registry recipes, old policy surfaces, compatibility/fallback wording, and large-file budget drift. New gates prove agent trace records are first-class durable data linked to invocation/session/model/VCS/resource evidence and visible to the agent. `rg` scans and compiler warnings are clean for touched areas. | Gates must avoid brittle prose matching that blocks future simplification. | pending |
