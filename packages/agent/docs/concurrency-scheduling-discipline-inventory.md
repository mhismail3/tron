...
Status: CSD-10 `passed_after_fix`; 129 production scheduling-surface rows and 22 static-gate/predecessor rows inventoried and classified.
...
Static-gate/predecessor rows: 22 `test_fixture` rows.
...
| `EventStoreManager` | `ios_event_persistence` | The weak-idle global lane consumes the bounded newest-value `EngineClient` stream. Rapid replacement predecessors and first/subsequent load chains have one idempotent terminal owner. Shutdown cancels and awaits accepted work before fixture database close; deinit cancellation is safety-only. |
