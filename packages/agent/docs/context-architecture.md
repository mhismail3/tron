...
| Capability schemas | Live provider projection of the single model-facing `capability::execute` primitive; search/inspect stay internal/operator surfaces | Every turn; reduced for local models only by capability policy | Session | Provider-native tool/function definitions | Session | `capabilities` bucket | Capability primitive spec, denied capabilities, local policy |
