...
| RIURD-4 | Setup, install, uninstall, restart, and clean-machine bootstrap | 12 | passed | Source proof covers Rust-owned `~/.tron`/managed-profile initialization, secure auth creation, narrow contributor payload staging, GUI-helper install skip, `/Applications/Tron.app` validation, health-gated start/restart, and uninstall preserving database/workspace by default. |
