...
| CSD-6 | Agent/session turn concurrency | 10 | passed_after_fix | agent orchestrator/session store | Inventory and focused tests cover run semaphore ownership, active-run state, event-store per-session serialization, forwarder drain, abort tokens, pending invocation trackers, and sequence counters. The event persister no longer owns a duplicate scheduling surface. | Closed. | CSD-6 agent/session concurrency checkpoint |
