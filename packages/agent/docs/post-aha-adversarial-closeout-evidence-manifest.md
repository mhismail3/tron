...
| PAC-8 | passed_after_fix | Aligned the local and GitHub CI closeout test sets. `scripts/tron ci test` now runs an explicit target list for `db_path_guard`, PET/PCC/HRA/AHA/PAC invariants, `primitive_trace_execution`, and serial `integration`; the GitHub Rust static-gates job runs the same named targets in the same order. README and CONTRIBUTING now document that parity. | PAC local/GitHub parity static gate passed; PAC scorecard formalization passed; `bash -n scripts/tron.d/quality.sh` passed; post-HRA closeout target passed. | Closed. | pending |
...
Completed local/GitHub CI parity:

- Local `scripts/tron ci test` runs Rust lib/bin tests first, then the explicit
  closeout target list.
- GitHub's Rust static-gates job runs the same named closeout targets in the
  same order for docs/template/iOS/Mac/script/CI changes.
- The `integration` target remains serial in both local and GitHub execution.

Focused proof:

```bash
bash -n scripts/tron.d/quality.sh
cargo test --manifest-path packages/agent/Cargo.toml --test post_aha_adversarial_closeout_invariants local_and_github_ci_run_the_same_static_closeout_targets -- --nocapture
cargo test --manifest-path packages/agent/Cargo.toml --test post_aha_adversarial_closeout_invariants post_aha_adversarial_closeout_scorecard_stays_formalized -- --nocapture
cargo test --manifest-path packages/agent/Cargo.toml --test post_hra_adversarial_hardening_invariants -- --quiet
```

Result: exit 0 for all focused commands on 2026-06-08. A full
`post_aha_adversarial_closeout_invariants` run remains intentionally red until
PAC-9 closes the provenance/privacy/residue gate.
