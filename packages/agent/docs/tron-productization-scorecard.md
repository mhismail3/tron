...
Status: **active; TPROD-H passed after fix; TPROD-I next**
...
| TPROD-G | Generated UI authoring | 9 | passed_after_fix | Engine-authored native surfaces show preview, plain diff, allowed actions, validation state, and Inspect details; no iOS app update is needed for new capability surfaces. | Added a server-authored source-control review matrix to generated surfaces using the existing fixed catalog: preview, plain diff preview, allowed action metadata, validation-state cue, and Inspect details render through stored `ui_surface` layout components without client target construction. Focused generated UI tests proved the matrix, stored actions, and absence of layout-embedded templates; iOS renderer/source guards proved the fixed renderer still submits only stored action coordinates and contains no target-function or payload-template construction. | Closed. |
| TPROD-H | Model, automation, and subagent routing | 10 | passed_after_fix | Automations support model presets; local models are opt-in per flow; agents choose models within policy; subagent chips show task/model/result/lineage. | Added `ModelPreset` and server-owned `ModelRoutingPresentation` with `Local when possible`, `Balanced`, and `Deep`; cron agent-turn schedules persist pending preset presentation and completed runs carry selected route evidence. Subagent spawn now resolves the selected model from active profile settings and optional presets instead of the retired hard-coded default, persists task profile/model routing in spawn/completion/failure events, final `agent_result` resources, generated subagent lineage UI, and iOS live/reconstructed chips. Focused Rust tests cover cron preset schedule truth, generated subagent lineage route rendering, and live subagent route/resource persistence; focused iOS `SubagentStateTests` cover route/profile chip data. | Closed. TPROD-I must now prove the full Tron-maintains-Tron local work loop. |
| TPROD-I | Tron replaces Codex for local work loop | 9 | pending | Flagship proof: a Tron chat session creates or updates a Tron helper capability/skill, tests it, updates docs, shows UI evidence, and reaches local review-ready state without push/merge/release. | The substrate self-extension loop now includes managed `self-extend`, local packs, created-by-agent lineage, trust presentation, generated UI authoring, cron model presets, and subagent model/task route evidence. Full flagship proof is still missing. | Run a live Tron chat session that updates Tron locally, captures generated UI/model/subagent evidence, updates docs, and stops before push/merge/release. |
...
TPROD-I is active. The next required proof is the flagship Tron-maintains-Tron
local work loop: a chat session must create or update a Tron helper, test it,
update docs/evidence, show generated UI plus model/subagent routing evidence,
and stop at review-ready state without push, merge, release, or deploy.
