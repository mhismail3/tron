...
Status: **active; TPROD-G passed after fix; TPROD-H next**
...
TPROD-H is active. The next required proof is model, automation, and subagent
routing product coverage: user-facing presets, automation routing, selected
model disclosure, local fallback disclosure, subagent task/model/result chips,
and parent/child lineage must remain server-owned and policy-backed.
