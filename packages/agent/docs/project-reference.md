...
| `tron version` | Central release version helper (`print`, `check`, `sync`, `bump`, `test`, and hosted-CI `github-output`). `VERSION.env` is the only hand-edited release identity source; `sync` updates active platform project mirrors, while `check` rejects duplicate Base.xcconfig version fields. |
