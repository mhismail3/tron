...
Continue JARVIS-1 by adding primary-path vocabulary gates after the replacement
Work surfaces exist, while keeping JARVIS-0 visual baseline and JARVIS-8
Guardrails UX open. The next iOS checkpoint should add plain Guardrails
settings, use paired-server simulator action checks when the surface is
interactive, and remove primary-path Engine Console/capability clutter rather
than renaming it.
