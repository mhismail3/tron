...
| JARVIS-3 | Worker-first orchestration | 10 | running | Main agent prompt, primer, and self-extend guidance default to delegating to workers for non-trivial work. Subagents and spawned helper capabilities share a product `Worker` projection while preserving distinct server primitives. Integration proof shows fan-out workers completing a local task without user approvals. | Partial: the Worker Guide and `execute` schema instruct non-trivial work delegation to workers/subagents, worker-guide resource docs are server-owned, provider-context tests prove the guide reaches hosted/local providers, and a real integration test fans out two session workers, invokes both through `execute`, verifies `agent::work_snapshot` projects both workers, and proves `approval::list` remains empty. | Remaining: project live subagent instances as product Workers in the Work snapshot/detail model before awarding points. |
...
Continue JARVIS-1/JARVIS-3 by projecting subagent instances as Workers and
adding primary-path vocabulary gates after the replacement Work surfaces exist,
while keeping JARVIS-0 visual baseline and JARVIS-8 Guardrails UX open. The
next iOS checkpoint should add plain Guardrails settings, use paired-server
simulator action checks when the surface is interactive, and remove
primary-path Engine Console/capability clutter rather than renaming it.
