...
| BPRC-2 | Frozen comparison conversion into restoration backlog | 10 | passed | The BPRC inventory TSV contains 24 machine-readable restoration backlog rows preserving every frozen comparison bucket, with old surface, current equivalent, dependencies, risk, server/iOS impact, restoration constraint, and `not_in_baseline` status. |
| BPRC-3 | Successor-feature absence guards | 10 | passed | The BPRC invariant rejects unapproved restored product domain roots, repo-managed skills, fixed iOS product-panel roots, runtime-authored worker claims, learned-memory/rule stores, tool-synthesis runtime claims, and provider-visible tool widening; approved Phase 2 restorations must be tracked in the Phase 2 inventory. |
...
| BPRC-6 | iOS baseline parity and UX readiness audit | 10 | passed | Source-owned focused iOS tests and the BPRC audit cover onboarding/pairing, chat, reconstruction, settings/auth/model setup, diagnostics/logs, attachments, generic runtime surfaces, and reconnect/error states. |
...
| BPRC-8 | Artifact inventory and provenance integrity | 8 | passed | The BPRC markdown/TSV inventory covers every active BPRC artifact, retained baseline and iii references, README/static-gate paths, key engine/iOS substrate paths, and all 24 restoration backlog rows. |
