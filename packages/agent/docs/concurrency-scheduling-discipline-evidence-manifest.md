...
| CSD-6 | passed_after_fix | Agent/session rows and focused tests cover run semaphore ownership, active-run guards, event-store per-session serialization, forwarder drain, abort tokens, invocation trackers, and sequence counters. The redundant event-persister worker was removed. | CSD inventory guard plus focused agent/orchestrator/event-store tests. | Closed. | CSD-6 agent/session concurrency checkpoint |
