...
| PMBD-4 | Provider stream/message converter tests cover OpenAI malformed arguments and duplicate deltas, Anthropic/Kimi/Ollama ID remapping and argument object handling, Google non-object argument fail-closed behavior, and exact canonical capability-result envelope text transport across OpenAI, Google, Anthropic, Kimi, MiniMax, and Ollama. Capability output tests reject inline media outside durable resource custody. The PMBD static guard rejects provider-local result truncation markers. |
