...
| TPC-1 | Retention inventory | 8 | passed_after_fix | architecture | Added the retention inventory and TSV, then regenerated it after TPC-2 through TPC-11, TMB closeout, Phase 2 Slice 3 memory foundation, Phase 2 Slice 6B Git index mutation foundation, and the Phase 2 Slice 8D web source archive candidate. Current coverage is 1,758 tracked and newly introduced source/docs/script paths in TPC scope: 111 `primitive`, 605 `implementation`, 385 `support`, 526 `test`, 131 `docs`, and 0 `delete`. Checkpoint commit: `92521b511`. | No unclassified tracked source remains. | TPC-1 inventory checkpoint |
