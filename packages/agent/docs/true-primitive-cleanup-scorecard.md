...
| `packages/agent/src/domains/capability/contract.rs` | capability contract owner | Phase 2 Slice 13 extended the single provider-visible `capability::execute` schema with durable device and notification operation fields while preserving provider portability; Slice 24C implementation-candidate docs keep the single execute contract surface unchanged pending review. | 1030 / 750 | Split operation-specific schema field builders into focused contract modules before adding more execute operations. |
...
| `packages/agent/src/domains/memory/tests.rs` | memory test owner | Slice 18A adds query/decision evidence regressions to the existing memory foundation test file. | 1101 / 800 | Split query/decision evidence regressions and older memory lifecycle fixtures into focused memory test modules before adding more memory coverage. |
...
| `packages/agent/src/engine/authority/grants/authorization.rs` | engine authority owner | Slice 24C implementation-candidate/fix work adds exact delegated subagent task selector enforcement to the shared capability authorization path while preserving module-program-execution exact runtime/job checks. | 2840 / 750 | Split operation/resource selector extraction and per-domain explicit-grant scanners into owner modules before adding more execute-resource families. |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant_tests.rs` | capability invocation test owner | Slice 24C implementation-candidate/fix work keeps delegated subagent runtime-grant regressions with existing execute-resource authority regressions pending independent review. | 1114 / 800 | Split resource-family and delegated subagent grant fixtures into focused modules before adding more execute-resource families. |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | capability runtime grant owner | Slice 24C implementation-candidate/fix work derives exact delegated subagent task selectors and delegated module refs in the shared provider execute grant derivation path. | 1302 / 750 | Split restored execute-resource family and delegated subagent grant derivation helpers into focused owner modules before adding more execute-resource families. |
| `packages/agent/src/domains/capability/operations/module_program_execution_tests.rs` | capability execute test owner | Slice 24C implementation-candidate/fix work extends accepted Slice 24B module-program-execution coverage with subagent delegation launch/replay exact-selector integration coverage. | 1210 / 800 | Split module-program-execution lifecycle tests from subagent delegation integration fixtures before adding more delegated module-pack coverage. |
| `packages/agent/src/domains/subagents/execution.rs` | subagents owner | Slice 24C implementation-candidate/fix work keeps controlled subagent launch/status/result/cancel lifecycle, exact task-selector checks, and delegated module binding checks together pending independent review. | 1171 / 750 | Split launch planning, follow-up inspection/cancel/result projection, and authority-selector helpers into focused subagent execution modules before expanding subagent behavior. |
| `packages/agent/src/domains/module_registry/tests.rs` | module registry test owner | Accepted Slice 23A module registry tests remain consolidated across manifest schema, seed redaction, authority scope, lifecycle, and side-effect coverage. | 871 / 800 | Split manifest schema, authority scope, lifecycle, and side-effect regression fixtures before expanding module registry coverage. |
...
| TPC-1 | Retention inventory | 8 | passed_after_fix | architecture | Added the retention inventory and TSV, then regenerated it after TPC-2 through TPC-11, TMB closeout, Phase 2 restoration slices, and accepted Phase 3 module-state slices through Slice 23H. Current coverage is 2,008 tracked and newly introduced source/docs/script paths in TPC scope: 112 `primitive`, 801 `implementation`, 385 `support`, 575 `test`, 135 `docs`, and 0 `delete`. Checkpoint commit: `92521b511`. | No unclassified tracked source remains; `grant.rs`, the Slice 23G `module_dependencies/service.rs` accepted row, and the Slice 23H `AgentCockpitState.swift` accepted row carry budget rows until focused execute-resource, dependency-service, and cockpit-state splits land. | TPC-1 inventory checkpoint |
