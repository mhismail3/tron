...
| `packages/agent/src/domains/capability/contract.rs` | capability contract owner | Phase 2 Slice 13 extended the single provider-visible `capability::execute` schema with durable device and notification operation fields while preserving provider portability; accepted Slice 24C keeps the single execute contract surface unchanged. | 1030 / 750 | Split operation-specific schema field builders into focused contract modules before adding more execute operations. |
...
| `packages/agent/src/domains/memory/tests.rs` | memory test owner | Slice 24D adds retrieval, prompt-inclusion, retention-policy, and provider-safe projection regressions to the existing memory foundation test file. | 1389 / 800 | Split retrieval/prompt-inclusion/retention regressions plus older memory lifecycle fixtures into focused memory test modules before adding more memory coverage. |
...
| `packages/agent/src/engine/authority/grants/authorization.rs` | engine authority owner | Accepted Slice 24C adds exact delegated subagent task selector enforcement to the shared capability authorization path while preserving module-program-execution exact runtime/job checks. | 2840 / 750 | Split operation/resource selector extraction and per-domain explicit-grant scanners into owner modules before adding more execute-resource families. |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant_tests.rs` | capability invocation test owner | Accepted Slice 24C keeps delegated subagent runtime-grant regressions with existing execute-resource authority regressions. | 1114 / 800 | Split resource-family and delegated subagent grant fixtures into focused modules before adding more execute-resource families. |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | capability runtime grant owner | Accepted Slice 24C derives exact delegated subagent task selectors and delegated module refs in the shared provider execute grant derivation path. | 1302 / 750 | Split restored execute-resource family and delegated subagent grant derivation helpers into focused owner modules before adding more execute-resource families. |
| `packages/agent/src/engine/tests/authority/execute_goal_authorization.rs` | engine authority test owner | Slice 24D adds exact memory module-pack execute selector regressions to the shared capability execute authorization test fixture. | 810 / 800 | Split memory/resource-family authorization tests into focused authority test modules before adding more execute-resource families. |
