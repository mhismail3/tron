...
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant_tests.rs` | capability invocation test owner | Accepted Slice 24E keeps procedural module-pack runtime-grant regressions with prior Slice 24D memory query/decision, execute-resource, and delegated subagent authority regressions. | 1284 / 800 | Split resource-family, procedural, memory, and delegated subagent grant fixtures into focused modules before adding more execute-resource families. |
...
| TPC-1 | Retention inventory | 8 | passed_after_fix | architecture | Added the retention inventory and TSV, then regenerated it after TPC-2 through TPC-11, TMB closeout, Phase 2 restoration slices, and accepted Phase 3 module-state slices through Slice 23H plus Slice 24H implementation-candidate manifest state. Current coverage is 2,033 tracked and newly introduced source/docs/script paths in TPC scope: 112 `primitive`, 821 `implementation`, 385 `support`, 580 `test`, 135 `docs`, and 0 `delete`. Checkpoint commit: `92521b511`. | No unclassified tracked source remains; `grant.rs`, the Slice 23G `module_dependencies/service.rs` accepted row, and the Slice 23H `AgentCockpitState.swift` accepted row carry budget rows until focused execute-resource, dependency-service, and cockpit-state splits land. | TPC-1 inventory checkpoint |
