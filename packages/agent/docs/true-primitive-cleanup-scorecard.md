...
| `packages/agent/tests/baseline_pre_restoration_closure_invariants.rs` | restoration static gate owner | Phase 2 baseline closure gate now covers Slice 6B docs, provider exposure, resource evidence, and non-goal source-control guards. | 861 / 800 | Split baseline documentation inventory checks from provider/resource/source-control guards before adding more restoration slices. |
