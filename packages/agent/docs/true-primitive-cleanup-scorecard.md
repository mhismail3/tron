...
| TPC-10 | Docs, guards, inventories | 5 | passed_after_fix | docs/static gates | Added the final docs/guards/inventories TPC gate, updated active README wording, refreshed HRA/PCC ownership inventories for `scripts/tron.d/manual-deploy.sh`, removed old deploy-command spelling from active reference docs, regenerated the TPC retention inventory for the new guard, and recorded the residual-term review policy. | No active docs/inventory stale path remains; final full verification remains TPC-11. | TPC-10 docs/guards/inventory checkpoint pending |
