...
| `packages/agent/src/domains/capability/operations/catalog.rs` | capability discovery adapter owner | Canonical model-facing search, inspect, unsupported-operation recovery, and conformance projection stay together so discovery semantics cannot drift. | 1938 / 750 | Split search, inspect, and conformance adapters behind one catalog facade before adding discovery behavior. |
...
| `packages/agent/src/domains/capability/operations/operation_contract/output.rs` | canonical provider-output owner | Schema validation, semantic validation, text-only transport, and structural budgeting share one output boundary. | 809 / 750 | Split schema construction from rendering and budgeting before adding another envelope feature. |
| `packages/agent/src/domains/capability/operations/operation_contract/output/projection.rs` | canonical provider-evidence projection owner | Provider-safe profile allowlists and redaction stay behind one boundary so raw details cannot bypass custody. | 1750 / 750 | Split profile-specific projectors before adding another output profile. |
...
| `packages/agent/src/domains/capability/pool.rs` | capability pool classification owner | The complete operation surface, audience, replacement class, visibility, minimality, and evolution inventory remains one typed owner. | 1297 / 750 | Split generated rows from validation helpers before adding another classification dimension. |
...
| `packages/ios-app/Tests/Session/WorkerLifecycle/AgentCockpitStateTests.swift` | cockpit state regression owner | Refresh, empty, degraded, package action, and server-truth projection cases remain under one state owner. | 694 / 650 | Split refresh/projection tests from command-action tests before adding behavior. |
...
| TPC-1 | Retention inventory | 8 | passed_after_fix | architecture | The retention inventory classifies the current tracked source, docs, test, and script surface. Current coverage is 2,135 paths in TPC scope: 113 `primitive`, 878 `implementation`, 385 `support`, 609 `test`, 150 `docs`, and 0 `delete`. Checkpoint commit: `92521b511`. | No unclassified tracked source remains; explicit post-closeout budget rows name each over-target owner and its split-before-growth condition. | TPC-1 inventory checkpoint |
