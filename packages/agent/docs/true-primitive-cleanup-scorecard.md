...
| `packages/agent/src/engine/authority/grants/authorization.rs` | engine authority owner | Accepted Slice 24D adds exact memory query/decision selector enforcement to the shared capability authorization path while preserving accepted Slice 24C delegated subagent and module-program-execution exact runtime/job checks. | 2896 / 750 | Split operation/resource selector extraction and per-domain explicit-grant scanners into owner modules before adding more execute-resource families. |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant_tests.rs` | capability invocation test owner | Accepted Slice 24D keeps memory query/decision runtime-grant regressions with existing execute-resource and delegated subagent authority regressions. | 1136 / 800 | Split resource-family, memory, and delegated subagent grant fixtures into focused modules before adding more execute-resource families. |
| `packages/agent/src/domains/agent/loop/capability_invocation_executor/grant.rs` | capability runtime grant owner | Accepted Slice 24D derives exact memory query/decision selectors alongside accepted Slice 24C delegated subagent task selectors and delegated module refs in the shared provider execute grant derivation path. | 1327 / 750 | Split restored execute-resource family, memory, and delegated subagent grant derivation helpers into focused owner modules before adding more execute-resource families. |
