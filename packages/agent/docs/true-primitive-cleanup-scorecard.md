...
| `packages/agent/tests/baseline_pre_restoration_closure_invariants.rs` | restoration static gate owner | Phase 2 baseline closure gate now covers Slice 6B docs, provider exposure, resource evidence, and non-goal source-control guards. | 906 / 800 | Split baseline documentation inventory checks from provider/resource/source-control guards before adding more restoration slices. |
| `packages/agent/src/domains/git/service.rs` | git owner | Read-only Git evidence service restored for Slice 6A and reused by Slice 6B index mutation preflight/evidence. | 827 / 750 | Split path/repository discovery, status summaries, and diff/bounded-output helpers into owner modules before expanding source-control behavior. |
| `packages/agent/src/domains/git/tests.rs` | git test owner | Git status/diff and index mutation regression coverage restored across Phase 2 Slice 6A and Slice 6B. | 932 / 800 | Split read-only status/diff tests from index-mutation/resource/replay tests before adding more source-control coverage. |
...
| TPC-1 | Retention inventory | 8 | passed_after_fix | architecture | Added the retention inventory and TSV, then regenerated it after TPC-2 through TPC-11, TMB closeout, Phase 2 Slice 3 memory foundation, and Phase 2 Slice 6B Git index mutation foundation. Current coverage is 1,733 tracked and newly introduced source/docs/script paths in TPC scope: 109 `primitive`, 589 `implementation`, 385 `support`, 519 `test`, 131 `docs`, and 0 `delete`. Checkpoint commit: `92521b511`. | No unclassified tracked source remains. | TPC-1 inventory checkpoint |
