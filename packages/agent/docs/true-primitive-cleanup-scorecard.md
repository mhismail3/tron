...
| TPC-1 | Retention inventory | 8 | passed_after_fix | architecture | Added the retention inventory and TSV, then regenerated it after TPC-2 through TPC-11, TMB closeout, Phase 2 restoration slices, accepted Phase 3 module-state slices through Slice 23H, accepted Slice 24H pending-review manifest state, the iOS event-surface inventory split, and the Context Control implementation-candidate surface. Current coverage is 2,069 tracked and newly introduced source/docs/script paths in TPC scope: 112 `primitive`, 849 `implementation`, 385 `support`, 587 `test`, 136 `docs`, and 0 `delete`. Checkpoint commit: `92521b511`. | No unclassified tracked source remains; `grant.rs`, the Slice 23G `module_dependencies/service.rs` accepted row, the Slice 23H `AgentCockpitState.swift` accepted row, and the Context Control provider guidance test row carry budget rows until focused execute-resource, dependency-service, cockpit-state, and provider-test splits land. | TPC-1 inventory checkpoint |
