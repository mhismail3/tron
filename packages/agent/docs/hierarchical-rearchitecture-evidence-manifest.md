...
|----|--------|----------|--------------|------------|--------|
| HRA-0 | passed_after_fix | Created the scorecard, evidence manifest, human inventory, generated TSV inventory, generated ownership map, Rust invariant target, README living-doc links, and `scripts/tron.d/quality.sh` CI hook. The invariant target intentionally fails against the current tree on loose Rust root files, flat engine root modules, broad iOS source buckets, non-mirrored iOS test buckets, and over-budget files without decomposition rows. | Red output captured below. | Red gates are expected until HRA-2 through HRA-13 move the tree and HRA-1 records final budgets. | `f14f7b60c` |
...
| HRA-8 | passed_after_fix | Added HRA SourceGuard red hierarchy checks, generated the iOS source/test Swift ownership map, recorded the XcodeGen/share-extension project map, added HRA artifact inventory rows, and added a Rust map-coverage invariant. | XcodeGen exited 0 with no generated project drift; focused SourceGuard red proof compiled and failed only the two new HRA hierarchy tests; HRA invariant target now has the new iOS ownership-map guard passing and later HRA-13 turns all iOS map rows green. | HRA-14/HRA-15/HRA-16 remain. | `c21cdc6f8` |
...
| HRA-15 | passed_after_fix | Added a live docs/scripts/workflows stale-path static gate, then updated README onboarding links, iOS test-organization docs, Mac Rust path mirror docs, and the personal-info guard allowlist to current paths. Regenerated HRA and cleanup inventories for the new gate. | HRA-15 red proof failed on seven stale live references, then the focused HRA-15 gate passed after fixes. Final HRA static target passes with the new gate. | Closed by HRA-16 final adversarial review. | `363bcf110` |
| HRA-16 | passed_after_fix | Adversarial review returned blockers: HRA scorecard still pending, live retired DB paths (`log.db`, `engine-ledger.sqlite`, `tron.db`) in storage/scripts/Mac docs, stale iOS README source tree, Rust `events/tests.rs` and `events/tron.rs` same-name file/folder pairs, generic iOS reconstruction `Handlers`, and WebSocket reconnect tests under the client test bucket. Fixes removed old-name DB cleanup from startup, changed reset/benchmark/Mac lock paths to `tron.sqlite`, moved Rust event modules to folder `mod.rs`, renamed iOS event projection files under `ChatMessageProjection`, moved reconnect tests to `Tests/Engine/Transport/WebSocket`, updated live docs, regenerated Xcode projects, and refreshed inventories. Red proof also caught stale iPad sheet source paths in the selected iOS batch; the helper now resolves the package root correctly. | `scripts/tron ci fmt check clippy test` passed on the final tree: fmt, check, clippy, 2920 lib tests, 13 `db_path_guard`, 27 primitive teardown invariants, 16 cleanup invariants, 34 HRA invariants, 3 primitive trace tests, and 2 integration tests. Focused storage and protocol-event Rust slices passed before full CI. `xcodegen generate` passed for both iOS and Mac. iOS selected SourceGuard/error projection/WebSocket reconnect/iPad sheet batch passed after the red path-helper fix, xcresult `/Users/<USER>/Library/Developer/Xcode/DerivedData/TronMobile-eqctauwqsqxkqyelqqpembdspvdk/Logs/Test/Test-Tron-2026.06.08_04-55-34--0700.xcresult`; focused iOS capability/reconstruction batch passed 35 XCTest and 63 Swift Testing tests, xcresult `/Users/<USER>/Library/Developer/Xcode/DerivedData/TronMobile-eqctauwqsqxkqyelqqpembdspvdk/Logs/Test/Test-Tron-2026.06.08_05-00-21--0700.xcresult`. Full Mac tests passed 271 tests in 42 suites, xcresult `/Users/<USER>/Library/Developer/Xcode/DerivedData/TronMac-fjgdtjmmbndhtwfqnfyaagtxvdvs/Logs/Test/Test-TronMac-2026.06.08_04-56-22--0700.xcresult`; focused Mac Tailscale/path/status batch passed 37 tests in 3 suites, xcresult `/Users/<USER>/Library/Developer/Xcode/DerivedData/TronMac-fjgdtjmmbndhtwfqnfyaagtxvdvs/Logs/Test/Test-TronMac-2026.06.08_05-00-11--0700.xcresult`. `git diff --check` passed; tracked generated-junk scan returned no matches; retired-word and old-hierarchy-path scans now match only absence tests plus scorecard/evidence/inventory records that name banned terms as regression needles or history, not implementation, README, scripts, or workflows. | No HRA implementation loops remain. Static absence tests and historical evidence retain old strings only as regression needles. | `d63e8646a` |
...
- First `cargo fmt --all -- --check`: failed only because the new Rust
  ownership-map guard needed rustfmt wrapping; `cargo fmt --all` was run.
- HRA invariant target before final doc staging: expected partial red with 23
  passed and 2 failed. The new
  `ios_hra8_ownership_map_covers_every_source_and_test_swift_file` guard
  passed.
...
- HRA invariant target: 29 passed, 0 failed. The iOS test mirror,
  iOS ownership-map coverage, and large-file budget gates are now green.
...
Checkpoint commit: `69f73bfa8`.

## HRA-14 Mac Wrapper Hierarchy Verification
...
- Added three red static gates for the Mac wrapper: production source owner
  roots, mirrored test roots, and Mac test files over the HRA line budget.
- First HRA rerun: exit 101, expected red proof. It ran 32 tests with 29
  passed and 3 failed:
...
- Moved Mac production sources into the target owner folders: App lifecycle,
  command mode, and composition; Server launch-agent, health, paths, pairing
  token, and process control; MenuBar actions, controller, and presentation;
  Wizard flow, steps, and components; Support diagnostics, feedback,
  foundation, onboarding, pairing, and theme.
- Split mixed helpers so pairing-token reading, settings-profile TOML caching,
  and onboarded sentinel writes have separate owner files.
- Moved Mac tests to mirror App, Server, MenuBar, Support, Wizard, and
  Infrastructure fake owners. Split the old 717-line
  `WizardStepTests.swift` into ordering, preferred-height, install-stage, and
  visual-layout suites.
- `xcodegen generate`: passed.
- First Mac test rerun: exit 65 red proof. It compiled and ran the Mac test
  target, then failed 21 split wizard tests because their source-reading helper
  still assumed the old `Tests/Wizard` depth and resolved paths under
  `packages/mac-app/Tests/Sources`. The helper now discovers the package root by
  walking upward to `project.yml`.
- Mac test rerun: exit 0, `** TEST SUCCEEDED **`; 271 tests in 42 Swift Testing
  suites passed.
- Mac test xcresult:
  `/Users/<USER>/Library/Developer/Xcode/DerivedData/TronMac-fjgdtjmmbndhtwfqnfyaagtxvdvs/Logs/Test/Test-TronMac-2026.06.08_04-12-43--0700.xcresult`
- Primitive cleanup reruns exposed direct retired-product labels in split iOS
  SourceGuard descriptions and path literals. The guard terms remain active
  through concatenated fragments, while live labels and function names are now
  neutral.
- Focused iOS SourceGuard rerun: exit 0, `** TEST SUCCEEDED **`; 34 tests in
  the Source Guards suite passed.
- SourceGuard xcresult:
  `/Users/<USER>/Library/Developer/Xcode/DerivedData/TronMobile-eqctauwqsqxkqyelqqpembdspvdk/Logs/Test/Test-Tron-2026.06.08_04-21-56--0700.xcresult`
- primitive code cleanup invariants: 16 passed.
- Final HRA static target after docs and inventory regeneration: 32 passed, 0
  failed.
- `cargo fmt --manifest-path packages/agent/Cargo.toml --all -- --check`:
  passed.
- `git diff --check`: passed.

Open loops after HRA-14:
