/**
 * @fileoverview Tests for OpenAI Codex Provider
 *
 * Comprehensive tests for the OpenAI Codex provider implementation including:
...
function createMockAuth(): CodexOAuth {
...
describe('OpenAI Codex Provider', () => {
  ...
  describe('Configuration', () => {
    it('should accept valid configuration', () => {
      ...
      const provider = new OpenAICodexProvider(config);
  ...
  describe('Model Registry', () => {
    it('should define GPT-5.2 Codex model', () => {
      expect(OPENAI_CODEX_MODELS['gpt-5.2-codex']).toBeDefined();
      expect(OPENAI_CODEX_MODELS['gpt-5.2-codex'].contextWindow).toBe(192000);
    });

    it('should define GPT-5.1 Codex Max model', () => {
      expect(OPENAI_CODEX_MODELS['gpt-5.1-codex-max']).toBeDefined();
      expect(OPENAI_CODEX_MODELS['gpt-5.1-codex-max'].supportsReasoning).toBe(true);
    });

    it('should define GPT-5.1 Codex Mini model', () => {
      expect(OPENAI_CODEX_MODELS['gpt-5.1-codex-mini']).toBeDefined();
    });

    it('should indicate reasoning support for all models', () => {
      for (const [modelId, model] of Object.entries(OPENAI_CODEX_MODELS)) {
