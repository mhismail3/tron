#!/usr/bin/env python3
"""Tron server benchmark — measures /engine protocol latencies.

Boots a fresh server with a temp database, connects via WebSocket,
and measures round-trip times for key canonical engine functions.

Scenarios:
  ping             system::ping round-trip (/engine + dispatch)
...
def read_bearer_token(home_dir=None):
    """Read the /engine bearer token generated by the server."""
    env_token = os.environ.get("TRON_BEARER_TOKEN")
    if env_token:
        return env_token
    home = home_dir or os.path.expanduser("~")
    auth_path = os.path.join(home, ".tron", "profiles", "auth.json")
    with open(auth_path) as f:
        data = json.load(f)
    token = data.get("bearerToken")
    if not isinstance(token, str) or not token:
        raise RuntimeError(f"missing bearerToken in {auth_path}")
    return token
...
async def ws_connect(port, home_dir=None):
    ...
    await ws.send(json.dumps({"type": "hello", "id": "hello", "protocolVersion": 1}))
    msg = await asyncio.wait_for(ws.recv(), timeout=5)
    data = json.loads(msg)
    assert data.get("type") == "hello.ok", f"unexpected: {data}"
    return ws


async def engine_invoke(ws, call_id, function_id, payload=None):
    """Invoke a canonical engine function and wait for the matching response."""
    req = {
        "type": "invoke",
        "id": f"r{call_id}",
        "functionId": function_id,
        "payload": payload or {},
        "idempotencyKey": f"bench-{call_id}-{function_id}",
    }
    ...
    while True:
        ...
        if data.get("id") == f"r{call_id}":
            return unwrap_engine_response(data)
...
def unwrap_engine_response(resp):
    ...
    if isinstance(child, dict):
        ...
        return {"success": True, "result": child.get("value")}
    return {"success": resp.get("ok") is True, "result": resp.get("result")}
...
async def scenario_ping(ws, iterations):
    ...
    for i in range(iterations):
        ...
        resp = await engine_invoke(ws, i, "system::ping", {"protocolVersion": 1})
...
async def scenario_session_create(ws, iterations):
    ...
    for i in range(iterations):
        ...
        # Clean up
        await engine_invoke(ws, 20000 + i, "session::delete", {"sessionId": session_id})
        latencies.append(elapsed_ms)
    return {
        "name": "session_create",
...
async def scenario_session_list(ws, iterations):
    ...
    for i in range(iterations):
        start = time.perf_counter()
        resp = await engine_invoke(ws, 30000 + i, "session::list")
        elapsed_ms = (time.perf_counter() - start) * 1000
...
async def run_benchmarks(port, scenario_names, iterations, home_dir=None):
    ws = await ws_connect(port, home_dir)
    results = []
