...
| `git_status` | Inspect trusted-root repository state: branch or detached HEAD, upstream/ahead-behind, dirty summaries, bounded porcelain evidence, and explicit `indexTreeTruncated` / `indexTreeOidUnavailable` evidence when the read-only staged-index hash is too large to compute; the model-facing result text and replay projection summarize branch, clean/dirty state, staged/unstaged/untracked/conflicted counts, porcelain emptiness, resource-ref count, and truncation so the agent does not need to infer the Git state from terse status text. |
