...
| `capability` | `capability.invocation.started`, `capability.invocation.completed`, `capability.invocation.progress` |
...
`capability.invocation.completed` are immutable capability lifecycle labels.
Active runtime/UI identity is capability-native: payloads carry `modelToolName`, `contractId`,
...
| `events` | Immutable append-only event log. Denormalized columns (`role`, `model_tool_name`, `invocation_id`, `turn`, token counts, `model`, `latency_ms`, `stop_reason`, `provider_type`, `cost`, ...) extracted from payloads for indexed queries |
...
- **Capability-native chat UI**: active work is rendered as `capabilityInvocation` / `capabilityResult` content from capability identity and schema/result metadata. Retired tool descriptors, old built-in names, and plugin source-specific tool sheets are not active UI routes.
