...
| `trace_list` | List durable Agent Trace-style records for the trusted current session as the default provider-safe trace proof path, optionally filtered by trace id; provider-visible projections include an explicit `traceRecordId` distinct from `traceId`, safe engine invocation refs, parent engine invocation refs, request/result hashes, completed-status summary, in-progress count, status/timing, safe errors, projection-boundary guidance, and per-record redaction proof. Invalid or unsupported execute attempts are recorded as failed traces before returning. When the provider byte budget is reached, the newest bounded record subset remains present with exact omission proof instead of dropping the entire record collection. The current `trace_list` call is pending at projection time and can appear in progress until it completes. Raw provider invocation ids, raw authority/grant ids, idempotency keys, process ids, working directories, raw requests/results, prompts, logs, commands, code, paths, file contents, and VCS payloads remain excluded. |
| `trace_get` | Read one durable trace record by exact `traceRecordId` within the trusted current session only when focused per-record detail is needed after `trace_list`; it uses the same provider-safe projection as `trace_list`, preserving separately named trace-record, trace, and invocation refs plus hashes without exposing raw provider call metadata or execution payloads. |
