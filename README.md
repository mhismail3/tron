...
envelope for enabled modules. It records `module_runtime_state` resources with
...
accepted Phase 3 module-plane additions include `module_registry`,
`module_authoring`, `module_validation`, `module_install`, `module_lifecycle`,
and `module_runtime`; they remain provider-visible only as operation values
