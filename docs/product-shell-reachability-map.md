...
| Prompt Library sheets/state | Generated `artifact:prompt-snippet:*` and `artifact:prompt-history:*` collection management surfaces | Composer insertion is a local editing affordance, not a safe server-authored action result consumed by the chat input | Low for management, medium for full sheet deletion: management is generated, but deleting the sheet would remove fast composer insertion | Decide whether composer insertion remains intentionally local or introduce a narrow generated-result-to-composer bridge | partially converted; management converted to generated UI, composer insertion retained |
...
|---|---|---|
| `prompt_library` | History and snippets are `artifact:prompt-history:*` and `artifact:prompt-snippet:*` resources. Fresh modular-engine-v3 databases do not create retired prompt-library tables. Prompt management now uses generated `ui_surface` resource-collection surfaces; the fixed sheet remains only for local composer insertion. | Partially converted; no fixed create/edit/delete/clear management path remains. |
