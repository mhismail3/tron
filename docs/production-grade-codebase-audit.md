...
remaining fixed iOS shell. Prompt Library is complete with a gated local
composer insertion boundary: snippet/history create, update, delete, and clear
management is server-authored generated UI over `artifact:prompt-*` resources,
while the fixed sheet is selection-only local draft editing through
`onSelect(text)`. The current phase decision remains `defer with proof` for
AgentControl, SourceChanges, subagent sheets/plugins, notification inbox/detail
views, display stream views, and voice recording affordances because each has
an active entrypoint or a missing generated/resource replacement.
...
|---:|---|---|
| 1 | Replace one remaining fixed iOS shell when generated/resource coverage exists | Product-shell replacement readiness map proves the replacement; old view/navigation/DTO/tests removed with absence gate |
...
| Completed | Prompt Library composer insertion boundary | Fixed sheet is documented and gated as selection-only local draft editing; it owns no management, durable truth, policy, grants, lineage, target functions, or payload templates |
