...
- **Started:** 2026-09-23
- **Status:** Active
- **Last updated:** 2026-09-25, T-GW-SESS-1
...
| C-COMMENTS-REFCHECK-1 | Claimed | Extend `scripts/check-documentation-policy.py` to check backticked repository paths inside code comments, the only check that would have caught the stale palette reference (about 30 lines). Adds a CI rule, so the user decides. Approved by the user on 2026-09-25 | S-COMMENTS-1 | simplification session, 2026-09-25 |
...
| S-GW-TRANS-1 | Claimed | Scope GW-TRANS (server, gateway service, receipts, transcript leases, logger, diagnostic export); keep `packages/gateway/docs/observability.md` accurate | V-0 | simplification session, 2026-09-25 |
...
| C-GW-KNOW-3 | Claimed | One credential-query-key predicate for the four copies (`x-public-post.ts`, `source-capture.ts`, `legacy-import.ts`, `knowledge-contract.ts`; source capture keeps its X-embed exception) and one normalized-URL helper for `source-capture.ts` and `knowledge-store.ts` | S-GW-KNOW-1 | simplification session, 2026-09-25 |
| C-GW-KNOW-4 | Claimed | Move the test-only `InMemoryConnectorCredentialStore` out of `connector-credentials.ts` into test support (7 test files across knowledge, sessions and integrations) | S-GW-KNOW-1 | simplification session, 2026-09-25 |
...
| S-GW-AUTO-1 | Claimed | Scope GW-AUTO | V-0 | simplification session, 2026-09-25 |
| S-GW-EXT-1 | Claimed | Scope GW-EXT | V-0 | simplification session, 2026-09-25 |
