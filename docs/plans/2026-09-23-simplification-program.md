...
| C-DEPS-1B | Ready | Land with the next signed Mac app build, never in a source-only rebuild: remove the now-unused direct `typebox` dependency and move `@types/node` to 22.x in `packages/gateway/package.json` and its lockfile. The source already imports `Type`/`Static` from `@earendil-works/pi-ai`. A lockfile change blocks "Rebuild from Source" until a newly signed app is installed (`captureReusableSourcePackage`), which is why this was reverted on 2026-09-25 | C-DEPS-1 | |
