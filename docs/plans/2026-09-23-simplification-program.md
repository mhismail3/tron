...
| C-KNOW-ORIGIN-1 | Ready | With the legacy importer gone, nothing produces `importOrigin`/`KnowledgeImportOrigin`; decide whether note update and correction still need to carry it (persisted lineage) or it can be removed from the contract, the service and the iOS models and dashboard | C-KNOW-UI-1 | |
