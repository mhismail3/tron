...
| C-COMMENTS-1 | Claimed | Fix the four verified misleading comments: the Mac `TronColors.swift` header names a nonexistent iOS file and claims hex values match iOS when they differ; `PairingURLBuilder.swift` and its test name a nonexistent `PairingURLParser` (the consumer is `PairingInvitationParser`); `MenuBarItemBuilder.swift` cites a nonexistent "plan §A"; `OnboardingModels.swift` narrates removed behavior. Also the two weak Gateway and scripts cases in the S-COMMENTS-1 findings | S-COMMENTS-1 | simplification session, 2026-09-24 |
