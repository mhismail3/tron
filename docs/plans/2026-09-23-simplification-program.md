...
| C-GW-BATCH-3 | Claimed | No-approval Gateway cleanups from S-DEAD-2, S-GW-SESS-PARTS-1 and S-GW-KNOW-2/S-GW-PROTO-1: delete the 11 dead declarations and `knowledge/index.ts` (C-DEAD-2), the dead session surface (C-GW-SESS-PARTS-1), unused protocol types (C-GW-PROTO-1), the knowledge connector/Jev items (C-GW-KNOW-5/6), and put the real-time connector retry test on the injected sleep (T-GW-KNOW-2) | S-DEAD-2 | simplification session, 2026-09-25 |
| C-GW-SESS-PARTS-2 | Claimed | Session-part refactors: one bounded UTF-8 text helper, one recency engine for extension and process activity, one process row builder, one semantic-vector admission helper, the test-only seams in blob store, lifecycle coordinator and resource invocation, and the test-policy fixes (C-GW-SESS-PARTS-2 to 5, T-GW-SESS-PARTS-1) | S-GW-SESS-PARTS-1 | simplification session, 2026-09-25 |
| C-RELAY-1 | Claimed | Push relay dead surface and duplication (C-RELAY-1, T-RELAY-1, C-RELAY-DOCS-1 in the S-RELAY-1 output) | S-RELAY-1 | simplification session, 2026-09-25 |
| C-SCRIPTS-MAC-1 | Claimed | Scripts and Mac payload hygiene: delete the unused `asc` toolchain path, wire the wizard migration test into CI, the deploy-test and reinstall-flag fixes (S-SCRIPTS-1), the launcher's missing package-file maxima and fail-fast checks, one owner for Node runtime pins, one Mach-O arch check, native-capture constants and the script test fixes (S-MAC-NATIVE) | S-SCRIPTS-1 | simplification session, 2026-09-25 |
| C-IOS-DEAD-1 | Claimed | Delete the unreferenced iOS Swift declarations S-DEAD-2 found, except `GatewayDiagnosticsView` (needs approval) | S-DEAD-2 | simplification session, 2026-09-25 |
| C-DEPS-1 | Ready | Import `Type`/`Static` from `@earendil-works/pi-ai` instead of a second `typebox` copy, and align `@types/node` with Node 22; needs a lockfile update, done by the supervisor | S-DEPS-1 | |
