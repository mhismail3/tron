...
| --- | --- | --- | --- | --- |
| V-0 | Ready | Baseline: run and time every focused and full suite (Gateway vitest, scripts `node --test` with the pinned Node, iOS `scripts/tron-ios-test`, Mac `xcodebuild test`); record pass state and wall times in a handoff entry for later comparison | none | |
...
| S-DOCS-1 | Ready | Docs: one owner per fact per `AGENTS.md`; flag docs that describe finished cutovers or duplicate another doc | none | |
