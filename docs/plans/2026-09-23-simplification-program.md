...
- **Started:** 2026-09-23
- **Status:** Active
- **Last updated:** 2026-09-25, C-KNOW-ORIGIN-1, C-STRUCT-RULES-1
...
| S-DEAD-2 | Done | Finish the dead-code sweep S-DEAD-1 did not cover: repo-wide unused TS exports (not only file-local unused symbols), unused Swift declarations in iOS and Mac, and unreferenced scripts, assets, docs and fixtures, each hand-verified against registration, RPC routing, generated projects and CI | S-DEAD-1 | simplification session, 2026-09-25 |
| S-DEPS-1 | Done | Dependency audit: every Gateway, relay, iOS and Mac dependency, what uses it, and whether platform API covers it. Output C-DEPS rows | V-0 | simplification session, 2026-09-25 |
...
| C-DOCS-PIN-1 | Done | Correct stale Pi pins: the Gateway README (two places), agent-home-reference-inventory.md, cutover-runbook.md and agent-home-cutover.md still say Pi 0.84.4 and `pi-subagents` 0.59.0; the pin is 0.87.1 and `pi-subagents` is runtime-installed, not a repository dependency | S-DOCS-1 | simplification session, 2026-09-24 |
...
| C-STRUCT-RULES-1 | Done | Make the structure self-maintaining with the least mechanism: (1) a short `AGENTS.md` rule that whoever adds, moves or removes a file updates its references, owning doc and ownership notes in the same change and commits no temporary files; (2) a two-line "owns / does not own" note in each package's existing README or doc where missing; (3) only if S-STRUCT-1 finds recurring leftovers, one fast CI check for the patterns actually found | S-STRUCT-1 | simplification session, 2026-09-25 |
| S-GW-ROOT-1 | Done | For each migration tool and cutover doc, determine whether it is complete on every supported install. Removal needs the user's approval; record the evidence and the question | V-0 | simplification session, 2026-09-25 |
...
| S-GW-SESS-PARTS-1 | Done | Scope the smaller GW-SESS sub-units in one pass: projection (`projection.ts`), catalog (`catalog-*`, `history.ts`, `summary-text.ts`), search (`session-search-*`, distinct from the performance row S-GW-SESS-SEARCH-1), blobs and export, process activity, extension activity (`extension-*`, `semantic-ui-broker.ts`, `hook-projection.ts`), receipts (`invocation-receipts.ts`, `resource-invocation.ts`, `context-delivery-receipts.ts`) and ownership (`run-markers.ts`, `delegated-provider.ts`, `gateway-work-registry.ts`, `fork-boundary.ts`, `restart-drain.ts`, `agent-runtime-lock.ts`). `delegated-root-migration.ts` stays with S-GW-ROOT-1 | S-GW-SESS-1 | simplification session, 2026-09-25 |
...
| S-GW-KNOW-2 | Done | Scope the knowledge provider sub-unit (`connectors.ts`, the Jev client, assessment and extension, `fixed-host-transport.ts`) and pass redaction and bounds overlaps to S-XMOD-1 | S-GW-KNOW-1 | simplification session, 2026-09-25 |
...
| T-GW-SMALL-2 | Superseded | Notifications test audit: classify each spy/call-count assertion as the requirement or setup; rewrite only setup | S-GW-SMALL-1 | fc9740856 test pruning |
| T-GW-SMALL-3 | Superseded | Providers and protocol test audit; decide whether `pi-image-serialization.test.ts` belongs beside `check-pi-sdk.mjs` | S-GW-SMALL-1 | fc9740856 test pruning |
| T-GW-SMALL-4 | Superseded | Integrations and client test audit | S-GW-SMALL-1 | fc9740856 test pruning |
| T-GW-SMALL-5 | Superseded | Workspace and runtime test audit | S-GW-SMALL-1 | fc9740856 test pruning |
| S-GW-PROTO-1 | Done | Scope `protocol/types.ts` against its Swift and TS consumers for unused types and fields | S-GW-SMALL-1 | simplification session, 2026-09-25 |
...
| C-MAC-RPC-1 | Done | One decoder for the `{type,id,ok,result,error}` envelope shared by `MenuBarLogReader.swift`, `ServerPing.swift` and `GatewayRestartClient.swift`; each client keeps its own errors and bounds | T-MAC-1 | simplification session, 2026-09-25 |
...
| S-MAC-2 | Done | Scope `Sources/NativeHost` and `Sources/Search` together with MAC-NATIVE | S-MAC-1 | simplification session, 2026-09-25 |
| S-MAC-3 | Done | Scope the payload and launcher contract across `GatewayPayloadStore.swift`, `tron-gateway-launcher.c` and `hash-gateway-payload.sh`, so the fail-closed policy is stated once | S-MAC-1 | simplification session, 2026-09-25 |
| S-MAC-NATIVE-1 | Done | Scope MAC-NATIVE, including the C launcher | V-0 | simplification session, 2026-09-25 |
| S-RELAY-1 | Done | Scope RELAY | V-0 | simplification session, 2026-09-25 |
| S-SCRIPTS-1 | Done | Scope SCRIPTS: which scripts a maintainer or CI actually runs; the Python reinstall tools and their tests; the deploy helper excluding progress reporting | V-0 | simplification session, 2026-09-25 |
...
| C-IOS-INSTALL-MSG-1 | Done | User-requested copy change (approved 2026-09-25 as an exception to the no-UX-change rule): `IosDeviceInstallService.install` (`packages/gateway/src/admin/ios-device-install-service.ts`) rejects every unusable bound target with one long message, which the iOS toast truncates and which does not say which check failed. Return one short message per cause, naming the device: not visible to the Mac, visible but not connected ("Moose's iPhone isn't reachable from the Mac: connect it by cable or join the Mac's Wi-Fi, then unlock it"), or Developer Mode off. Keep the `not_found` code and retryable flag, add a test per cause, and check that the iOS toast shows each message in full | C-GW-ADMIN-4 | simplification session, 2026-09-25 |
| C-GW-BATCH-3 | Done | No-approval Gateway cleanups from S-DEAD-2, S-GW-SESS-PARTS-1 and S-GW-KNOW-2/S-GW-PROTO-1: delete the 11 dead declarations and `knowledge/index.ts` (C-DEAD-2), the dead session surface (C-GW-SESS-PARTS-1), unused protocol types (C-GW-PROTO-1), the knowledge connector/Jev items (C-GW-KNOW-5/6), and put the real-time connector retry test on the injected sleep (T-GW-KNOW-2) | S-DEAD-2 | simplification session, 2026-09-25 |
| C-GW-SESS-PARTS-2 | Done | Session-part refactors: one bounded UTF-8 text helper, one recency engine for extension and process activity, one process row builder, one semantic-vector admission helper, the test-only seams in blob store, lifecycle coordinator and resource invocation, and the test-policy fixes (C-GW-SESS-PARTS-2 to 5, T-GW-SESS-PARTS-1) | S-GW-SESS-PARTS-1 | simplification session, 2026-09-25 |
| C-RELAY-1 | Done | Push relay dead surface and duplication (C-RELAY-1, T-RELAY-1, C-RELAY-DOCS-1 in the S-RELAY-1 output) | S-RELAY-1 | simplification session, 2026-09-25 |
| C-SCRIPTS-MAC-1 | Done | Scripts and Mac payload hygiene: the deploy-test and reinstall-flag fixes (S-SCRIPTS-1), the launcher's missing package-file maxima and fail-fast checks, one owner for Node runtime pins, one Mach-O arch check, native-capture constants and the script test fixes (S-MAC-NATIVE) | S-SCRIPTS-1 | simplification session, 2026-09-25 |
| C-IOS-DEAD-1 | Done | Delete the unreferenced iOS Swift declarations S-DEAD-2 found, except `GatewayDiagnosticsView` (needs approval) | S-DEAD-2 | simplification session, 2026-09-25 |
| C-DEPS-1 | Done | Import `Type`/`Static` from `@earendil-works/pi-ai` instead of a second `typebox` copy, and align `@types/node` with Node 22; needs a lockfile update, done by the supervisor | S-DEPS-1 | |
| S-GW-SESS-PROJ-1 | Ready | Scope `projection.ts` in four passes by line range, and `extension-run-projection.ts` with `semantic-ui-broker.ts` | S-GW-SESS-PARTS-1 | |
| S-MAC-NATIVE-2 | Ready | Scope the native capture addon against the Gateway transport and client (bounds, error codes, identity fields) | S-MAC-NATIVE-1 | |
| C-GW-ROOT-CUTOVER-1 | Done | Approved by the user on 2026-09-25: delete the finished one-time cutover tools and runbooks (agent-home migration and preflight, `tron_agent_home_cutover.py`, internal-layout migration, the mutating half of the delegated-root migration keeping its startup gate, connection migration, wizard migration) with their `scripts/tron` subcommands, tests and docs, keeping the startup refusals in `config.ts` and `gateway-main.ts`. Rows C-GW-ROOT-AGENTHOME-1/2, STATE-1/2, DELEGATED-1, WIZARD-1 and DOCS-1 in the S-GW-ROOT-1 output | S-GW-ROOT-1 | simplification session, 2026-09-25 |
| C-GW-ROOT-FORMAT-1 | Done | Approved by the user on 2026-09-25: drop the upload metadata v1, pre-inbox notification and device `lastSeenAt` acceptances and their tests. On this Mac 0 of 860 upload records are v1, the notification document has an inbox and no device carries `lastSeenAt` | S-GW-ROOT-1 | simplification session, 2026-09-25 |
| C-APPROVED-SMALL-1 | Done | Approved by the user on 2026-09-25: document `scripts/install-hooks.sh` and `scripts/tron-version sync|bump` in CONTRIBUTING; drop node-pty's win32 prebuilds during payload staging (C-DEPS-3); delete `GatewayDiagnosticsView`, the 7 unused provider icon imagesets, the unread APNs `category` field (C-RELAY-2) and the two unused drain-status fields (C-GW-PROTO-2) | S-DEAD-2 | simplification session, 2026-09-25 |
| C-MAC-PAYLOAD-IDENTITY-1 | Done | Approved by the user on 2026-09-25: one set of payload identity rules and manifest keys across `GatewayPayloadStore.swift`, the C launcher and `gateway-payload-deploy.mjs`, documented once (S-MAC-NATIVE output) | S-MAC-NATIVE-1 | simplification session, 2026-09-25 |
| C-KNOW-UI-1 | Done | User decision 2026-09-25. Connectors stay agent-driven with no iOS screen: delete the iOS client code left for the removed connector and import screens (five `KnowledgeRPCClient` methods, `KnowledgeImportPresentationPolicy` and its test cases, and any Swift models only they use), keeping only what the app still calls. Delete the legacy importer entirely: `knowledge/legacy-import.ts` and its test, the `KnowledgeExtensionSeam.importer` registration, the `knowledge.import.dry-run`/`knowledge.import.run` RPCs and their protocol types, the named-root configuration (`TRON_PERSONAL_OS_ROOT`, `TRON_LLM_WIKI_ROOT`) in `gateway-main.ts`, the `imports` catalog collection if nothing else writes it, and the importer docs. One-time imports are done by agents, never app code. Keep the connector RPCs the agent tool uses | C-IOS-DEAD-1 | simplification session, 2026-09-25 |
| C-KNOW-ORIGIN-1 | Done | With the legacy importer gone, nothing produces `importOrigin`/`KnowledgeImportOrigin`; decide whether note update and correction still need to carry it (persisted lineage) or it can be removed from the contract, the service and the iOS models and dashboard | C-KNOW-UI-1 | simplification session, 2026-09-25 |
| C-DEPS-1B | Done | Land with the next signed Mac app build, never in a source-only rebuild: remove the now-unused direct `typebox` dependency and move `@types/node` to 22.x in `packages/gateway/package.json` and its lockfile. The source already imports `Type`/`Static` from `@earendil-works/pi-ai`. A lockfile change blocks "Rebuild from Source" until a newly signed app is installed (`captureReusableSourcePackage`), which is why this was reverted on 2026-09-25 | C-DEPS-1 | simplification session, 2026-09-25 |
...
### T-GW-SMALL-2 to T-GW-SMALL-5 · Superseded · 2026-09-25 · simplification session

- Commit `fc9740856` (another session) audited every non-E2E test file and pruned about 430 cases under the new `AGENTS.md` testing policy (E2E-first; isolated tests only for written failure modes the E2E oracles miss), including the notifications, providers, protocol, integrations, client, workspace and runtime tests these rows covered. Test audits from here follow that policy, which supersedes this plan's test bar where they conflict (for example, T-GW-SESS-2 should delete private-seam tests the integration suite already covers rather than rewrite them).

### C-IOS-INSTALL-MSG-1, C-MAC-RPC-1 and eight scopings · Done · 2026-09-25 · simplification session (DeepSeek v4.1 Flash lanes, reviewed by the supervisor)

- C-IOS-INSTALL-MSG-1: install admission names the one failed check: "<name> isn't visible to this Mac. Pair it in Xcode.", "<name> isn't reachable. Plug it in or join the Mac's Wi-Fi." or "Turn on Developer Mode on <name>." (`not_found`, retryable). The lane measured the iOS toast (three lines of a 168 pt title) and found the first proposed copy truncated; the final copy fits in at most three lines in all four app fonts. Written test-first.
- C-MAC-RPC-1: `GatewayResponseDecoder` is the one reader of the response envelope for the ping, restart/update/command-status and log clients; each keeps its own errors and text. Existing fixtures classify as before; envelopes the Gateway never sends (`ok` with an error, a failure with a result) are now `malformed` rather than `error`, which the ping path treats the same. `undeclaredResultFieldsAreTolerated` restores the unknown-field assertion `fc9740856` removed. Mac suite 285 tests.
- Scopings (outputs retained with the run): dead code, dependencies, relay, scripts, migrations, session parts, knowledge providers and protocol, and Mac native are mostly load-bearing. No-approval findings became C-GW-BATCH-3, C-GW-SESS-PARTS-2, C-RELAY-1, C-SCRIPTS-MAC-1, C-IOS-DEAD-1 and C-DEPS-1. Items needing the user were asked in chat.
- Migration evidence checked by the supervisor on this Mac: 0 of 860 upload metadata files are v1, the notifications document has an inbox, no device entry carries `lastSeenAt`, the knowledge catalog control holds zero legacy connectors, `~/.tron-machine-group-id` is absent and `~/.pi/agent` holds only `auth.json`.
- Evidence (verified): full Gateway suite on combined `main` 178 files, 1,921 tests; documentation policy exit 0.

### C-GW-BATCH-3, C-GW-SESS-PARTS-2, C-RELAY-1, C-IOS-DEAD-1, C-DEPS-1, C-SCRIPTS-DEAD-1 · Done · 2026-09-25 · simplification session (DeepSeek v4.1 Flash lanes, reviewed by the supervisor)

- C-GW-BATCH-3: 11 dead declarations and `knowledge/index.ts`, 69 own-file-only exports (two kept: `HistoryPage` types are named by an exported signature), 4 unreferenced `SessionSearch*` protocol types, connector read-path cancellable sleep and single-use factory, one Jev model spelling, and the cooldown test on the fake clock (`connectors.test.ts` 9.7 s → 5.7 s). False claims skipped: `SessionSearchIndex.open` options are used by a test; the two connector health expressions differ.
- C-GW-SESS-PARTS-2: `util/bounded-text.ts` owns the whole-code-point UTF-8 cut for projection and summaries (process-activity and the admin helpers keep their own cut: 31–55k fuzz divergences, so not the same job); `sessions/recency-deadlines.ts` is the one clock/deadline/timer owner for extension and process recency with per-kind semantics unchanged; one subagent lifecycle builder, history page assembler and semantic-vector admission; `BlobStore.get()` and two unset parameters gone; four tests assert outcomes instead of internals. `maximumReaders` stays (production sets it).
- C-RELAY-1: nine relay exports removed, `Env` is `Cloudflare.Env`, the DO routes by the shared path constants, one bounded stream reader for request and APNs response (new bound test), two tests assert outcomes (pinned root digest, persisted body hash), README states current scope. Relay 39 tests.
- C-IOS-DEAD-1: 33 unreferenced iOS declarations and `SessionShellPreviewSupport.swift` deleted (−597 lines), iOS docs corrected in the same change; iOS unit suite 1,715 passed, 0 failed. Found C-KNOW-UI-1.
- C-DEPS-1 (supervisor): tool schemas import `Type`/`Static` from `@earendil-works/pi-ai`, so the stray `typebox@1.3.7` is gone and schemas match the SDK's 1.3.27; `@types/node` is 22.20.4 to match Node 22.22.0. The main checkout's dependencies were reinstalled from the new lockfile.
- C-SCRIPTS-DEAD-1: the uninstalled `asc` toolchain path and its seven pins are gone. The C-SCRIPTS-MAC-1 lane timed out after this commit and left an uncommitted negative-control edit in `gateway-payload-deploy.mjs`, which the supervisor discarded unmerged; its other items return to Ready. Its wizard-migration CI wiring was not merged, because C-GW-ROOT-CUTOVER-1 deletes that tool.
- Supervisor merge work: two conflicts between the export-drop and refactor lanes, resolved by keeping the refactor and re-applying the export drops.
- Evidence (verified): on combined `main`, Gateway `tsc` (including declaration emit), `check-pi-sdk` coherent, 178 files / 1,921 tests; relay check and 39 tests; documentation policy exit 0.

### C-GW-ROOT-CUTOVER-1, C-GW-ROOT-FORMAT-1, C-APPROVED-SMALL-1, C-MAC-PAYLOAD-IDENTITY-1, C-SCRIPTS-MAC-1 · Done · 2026-09-25 · simplification session (DeepSeek v4.1 Flash lanes, reviewed by the supervisor)

- C-GW-ROOT-CUTOVER-1: the agent-home migration and preflight, `tron_agent_home_cutover.py`, the internal-layout and connection migrations, the wizard migration and the mutating half of the delegated-root migration are deleted with their `scripts/tron` subcommands, tests, CI steps and runbooks (about 5,400 lines). The startup refusals stay: `config.ts` still refuses a legacy machine-group file, and `assertDelegatedRootCutoverReady` still refuses a legacy provider root with work (negative control: disabling the gate fails exactly its two refusal tests).
- C-GW-ROOT-FORMAT-1: upload metadata v1, pre-inbox notification and device `lastSeenAt` acceptance removed; `UploadMetadata` has one shape and `repairMetadata` keeps the crash-repair half, now witnessed by "repairs a lost content-addressed object from its logical file"; the notification `inbox` is required.
- C-APPROVED-SMALL-1: `install-hooks.sh` and `tron version sync|bump` documented in CONTRIBUTING; `GatewayDiagnosticsView` and 7 unused provider icons deleted; the unread APNs `category` removed with the payload asserted through `sendToApns`; four unread drain-snapshot timing fields removed (the approval said two; the other two fed a reduce only those fields used, and no iOS or Mac decoder reads any of them).
- C-MAC-PAYLOAD-IDENTITY-1 and C-SCRIPTS-MAC-1 (with C-DEPS-3): one payload identity policy across the Swift validator, C launcher, deploy helper and bundler, tabled in the Mac architecture doc, each rule at least as strict as the strictest side before; the launcher bounds the package documents; the verifier refuses any writable payload entry (the scoping's `-perm +022` advice was too weak; fixed); one owner for the Node runtime digests; one Mach-O architecture check; node-pty win32 prebuilds pruned before fingerprinting (about 58 MB). The lane broke its commit boundaries with an amend, so the supervisor squashed it into `528484184`, leaving out its guard for the deleted agent-home tool. The supervisor checked that all 8 installed payload manifests, the bundled app manifest and both selection files already satisfy the new identity and key rules.
- Supervisor fix: `scripts/tron version check` had failed since 2026-09-10 because the Mac project versions two bundles; it now requires every occurrence to carry the canonical value (`cf1d00a93`).
- Evidence (verified on combined `main`): Gateway `tsc` with declarations and 174 files / 1,871 tests; relay 40 tests; Mac build and 286 tests; launcher test (550 s); deploy helper 52 tests; reinstall tests; documentation policy exit 0. The payload-size change takes effect on the user's next Mac build.

### C-KNOW-UI-1 · Done · 2026-09-25 · simplification session (DeepSeek v4.1 Flash lane, reviewed by the supervisor)

- Result, per the user's decision: the legacy importer is gone (`legacy-import.ts`, the importer seam, the `knowledge.import.*` RPCs and types, the named-root environment, the catalog `imports` collection and checkpoints, its docs, and the iOS import client and models). An existing stored `imports` map still loads and is simply not carried forward. The iOS connector client methods are gone. All four `knowledge.connector.*` RPCs stay: `run` backs the agent's `connectorSweep`, `configure` is the only path that writes connector policy, `status` is the only admission read, and `assessment.approve` is the only renewal path for a paid cohort.
- Agent-facing connector failures name the fix without a secret: "Raindrop credential is unavailable. Add it to the Mac Keychain: service 'Tron Connector Credentials', account 'connector:raindrop:personal'." The iOS capability detail names the service and points to the agent for the account, because `connections.list` deliberately carries no credential reference. The supervisor made the detail cover a provider-rejected token too, since both record `credentialAvailability: unavailable`. Written test-first; negative control reproduced.
- Root cause of the user's Raindrop `auth-error` on 2026-09-25: the Keychain item `connector:raindrop:personal` was absent (not deleted by any Tron code); the user re-added it with `security add-generic-password -U -s "Tron Connector Credentials" -a "connector:raindrop:personal" -w`, and a read then verified account 915401.
- What an agent still cannot do on its own: configure a connection (destination, enablement, write approval, budget), read its status before a sweep, or renew a paid cohort. Left as-is by the user's choice to add pieces only when missing.
- Evidence (verified on combined `main`): Gateway `tsc` with declarations, 173 files / 1,867 tests; iOS unit suite 1,639 tests in 131 suites on the branch; documentation policy exit 0.
- Tasks added: C-KNOW-ORIGIN-1.

### C-DEPS-1 partial revert · 2026-09-25 · simplification session
...
### C-KNOW-ORIGIN-1, C-STRUCT-RULES-1 · Done · 2026-09-25 · simplification session

- C-KNOW-ORIGIN-1: `importOrigin`/`KnowledgeImportOrigin` removed from the knowledge contract, its validation, the note-update carry-through and the iOS models and dashboard. Evidence (verified): 0 record files and 0 catalog rows on this Mac carry it, and its only producer was the deleted importer; Gateway `tsc` with declarations and 256 knowledge tests pass; the iOS build and `KnowledgeModelsTests` (34) pass.
- C-STRUCT-RULES-1: `AGENTS.md` now has the "Keep the tree owned" rule; the iOS README and the Mac architecture doc state what each app owns and does not own (the Gateway and relay docs already did). No CI check: S-STRUCT-1 found no recurring leftover pattern.

### T-GW-SESS-2 pilot · 2026-09-25 · simplification session (deepseek-worker, reviewed by the supervisor)

- Result: of the first 16 flagged registry tests, 13 stay because the private observer is the only witness of a bound, lock order or fence (each now carries a comment naming the requirement), 2 were rewritten through public paths (restart plus the published row; on-disk topology asserted through catalog, acquire and delete), and 1 direct private-helper test was deleted because two other tests now cover both directions. One title that promised an assertion the body never made was corrected.
- Evidence (verified): focused file 239 → 238 tests (41.5 s both ways); full suite 173 files / 1,866 tests; five negative controls, each failing the intended test.
- For the next agent: about 80% of the remaining 48 are expected to be keeps, so the remaining value is mostly naming requirements rather than removing code. Proceed only if that is worth the review time. The unused `catalogDiscoveryLimits` constructor option is a test-only seam for a later row.
