...
| C-SCRIPTS-MAC-1 | Ready | Scripts and Mac payload hygiene: the deploy-test and reinstall-flag fixes (S-SCRIPTS-1), the launcher's missing package-file maxima and fail-fast checks, one owner for Node runtime pins, one Mach-O arch check, native-capture constants and the script test fixes (S-MAC-NATIVE) | S-SCRIPTS-1 | |
...
| C-GW-ROOT-CUTOVER-1 | Ready | Approved by the user on 2026-09-25: delete the finished one-time cutover tools and runbooks (agent-home migration and preflight, `tron_agent_home_cutover.py`, internal-layout migration, the mutating half of the delegated-root migration keeping its startup gate, connection migration, wizard migration) with their `scripts/tron` subcommands, tests and docs, keeping the startup refusals in `config.ts` and `gateway-main.ts`. Rows C-GW-ROOT-AGENTHOME-1/2, STATE-1/2, DELEGATED-1, WIZARD-1 and DOCS-1 in the S-GW-ROOT-1 output | S-GW-ROOT-1 | |
| C-GW-ROOT-FORMAT-1 | Ready | Approved by the user on 2026-09-25: drop the upload metadata v1, pre-inbox notification and device `lastSeenAt` acceptances and their tests. On this Mac 0 of 860 upload records are v1, the notification document has an inbox and no device carries `lastSeenAt` | S-GW-ROOT-1 | |
| C-APPROVED-SMALL-1 | Ready | Approved by the user on 2026-09-25: document `scripts/install-hooks.sh` and `scripts/tron-version sync|bump` in CONTRIBUTING; drop node-pty's win32 prebuilds during payload staging (C-DEPS-3); delete `GatewayDiagnosticsView`, the 7 unused provider icon imagesets, the unread APNs `category` field (C-RELAY-2) and the two unused drain-status fields (C-GW-PROTO-2) | S-DEAD-2 | |
| C-MAC-PAYLOAD-IDENTITY-1 | Ready | Approved by the user on 2026-09-25: one set of payload identity rules and manifest keys across `GatewayPayloadStore.swift`, the C launcher and `gateway-payload-deploy.mjs`, documented once (S-MAC-NATIVE output) | S-MAC-NATIVE-1 | |
