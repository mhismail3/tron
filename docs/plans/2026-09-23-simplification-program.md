...
| C-MAC-REDACT-2 | Claimed | Add TronLog's bare key/value patterns (for example `api-key=…`) to `DiagnosticsRedactor`, which widens what feedback exports redact; then C-MAC-REDACT-1 can proceed behind its corpus test. Approved by the user on 2026-09-25 | C-MAC-REDACT-1 | simplification session, 2026-09-25 |
