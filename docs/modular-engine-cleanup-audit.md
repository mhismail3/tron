...
classification, but the stricter capability-backed-truth audit found several
high-impact durable paths that needed stronger substrate ownership. The
2026-05-26 Phase 1 slice converted memory retain to resource-backed truth.
...
- `notifications`: durable inbox/read truth is converted, but the fixed iOS
  ...
  that behavior.
- `AgentControl` and `SourceChanges`: still have chat-sheet callers and
  event-state dependencies. Replace with resource lineage/control projections
  before deletion.
- subagent sheets: completed-result truth is now server-owned, but fixed chat
  shells still preserve pending/result navigation ergonomics. Remove only after
  the generated lineage UX covers the current operator role with absence gates.
...
| Area | Evidence | Decision |
|---|---|---|
| Capability execution ownership | `domains/capability/operations.rs` was replaced by `operations/mod.rs` plus `operations/execute.rs`, `operations/run.rs`, `operations/search.rs`, `operations/inspect.rs`, and `operations/audit.rs`; provider-visible tools remain execute-only | Keep `mod.rs` as the operator/internal coordination surface, isolate model-facing orchestration from child execution/approval projection, and keep operator search/inspect/audit helpers in concern-owned files |
