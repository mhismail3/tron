...
| What | How | Cadence |
|---|---|---|
| iOS Beta to TestFlight | Tag `server-v0.1.0-beta.1`-style versions on a green main commit. CI workflow `release-ios.yml` archives the `Tron` / `Prod` iOS app with `Apple Distribution`, manually packages and re-signs the IPA with archive-embedded or cached App Store profiles, uploads to App ID `6761511764`, waits for processing, and assigns the build to the internal + public TestFlight groups. | Same tag as server release. |
